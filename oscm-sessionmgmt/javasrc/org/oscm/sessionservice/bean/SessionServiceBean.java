...
public class SessionServiceBean implements SessionServiceLocal, SessionService {
  ...
  private static final Log4jLogger logger = LoggerFactory.getLogger(SessionServiceBean.class);
  ...
  @EJB private EventServiceBean evtMgmt;
  ...
  public void deleteAllSessions() {
    ...
    for (Session session : ParameterizedTypes.iterable(query.getResultList(), Session.class)) {
  ...
  public void createServiceSession(long subscriptionKey, String sessionId, String userToken)
    ...
    if (currentSessionData == null) {
      try {
        Subscription sub = subMgmt.loadSubscription(sessionData.getSubscriptionTKey().longValue());
        // check that the subscription belongs to the current user's
        // organization
        PermissionCheck.owns(sub, currentUser.getOrganization(), logger);
        if (sub.getStatus() == SubscriptionStatus.EXPIRED) {
          ServiceParameterException ex =
              new ServiceParameterException(
                  ...
                  null);
          logger.logWarn(
              Log4jLogger.SYSTEM_LOG, ex, LogMessageIdentifier.WARN_CREATE_SERVICE_SESSION_FAILED);
          throw ex;
        }
        if (sub.getStatus() != SubscriptionStatus.ACTIVE
          ...
          OperationNotPermittedException ex =
              new OperationNotPermittedException(
                  "subscription '" + sub.getKey() + "' is not active or pending update.");
          logger.logWarn(
              Log4jLogger.SYSTEM_LOG, ex, LogMessageIdentifier.WARN_CREATE_SERVICE_SESSION_FAILED);
          throw ex;
        }
        ...
        SaaSSystemException sse =
            new SaaSSystemException("Session entry already exists with the same business key", e);
        logger.logError(
            Log4jLogger.SYSTEM_LOG, sse, LogMessageIdentifier.ERROR_CREATE_PRODUCT_SESSION_FAILED);
        throw sse;
      }
    }
  }
    ...
    if (storedSession != null) {
      ...
      if (storedSession.getUserToken().equals(userToken)) {
        ...
        try {
          ...
          Subscription sub = dm.getReference(Subscription.class, subscriptionKey);
          UsageLicense license = findUsageLicense(sub, userId);
          if (license != null && license.getApplicationUserId() != null) {
        ...
        evt.setSubscriptionTKey(storedSession.getSubscriptionTKey().longValue());
        try {
  ...
  @Override
  public UsageLicense findUsageLicense(Subscription subscription, String userId) {
    Query query = dm.createNamedQuery("Subscription.findUsageLicense");
    query.setParameter("userId", userId);
    query.setParameter("subscriptionKey", Long.valueOf(subscription.getKey()));
  ...
  public void deleteSessionsForSessionId(String sessionId) {
    ...
    for (Session session : productSessions) {
      ...
      try {
        sub = subMgmt.loadSubscription(session.getSubscriptionTKey().longValue());
      } catch (ObjectNotFoundException e) {
        logger.logWarn(
            Log4jLogger.SYSTEM_LOG,
            e,
  ...
  @Override
  @TransactionAttribute(TransactionAttributeType.MANDATORY)
  public List<Session> getProductSessionsForSubscriptionTKey(long subscriptionTKey) {
  ...
  public boolean hasTechnicalProductActiveSessions(long technicalProductKey) {
    ...
    Query query = dm.createNamedQuery("Session.getNumOfActiveSessionsForTechProduct");
    query.setParameter("technicalProductKey", Long.valueOf(technicalProductKey));
  ...
  public int deletePlatformSession(String sessionId) {
    ArgumentValidator.notNull("sessionId", sessionId);
    Query query = dm.createNamedQuery("Session.deletePlatformSessionsForSessionId");
    query.setParameter("sessionId", sessionId);
    query.setParameter("sessionType", SessionType.PLATFORM_SESSION);
   ...
   */
  private List<Session> retrieveProductSessionDataForSessionId(String sessionId) {
   ...
   *
   * <p>A logout event is only generated in case the user token is not set anymore, what means that
   * the user has really logged in earlier
   *
   * @param session The session the logout event will be generated for.
   */
  ...
  /**
   * Checks if the session count for the subscription is lower or equal to the value of the product
   * parameter CONCURRENT_USER. If this condition does not hold, an ProductParameterException will
   * be thrown.
   *
   * @param subscription The subscription to be checked.
   * @throws ServiceParameterException Thrown in case the session count of the subscription is
   *     greater than the value of the product parameter CONCURRENT_USER
   */
  private void verifyParameterConcurrentUser(Subscription subscription)
    ...
    for (Parameter parameter : parameterSet.getParameters()) {
      if (parameter.getParameterDefinition().getParameterType() == ParameterType.PLATFORM_PARAMETER
          && PlatformParameterIdentifiers.CONCURRENT_USER.equals(
              parameter.getParameterDefinition().getParameterId())
          && parameter.getValue() != null) {
        List<Session> list = getProductSessionsForSubscriptionTKey(subscription.getKey());
  ...
  public void deleteServiceSessionsForSubscription(long subscriptionKey)
    ...
    List<UserGroup> administratedUserGroups =
        userGroupService.getUserGroupsForUserWithRole(
            dm.getCurrentUser().getKey(), UnitRoleType.ADMINISTRATOR.getKey());
    PermissionCheck.owns(sub, dm.getCurrentUser(), administratedUserGroups, logger);
    PermissionCheck.owns(sub, dm.getCurrentUser().getOrganization(), logger);
