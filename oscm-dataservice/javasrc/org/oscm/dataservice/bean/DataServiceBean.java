...
public class DataServiceBean implements DataService {
  ...
  private static final String EVALUATING_SESSION_CONTEXT_FAILED =
      "Evaluating session context failed";

  /** Cache for the logged in user. */
  private static ThreadLocal<String> CURRENT_USER = new ThreadLocal<String>();

  /**
   * Cache for the user to make is accessible to the history listener. This user might be different
   * to the logged in user, in case a user acts on behalf of another organization
   */
  ...
  private static final Log4jLogger logger = LoggerFactory.getLogger(DataServiceBean.class);
   ...
   *
   * @param objclass the object class of the not found object
   * @param businessKey the not found business key
   */
  ...
  public DomainObject<?> find(DomainObject<?> idobj) {
    setThreadLocals();
    String[] businessKeyElements =
        PersistenceReflection.getBusinessKey(DomainObject.getDomainClass(idobj));
    String className = DomainObject.getDomainClass(idobj).getName();
    String queryName = className.substring(className.lastIndexOf(".") + 1) + ".findByBusinessKey";
    if (businessKeyElements == null) {
      throw new SaaSSystemException("No BusinessKey defined for " + className);
    }
    ...
    try {
      ...
      for (String par : businessKeyElements) {
        qrykey.append(par + "='");
        qrykey.append(PersistenceReflection.getValue(idobj, par).toString());
        qrykey.append("',");
      }
      qrykey.append(")");
      String msgText =
          "Non-Unique Business Key Search for "
  ...
  public PlatformUser find(PlatformUser pu) {
    logger.logDebug(
        "Find user (" + pu.getKey() + ":" + pu.getUserId() + ") in tenant " + pu.getTenantId());
    Query qry;
    ...
    try {
      ...
      String qrykey =
          "(" + "tenantId='" + pu.getTenantId() + "'," + "userId='" + pu.getUserId() + ")";
  ...
  public DomainObject<?> getReferenceByBusinessKey(DomainObject<?> findTemplate)
    ...
    if (result == null) {
      DomainObjectException.ClassEnum classEnum = class2Enum(findTemplate.getClass());
      Map<String, String> businessKeyMap = PersistenceReflection.getBusinessKeys(findTemplate);
      throw new ObjectNotFoundException(classEnum, getBusinessKey(businessKeyMap, classEnum));
    }
    return result;
  }
  ...
  public List<DomainHistoryObject<?>> findHistory(DomainObject<?> obj) {
    ...
    String histClassName = className.substring(className.lastIndexOf(".") + 1) + "History";
    ...
    for (Object o : qryresult) {
      if (!(o instanceof DomainHistoryObject<?>)) {
        throw new SaaSSystemException("findHistory loaded Non-History Object !");
      }
  ...
  public <T extends DomainObject<?>> T getReference(Class<T> objclass, long id)
    ...
    if (result == null) {
      throw new ObjectNotFoundException(class2Enum(objclass), String.valueOf(id));
  ...
  /**
   * Returns the key user to be used in the history objects. In case a user acts on behalf of the
   * current in user, then the key of the master user will be returned.
   */
  ...
  public void validateBusinessKeyUniqueness(DomainObject<?> obj)
    ...
    Map<String, String> businessKeyMap = PersistenceReflection.getBusinessKeys(obj);
    if (businessKeyMap != null) {
      ...
      if (search != null && obj.getKey() != search.getKey()) {
        DomainObjectException.ClassEnum classEnum = class2Enum(obj.getClass());
        String businessKey = getBusinessKey(businessKeyMap, classEnum);
        NonUniqueBusinessKeyException e = new NonUniqueBusinessKeyException(classEnum, businessKey);
        logger.logWarn(
  ...
  private PlatformUser getUserFromSessionContext(boolean lookupOnly) {
    ...
    try {
      ...
      String name = "-";
      try {
        if (sessionCtx != null && sessionCtx.getCallerPrincipal() != null)
          name = sessionCtx.getCallerPrincipal().getName();
      } catch (IllegalStateException ex) {
        // name is set before
      }
      InvalidUserSession ius =
          new InvalidUserSession(
              "User with key '"
                  + name
      ...
      InvalidUserSession ius = new InvalidUserSession(EVALUATING_SESSION_CONTEXT_FAILED, e);
      logger.logWarn(
  ...
  private PlatformUser loadUserFromSessionContext(boolean lookupOnly)
      throws ObjectNotFoundException {
    String name = CURRENT_ASYNC_USER.get() == null ? null : CURRENT_ASYNC_USER.get().toString();
    if (name == null) {
  ...
  private PlatformUser loadUserFromSSLContext(boolean lookupOnly) {
    ...
    InvalidUserSession ius = new InvalidUserSession(EVALUATING_SESSION_CONTEXT_FAILED);
    logger.logWarn(
        Log4jLogger.SYSTEM_LOG | Log4jLogger.ACCESS_LOG,
        ius,
        LogMessageIdentifier.WARN_ACCESS_DINIED_NO_USER);
  ...
  private Organization getOrganizationForDistinguishedName(boolean lookupOnly) {
    final Set<String> principalNames =
        Collections.singleton(webServiceContext.getUserPrincipal().getName());
    final TypedQuery<Organization> query =
        createNamedQuery("Organization.getOrgsForDN", Organization.class);
    ...
    try {
      ...
      final InvalidUserSession ius = new InvalidUserSession(EVALUATING_SESSION_CONTEXT_FAILED, e);
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG,
          ius,
   ...
   *
   * @param org the organization is question
   * @param lookupOnly defines whether a deregistered organization causes an exception or just
   *     returns <code>false</code>
   * @return <code>true</code> if the organization is still registered, <code>false</code> or
   *     exception otherwise
   */
  private boolean checkOrgDeregistration(Organization org, boolean lookupOnly) {
    if (org.getDeregistrationDate() != null) {
      ...
      InvalidUserSession ius =
          new InvalidUserSession("Organization with key '" + org.getKey() + "' is deregistered!");
      logger.logError(
          Log4jLogger.SYSTEM_LOG | Log4jLogger.AUDIT_LOG,
          ius,
   ...
   *
   * @return Result string or "" if no business key attribute is defined in classEnum
   */
  private String getBusinessKey(
      Map<String, String> businessKeyMap, DomainObjectException.ClassEnum classEnum) {
  ...
  public DataSet executeQueryForRawData(SqlQuery sqlQuery) {
    ...
    try {
      ...
      for (Integer parameterIndex : sqlQuery.getParameters().keySet()) {
        stmt.setObject(parameterIndex.intValue(), sqlQuery.getParameters().get(parameterIndex));
      }
      rs = stmt.executeQuery();
      return convertToDataSet(rs, sqlQuery.getMax());
    } catch (Exception e) {
      SaaSSystemException sse = new SaaSSystemException(e);
      logger.logError(Log4jLogger.SYSTEM_LOG, sse, LogMessageIdentifier.ERROR_SQL);
      throw sse;
    } finally {
      closeResultSet(rs);
      closeStatement(stmt);
  ...
  private void closeResultSet(ResultSet rs) {
    if (rs != null) {
      try {
        ...
        logger.logError(
            Log4jLogger.SYSTEM_LOG, ignore, LogMessageIdentifier.ERROR_CLOSE_RESOURCE_FAILED);
      }
    }
  }
  ...
  public void persistPlatformUserWithTenant(PlatformUser pu, String tenantId)
    ...
    if (user != null) {
      DomainObjectException.ClassEnum classEnum = class2Enum(pu.getClass());
