...
public class UserServiceBeanTest {
  ...
  @Captor ArgumentCaptor<DisplaySettings> displaySettings;
  ...
  public void setup() throws Exception {
    ...
    when(usb.ds.getReference(eq(PlatformUser.class), eq(pu.getKey()))).thenReturn(pu);
    when(usb.ds.getCurrentUser()).thenReturn(pu);
    doNothing().when(usb.ds).persist(any(DisplaySettings.class));
    when(usb.ds.merge(any(DisplaySettings.class))).thenReturn(null);

    when(usb.lsmsl.getMappedAttributes()).thenReturn(mappedAttributes);

    when(usb.lsl.getLocalizedTextFromDatabase(
            anyString(), anyLong(), eq(LocalizedObjectTypes.ROLE_DEF_NAME)))
        .thenReturn(ROLENAME);

    when(usb.slsl.getSubcsriptionsWithRoles(
            any(Organization.class), anySetOf(SubscriptionStatus.class)))
        .thenReturn(new ArrayList<SubscriptionWithRoles>());
    when(usb.slsl.getSubscriptionAssignments(
            any(PlatformUser.class), anySetOf(SubscriptionStatus.class)))
        .thenReturn(new ArrayList<UsageLicense>());
        ...
        .thenAnswer(
            new Answer<VOUserDetails>() {
              ...
              public VOUserDetails answer(InvocationOnMock invocation) throws Throwable {
                ...
                VOUserDetails ud = (VOUserDetails) invocation.getArguments()[0];
                if (ud != null) {
                  ud.setKey(pu.getKey());
                }
                return ud;
    ...
    when(usb.isl.createUserWithGroups(
            any(VOUserDetails.class), anyListOf(UserRoleType.class), anyString(), anyMap()))
        .thenAnswer(
            new Answer<VOUserDetails>() {
    ...
    when(Boolean.valueOf(
            usb.ssl.addRevokeUser(
                anyString(), anyListOf(VOUsageLicense.class), eq((List<VOUser>) null))))
  ...
  public void getUserDetails() throws Exception {
    ...
    verify(usb.dc, times(1)).toPOUserDetails(eq(pu), anySetOf(UserRoleType.class));
  ...
  public void resetUserPassword() throws Exception {
    ...
    verify(usb.isl, times(1)).getPlatformUser(eq(user.getUserId()), eq(true));
  ...
  public void resetUserPassword_Rollback() throws Exception {
    when(usb.isl.getPlatformUser(anyString(), anyBoolean()))
  ...
  public void deleteUser() throws Exception {
    POUser user = createPOUser(pu);
    String marketplaceId = "marketplaceId";
    when(usb.isl.getPlatformUser(eq(user.getUserId()), anyString(), eq(true))).thenReturn(pu);

    usb.deleteUser(user, marketplaceId, TENANT_ID);

    verify(usb.isl, times(1)).getPlatformUser(eq(user.getUserId()), anyString(), eq(true));
    verify(usb.userGroupService, times(1)).addLogEntryWhenDeleteUser(eq(pu));
  ...
  public void saveUser() throws Exception {
    ...
    verify(usb.isl, times(1)).verifyIdUniquenessAndLdapAttributes(eq(pu), any(PlatformUser.class));
    verify(usb.isl, times(1)).setUserRolesInt(eq(user.getAssignedRoles()), eq(pu));
    verify(usb.isl, times(1)).notifySubscriptionsAboutUserUpdate(eq(pu));
    verify(usb.isl, times(1)).sendUserUpdatedMail(eq(pu), any(PlatformUser.class));

    verify(usb.dc, times(2)).updatePlatformUser(eq(user), (PlatformUser) notNull());
    verifyZeroInteractions(usb.sc);
  }
  ...
  public void saveUser_NoMailSet() throws Exception {
    ...
    verify(usb.isl, times(1)).setUserRolesInt(eq(user.getAssignedRoles()), eq(pu));
    verify(usb.isl, times(1)).notifySubscriptionsAboutUserUpdate(eq(pu));
    verify(usb.isl, times(1)).sendUserUpdatedMail(eq(pu), eq((PlatformUser) null));

    verify(usb.dc, times(2)).updatePlatformUser(eq(user), any(PlatformUser.class));
    verifyZeroInteractions(usb.sc);
  }
  ...
  public void saveUser_SendAdministratorNotifyMail() throws Exception {
    ...
    verify(usb.isl, times(1))
        .sendAdministratorNotifyMail(any(PlatformUser.class), eq(user.getUserId()));
  }
  ...
  public void getNewUserData() {
    ...
    verify(usb.dc, times(1))
        .toPOUserAndSubscriptionsNew(
            anyListOf(SubscriptionWithRoles.class),
            anySetOf(UserRoleType.class),
            any(LocalizerFacade.class));
    verify(usb.slsl, times(1))
        .getSubcsriptionsWithRoles(
            any(Organization.class), eq(Subscription.ASSIGNABLE_SUBSCRIPTION_STATUS));
    verifyZeroInteractions(usb.sc);
  }
  ...
  public void toRoleKeyList() {
    ...
    HashSet<Long> keys =
        new HashSet<Long>(
            Arrays.asList(Long.valueOf(10), Long.valueOf(11), Long.valueOf(20), Long.valueOf(21)));
  ...
  public void createNewUser() throws Exception {
    ...
    verify(usb.isl, times(1)).createUser(any(VOUserDetails.class), anyString());

    verify(usb.isl, times(1))
        .sendMailToCreatedUser(anyString(), eq(true), any(Marketplace.class), eq(pu));
  }

  @Test
  public void createNewUser_CreateSuspended() throws Exception {
    when(usb.isl.createUser(any(VOUserDetails.class), anyString())).thenReturn(null);
    POUserAndSubscriptions user = new POUserAndSubscriptions();
    user.setAssignedRoles(EnumSet.of(UserRoleType.ORGANIZATION_ADMIN));
    user.setSubscriptions(createSubscriptions());
    ...
    verify(usb.isl, never())
        .sendMailToCreatedUser(
            anyString(), anyBoolean(), any(Marketplace.class), any(PlatformUser.class));
  }
  ...
  public void createNewUser_AddRevokeSuspended() throws Exception {
    when(Boolean.valueOf(
            usb.ssl.addRevokeUser(
                anyString(), anyListOf(VOUsageLicense.class), anyListOf(VOUser.class))))
  ...
  public void getUserAndSubscriptionDetails() throws Exception {
    ...
    POUserAndSubscriptions result = usb.getUserAndSubscriptionDetails(userId, TENANT_ID);

    assertNotNull(result);
    verify(usb.lsmsl, times(1)).getMappedAttributes();
    verify(usb.dc, times(1))
        .toPOUserAndSubscriptionsExisting(
            eq(pu),
            anyListOf(SubscriptionWithRoles.class),
            anySetOf(UserRoleType.class),
            anyListOf(UsageLicense.class),
            any(LocalizerFacade.class),
            same(mappedAttributes));
  ...
  public void getAssignments() {
    ...
    Map<String, VOUsageLicense> result = usb.getAssignments(assignments, subs, u);
  ...
  public void saveUserAndSubscriptionAssignment() throws Exception {
    ...
    doReturn(Boolean.TRUE)
        .when(usb.dc)
        .isUserRoleUpdated(anySetOf(UserRoleType.class), any(PlatformUser.class));
    ...
    verify(usb.isl, times(1)).setUserRolesInt(eq(user.getAssignedRoles()), eq(pu));
    verify(usb.isl, times(1)).notifySubscriptionsAboutUserUpdate(eq(pu));
    verify(usb.isl, times(1)).sendUserUpdatedMail(eq(pu), (PlatformUser) notNull());
  }

  @Test(expected = ObjectNotFoundException.class)
  public void saveUserAndSubscriptionAssignment_UserGroupNotExist() throws Exception {
    ...
    when(usb.ds.getReference(eq(UserGroup.class), eq(GROUPKEY)))
        .thenThrow(new ObjectNotFoundException());
  ...
  public void saveUserAndSubscriptionAssignment_Rollback() throws Exception {
    ...
    try {
      ...
      verify(usb.sc, times(1)).setRollbackOnly();
      verifyZeroInteractions(usb.dc, usb.isl, usb.isl, usb.lsl, usb.lsmsl, usb.slsl, usb.ssl);
    }
  }
  ...
  @Test
  public void setDisplaySetting_notExisting() throws Exception {
    // given
    givenNoDisplaySettings();

    // when
    usb.setDisplaySettings(1000, "json: setting");

    // then
    verify(usb.ds, times(1)).persist(any(DisplaySettings.class));
  }

  @Test(expected = ConcurrentModificationException.class)
  public void setDisplaySetting_createConcurrent() throws Exception {
    // given
    givenNoDisplaySettings();
    doThrow(new NonUniqueBusinessKeyException()).when(usb.ds).persist(any(DisplaySettings.class));
  ...
  public void setDisplaySetting_empty() throws Exception {
    // given
    ...
    // when
    usb.setDisplaySettings(1000, null);
  }

  @Test
  public void getDisplaySetting_existing() throws Exception {
    // given
    givenDisplaySettings();

    // when
    String json = usb.getDisplaySettings(1000);
  ...
  @Test
  public void getDisplaySetting_notExisting() throws Exception {
    // given
    givenNoDisplaySettings();
  ...
  @Test
  public void setDisplaySetting_existing() throws Exception {
    // given
    givenDisplaySettings();
    String settings = "json: setting";

    // when
    usb.setDisplaySettings(1000, settings);

    // then
    verify(usb.ds, times(0)).persist(any(DisplaySettings.class));
    verify(usb.ds, times(1)).merge(any(DisplaySettings.class));
  }

  @SuppressWarnings({"unchecked", "rawtypes"})
  private void givenDisplaySettings() {
    DisplaySettings ds = new DisplaySettings();
    ds.setData("test");
    ds.setUserTKey(1000);
    when(usb.ds.find(any(DisplaySettings.class))).thenReturn((DomainObject) ds);
  }

  private void givenNoDisplaySettings() {
    when(usb.ds.find(any(DisplaySettings.class))).thenReturn(null);
  }

  private static void verifyUserAndAssignmentUpdate(
      Response response, UserServiceBean bean, boolean suspended) throws Exception {
    assertNotNull(response);
    if (suspended) {
  ...
  private static void verifyAddRevokeCall(
      String subId, List<VOUsageLicense> lics, List<VOUser> users, PlatformUser pu) {
  ...
  private static POUserAndSubscriptions initUserWithAssignments(
      final boolean suspend, UserServiceBean bean, final PlatformUser pu) throws Exception {
    POUserAndSubscriptions user = new POUserAndSubscriptions();
    populateUserDetails(pu, user);
    user.setAssignedRoles(EnumSet.of(UserRoleType.ORGANIZATION_ADMIN));
    user.setSubscriptions(createSubscriptions());

    when(bean.slsl.getSubscriptionAssignments(
            any(PlatformUser.class), anySetOf(SubscriptionStatus.class)))
        .thenReturn(createAssignments());
    when(Boolean.valueOf(
            bean.ssl.addRevokeUser(
                anyString(), anyListOf(VOUsageLicense.class), anyListOf(VOUser.class))))
        .thenAnswer(
            new Answer<Boolean>() {
              ...
              public Boolean answer(InvocationOnMock invocation) throws Throwable {
                ...
                List<VOUsageLicense> lics = (List<VOUsageLicense>) invocation.getArguments()[1];
                List<VOUser> users = (List<VOUser>) invocation.getArguments()[2];
  ...
  private static void verifyAssignment(Map<String, VOUsageLicense> assignments, VOUser u) {
  ...
  private POUserAndSubscriptions preparedForUpdateUserGroups() throws Exception {
    ...
    doReturn(Boolean.TRUE)
        .when(usb.dc)
        .isUserRoleUpdated(anySetOf(UserRoleType.class), any(PlatformUser.class));
    doReturn(assginedUserGroupsInDB)
        .when(usb.userGroupService)
