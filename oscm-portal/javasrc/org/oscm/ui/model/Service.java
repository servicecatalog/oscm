...
public class Service extends BaseBean implements Serializable {
  ...
  /** @return true, if the configurator URL is not null and not empty, otherwise false. */
  public boolean useExternalConfigurator() {
    return vo.getConfiguratorUrl() != null && !vo.getConfiguratorUrl().trim().isEmpty();
  }
   ...
   *
   * <p>The real value will be exchanged within a hidden field for supporting a dynamically
   * disabling.
   */
   ...
   *
   * <p>The real value will be exchanged within a hidden field for supporting a dynamically
   * disabling. Therefore this "set" is ignored.
   *
   * @param checked
   */
  ...
  /**
   * Returns boolean which informs the UI whether the flag "visibleInCatalog" can be adjusted (only
   * for global -not customer specific- services).
   */
   ...
   * This affects only the activation and visibility state.
   */
  public boolean isModified() {
    boolean modified =
        ((isSelected() && vo.getStatus() == ServiceStatus.INACTIVE)
            || (!isSelected() && vo.getStatus() == ServiceStatus.ACTIVE));
    if (!modified && isVisibleInCatalogSupported() && isVisibleInCatalog() != oldVisibility)
      modified = true;
    return modified;
  }
  ...
  public String getPriceModelTextKey() {
    ...
    if (priceModel == null || priceModel.getVo() == null || priceModel.getKey() == 0) {
   ...
   *
   * @param priceModel the price model to get the price
  ...
  String[] getPriceTextNative(VOPriceModel priceModel) {
    String[] result =
        new String[] {JSFUtils.getText(BaseBean.LABEL_PRICE_MODEL_FREE, null), "", ""};
    ...
    if (priceModel.isPricePerPeriodSet()) {
      result[0] =
          JSFUtils.getText(
              ...
              new Object[] {
                getCurrencySymbol(priceModel), getDisplayPrice(priceModel.getPricePerPeriod())
              });
      result[1] =
          JSFUtils.getText(
              BaseBean.LABEL_PRICE_MODEL_PER_SUB,
      ...
      result[0] =
          JSFUtils.getText(
              ...
              new Object[] {
                ...
                getDisplayPrice(priceModel.getPricePerUserAssignment())
              });
      result[1] =
          JSFUtils.getText(
              BaseBean.LABEL_PRICE_MODEL_PER_USER,
              new Object[] {getPeriodText(priceModel.getPeriod())});
      ...
      result[0] =
          JSFUtils.getText(
              BaseBean.LABEL_PRICE_MODEL_PRICE,
              new Object[] {
                getCurrencySymbol(priceModel), getDisplayPrice(priceModel.getOneTimeFee())
              });
    } else {
      result[0] = JSFUtils.getText(BaseBean.LABEL_PRICE_MODEL_SEE_DETAILS, new Object[0]);
    }
    if (result[1].trim().length() > 0) {
      // when both price and unit are set, provide combined string as well
      result[2] =
          JSFUtils.getText(
              BaseBean.LABEL_PRICE_MODEL_PRICE_AND_UNIT, new Object[] {result[0], result[1]});
    } else {
      result[2] = result[0];
    }
    return result;
  ...
  private static String getCurrencySymbol(VOPriceModel priceModel) {
    return Currency.getInstance(priceModel.getCurrencyISOCode())
        .getSymbol(JSFUtils.getViewLocale());
  }
  ...
  static final String getPeriodText(PricingPeriod period) {
    return JSFUtils.getText("PricingPeriod." + period.name(), new Object[0]);
  }
  ...
  /**
   * Returns the short description of this service with a max. length of 120 characters. If the
   * description is longer it will be cut accordingly.
   */
