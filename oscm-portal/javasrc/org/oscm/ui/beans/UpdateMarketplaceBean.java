...
public class UpdateMarketplaceBean extends BaseBean {
  ...
  /**
   * Calls the marketplace service and persists the changes to the marketplace.
   *
   * @return <code>OUTCOME_SUCCESS</code> if the operation was executed successfully
   */
  public String updateMarketplace() {
    ...
    addMessage(null, FacesMessage.SEVERITY_INFO, INFO_MARKETPLACE_SAVED, mId);
    return OUTCOME_SUCCESS;
  }

  /**
   * Updates the label of the {@link SelectItem} matching the passed {@link VOMarketplace}.
   *
   * @param mp the {@link VOMarketplace} to get the new label from
   * @param list the list of {@link SelectItem} to update
   */
  ...
  String applyOrgChange(String mId) {
    ...
    try {
      ...
      Response response = getPricingService().getPricingForMarketplace(mId);
  ...
  public List<SelectItem> getSelectableMarketplaces() {
    if (selectableMarketplaces == null) {
      ...
      if (isLoggedInAndPlatformOperator()) {
        marketplaces = getMarketplaceService().getMarketplacesForOperator();
      } else {
  ...
  Marketplace convertToModel(VOMarketplace vmp) {
    ...
    mp.setTenantSelectVisible(
        isLoggedInAndPlatformOperator() && !menuBean.getApplicationBean().isInternalAuthMode());
  ...
  void addToModel(POPartnerPriceModel priceModel) {
    if (model != null) {
      model.setResellerRevenueShareObject(priceModel.getRevenueShareResellerModel());
      model.setBrokerRevenueShareObject(priceModel.getRevenueShareBrokerModel());
    }
  }
  ...
  boolean isMpOwner(VOMarketplace vmp) {
    ...
    return user.isMarketplaceOwner()
        && user.getOrganizationId().equals(vmp.getOwningOrganizationId());
  }
