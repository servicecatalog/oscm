...
public class SessionBean implements Serializable {
  ...
  private static final Log4jLogger logger = LoggerFactory.getLogger(SessionBean.class);
  ...
  /**
   * The key of the last selected technical service - applies to technology provider and supplier
   * (operations on technical and marketable services).
   */
  private long selectedTechnicalServiceKey;

  /**
   * The key of the last selected marketable service - applies to supplier (operations on price
   * model and marketable services).
   */
  private Long selectedServiceKeyForSupplier;

  /**
   * The id of the last selected customer - applies to supplier (operations on customer and price
   * model).
   */
  private String selectedCustomerId;

  /**
   * The id of the last selected subscription id - applies to customer (operations on
   * subscriptions).
   */
  ...
  /**
   * The key of the last selected marketable service - applies to customer (browsing in
   * marketplace).
   */
  ...
  public boolean isIe() {
    ...
    if (useragent.toLowerCase().contains("trident") && useragent.toLowerCase().contains("rv:11")) {
      ie = Boolean.TRUE;
    } else {
      ie = Boolean.FALSE;
    }
  ...
  public boolean isAutoOpenMpLogonDialog() {
    ...
    return obj instanceof HttpServletRequest
        && Boolean.TRUE
            .toString()
            .equals(
                ((ServletRequest) obj).getParameter(Constants.REQ_PARAM_AUTO_OPEN_MP_LOGIN_DIALOG));
  }
   ...
   *
   * @param selectedSubscriptionId the subscription id
   */
  ...
  public long determineSelectedServiceKeyForCustomer() {
    if (selectedServiceKeyForCustomer == SERIVE_KEY_NOT_SET) {
      ...
      String key = httpRequest.getParameter(Constants.REQ_PARAM_SELECTED_SERVICE_KEY);
  ...
  public void setSelectedServiceKeyForCustomer(long selectedServiceKeyForCustomer) {
    ...
    if (isValidServiceKey(selectedServiceKeyForCustomer)) {
      try {
        ...
        if (httpResponse != null) {
          ...
          // be able to continue a subscription in case of a possible
          // session timeout
          JSFUtils.setCookieValue(
              JSFUtils.getRequest(),
              httpResponse,
              Constants.REQ_PARAM_SERVICE_KEY,
              URLEncoder.encode(
                  Long.valueOf(selectedServiceKeyForCustomer).toString(),
  ...
  public boolean isErrorMessageDuplicate() {
    ...
    for (int i = 0; i < 5; i++) {
      Object param = getRequest().getAttribute(Constants.REQ_ATTR_ERROR_PARAM + i);
      if (param != null) {
        params.add(param);
      }
    }
  ...
  /** @param selectedSubscriptionKey - the selected subscription key */
  ...
  public void setSelectedExternalPriceModel(PriceModel selectedExternalPriceModel) {
  ...
  public String getSamlLogoutRequest() {
    if (samlLogoutRequest == null) {
      samlLogoutRequest =
          (String) new UiDelegate().getSession().getAttribute(SESSION_PARAM_SAML_LOGOUT_REQUEST);
    }
  ...
  private String getTenantIDFromMarketplace() throws MarketplaceRemovedException {
    ...
    if (StringUtils.isNotBlank(marketplaceId)) {
      tenantID = mkpCache.getConfiguration(marketplaceId).getTenantId();
      if (StringUtils.isBlank(tenantID)) {
        try {
          tenantID = mkpService.getMarketplaceById(marketplaceId).getTenantId();
        } catch (ObjectNotFoundException e) {
