...
/**
 * Backing bean for operator functionality for exporting billing data, starting billing, retry of
 * failed payment processes and start for SOP data handling/billing.
 *
 * @author weiser
 */
@ViewScoped
...
public class BillingBean extends BaseOperatorBean implements Serializable {
  ...
  private static final Log4jLogger logger = LoggerFactory.getLogger(BillingBean.class);
   ...
   * @throws OrganizationAuthoritiesException
   */
  public String retryFailedPaymentProcesses() throws OrganizationAuthoritiesException {
  ...
  /**
   * Starts the payment processing for the not yet processed billing results that must be handled by
   * the PSP.
   *
   * @return a generic success or error message
   * @throws OrganizationAuthoritiesException if the user calling this method belongs to an
   *     organization which has no authorities to start the payment process.
   */
  public String startPaymentProcessing() throws OrganizationAuthoritiesException {
    getLogger().logInfo(Log4jLogger.SYSTEM_LOG, LogMessageIdentifier.INFO_USER_PAYMENT_RUN_STARTED);

    boolean result = getOperatorService().startPaymentProcessing();
    getLogger()
        .logInfo(Log4jLogger.SYSTEM_LOG, LogMessageIdentifier.INFO_USER_PAYMENT_RUN_FINISHED);
    return getOutcome(result);
  }

  /**
   * Exports the billing data for the specified start and end date and the supplier id. The read
   * data will be written to the response as type text/xml.
   *
   * @return the logical outcome.
   * @throws ObjectNotFoundException
   * @throws OrganizationAuthoritiesException
   */
  public String getBillingData() throws ObjectNotFoundException, OrganizationAuthoritiesException {

    String orgId = operatorSelectOrgCtrl.getOrganizationId();
    if (fromDate == null || toDate == null || orgId == null || orgId.trim().length() == 0) {
      addMessage(null, FacesMessage.SEVERITY_ERROR, ERROR_EXPORT_BILLING_DATA);
      logger.logError(
    ...
    long from = DateConverter.getBeginningOfDayInCurrentTimeZone(fromDate.getTime());
    long to = DateConverter.getBeginningOfNextDayInCurrentTimeZone(toDate.getTime());

    billingData = getOperatorService().getOrganizationBillingData(from, to, orgId);
    if (billingData == null) {
      addMessage(null, FacesMessage.SEVERITY_ERROR, ERROR_SHOW_BILLING_DATA);
  ...
  public void validateFromAndToDate(
    ...
    try {
      ...
      context.addMessage(
          ...
          new FacesMessage(FacesMessage.SEVERITY_ERROR, ex.getLocalizedMessage(), null));
    }
  }
