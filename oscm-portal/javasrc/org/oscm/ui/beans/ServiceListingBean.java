...
public class ServiceListingBean extends BaseBean implements Serializable {
  ...
  /**
   * Needed to reset the categories of a marketplace if a selected category was deleted
   * concurrently.
   */
  @ManagedProperty(value = "#{categorySelectionBean}")
  private CategorySelectionBean categorySelectionBean;

  /** Needed to reset the tags of a marketplace if a selected tag was deleted concurrently. */
  ...
  void updateServiceListContainsChargeableResellerService(List<VOService> resultList) {
  ...
  public List<Service> getServices() {
    if (services == null && isMarketplaceSet()) {
      if (isLoggedIn()) {
        services =
            DEFAULT_VOSERVICE_MAPPER.map(
                getProvisioningServiceInternal()
                    .getServicesForMarketplace(
                        getMarketplaceId(), PerformanceHint.ONLY_FIELDS_FOR_LISTINGS));
  ...
  /**
   * Returns a list of all public services that can be viewed in the public catalog by a potential
   * customer. The supplier ID or marketplace ID must be given in the request URL.
   *
  ...
  public List<Service> getPublicServices() {
    if (services == null) {
      ...
      if (marketplaceId != null) {
        voList =
            getProvisioningServiceInternal()
                .getServicesForMarketplace(marketplaceId, PerformanceHint.ONLY_FIELDS_FOR_LISTINGS);
  ...
  /**
   * Executes the service for paging, sorting and filtering and updates the result size if the
   * service list is not initialized.
   *
  ...
  public List<Service> searchWithCriteria() {
    ...
    if (categorySelectionBean.isCategorySelected()) {
      criteria.setCategoryId(categorySelectionBean.getSelectedCategoryId());
    }
    if (services == null) {
      try {
        VOServiceListResult result =
            getSearchServiceInternal()
        ...
        updateServiceListContainsChargeableResellerService(result.getServices());
   ...
   *
   * @param ae required by faces actionListener
   */
  ...
  /**
   * Return the service list for the marketplace portal. This is either the complete, the filtered
   * list a list resulting from the search execution.
   *
   * <p>The first one is also paged and sorted while the second is not (realized in future).
   *
  ...
  public List<Service> searchWithPhrase() {
    ...
    if (searchPhrase != null
        ...
        && (searchResult == null || getServicePagingBean().isNewSearchRequired())) {
        ...
        VOServiceListResult result =
            getSearchServiceInternal()
                .searchServices(
                    ...
                    PerformanceHint.ONLY_FIELDS_FOR_LISTINGS);
        updateServiceListContainsChargeableResellerService(result.getServices());
        searchResult = DEFAULT_VOSERVICE_MAPPER.map(result.getServices());
   ...
   *
   * <p>All defined filter, sorting and paging criteria will be evaluated.
   *
   * @see org.oscm.ui.beans.marketplace.ServicePagingBean
   * @return OUTCOME_SHOW_SERVICE_LIST
   */
   ...
   *
   * @param servicePagingBean paging bean instance
   */
  ...
  LandingpageType loadLandingpageType() {
    if (landingpageType == null) {
      try {
        landingpageType = getLandingpageService().loadLandingpageType(ui.getMarketplaceId());
      } catch (ObjectNotFoundException e) {
        ExceptionHandler.execute(e);
      }
    }
