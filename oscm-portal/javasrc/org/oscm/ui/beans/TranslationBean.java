 ...
 *
 * <p>Author: pock, Enes Sejfi
 *
 * <p>Creation Date: 12.11.2009
 *
 * <p>*****************************************************************************
 */
...
public class TranslationBean extends BaseBean implements Serializable {
  ...
  private static final Log4jLogger logger = LoggerFactory.getLogger(TranslationBean.class);
  ...
  /**
   * Holds the locale specific properties for the selected marketplace. In case the marketplace or
   * the locale changes, it needs to be refreshed.
   */
  private Properties messageProperties = null;

  /**
   * Holds the default messages bundle for the selected locale. It's values will be used in case the
   * value is not part of the messagerProperties.
   */
   ...
   *
   * @param marketplaceBean the new marketplace bean
   */
  ...
  public String getPrivacypolicy() {
    if (privacypolicy == null && getMarketplaceBean().getMarketplaceId() != null) {
  ...
  /**
   * Retrieves the imprint in the locale of the currently logged-in user for the marketplace the
   * user is logged in (or in the default locale if not defined for the user-locale). If no imprint
   * is defined neither in the current user locale nor in the default locale, the empty string is
   * returned.
   *
   * <p>This function is used directly by the JSF pages.
   */
  public String getImprintSessionLocale() {
    ...
    if (bundle != null) {
      ...
      if (imprintInclFallback.trim().length() == 0 && !DEFAULT_LOCALE.equals(getUserLanguage())) {
        ResourceBundle bundleDefaultLocale = getResourceBundle(new Locale(DEFAULT_LOCALE));
        imprintInclFallback = bundleDefaultLocale.getString(KEY_IMPRINT);
      }
    }
  ...
  /**
   * Retrieves the imprint in the locale of the currently logged-in user (or in the default locale
   * if not defined for the user-locale). If no imprint is defined neither in the current user
   * locale nor in the default locale, a descriptive text in the current user locale is returned.
   */
  public String getImprint() {
    ...
    if (getMarketplaceBean().getMarketplaceId() != null) {
      ...
      if (displayValue != null && displayValue.trim().length() == 0 || imprintDefaultLocaleUsed) {
  ...
  /**
   * Retrieves the imprint in the given locale (or in the default locale if not defined for the
   * user-locale). If no imprint is defined neither in the current user locale nor in the default
   * locale, the empty string is returned.
   *
   * @param loc the locale for the imprint
   */
  private String getImprintForLocale(String loc) {
    ...
    if (imprintInclFallback == null
        || imprintInclFallback.trim().length() == 0 && !DEFAULT_LOCALE.equals(loc)) {
      imprintInclFallback = getProperty(KEY_IMPRINT, DEFAULT_LOCALE);
      imprintDefaultLocaleUsed =
          imprintInclFallback != null && imprintInclFallback.trim().length() > 0;
    }
    return imprintInclFallback;
  }
  ...
  /**
   * Resolves the resource bundle for the current locale and returns the description of the imprint
   * field.
   *
  ...
  private String getImprintDescription() {
    if (imprintDescription == null) {
      imprintDescription = getProperty(KEY_IMPRINT_DESCRIPTION, getLocale());
    }
  ...
  public byte[] createExcel() throws IOException {
    ...
    Iterator<Locale> it = getApplicationBean().getSupportedLocalesIterator();
    while (it.hasNext()) {
      Locale locale = it.next();
      defaultProperties.put(
          locale.getLanguage(), ResourceBundle.getBundle(DefaultMessages.class.getName(), locale));
    }
      ...
      localizedProperties.put(
          ...
          getBrandManagementService()
              .loadMessageProperties(
                  getMarketplaceBean().getMarketplaceId(), locale.getLanguage()));
    }
    ...
    ResourceBundle defaultBundle =
        ResourceBundle.getBundle(
            DefaultMessages.class.getName(), fc.getApplication().getDefaultLocale());
  ...
  public String deleteTranslations() throws SaaSApplicationException {
    ...
    try {
      getBrandManagementService()
          .deleteAllMessageProperties(getMarketplaceBean().getMarketplaceId());
    ...
    addMessage(null, FacesMessage.SEVERITY_INFO, INFO_SHOP_TRANSLATIONS_DELETED);
    return null;
  }
   ...
   *
   * @return the logical outcome.
   * @throws IOException Thrown in case the access to the uploaded file failed.
   */
  public String exportTranslations() throws IOException {
    ...
    String filename = sdf.format(Calendar.getInstance().getTime()) + "_Translations.xls";
  ...
  /**
   * Read the translations from the uploaded excel file and store them in the database for the
   * current supplier. Translations for unknown locales will be ignored.
   *
   * @return the logical outcome.
   */
  public String importTranslations() {
    ...
    if (excel == null || excel.getName() == null) {
      addMessage(null, FacesMessage.SEVERITY_ERROR, ERROR_SHOP_TRANSLATIONS_FILEFORMAT);
      return OUTCOME_ERROR;
    }
    ...
    try {
      ...
      try {
        getBrandManagementService()
            .saveMessageProperties(propertiesMap, getMarketplaceBean().getMarketplaceId());
      } catch (SaaSApplicationException e) {
        marketplaceBean.checkMarketplaceDropdownAndMenuVisibility(e);
        throw e;
      }
      ...
      addMessage(
          ...
          new Object[] {String.valueOf(numOfValues), String.valueOf(propertiesMap.size())});
    } catch (ValidationException ex) {
      logger.logError(
          Log4jLogger.SYSTEM_LOG, ex, LogMessageIdentifier.ERROR_PROCESS_TRANSLATION_FILE);
      addMessage(
          null,
      ...
      logger.logError(
          Log4jLogger.SYSTEM_LOG, e, LogMessageIdentifier.ERROR_PROCESS_TRANSLATION_FILE);
      addMessage(null, FacesMessage.SEVERITY_ERROR, ERROR_SHOP_TRANSLATIONS_FILEFORMAT);
      return OUTCOME_ERROR;
    }
  ...
  private void validateTextLength(String key, String text) throws ValidationException {
    if (text.length() > ADMValidator.LENGTH_TEXT) {
      String title = JSFUtils.getText(key, null);

      throw new ValidationException(
          ReasonEnum.LENGTH_TEXT,
          key,
          new Object[] {
            title, Integer.toString(text.length()), Integer.toString(ADMValidator.LENGTH_TEXT)
          });
    }
  }
  ...
  public String save() throws SaaSApplicationException {
    ...
    Properties properties =
        getBrandManagementService()
            .loadMessageProperties(getMarketplaceBean().getMarketplaceId(), locale);
    ...
    addMessage(null, FacesMessage.SEVERITY_INFO, INFO_TRANSLATIONS_SAVED, new Object[] {"3", "1"});
    return null;
  }
  ...
  /**
   * Returns the stage content for the specified locale if existing from the provided list of
   * localized texts. If no matching locale was found, <code>null</code> is returned.
   *
   * @param locale the locale to get the stage for
   * @param stages the {@link VOLocalizedText}s to get the stage from
   * @return the stage or <code>null</code>
   */
  private static String getStageForLocale(String locale, List<VOLocalizedText> stages) {
    if (stages != null) {
  ...
  private void initStage() {
    if (getMarketplaceBean().getMarketplaceId() != null) {
      try {
        stages =
            getBrandManagementService()
                .getMarketplaceStageLocalization(getMarketplaceBean().getMarketplaceId());
      } catch (SaaSApplicationException e) {
        marketplaceBean.checkMarketplaceDropdownAndMenuVisibility(e);
  ...
  /** Initializes the marketplace id and the localized mobile stages if not already done. */
  private void initMobileStage() {
    if (getMarketplaceBean().getMarketplaceId() != null) {
      try {
        mobileStages =
            getBrandManagementService()
                .getMarketplaceMobileStageLocalization(getMarketplaceBean().getMarketplaceId());
      } catch (SaaSApplicationException e) {
        marketplaceBean.checkMarketplaceDropdownAndMenuVisibility(e);
        ExceptionHandler.execute(e);
      }
  ...
  public void setMobileStage(String stage) {
    mobileStageContent = stage;
  }
  ...
  /**
   * Returns the stage for the preview - handling is done lime for the real stage - if the current
   * locale's stage is empty, try to get the english one. If this is also empty, return the default
   * image.
   *
  ...
  /**
   * Returns the value for the passed key in the specified locale. If the value exists in the
   * properties set on the server it'll be used. in case the property is not defined, the default
   * value will be loaded from the corresponding resourcebundle.
   */
  ...
  private Properties getMessagePropertiesFromDb(String mid, String locale) {
    ...
    Properties propsFromLocalizedDataService =
        getLocalizedDataService().loadMessageProperties(locale);
    if (propsFromLocalizedDataService != null) {
      props.putAll(propsFromLocalizedDataService);
    }
    Properties propsFromBrandMgmtService =
        getBrandManagementService().loadMessageProperties(mid, locale);
  ...
  private boolean checkIfDefaultText(String value) {
    ...
    String defaultTextLang1 = getProperty(KEY_IMPRINT_DESCRIPTION, getLocale());
    defaultTexts.add(defaultTextLang1);
    String defaultTextLang2 = JSFUtils.getText(KEY_IMPRINT_DESCRIPTION, null);
  ...
  private ValidatorException constructValidatorException(
    ...
    ValidationException ex =
        new ValidationException(ValidationException.ReasonEnum.URL, label, null);
    ...
    return new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, text, null));
  }

  public void validateImprint(
      final FacesContext context, final UIComponent component, final Object valueObj) {
