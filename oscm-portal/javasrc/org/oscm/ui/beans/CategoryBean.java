...
public class CategoryBean extends BaseBean implements Serializable {
  ...
  public List<CategoryRow> getCategoriesRows() {
    if (categoriesRows == null) {
      ...
      if (marketplaceBean.getMarketplaceId() != null
          && marketplaceBean.getMarketplaceId().trim().length() > 0
          && selectedLocale != null
  ...
  private void loadCategories(String selectedLocale) {
    ...
    List<VOCategory> fetchedCategories =
  ...
  /**
   * reset categories for marketplace so that fresh data is loaded if currently logged in user wants
   * to preview his changes.
   */
  ...
  public String getDisplayAsColumnHeader() {
    ...
    return JSFUtils.getText(
        "category.displayLocale", new Object[] {chooselanguage.getDisplayLanguage(currentLocale)});
  }
  ...
  /**
   * Returns the message shown in the confirmation box indicating that some of the categories to be
   * deleted have assigned services.
   *
      ...
      for (VOCategory voCategory : toDelete) {
        if (numOfMessages < MAX_NUMBER_OF_APPENDED_MESSAGES) {
          ...
          if (categoryMsg.length() > 0) {
            if (numOfMessages > 0) {
              buf.append(JSFUtils.getText("confirm.category.comma", null));
            }
            buf.append(categoryMsg);
            numOfMessages++;
          }
        } else {
          buf.append(JSFUtils.getText("confirm.category.points", null));
          break;
        }
      }
      return JSFUtils.getText("confirm.categories.delete", new Object[] {buf.toString()});
    }
  ...
  public String getNumOfServicesToDisplay(VOCategory voCategory) {
    if (voCategory != null) {
      ...
      List<VOService> services =
          getCategorizationService().getServicesForCategory(voCategory.getKey());
