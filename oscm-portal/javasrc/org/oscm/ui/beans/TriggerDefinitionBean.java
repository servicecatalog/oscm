...
public class TriggerDefinitionBean extends BaseBean implements Serializable {
  ...
  // -------------------------------------------------------------------------
  /**
   * Get the existing trigger definitions for the current organization. If not initialized, the
   * values will be read from server.
   *
   * @return the list of {@link VOTriggerDefinition}s
   */
  public List<VOTriggerDefinition> getTriggerDefinitionList() {

    if (this.triggerDefinitionList == null
        && (this.isLoggedInAndAdmin() || this.isLoggedInAndPlatformOperator())) {
  ...
  public SelectItem[] getTriggerTargetTypeValues() {
    ...
    for (final TriggerTargetType targetType : TriggerTargetType.values()) {
      items[i++] =
          new SelectItem(targetType, JSFUtils.getText("TriggerTargetType." + targetType, null));
    }
    return items;
  }
