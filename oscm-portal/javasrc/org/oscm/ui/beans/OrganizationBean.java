...
public class OrganizationBean extends BaseBean implements Serializable {
  ...
  private static final Log4jLogger logger = LoggerFactory.getLogger(OrganizationBean.class);
        ...
        try {
          VOOrganization vo =
              getAccountingService()
                  .getMyCustomer(customer.getVOOrganization(), getUserFromSession().getLocale());
          voCustomer = vo;
   ...
   *
   * @param organizationId Organization ID of needed customer.
   * @param voOrganization VO for set for needed organization.
   */
  public void updateSelectedCustomer(String organizationId, VOOrganization voOrganization) {
   ...
   * @throws DeletionConstraintException Thrown if the organization has active subscriptions
   */
  public String delete() throws DeletionConstraintException {
    ...
    try {
      ...
      logger.logError(
          Log4jLogger.SYSTEM_LOG, e, LogMessageIdentifier.ERROR_ORGANIZATION_DELETION_FAILED);
      addMessage(null, FacesMessage.SEVERITY_ERROR, ERROR_DELETE_USER_FROM_EXPIRED_SUBSCRIPTION);
      return "";
    }
    if (logger.isDebugLoggingEnabled()) {}
  ...
  public String save() throws SaaSApplicationException {
    ...
    if (user != null) {
      ...
      addMessage(
          ...
          getUserFromSession().getOrganizationId());
    }
  ...
  public List<Organization> getSuppliersForTechnicalService()
    ...
    if (suppliersForTechnicalService == null && voTechnicalService != null) {
      ...
      List<VOOrganization> suppliers =
          getAccountingService().getSuppliersForTechnicalService(voTechnicalService);
    ...
    if (suppliersForTechnicalService != null && suppliersForTechnicalService.size() > 1) {
  ...
  boolean isAnySupplierSelectedForDeletion()
      throws ObjectNotFoundException, OperationNotPermittedException {
    for (Organization org : getSuppliersForTechnicalService()) {
      if (org.isSelected()) return true;
    }
    return false;
  }
  ...
  public String addSuppliersForTechnicalService()
    ...
    if (voTechnicalService != null && !isBlank(supplierIdToAdd)) {
      ...
      try {
        getAccountingService().addSuppliersForTechnicalService(voTechnicalService, ids);
      } finally {
        suppliersForTechnicalService = null;
        instanceIdsForSuppliers = null;
      }
      addMessage(null, FacesMessage.SEVERITY_INFO, INFO_SUPPLIER_ADDED, supplierIdToAdd);
      resetToken();
  ...
  public String removeSuppliersFromTechnicalService()
    ...
    if (supplierIds.length > 0 && voTechnicalService != null) {
      ...
      if (isConfirmed()) {
        getAccountingService().removeSuppliersFromTechnicalService(voTechnicalService, ids);

        suppliersForTechnicalService = null;
        instanceIdsForSuppliers = null;

        addMessage(null, FacesMessage.SEVERITY_INFO, INFO_SUPPLIER_REMOVED, supplierIdToAdd);

        resetToken();
      } else {
        List<String> instanceIds = getProvisioningService().getInstanceIdsForSellers(ids);
        if (instanceIds != null) {
  ...
  protected String[] getSelectedOrganizationIds() {
    if (suppliersForTechnicalService == null || suppliersForTechnicalService.isEmpty()) {
      return new String[] {};
    }
  ...
  public User getCustomerUserToAdd() throws MarketplaceRemovedException {
    if (customerUserToAdd == null) {
      ...
      customerUserToAdd.getVOUserDetails().setTenantId(sessionBean.getTenantID());
    }
    return customerUserToAdd;
  }

  public String addCustomer()
      throws NonUniqueBusinessKeyException, OrganizationAuthoritiesException, ValidationException,
          MailOperationException, ObjectNotFoundException, OperationPendingException {

    try {
      Properties properties = null;
      if (ldapManaged && organizationProperties != null) {
        properties = PropertiesLoader.loadProperties(organizationProperties.getInputStream());
      }
      VOUserDetails user = null;
      if (!isInternalAuthMode()) {
        getManageTenantService()
            .validateOrgNameUniquenessInTenant(customerToAdd.getName(), sessionBean.getTenantID());
        user =
            getIdService()
                .loadUserDetailsFromOIDCProvider(
                    customerUserToAdd.getUserId(), sessionBean.getTenantID());

        String groupId =
            getIdService()
                .createAccessGroupInOIDCProvider(
                    sessionBean.getTenantID(), customerToAdd.getName());
        customerToAdd.setOidcGroupId(groupId);
        getIdService()
            .addMemberToAccessGroupInOIDCProvider(groupId, sessionBean.getTenantID(), user);
      } else {
      ...
      VOOrganization org =
          getAccountingService()
              .registerKnownCustomer(
                  customerToAdd, user, LdapProperties.get(properties), marketplaceId);
      ...
      addInfoOrProgressMessage(org != null, INFO_ORGANIZATION_CREATED, orgId);
    } catch (IOException e) {
      logger.logError(Log4jLogger.SYSTEM_LOG, e, LogMessageIdentifier.ERROR_ADD_CUSTOMER);
      ...
      logger.logError(
          ...
          "Can not load user from OIDC Provider");
      ...
      logger.logError(
          ...
          e,
          LogMessageIdentifier.ERROR_ADD_CUSTOMER,
          "Error while trying to get the tenand id");
      addMessage(null, FacesMessage.SEVERITY_ERROR, ERROR_UPLOAD);
    }
  ...
  public String showCustomerBillingData() throws IOException {
    ...
    String filename = sdf.format(Calendar.getInstance().getTime()) + "_BillingData.xml";
   ...
   *
   * @return <code>true</code> if billing data is available otherwise <code>false</code>.
   */
  ...
  void initializeGroups() {
    ...
    List<POUserGroup> groups =
        getUserGroupService().getUserGroupsForUserWithoutDefault(currentUser.getKey());
    for (POUserGroup group : groups) {
      groupsToDisplay.append(group.getGroupName());
      groupsToDisplay.append(", ");
    }
    if (groupsToDisplay.length() > 0) {
      groupsToDisplay.replace(groupsToDisplay.length() - 2, groupsToDisplay.length(), "");
    }
  ...
  public String updateCustomer() throws SaaSApplicationException {
    ...
    if (voDiscount != null) {
      ...
      long currentTimeMonthYear = getTimeInMillisForFirstDay(System.currentTimeMillis());
      if (begin == null) {
      ...
      if (end != null) {
        ...
        long discountEndTimeMonthYear = getTimeInMillisForFirstDay(end.longValue());

        // end discount can not be less then current date
        if (currentTimeMonthYear > discountEndTimeMonthYear) {
          addMessage(COMPONENT_THRU_ID, FacesMessage.SEVERITY_ERROR, ERROR_DISCOUNT_DATE_FUTURE);
          return OUTCOME_ERROR;
        }

        if (begin.longValue() > end.longValue()) {
          addMessage(COMPONENT_THRU_ID, FacesMessage.SEVERITY_ERROR, ERROR_DISCOUNT_DATE_BEFORE);
          return OUTCOME_ERROR;
        }
      }
    ...
    VOOrganization org = getAccountingService().updateCustomerDiscount(voOrganization);
    ...
    addMessage(
        null, FacesMessage.SEVERITY_INFO, INFO_ORGANIZATION_UPDATED, org.getOrganizationId());
   ...
   *
   * @param timeInMilis Time of any day of month.
  ...
  public List<UdaRow> getOrganizationUdas() {
    if (customerUdaRows == null) {
      ...
      if (selectedCustomer != null) {
        ...
        List<VOUdaDefinition> customerUdaDefinitions = udaBean.getForType(UdaBean.CUSTOMER);
        return UdaRow.getUdaRows(customerUdaDefinitions, new ArrayList<VOUda>());
      }
    }
  ...
  public String getInstanceIdsForSuppliers() {
    if (instanceIdsForSuppliers == null) return "";
  ...
  public boolean isCustomerOrganization() {
    ...
    return !u.isSupplier() && !u.isTechnologyProvider() && !u.isReseller() && !u.isBroker();
  }
