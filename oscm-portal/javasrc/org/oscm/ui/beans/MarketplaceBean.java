...
public class MarketplaceBean extends BaseBean implements Serializable {
  ...
  private MarketplaceConfiguration getConfig() {
    return marketplaceService.getCachedMarketplaceConfiguration(BaseBean.getMarketplaceIdStatic());
  }

  /**
   * Retrieve the list of marketplaces available for the supplier to publish the services on.
   *
  ...
  /**
   * Publishes the provided service to the selected marketplace. In case no marketplace is selected,
   * the service will be unpublished .
   *
   * @param service the service to (un)publish
   * @param categories list of assigned categories
   * @return a value object with the updated service definition
   * @throws SaaSApplicationException
   */
  VOServiceDetails publishService(ServiceDetails service, List<VOCategory> categories)
      throws SaaSApplicationException {
    ...
    try {
      return getMarketplaceService()
          .publishService(service.getVoServiceDetails(), Arrays.asList(voCE));
  ...
  public String marketplaceChangedForManageSeller() {
    ...
    if (marketplaceId == null || marketplaceId.equals("0")) {
      ...
      if ((result = this.validateMarketplaceId(marketplaceId)) && this.marketplaces != null) {
        this.marketplace = this.findMarketplace(this.marketplaces, this.marketplaceId);
      }
    }
  ...
  /**
   * Returns the message identifiers for message properties files, depending on the marketplace
   * type.
   *
  ...
  /**
   * Returns the list of catalog entries to all marketplaces the provided service is published on.
   *
   * @param service the service to get the marketplaces it is published on for
   * @return the list of marketplaces
  ...
  public List<Marketplace> getMarketplacesForOperator() {
    if (marketplaces == null) {
      marketplaces = new ArrayList<Marketplace>();
      List<VOMarketplace> voMarketplaces = getMarketplaceService().getMarketplacesForOperator();
      Collections.sort(voMarketplaces, new MarketplacesComparator());
      for (VOMarketplace mp : voMarketplaces) {
        marketplaces.add(new Marketplace(mp));
      }
  ...
  /**
   * Returns a list of all marketplaces the logged in user is allowed to manage (i.e. that he owns).
   *
  ...
  public List<Marketplace> getMarketplacesOwned() {
    if (this.marketplaces == null) {
      ...
      List<VOMarketplace> voMarketplaces = this.getMarketplaceService().getMarketplacesOwned();
      Collections.sort(voMarketplaces, new MarketplacesComparator());
  ...
  /**
   * Get the current VOMarketplace object. If it doesn't exist a default VOMarketplace object will
   * be created.
   *
  ...
  /**
   * Returns the VO for the currently selected marketplace ID. If there is no ID selected <code>null
   * </code> will be returned.
   *
   * <p>Note: This function expects that the select ID belongs to a VO which was previously acquired
   * by the <code>getGlobalMarketplaces</code> or <code>getMarketplaces</code> function. There will
   * be no additional server call to obtain the VO for the selected ID.
   *
   ...
   *
   * <p>This function can be used to control the disable state of UI components.
   *
   * @return <code>true</code> in case a marketplace id is selected, otherwise <code>false</code>
   */
  ...
  public String deleteMarketplace() throws SaaSApplicationException {
    try {
      if (isTokenValid()) {
        getMarketplaceService().deleteMarketplace(marketplace.getMarketplaceId());
    ...
    try {
      if (suppliersForMarketplace == null) {
        ...
        if (marketplaceId != null) {
          ...
          if (isOpen()) {
            suppliers = getMarketplaceService().getBannedOrganizationsForMarketplace(marketplaceId);
          } else {
            suppliers = getMarketplaceService().getOrganizationsForMarketplace(marketplaceId);
          }
  ...
  public String addSupplierMarketplaceRelation() throws SaaSApplicationException {

    if (!isTokenValid()) {
      return OUTCOME_SUCCESS;
    }
    if (!Strings.isEmpty(supplierIdToAdd) && !Strings.isEmpty(marketplaceId)) {
      List<String> ids = new ArrayList<String>();
      ids.add(supplierIdToAdd);
      try {
        if (isOpen()) {
          getMarketplaceService().banOrganizationsFromMarketplace(ids, marketplaceId);
          addMessage(null, FacesMessage.SEVERITY_INFO, INFO_SUPPLIER_BANNED, supplierIdToAdd);
        } else {
          getMarketplaceService().addOrganizationsToMarketplace(ids, marketplaceId);
  ...
  public String removeSupplierMarketplaceRelation() throws SaaSApplicationException {
    ...
    if (ids.size() > 0) {
      try {
        if (isOpen()) {
          getMarketplaceService().liftBanOrganizationsFromMarketplace(ids, marketplaceId);
          addMessage(null, FacesMessage.SEVERITY_INFO, INFO_SUPPLIER_BANLIFTED);
        } else {
          getMarketplaceService().removeOrganizationsFromMarketplace(ids, marketplaceId);
          addMessage(null, FacesMessage.SEVERITY_INFO, INFO_SUPPLIER_REMOVED);
        }
      } catch (SaaSApplicationException e) {
        checkMarketplaceDropdownAndMenuVisibility(e);
        throw e;
  ...
  /**
   * @return true, if there is a marketplace with a list of suppliers and one or more elements of
  ...
  private List<String> getSelectedOrganizationIds() {
    ...
    if (suppliersForMarketplace == null || suppliersForMarketplace.isEmpty()) {
      return list;
    }
  ...
  /**
   * Resets the cached marketplaces and the menu visibility upon certain server-side exceptions.
   * Should be invoked after e.g. concurrent deletion or reassignment of marketplace. Can be
   * enforced by passing <code>null</code> as argument.
   */
  protected void checkMarketplaceDropdownAndMenuVisibility(SaaSApplicationException ex) {
    if (ex == null
        || (ex instanceof ObjectNotFoundException
            && ((ObjectNotFoundException) ex).getDomainObjectClassEnum() == ClassEnum.MARKETPLACE)
