...
public class ServiceBean extends BaseBean implements Serializable {
  ...
  private static final Log4jLogger logger = LoggerFactory.getLogger(ServiceBean.class);
  ...
  private final ImageUploader imageUploader = new ImageUploader(ImageType.SERVICE_IMAGE);
  ...
  /** Sort available technical services Ids in locale-sensitive alphabetical order. */
  private class AvailableTechServicesIdsComparator implements Comparator<VOTechnicalService> {
    Collator collator = Collator.getInstance();

    @Override
    public int compare(VOTechnicalService svc1, VOTechnicalService svc2) {
      return collator.compare(svc1.getTechnicalServiceId(), svc2.getTechnicalServiceId());
    }
  }
   ...
   * @throws SaaSApplicationException Thrown from the business logic.
  ...
  public String create() throws SaaSApplicationException {
    ...
    serviceForCreation =
        new ServiceDetails(cleanupParameter(serviceForCreation.getVoServiceDetails()));
    ...
    try {
      ...
      if (ClassEnum.MARKETPLACE == e.getDomainObjectClassEnum()) {
        addMessage(
            ...
            ERROR_SERVICE_SAVED_MARKETPLACE_DELETED,
            new String[] {
    ...
    if (!addedMsg) {
      addMessage(
          null, FacesMessage.SEVERITY_INFO, INFO_SERVICE_CREATED, selectedService.getServiceId());
    }
  ...
  private void rewritePasswordsValues() {
    for (ParameterRow parameterRow : parameterRows) {
      if (!parameterRow.getParameterDefinition().isValueTypePWD()
          & !parameterRow.isPasswordType()) {
        continue;
      }
      if (parameterRow.getPasswordValueToStore() == null
          || !parameterRow.getPasswordValueToStore().trim().equals(HIDDEN_PWD)) {
  ...
  public String delete() throws SaaSApplicationException {
    ...
    try {
      getProvisioningService().deleteService(selectedService.getVoServiceDetails());
      addMessage(
          null, FacesMessage.SEVERITY_INFO, INFO_SERVICE_DELETED, selectedService.getServiceId());
      ...
      String localizedStatus =
          JSFUtils.getText("ServiceStatus." + sse.getMessageParams()[1], params);
      addMessage(null, FacesMessage.SEVERITY_ERROR, CANNOT_DELETE_ACTIVE_SERVICE, localizedStatus);
      return OUTCOME_ERROR;
    }
  ...
  public boolean isDeleteDisabled() {
    ...
    try {
      if (getSelectedService() != null && getSelectedService().getVoServiceDetails() != null) {
        deleteAllowed =
            (getProvisioningService().statusAllowsDeletion(selectedService.getVoServiceDetails()));
      }
   ...
   * @param parameterType the parameter type of the searched parameter
   * @param parameterId the parameter id of the searched parameter
   * @return the found parameter value object or null if no parameter value object is found.
   */
  private VOParameter findParameter(
      final List<VOParameter> list, final ParameterType parameterType, final String parameterId) {
    if (list != null) {
  ...
  public List<VOTechnicalService> getAvailableTechServices() {
    if (availableTechServices == null) {
      try {
        availableTechServices =
            getProvisioningService().getTechnicalServices(OrganizationRoleType.SUPPLIER);
        Collections.sort(availableTechServices, new AvailableTechServicesIdsComparator());
  ...
  public VOServiceLocalization getLocalization()
    ...
    if (getSelectedService() == null || getSelectedService().getVoServiceDetails() == null) {
      ...
      localization =
          getProvisioningService()
              .getServiceLocalization(getSelectedService().getVoServiceDetails());
      List<Locale> supportedLocales = getApplicationBean().getSupportedLocaleList();
      localization.setNames(LocaleUtils.trim(localization.getNames(), supportedLocales.iterator()));
      localization.setDescriptions(
          LocaleUtils.trim(localization.getDescriptions(), supportedLocales.iterator()));
      localization.setShortDescriptions(
          LocaleUtils.trim(localization.getShortDescriptions(), supportedLocales.iterator()));
      localization.setCustomTabNames(
          LocaleUtils.trim(localization.getCustomTabNames(), supportedLocales.iterator()));
    }
    return localization;
  }
  ...
  public List<Service> getPossibleCompatibleServices() {
    ...
    if (possibleCompatibleServices == null) {
      try {
        final String mpName = getMarketplaceBean().getSelectedMarketplaceName();
        List<VOCompatibleService> list =
            getProvisioningService().getPotentialCompatibleServices(service.getVoServiceDetails());
        Vo2ModelMapper<VOCompatibleService, Service> mapper =
            new Vo2ModelMapper<VOCompatibleService, Service>() {
              @Override
  ...
  private List<VOService> getServices(PerformanceHint performanceHint) {
    if (services == null) {
      services = getProvisioningServiceInternal().getSuppliedServices(performanceHint);
    }
    return services;
  }
  ...
  /**
   * Merge the set parameter of the selected service with the possible parameter definitions and
   * initialize the paramtersRow array.
   */
  private void prepareParameters() {
    ...
    if (selectedService.getTechnicalService().getParameterDefinitions() != null) {
      for (VOParameterDefinition paramDef :
          selectedService.getTechnicalService().getParameterDefinitions()) {
        VOParameter parameter =
            findParameter(
   ...
   *
   * @param parameters parameters for the array.
   */
  private void initParameterRows(List<VOParameter> parameters, boolean initDefault) {
    parameterRows = new ArrayList<ParameterRow>();
    for (VOParameter voParameter : parameters) {
      VOParameterDefinition parameterDefinition = voParameter.getParameterDefinition();
      ...
      if (parameterDefinition.getValueType() == ParameterValueType.ENUMERATION) {
        int optionIndex = 0;
        for (VOParameterOption option : parameterDefinition.getParameterOptions()) {
          ParameterRow optionRow = new ParameterRow(voParameter, option, initDefault);
  ...
  public String saveLocalizedValues() {
    ...
    if (selectedService != null) {
      ...
      selectedService.setName(LocaleUtils.get(localization.getNames(), locale));
      selectedService.setShortDescription(
          LocaleUtils.get(localization.getShortDescriptions(), locale));
      selectedService.setDescription(LocaleUtils.get(localization.getDescriptions(), locale));
      selectedService.setCustomTabName(LocaleUtils.get(localization.getCustomTabNames(), locale));
    }
    dirty = true;
    menuBean.setCurrentPageLink(MenuBean.LINK_SERVICE_EDIT);
  ...
  /**
   * Transfers the name and description currently set on the service to the corresponding localized
   * texts.
   *
  ...
  private void setServiceAttributesToLocalization()
    ...
    if (selectedService != null) {
      String locale = getUserFromSession().getLocale();
      LocaleUtils.set(getLocalization().getNames(), locale, selectedService.getName());
      LocaleUtils.set(
          getLocalization().getDescriptions(), locale, selectedService.getDescription());
      LocaleUtils.set(
          getLocalization().getShortDescriptions(), locale, selectedService.getShortDescription());
      LocaleUtils.set(
          getLocalization().getCustomTabNames(), locale, selectedService.getCustomTabName());
    }
  }
  ...
  public String saveUpgradeOptions() throws SaaSApplicationException {
    ...
    getProvisioningService()
        .setCompatibleServices(selectedService.getVoServiceDetails(), compatibleServices);
  ...
  public void setSelectedServiceKey(final long key) {
    ...
    try {
      ...
      VOServiceDetails serviceDetails = getProvisioningService().getServiceDetails(service);
  ...
  private void updateMarketplacePublishedTo() throws SaaSApplicationException {
    List<VOCatalogEntry> list =
        marketplaceBean.getMarketplacesForService(selectedService.getVoServiceDetails());
    ...
    if (!list.isEmpty()) {
      ...
      if (list.get(0).getMarketplace() != null) {
        marketplaceBean.setMarketplaceId(ce.getMarketplace().getMarketplaceId());
  ...
  public void setSelectedServiceKeyReadOnly(@SuppressWarnings("unused") final long key) {}
  ...
  public void setSelectedTechServiceKey(final long key) {
    ...
    for (VOTechnicalService techService : getAvailableTechServices()) {
      if (techService.getKey() == key) {
        ...
        serviceForCreation.setDescription(selectedTechService.getTechnicalServiceDescription());
        serviceForCreation
            .getVoServiceDetails()
            .setTechnicalId(selectedTechService.getTechnicalServiceId());
        serviceForCreation.getVoServiceDetails().setTechnicalService(selectedTechService);
        List<VOParameter> parameters = new ArrayList<VOParameter>();
        if (selectedTechService.getParameterDefinitions() != null) {
          for (VOParameterDefinition paramDef : selectedTechService.getParameterDefinitions()) {
            parameters.add(new VOParameter(paramDef));
          }
        }
        serviceForCreation.getVoServiceDetails().setParameters(parameters);
  ...
  public String update() throws SaaSApplicationException {
    ...
    rewritePasswordsValues();
    selectedService = new ServiceDetails(cleanupParameter(selectedService.getVoServiceDetails()));
    ...
    try {
      selectedService =
          new ServiceDetails(marketplaceBean.publishService(selectedService, categories));
      selectedService.setPublicService(isPublicService);
      setMarketplaceCategories(marketplaceBean.getMarketplaceId());
      updateMarketplacePublishedTo();
    } catch (ObjectNotFoundException e) {
    ...
    if (!addedMsg) {
      addMessage(
          null, FacesMessage.SEVERITY_INFO, INFO_SERVICE_SAVED, selectedService.getServiceId());
    }
    ...
    if (saveLocalizedValues) {
      setServiceAttributesToLocalization();
      getProvisioningService()
          .saveServiceLocalization(selectedService.getVoServiceDetails(), localization);
    ...
    if (servicesForDeActivation == null) {
      ...
      try {
        ...
        for (VOService service : services) {
          ...
          List<VOCatalogEntry> catEntries =
              getMarketplaceServiceInternal()
                  .getMarketplacesForService(service, PerformanceHint.ONLY_FIELDS_FOR_LISTINGS);
        ...
        Collections.sort(servicesForDeActivation, new DefaultSortingOfDeActivations());

        // Append customer specific services (at correct position)
        List<VOCustomerService> custServices =
            getProvisioningService().getAllCustomerSpecificServices();
        ListIterator<Service> iterList = servicesForDeActivation.listIterator();
        while (iterList.hasNext()) {
          Service prod = iterList.next();
          for (VOCustomerService voCustomerService : custServices) {
            if (!voCustomerService.getServiceId().equals(prod.getServiceId())) {
              continue;
            }
            CustomerService custProd = new CustomerService(voCustomerService, prod);
  ...
  public String doDeActivate() throws SaaSApplicationException {
    ...
    try {
      // invoke update
      List<VOService> result = getProvisioningService().setActivationStates(updateList);
      boolean operationCompleted = (result.size() == updateList.size());
      // display success
      addInfoOrProgressMessage(operationCompleted, INFO_SERVICE_STATE_SAVED, null);
    } catch (ServiceStateException e) {

      String causedStatus = e.getMessageParams()[1];
      if (causedStatus.equals(ServiceStatus.DELETED.name())) {
        addMessage(null, FacesMessage.SEVERITY_ERROR, SERVICE_NO_LONGER_EXISTS);
        return null;
      } else throw e;

    } catch (RuntimeException e) {
      addMessage(null, FacesMessage.SEVERITY_ERROR, SERVICE_NOT_ALIVE);
    } finally {
  ...
  /**
   * Remove the parameters which are not configurable and which have no value from the parameter
   * list of the service
   *
  ...
  private static VOServiceDetails cleanupParameter(VOServiceDetails service) {
    if (service != null) {
      for (Iterator<VOParameter> it = service.getParameters().iterator(); it.hasNext(); ) {
  ...
  public String copy() throws SaaSApplicationException {
    ...
    try {
      selectedService =
          new ServiceDetails(
              getProvisioningService().copyService(service.getVoServiceDetails(), newServiceId));
      sessionBean.setSelectedServiceKeyForSupplier(Long.valueOf(selectedService.getKey()));
      updateMarketplacePublishedTo();
      prepareParameters();
      addMessage(
  ...
  public String getNewServiceId() {
    ServiceDetails service = getSelectedService();
    if (newServiceId == null && service != null && service.getVoServiceDetails() != null) {
      newServiceId = new IdGenerator("Copy of ", service, getServices()).generateNewId();
    }
    return newServiceId;
  }
  ...
  private class DefaultSortingOfDeActivations implements Comparator<Service> {
    ...
    public int compare(Service svc1, Service svc2) {
      int rc = collator.compare(svc1.getMarketplace(), svc2.getMarketplace());
      if (rc == 0) rc = collator.compare(svc1.getTechnicalId(), svc2.getTechnicalId());
      if (rc == 0)
        rc = collator.compare(svc1.getServiceIdToDisplay(), svc2.getServiceIdToDisplay());
      return rc;
    }
  }
    ...
    if (partOfUpgradePath == null) {
      ...
      if (s != null) {
        try {
          partOfUpgradePath =
              Boolean.valueOf(
                  getProvisioningService().isPartOfUpgradePath(s.getVoServiceDetails()));
        } catch (SaaSApplicationException e) {
          ExceptionHandler.execute(e);
  ...
  public void marketplaceChanged(ValueChangeEvent event) {
    ...
    if (ce != null
        ...
        && ce.getMarketplace().getMarketplaceId().equals(selectedMarketplaceId)) {
      checkCategorySelection(ce.getCategories());
    }
  }
  ...
  private void setMarketplaceCategories(String selectedMarketplaceId) {
    ...
    if (selectedMarketplaceId != null) {
      allMarketplaceCategories =
          this.getCategorizationService().getCategories(selectedMarketplaceId, getUserLanguage());
    }
  ...
  private final Comparator<CategoryRow> categoriesComparator =
      new Comparator<CategoryRow>() {

        @Override
        public int compare(CategoryRow arg0, CategoryRow arg1) {
          return Category.getDisplayName(arg0.getCategory())
              .compareTo(Category.getDisplayName(arg1.getCategory()));
        }
      };

  private List<VOCategory> selectedCategoryToVOUnwrapper(List<CategoryRow> selectedCategories) {
  ...
  public boolean isShowConfirm() {
    ...
    for (VOService serviceToUpDowngrade : compatibleServices) {
      ...
      if ((servicePMType.equals(PriceModelType.PER_UNIT)
              && !serviceToUpDowngradePMType.equals(PriceModelType.FREE_OF_CHARGE))
          || (serviceToUpDowngradePMType.equals(PriceModelType.PER_UNIT)
              && !servicePMType.equals(PriceModelType.FREE_OF_CHARGE))) {
