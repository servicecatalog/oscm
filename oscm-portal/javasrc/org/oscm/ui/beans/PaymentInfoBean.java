 ...
 * <p>Author: weiser
 *
 * <p>Creation Date: 05.07.2011
 *
 * <p>Completion Time: 06.07.2011
 *
 * <p>*****************************************************************************
 */
...
// TODO: class should be refactored and creation of payment info extracted, like
// PaymentInfoEditBean.
public class PaymentInfoBean extends BaseBean implements Serializable {
  ...
  private Comparator<VOPaymentType> paymentTypeComparator =
      new Comparator<VOPaymentType>() {
  ...
  /**
   * Set the the selected payment type for a new payment option created by the administrator.
   *
   * <p>In distinction to the supplier, the administrator selects from all available payment types.
   *
  ...
  /** Return the selected payment type for a new payment option created by the administrator. */
  public String getSelectedPaymentTypeId() {
    if (selectedPaymentType != null) {
      return selectedPaymentType.getPaymentTypeId();
    }
    // By default the first is selected.
    return (getPaymentTypeChoice().size() > 0)
        ? getPaymentTypeChoice().iterator().next().getPaymentTypeId()
        : "";
  }
   ...
   *
   * <p>In context of subscriptions only those types are available which are enabled for the
   * supplier.
   */
  ...
  /**
   * Return the list of payment types which are available in the given choice context.
   *
   * <p>In context of subscriptions only those types are available which are enabled for the
   * supplier.
   *
  ...
  public Collection<VOPaymentType> getPaymentTypeChoice() {
    if (context.equals(CONTEXT_SUBSCRIPTION)) return getAvailablePaymentTypesForCreation();
  ...
  private String generateHeaderText(String msgPrefix, String msgSuffix) {
    ...
    return JSFUtils.getText(buffer.toString(), new Object[] {""});
  }
  ...
  /**
   * Tries to read the payments that are enabled (intersection of product and customer). The
   * resulting list is sorted by the payment type id. The function returns null in case the
   * serviceKey is not set.
   *
  ...
  public Collection<VOPaymentType> getEnabledPaymentTypes() {
    ...
    if (enabledPaymentTypes == null || serviceKeyForPaymentChanged(serviceKeyForPayment)) {
      currentServiceKeyForPayment = serviceKeyForPayment;
      if (serviceKeyForPayment == null) {
        return null;
      }
      enabledPaymentTypes = new HashSet<>();
      enabledPaymentTypes.addAll(
          getEnabledPaymentTypes(serviceKeyForPayment, getAccountingService()));
    }
  ...
  public Collection<VOPaymentType> getEnabledPaymentTypes(Long key, AccountService accountService) {
    ...
    try {
      ...
      enabledPaymentTypes.addAll(accountService.getAvailablePaymentTypesFromOrganization(key));
  ...
  /**
   * Returns the list of payment types a customer can define payment information for.
   *
  ...
  public Collection<VOPaymentType> getAvailablePaymentTypes() {
    if (availablePaymentTypes == null) {
      availablePaymentTypes =
          new ArrayList<VOPaymentType>(getAccountingService().getAvailablePaymentTypes());
    }
    return availablePaymentTypes;
  }

  /**
   * Returns all payment types for which a costumer can create new payment infos in the context of
   * an subscription. The returned collection reflect all payment types the supplier has enabled for
   * this customer, without INVOICE. The function returns null in case the suppliedId is not set.
   *
   * @return a list of payment type VOSs.
   */
  ...
  private List<VOPaymentType> getAvailablePaymentTypesForCreation(
      Collection<VOPaymentType> enabledPaymentTypes) {
  ...
  public List<VOPaymentType> getAvailablePaymentTypesForCreation(
      long key, AccountService accountService) {
    ArrayList<VOPaymentType> availablePaymentTypesForCreation = new ArrayList<>();
    Collection<VOPaymentType> enabledPaymentTypes =
        getEnabledPaymentTypes(Long.valueOf(key), accountService);
    for (VOPaymentType pt : enabledPaymentTypes) {
      if (!pt.getPaymentTypeId().equals("INVOICE")) {
        availablePaymentTypesForCreation.add(pt);
      }
  ...
  public List<VOPaymentInfo> getPaymentInfos() {
    if (allPaymentInfos == null) {
      allPaymentInfos = getAccountingService().getPaymentInfosForOrgAdmin();
    }
  ...
  public String handlePspResult() {
    ...
    if (SUCCESS.equals(pspResult)) {
      ...
      addMessage(null, FacesMessage.SEVERITY_ERROR, "info.pspregistrationresult.cancel");
      outcome = OUTCOME_PSP_ERROR;
    } else if (FAILURE.equals(pspResult)) {
      addMessage(null, FacesMessage.SEVERITY_ERROR, "error.pspregistrationresult.failure");
      outcome = OUTCOME_PSP_ERROR;
    ...
    if (OUTCOME_PSP_SUCCESS.equals(outcome)) {
      ...
      selectedPaymentInfoForSubscription = findCreatedPaymentInfoById(paymentInfo.getId());
    } else {
      resetCachedPaymentInfo();
    }
    // reset page to edit
  ...
  public String getPaymentRegistrationLink() {
    if (paymentRegistrationLink == null) {
      try {
        paymentRegistrationLink = getPaymentService().determineRegistrationLink(getPaymentInfo());
      } catch (SaaSApplicationException e) {
        ExceptionHandler.execute(e);
      }
    }
  ...
  public List<VOPaymentInfo> getPaymentInfosForSubscription() {
    ...
    if (paymentInfosForSubscription == null || serviceKeyForPaymentChanged(serviceKeyForPayment)) {
      currentServiceKeyForPayment = serviceKeyForPayment;
      paymentInfosForSubscription = new ArrayList<>();
      if (currentServiceKeyForPayment != null) {
        paymentInfosForSubscription =
            getPaymentInfosForSubscription(
                serviceKeyForPayment.longValue(), getAccountingService());
      }
    }
    Collections.sort(paymentInfosForSubscription, new PaymentInfoComparator());
  ...
  public List<VOPaymentInfo> getPaymentInfosForSubscription(
    ...
    try {
      ...
      types = accountService.getAvailablePaymentTypesFromOrganization(Long.valueOf(key));

      List<VOPaymentInfo> infos = accountService.getPaymentInfos();
  ...
  public boolean isPaymentVisible(UserBean userBean, long key, AccountService accountingService) {
    if (!userBean.isLoggedInAndAdmin() && userBean.isLoggedInAndSubscriptionManager()) {
      List<VOPaymentInfo> paymentInfosForSubscription =
          getPaymentInfosForSubscription(key, accountingService);
      return (paymentInfosForSubscription != null && !paymentInfosForSubscription.isEmpty());
    }

    if (userBean.isLoggedInAndAdmin()) {
      Collection<VOPaymentType> enabledPaymentTypes =
          getEnabledPaymentTypes(Long.valueOf(key), accountingService);
