...
public class PriceModelBean extends BaseBean implements Serializable {
  ...
  private static final Log4jLogger logger = LoggerFactory.getLogger(PriceModelBean.class);
  ...
  static final int PRICEMODEL_FOR_SERVICE = 1;
  static final int PRICEMODEL_FOR_CUSTOMER = 2;
  static final int PRICEMODEL_FOR_SUBSCRIPTION = 3;
  ...
  /**
   * Specific prices of parameters and options for one selected role. These values are displayed.
   * Dimension is roles.
   */
  private PricedParameterRow[] pricedParametersOfSelectedRole;

  /**
   * Specific prices of parameters and options for all roles. This is a cache for saving prices for
   * all roles before saving. [Dimension_y][Dimension_x]: Dimension_y - roles, Dimension_x -
  ...
  public List<String> getDataTableHeaders() {
    if (dataTableHeaders == null || dataTableHeaders.isEmpty()) {
      try {
        dataTableHeaders =
            DataTableHandler.getTableHeaders(POSubscriptionAndCustomer.class.getName());
      } catch (Exception ex) {
  ...
  public void setDescription(String description) {
    ...
    if (!LocaleUtils.set(localizationTmp.getDescriptions(), locale, description)) {
  ...
  void initServices() {
    Vo2ModelMapper<VOService, Service> mapper =
        new Vo2ModelMapper<VOService, Service>() {
          @Override
    ...
    switch (getCurrentPMPage()) {
      ...
      case PRICEMODEL_FOR_CUSTOMER:
        setServices(
            mapper.map(
                getProvisioningServiceInternal()
                    .getSuppliedServices(PerformanceHint.ONLY_FIELDS_FOR_LISTINGS)));
  ...
  public List<Organization> getCustomers() {
    if (customers == null) {
      ...
      try {
        customers = mapper.map(getAccountingService().getMyCustomersOptimization());
      } catch (OrganizationAuthoritiesException e) {
        ExceptionHandler.execute(e);
      }
      if (getCustomerID() == null && customers != null && customers.size() > 0) {
    ...
    try {
      if (selectedService == null) {
        ...
        if (pm.getKey() == 0) {
          ...
          List<VOLocalizedText> templLicenses =
              getProvisioningService()
                  .getPriceModelLicenseTemplateLocalization(selectedService.getVoServiceDetails());
          localization.setDescriptions(new ArrayList<VOLocalizedText>());
          if (templLicenses != null) {
            localization.setLicenses(templLicenses);
          } else {
            localization.setLicenses(new ArrayList<VOLocalizedText>());
          }
        } else {
          PartnerService partnerService = getParterService();
          try {
            Response r = partnerService.getPriceModelLocalization(getSelectedService());
            localization = r.getResult(VOPriceModelLocalization.class);
        ...
        if (!error) {
          List<Locale> supportedLocales = appBean.getSupportedLocaleList();
          localization.setDescriptions(
              LocaleUtils.trim(localization.getDescriptions(), supportedLocales.iterator()));
          localization.setLicenses(
              LocaleUtils.trim(localization.getLicenses(), supportedLocales.iterator()));
        }
      }
    } catch (SaaSApplicationException e) {
      ui.handleException(e);
  ...
  public List<String> getSupportedCurrencies() {
    if (supportedCurrencies == null) {
      supportedCurrencies = getProvisioningService().getSupportedCurrencies();
    }
    ...
    switch (getCurrentPMPage()) {
      ...
      case PRICEMODEL_FOR_SUBSCRIPTION:
        return model.getCachedList() == null
            ...
            || isDisableSaveForExternalPriceModel(extSubBean);
    }
    return false;
  }

  private boolean isDisableSaveForExternalPriceModel(
      ExternalPriceModelCtrl externalPriceModelCtrl) {
    if (selectedService == null || selectedService.getPriceModel() == null) {
      return false;
    }
    if (!selectedService.getPriceModel().isExternal()
        && !selectedService.getTechnicalService().isExternalBilling()) {
      return false;
    }
  ...
  /** @param currentPriceModel the currentPriceModel to set */
  ...
  public void setLocalizedDescriptions(final List<VOLocalizedText> localizedDescriptions) {
    if (localization == null) {
      localization = new VOPriceModelLocalization();
    }
  ...
  /** @param key the selectedService to set */
  ...
  public String selectSubscriptionIdAndCustomerId() {
    ...
    String result = setSelectedSubscription(subscriptionId, customerId, false);
    updatePriceModel();
  ...
  String setSelectedSubscription(String subscriptionId, String customerId, boolean isInitPage) {
    ...
    POSubscriptionAndCustomer subscriptionAndCustomer =
        getSubscriptionAndCustomer(subscriptionId, customerId);
  ...
  private POSubscriptionAndCustomer getSubscriptionAndCustomer(
    ...
    for (POSubscriptionAndCustomer subscriptionAndCustomer : model.getCachedList()) {
      if (subscriptionAndCustomer.getCustomerId().equals(customerId)
          && subscriptionAndCustomer.getSubscriptionId().equals(subscriptionId)) {
  ...
  public String setSelectedSubscription(POSubscriptionAndCustomer subscriptionAndCustomer) {
    ...
    try {
      ...
      setSubscriptionID(model.getSelectedSubscriptionAndCustomer().getSubscriptionId());
      setCustomerID(model.getSelectedSubscriptionAndCustomer().getCustomerId());
      sessionBean.setSelectedCustomerId(model.getSelectedSubscriptionAndCustomer().getCustomerId());
      sessionBean.setSelectedSubscriptionId(
          model.getSelectedSubscriptionAndCustomer().getSubscriptionId());
      model.setSelectedSubscription(
          getSubscriptionService()
  ...
  public void updatePriceModel() {
    ...
    System.out.println("Price model: " + priceModel);
    ...
    pricedEvents = PricedEventRow.createPricedEventRowList(selectedService.getVoServiceDetails());
    parameters =
        PricedParameterRow.createPricedParameterRowListForPriceModel(
            selectedService.getVoServiceDetails());
    parametersRoles =
        PricedParameterRow.createPricedParameterRowListForPriceModelRoles(
            selectedService.getVoServiceDetails());
  ...
  /**
   * When some parameters are not initialized in existing price model, the parameter in
   * member"parameters" should be the same in member "parametersRoles" as it is initialized for
   * first time price model saving.
   */
    ...
    for (PricedParameterRow row : getParameters()) {
      if (row.getPricedParameter().getKey() == 0) {
        for (PricedParameterRow roleRow : parametersRoles) {
          if (roleRow.getParameterDefinition().getKey() == row.getParameterDefinition().getKey()) {
  ...
  void updateVOServiceDetails() {
      ...
      if (service != null) {
        ...
        if (getCurrentPMPage() == PRICEMODEL_FOR_CUSTOMER) {
          if (getCustomerID() != null) {
            vopd =
                getProvisioningService()
                    .getServiceForCustomer(getCustomer().getVOOrganization(), service.getVO());
          }
          if (vopd == null) {
            customerPricemodelCreation = true;
            // there is no customer specific one yet so use the
            // global template
            vopd = getProvisioningService().getServiceDetails(service.getVO());
            if (vopd != null) {
        ...
        if (vopd == null) {
          ...
          ObjectNotFoundException ex =
              new ObjectNotFoundException(ClassEnum.SERVICE, storedServiceId);
          throw ex;
        }
        List<VOCatalogEntry> catalogEntries =
            getMarketplaceService().getMarketplacesForService(vopd);
        if (catalogEntries != null && !catalogEntries.isEmpty()) {
          final VOMarketplace mp = catalogEntries.get(0).getMarketplace();
          if (mp != null) {
        ...
        vopd =
            getProvisioningService()
                .getServiceForSubscription(getCustomer().getVOOrganization(), getSubscriptionID());
      }
    } catch (SaaSApplicationException e) {
      ui.handleException(e);
    }
    ...
    editDisabled = true;
    if (selectedServiceKey != null) {
      if (customerPricemodelCreation && (getCurrentPMPage() == PRICEMODEL_FOR_CUSTOMER)) {
  ...
  public String localize() {
    ...
    if (priceModel != null) {
      String locale = getUserFromSession().getLocale();
      LocaleUtils.set(getLocalization().getDescriptions(), locale, priceModel.getDescription());
    }
    return null;
  }
  ...
  public String save() throws SaaSApplicationException {
    ...
    if (voServiceDetails == null) {
      ...
      logger.logDebug("save() selectedService is null and " + "hence returning without saving");

      dirty = false;

      if (getCurrentPMPage() == PRICEMODEL_FOR_SUBSCRIPTION) {
        throw new ObjectNotFoundException(ClassEnum.SUBSCRIPTION, model.getSubscriptionId());
      } else {
        throw new ObjectNotFoundException(ClassEnum.SERVICE, storedServiceId);
      }

    } else {
      if (voServiceDetails.getTechnicalService().isExternalBilling()) {
        return saveExternalPriceModel(voServiceDetails, getPriceModel());
      } else {
  ...
  public String saveNativePriceModel(VOServiceDetails voServiceDetails, VOPriceModel voPriceModel)
    ...
    for (PricedEventRow row : pricedEvents) {
      if (row.getSteppedPrice() == null && row.getEventPrice().compareTo(BigDecimal.ZERO) != 0
    ...
    voPriceModel.setRoleSpecificUserPrices(getRoleSpecificUserPricesForSaving());
    // ------------------------------------------
    ...
    try {
      switch (getCurrentPMPage()) {
        case PRICEMODEL_FOR_SERVICE:
          voServiceDetails =
              getProvisioningService().savePriceModel(voServiceDetails, voPriceModel);
          saveLocalization(voServiceDetails);
          addMessage(
              null,
              FacesMessage.SEVERITY_INFO,
              INFO_PRICEMODEL_SAVED,
        ...
        case PRICEMODEL_FOR_CUSTOMER:
          ...
          voServiceDetails =
              getProvisioningService()
                  .savePriceModelForCustomer(
                      voServiceDetails, voPriceModel, getCustomer().getVOOrganization());
          saveLocalization(voServiceDetails);
          addMessage(
              null,
              FacesMessage.SEVERITY_INFO,
              ...
              new String[] {
                getCustomer().getNameWithOrganizationId(), voServiceDetails.getServiceId()
              });
          break;
        ...
        case PRICEMODEL_FOR_SUBSCRIPTION:
          voServiceDetails =
              getProvisioningService()
                  .savePriceModelForSubscription(voServiceDetails, voPriceModel);
          ...
          addMessage(
              ...
              new String[] {getSubscriptionID(), getCustomer().getNameWithOrganizationId()});
          break;

        default:
          throw new IllegalStateException("PRICEMODEL OF UNKNOWN TYPE" + getCurrentPMPage());
      }
    } catch (SaaSApplicationException ex) {
      if ((ex instanceof ObjectNotFoundException)
          && ((ObjectNotFoundException) ex).getDomainObjectClassEnum() == ClassEnum.SERVICE) {
  ...
  String saveExternalPriceModel(VOServiceDetails voServiceDetails, VOPriceModel voPriceModel)
      throws SaaSApplicationException {
    ...
    if (priceModel == null) {
      addMessage(null, FacesMessage.SEVERITY_ERROR, ERROR_EXTERNAL_PRICEMODEL_NOT_AVAILABLE);
   ...
   *
   * @param voServiceDetails the service with the price model for which the save is performed
  ...
  private void saveLocalization(VOServiceDetails voServiceDetails)
    ...
    if (localization != null) {
      // We need to remove current localization from the list as it has been saved already during
      // price model
  ...
  private List<VOLocalizedText> removeCurrentLocalizationFromList(
      List<VOLocalizedText> textsToFilter) {
  ...
  public List<VORoleDefinition> getRoles() {
    if (roles == null) {
      ...
      if (vopd != null) {
        VOTechnicalService technicalService = vopd.getTechnicalService();
      ...
      if (index != -1) {
        ...
        for (int i = 0; i < paramNum; i++) {
          if ((pricedParametersOfSelectedRole[i].getPricedParameter() != null)
              && (pricedParametersOfAllRoles[index][i].getPricedParameter() != null)) {
            pricedParametersOfSelectedRole[i]
                .getPricedParameter()
                .setPricePerUser(
                    pricedParametersOfAllRoles[index][i].getPricedParameter().getPricePerUser());
          }
          if ((pricedParametersOfSelectedRole[i].getPricedOption() != null)
              && (pricedParametersOfAllRoles[index][i].getPricedOption() != null)) {
            pricedParametersOfSelectedRole[i]
                .getPricedOption()
                .setPricePerUser(
                    pricedParametersOfAllRoles[index][i].getPricedOption().getPricePerUser());
          }
        }
        // initialize user role price
        pricePerUserSelectedRole = priceModelPricedRoles[index].getPricePerUser();
      }
    }
  }
   ...
   *
   * @param inputRow parameters for selected role Value to set.
   */
   ...
   *
   * @param pricePerUserCurrentRole Current role price per user
   */
  ...
  public void saveRoles() {
    ...
    if (index != -1) {
      ...
      priceModelPricedRoles[index].setPricePerUser(pricePerUserSelectedRole);
      ...
      for (int i = 0; i < paramNum; i++) {
        if ((pricedParametersOfSelectedRole[i].getPricedParameter() != null)
          ...
          pricedParametersOfAllRoles[index][i]
              .getPricedParameter()
              .setPricePerUser(
                  pricedParametersOfSelectedRole[i].getPricedParameter().getPricePerUser());
        }
        if ((pricedParametersOfSelectedRole[i].getPricedOption() != null)
            && (pricedParametersOfAllRoles[index][i].getPricedOption() != null)) {
          pricedParametersOfAllRoles[index][i]
              .getPricedOption()
              .setPricePerUser(
                  pricedParametersOfSelectedRole[i].getPricedOption().getPricePerUser());
        }
      }
    }
  }
  ...
  /**
   * Predicate for checking are there roles. Needed for enabling a button on GUI.
   *
   * @return Confirmation about no selected role.
   */
  ...
  private void initRoleSpecificUserPrice() {
    ...
    if (priceModel != null) {
      List<VOPricedRole> roleSpecificUserPrice = priceModel.getRoleSpecificUserPrices();
      if (roleSpecificUserPrice != null) {
        for (int i = 0; i < roleCount; i++) {
          VORoleDefinition curRole = roles.get(i);
          for (int j = 0; j < roleSpecificUserPrice.size(); j++) {
            VOPricedRole curUserPrice = roleSpecificUserPrice.get(j);
            if (curRole.getRoleId().equals(curUserPrice.getRole().getRoleId())) {
  ...
  private void initParametersForRole() {
    ...
    List<PricedParameterRow> paramForSelectedRole =
        PricedParameterRow.createPricedParameterRowListForPriceModelRoles(
            selectedService.getVoServiceDetails());
    for (int i = 0; i < numParam; i++) {
      pricedParametersOfSelectedRole[i] = copyParameterRow(paramForSelectedRole.get(i));
    }
    ...
    for (int i = 0; i < numRoles; i++) {
      List<PricedParameterRow> paramForRoleInCash =
          PricedParameterRow.createPricedParameterRowListForPriceModelRoles(
              selectedService.getVoServiceDetails());
      for (int j = 0; j < numParam; j++) {
        pricedParametersOfAllRoles[i][j] = copyParameterRow(paramForRoleInCash.get(j));
      }
    }

    // initialize with real prices
    for (int indexForParameters = 0; indexForParameters < numParam; indexForParameters++) {
      PricedParameterRow curParameter = parametersRoles.get(indexForParameters);
      if (!curParameter.isOption()) {
        // looking for only for parameter entry, not for option
        if (curParameter.getParameterDefinition().getValueType()
            != ParameterValueType.ENUMERATION) {
          initNotEnumerationParameter(indexForParameters, curParameter);
  ...
  /**
   * Initialize specific user prices for roles of parameters with options - enumeration type.
   *
   * @param curParameter Parameter for initialization. @TODO Optimize this method as method for
   *     adding options role prices before price model saving. Initialization method uses compare of
   *     keys, it will work as reason keys are initialized or equals 0, in this case prices are also
   *     0 - the first saving of price model.
   */
  private void initEnumerationParameter(PricedParameterRow curParameter) {
    ...
    List<VOPricedOption> pricedOptionList = curParameter.getPricedParameter().getPricedOptions();
    ...
    for (int optionIndex = 0; optionIndex < pricedOptionList.size(); optionIndex++) {
      ...
      List<VOPricedRole> oldRoleSpecificUserPrices = pricedOption.getRoleSpecificUserPrices();
      // looking for position in displayed parameter-option
      // list for the option
      for (int indexInCache = 0; indexInCache < parametersRoles.size(); indexInCache++) {
        VOPricedOption curPricedOption = parametersRoles.get(indexInCache).getPricedOption();
        if (curPricedOption != null) {
          ...
          if (pricedOption.getKey() == curPricedOption.getKey()) {
            ...
            for (int j = 0; j < roles.size(); j++) {
              ...
              for (int roleIndex = 0; roleIndex < oldRoleSpecificUserPrices.size(); roleIndex++) {
                VORoleDefinition oldCurRole = oldRoleSpecificUserPrices.get(roleIndex).getRole();
                if (oldCurRole.getRoleId().equals(role.getRoleId())) {
                  ...
                  BigDecimal pricePerUser =
                      oldRoleSpecificUserPrices.get(roleIndex).getPricePerUser();
                  VOPricedOption po = pricedParametersOfAllRoles[j][indexInCache].getPricedOption();
   ...
   *
   * @param indexForParameters Number in parameter-option list.
   * @param curParameter Parameter for initialization.
   */
  private void initNotEnumerationParameter(
      int indexForParameters, PricedParameterRow curParameter) {
    // specific prices for all roles
    List<VOPricedRole> roleSpecificUserPrices =
        curParameter.getPricedParameter().getRoleSpecificUserPrices();
    if (roleSpecificUserPrices != null) {
      for (VOPricedRole rolePrice : roleSpecificUserPrices) {
        ...
        for (int roleIndex = 0; roleIndex < roles.size(); roleIndex++) {
          if (curParamRole.getRoleId().equals(roles.get(roleIndex).getRoleId())) {
  ...
  /**
   * Prepare parameterRow with new internal member pricedParameter and pricedOption. We need copy of
   * objects, because we use the same structure for another functionality. Without copy we get
   * problems with object references.
   *
   * @param pricedParameterRow Original parameterRow.
   * @return parameterRow with new internal member pricedParameter and pricedOption.
   */
  private PricedParameterRow copyParameterRow(PricedParameterRow pricedParameterRow) {
  ...
  private void clearParametersSelectedRole() {
    ...
    for (int i = 0; i < paramNum; i++) {
      if (pricedParametersOfSelectedRole[i].getPricedParameter() != null) {
        pricedParametersOfSelectedRole[i].getPricedParameter().setPricePerUser(BigDecimal.ZERO);
      }
      if (pricedParametersOfSelectedRole[i].getPricedOption() != null) {
        pricedParametersOfSelectedRole[i].getPricedOption().setPricePerUser(BigDecimal.ZERO);
      }
    }
  }

  /**
   * Copy parameter. We need copy of objects, because we use the same structure for another
   * functionality. Without copy we get problems with object references.
   *
   * @param paramOld
  ...
  /**
   * Copy option. We need copy of objects, because we use the same structure for another
   * functionality. Without copy we get problems with object references.
   *
   * @param optionOld
  ...
  private VOPricedOption copyOption(VOPricedOption optionOld) {
    ...
    optionNew.setRoleSpecificUserPrices(optionOld.getRoleSpecificUserPrices());
  ...
  /**
   * Prepare price model for saving: fill list of user-role specific price list.
   *
  ...
  /**
   * Prepare price model for saving: specific role prices of options are added from cache.
   *
   * @param curParam Current enumeration parameter.
   * @param paramIndex index in displayed array.
   */
  private void addForEnumeration(PricedParameterRow curParam, int paramIndex) {
    // get all priced option for the parameter
    List<VOPricedOption> pricedOptionList = curParam.getPricedParameter().getPricedOptions();
    for (int optionIndex = 0; optionIndex < pricedOptionList.size(); optionIndex++) {
      VOPricedOption pricedOption = pricedOptionList.get(optionIndex);
      List<VOPricedRole> oldRoleSpecificUserPrices = pricedOption.getRoleSpecificUserPrices();
      ...
      for (int j = 0; j < roles.size(); j++) {
        ...
        for (int roleIndex = 0; roleIndex < numOldRoleWithPrice; roleIndex++) {
          ...
          if (oldCurRole.getRoleId().equals(role.getRoleId())) {
            ...
            BigDecimal price =
                pricedParametersOfAllRoles[j][indexInCache].getPricedOption().getPricePerUser();
            oldRoleSpecificUserPrices.get(roleIndex).setPricePerUser(price);
            isUpdated = true;
            break;
          }
        }
        if (!isUpdated) {
          ...
          if (pricedParametersOfAllRoles[j][indexInCache].getPricedOption() != null) {
  ...
  /**
   * Prepare price model for saving: specific role prices of not enumeration parameters are added
   * from cache.
   *
   * @param curParam Current parameter.
   * @param paramIndex parameter index in displayed array.
   */
  private void addForNotEnumeration(PricedParameterRow curParam, int paramIndex) {
    List<VOPricedRole> oldRoleSpecificUserPrices =
        curParam.getPricedParameter().getRoleSpecificUserPrices();
    ...
    for (int j = 0; j < roles.size(); j++) {
      ...
      for (int roleIndex = 0; roleIndex < numOldRoleWithPrice; roleIndex++) {
        ...
        if (oldCurRole.getRoleId().equals(role.getRoleId())) {
          // role is already has price
          // just change old price to new price
          oldRoleSpecificUserPrices
              .get(roleIndex)
              .setPricePerUser(
                  pricedParametersOfAllRoles[j][paramIndex].getPricedParameter().getPricePerUser());
          isUpdated = true;
        }
      }
      if (!isUpdated) {
        ...
        // take prices for this role and add to list
        VOPricedRole price = new VOPricedRole();
        if (pricedParametersOfAllRoles[j][paramIndex].getPricedParameter() != null) {
          price.setPricePerUser(
              pricedParametersOfAllRoles[j][paramIndex].getPricedParameter().getPricePerUser());
          price.setRole(role);
        }
   ...
   *
   * @return true if not price model is selected or the current price model is not chargeable.
   */
   ...
   *
   * @return true if the selected service is published in a global marketplace.
   */
  ...
  public void setFreePeriodAvailable(@SuppressWarnings("unused") boolean isFreePeriodAvailable) {
    // not persisted, calculated from VOPriceModel.freePeriod
  }

  /**
   * Needed by <code>ParameterValueValidator</code> to validate the minimum value.
   *
   * @return The minimum value as <code>Long</code>, as expected by the ParameterValueValidator, in
   *     order to reuse it.
   */
  ...
  /**
   * Needed by <code>ParameterValueValidator</code> to validate the maximum value.
   *
   * @return The maximum value as <code>Long</code>, as expected by the ParameterValueValidator, in
   *     order to reuse it.
   */
  ...
  void throwObjectNotFoundException(ClassEnum objectClass, String objectId)
      throws ObjectNotFoundException {
    ObjectNotFoundException ex = new ObjectNotFoundException(objectClass, objectId);
    dirty = false;
    throw ex;
  }
  ...
  public void setExternalPriceModelUploaded(boolean isExternalPriceModelUploaded) {
  ...
  public void upload() throws SaaSApplicationException {
    if (getCurrentPMPage() == PRICEMODEL_FOR_SERVICE) {
      ...
      extCustBean.upload(getSelectedService(), getCustomer().getVOOrganization());
    } else if (getCurrentPMPage() == PRICEMODEL_FOR_SUBSCRIPTION) {
  ...
  public void display() throws SaaSApplicationException, IOException {
    if (getCurrentPMPage() == PRICEMODEL_FOR_SERVICE) {
      ...
      extSubBean.display(
          isExternalPriceModelUploaded, getSelectedSubscription().getSubscribedService());
    }
  }
