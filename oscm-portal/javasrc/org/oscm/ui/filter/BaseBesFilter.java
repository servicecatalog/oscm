...
public abstract class BaseBesFilter implements Filter {
  ...
  protected static final String PARAM_LOGIN_PAGE = "login-page";
  protected static final String PARAM_PUBLIC_URL_PATTERN = "public-url-pattern";
  protected static final String PARAM_PWD_PAGE = "pwd-page";
  protected static final String PARAM_ERROR_PAGE = "error-page";
  ...
  /**
   * Read the parameter from filter configuration. Called by the web container to indicate to a
  ...
  public void init(FilterConfig filterConfig) throws ServletException {
    ...
    publicUrlPattern = filterConfig.getInitParameter(PARAM_PUBLIC_URL_PATTERN);
    ...
    try {
      ...
      logger.logError(Log4jLogger.SYSTEM_LOG, e, LogMessageIdentifier.ERROR_TENANT_MISCONFIGURED);
    }
  }
  ...
  protected AuthorizationRequestData initializeRequestDataObject(HttpServletRequest httpRequest) {
    final AuthorizationRequestData ard = new AuthorizationRequestData();

    ard.setRelativePath(BesServletRequestReader.getRelativePath(httpRequest));
    if (ard.getRelativePath().startsWith(pwdPage)) {
      ard.setPassword(httpRequest.getParameter(BesServletRequestReader.REQ_PARAM_PASSWORD));
      ard.setRequestedToChangePwd(BesServletRequestReader.isRequestedToChangePassword(httpRequest));

      if (ard.isRequestedToChangePwd()) {
        ard.setNewPassword(
            httpRequest.getParameter(BesServletRequestReader.REQ_PARAM_PASSWORD_NEW));
        ard.setNewPassword2(
            httpRequest.getParameter(BesServletRequestReader.REQ_PARAM_PASSWORD_NEW2));
      }
    }
    ard.setMarketplace(BesServletRequestReader.isMarketplaceRequest(httpRequest));

    if (authSettings.isServiceProvider()) {
      ard.setMarketplaceLoginPage(
          httpRequest.getServletPath().startsWith(BaseBean.SAML_SP_LOGIN_AUTOSUBMIT_PAGE));
    } else {
      ard.setMarketplaceLoginPage(
          BesServletRequestReader.isMarketplaceLoginPageRequest(httpRequest));
    }

    ard.setMarketplaceErrorPage(BesServletRequestReader.isMarketplaceErrorPageRequest(httpRequest));

    String mId = (String) httpRequest.getSession().getAttribute(Constants.REQ_PARAM_MARKETPLACE_ID);
  ...
  /**
   * If the value is set check if it starts with a /. Otherwise use the default value
   *
   ...
   *
   * @param page the page to which the request is forwarded
   * @param request the current http servlet request
   * @param response the current http servlet response
   * @throws ServletException
   * @throws IOException
   */
  protected void forward(String page, ServletRequest request, ServletResponse response)
      throws ServletException, IOException {
    filterConfig.getServletContext().getRequestDispatcher(page).forward(request, response);
  }

  /**
   * Redirect to the given page overtaking errors if exist.
   *
   * @param relativePath the relative path of the page to which to be redirected
   * @param request the current http servlet request
   * @param response the current http servlet response
   * @throws IOException
   */
  protected void sendRedirect(
      HttpServletRequest httpRequest, HttpServletResponse response, String relativePath)
      throws IOException {
    sendRedirectStatic(httpRequest, response, relativePath);
  }

  public static void sendRedirectStatic(
      HttpServletRequest httpRequest, HttpServletResponse response, String relativePath)
      throws IOException {
    // Bug 7607: Avoid error message is lost
    final String errMsg = (String) httpRequest.getAttribute(Constants.REQ_ATTR_ERROR_KEY);
    if (errMsg != null) {
      StringBuffer sb = new StringBuffer(relativePath);
      if (relativePath.indexOf('?') > 0) sb.append('&');
      else sb.append('?');
    ...
    // And redirect...
    JSFUtils.sendRedirect(response, httpRequest.getContextPath() + relativePath);
  }

  /**
   * Get the default URL for the given user
   *
   * @param serviceAccess the service access implementation for the current session.
   * @param userDetails the user for which we want to get the default URL.
   * @param httpRequest
   * @return the default URL.
   */
  protected String getDefaultUrl(
      ServiceAccess serviceAccess, AuthorizationRequestData rdo, HttpServletRequest httpRequest) {
    ...
    if (userDetails == null) {
    ...
    } else if (BesServletRequestReader.isMarketplaceLogin(httpRequest) && hasMpId) {
      ...
      if (roles.contains(UserRoleType.PLATFORM_OPERATOR)) {
        ...
        if (roles.contains(UserRoleType.TECHNOLOGY_MANAGER)) {
          ServiceProvisioningServiceInternal sps =
              serviceAccess.getService(ServiceProvisioningServiceInternal.class);
          try {
            if (sps.getTechnicalServices(
                    OrganizationRoleType.SUPPLIER, PerformanceHint.ONLY_IDENTIFYING_FIELDS)
                .isEmpty()) {
        ...
        defaultUrl = MenuBean.LINK_TECHSERVICE_IMPORT;
          ...
          || (userDetails.getOrganizationRoles().size() == 1
              && userDetails.getOrganizationRoles().contains(OrganizationRoleType.CUSTOMER))) {
    ...
    if (BesServletRequestReader.isServiceLogin(httpRequest) && (userDetails != null)) {
      // Ensure user is not removed from session after service login, for
      // example if user is not assigned to subscription!
      httpRequest.getSession().setAttribute(Constants.SESS_ATTR_USER, userDetails);
    }
    return defaultUrl;
  }

  /**
   * Forward to the login page if the given relative path is not the login page.
   *
   * @param relativePath the current request's URL
   * @param save if true the value of the relativePath is store in the session
   * @param request the current HTTP servlet request
   * @param response the current HTTP servlet response
   * @param chain
   * @throws ServletException
   * @throws IOException
   */
  protected void forwardToLoginPage(
      String relativePath,
      boolean save,
    ...
    String actualLoginPage = getActualLoginPage(request, loginPage, authSettings);
  ...
  MarketplaceService getMarketplaceService(HttpServletRequest request) {
    ServiceAccess serviceAccess = ServiceAccess.getServiceAcccessFor(request.getSession());
    if (mkpService == null) {
      mkpService = serviceAccess.getService(MarketplaceService.class);
    }
    return mkpService;
  }
  ...
  MarketplaceCacheService getMarketplaceServiceCache(HttpServletRequest request) {
    ...
    if (mkpServiceCache == null) {
      mkpServiceCache = serviceAccess.getService(MarketplaceCacheService.class);
    }
  ...
  private void saveForwardUrl(String relativePath, HttpServletRequest request) {
    ...
    if (BesServletRequestReader.isMarketplaceLogin(request)) {
      ...
      forwardUrl =
          BesServletRequestReader.copyRequestAttributeToURLParam(
              request, Constants.REQ_ATTR_SERVICE_LOGIN_TYPE, forwardUrl);
      request.setAttribute(Constants.REQ_ATTR_LOGIN_REDIRECT_TARGET, forwardUrl);
    }
  ...
  /**
   * Get the relative URL for the login page. In case of a marketplace login the path of the
   * marketplace login page is returned. In case of a BES login the passed default page is returned.
   *
   * @param httpRequest - the HTTP request
   * @param defaultLoginPage - the path of page to be shown if it is no marketplace login.
   * @return see above.
   */
  protected String getActualLoginPage(
    ...
    // login panel.
    String loginType = (String) httpRequest.getAttribute(Constants.REQ_ATTR_SERVICE_LOGIN_TYPE);
    if (Constants.REQ_ATTR_LOGIN_TYPE_MPL.equals(loginType)) {
      ...
      if (authSettings != null && authSettings.isServiceProvider()) {
        return BaseBean.SAML_SP_LOGIN_AUTOSUBMIT_PAGE + serviceLoginType;
      } else {
  ...
  protected boolean redirectToMpUrl(
    ...
    if (uri != null && !ADMStringUtils.isBlank(mpRedirect)) {
      String suffix =
          mpRedirect.contains("?") ? mpRedirect.substring(mpRedirect.lastIndexOf('?') + 1) : "";
      if (!mpRedirect.contains(uri.replaceFirst("/index.jsf", "")) || !suffix.equals(queryString)) {
  ...
  /**
   * @param cs the ConfigurationService for getting VOConfigurationSetting.
   * @return mpRedirect the value of ConfigurationSetting by key "HTTP_MP_ERROR_REDIRECT"
   */
  String getRedirectMpUrlHttp(ConfigurationService cs) {
    String mpRedirect =
        cs.getVOConfigurationSetting(
                ConfigurationKey.MP_ERROR_REDIRECT_HTTP, Configuration.GLOBAL_CONTEXT)
            .getValue();
    return mpRedirect;
  }
  ...
  /**
   * @param cs the ConfigurationService for getting VOConfigurationSetting.
   * @return mpRedirect the value of ConfigurationSetting by key "HTTPS_MP_ERROR_REDIRECT"
   */
