...
public class ManageOperatorRevenueShareCtrl implements Serializable {
  ...
  void initTemplateServiceSelector() {
    ...
    Response response = sl.findService(ServiceSelector.class).getTemplateServices();
  ...
  void intializeRevenueShares(long serviceKey) {
    try {
      Response response = sl.findService(PricingService.class).getOperatorRevenueShare(serviceKey);
      POOperatorPriceModel operatorPriceModel = response.getResult(POOperatorPriceModel.class);
      model.setOperatorRevenueShare(operatorPriceModel.getRevenueShare());
      model.setDefaultOperatorRevenueShare(operatorPriceModel.getDefaultRevenueShare());
  ...
  public void save() {
    try {
      Response response =
          sl.findService(PricingService.class)
              .saveOperatorRevenueShare(
                  model.getSelectedTemplateKey(), model.getOperatorRevenueShare());
      ui.handle(response, INFO_SAVED);
    } catch (ObjectNotFoundException e) {
      model.setSelectedTemplateKey(0L);
      ui.handleException(e);
    } catch (ValidationException | ServiceOperationException | ConcurrentModificationException e) {
      ui.handleException(e);
    }
  }
