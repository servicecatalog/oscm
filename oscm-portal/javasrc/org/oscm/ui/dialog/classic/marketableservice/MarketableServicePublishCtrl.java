...
@ManagedBean
@ViewScoped
public class MarketableServicePublishCtrl extends BaseBean implements Serializable {
  ...
  @EJB PublishService publishService;
  @EJB ResaleService resaleService;
  @EJB PricingService pricingService;
  @EJB CategorizationService categorizationService;
  @EJB MarketplaceService mplService;
  @EJB ServiceProvisioningService serviceProvisioning;

  /** Sort template ids in locale-sensitive alphabetical order. */
  private class TemplateIdsComparator implements Comparator<POServiceDetails> {
   ...
   *     <p>workaround: to be refactored under jsf 2.0
   */
  public String getInitializePublish() {
    if (!model.isInitialized()) {
      ...
      // (#9481)
      initializeModel(key == null ? 0L : key.longValue(), null, ui.hasErrors());
  ...
  void initializeModel(long serviceKey, String marketplaceId, boolean handleExceptions) {
    ...
    if (serviceKey > 0) {
      ...
      try {
        ...
        servicePartnerPriceModel = response.getResult(POPartnerPriceModel.class);
    ...
    model.setServiceDetails(service == null ? new POServiceForPublish() : service);
    model.setInitialMarketplaceId(getMarketplaceId(service));
    model.setOperatorPriceModel(
        operatorPriceModel == null ? new POOperatorPriceModel() : operatorPriceModel);
    model.setServicePartnerPriceModel(
        servicePartnerPriceModel == null ? new POPartnerPriceModel() : servicePartnerPriceModel);
    model.setSelectedServiceKey(model.getServiceDetails().getService().getKey());
    User sessionUser = userBean.getUserFromSession();
    model.setSupplier(sessionUser.isSupplier());
    model.setOperatorShareVisible(sessionUser.isSupplier() && !isExternalPMUsed());
    model.setBrokerShareVisible(
        (sessionUser.isBroker() || sessionUser.isSupplier()) && !isExternalPMUsed());
    model.setResellerShareVisible(
        (sessionUser.isReseller() || sessionUser.isSupplier()) && !isExternalPMUsed());
  ...
  void initializeServiceTemplates() {
    ...
    List<POServiceDetails> templates = response.getResultList(POServiceDetails.class);
    Collections.sort(templates, new TemplateIdsComparator());
    List<SelectItem> templateItems = new ArrayList<SelectItem>();
    templateItems.add(new SelectItemBuilder(ui).pleaseSelect(Long.valueOf(0L)));
    for (POServiceDetails template : templates) {
      String id =
          model.isSupplier()
              ? template.getServiceId()
              : template.getServiceId() + "  (" + template.getOrganizationId() + ")";
      templateItems.add(new SelectItem(Long.valueOf(template.getKey()), id));
    }
  ...
  private void initializeCategories(String marketplaceId) {
    ...
    if (!ADMStringUtils.isBlank(marketplaceId)) {
      ...
      try {
        response =
            getPublishService().getCategoriesAndRvenueShare(marketplaceId, getUserLanguage());
        allMarketplaceCategories = response.getResultList(VOCategory.class);
        marketplacePriceModel = response.getResult(POMarketplacePriceModel.class);
        partnerPriceModel = response.getResult(POPartnerPriceModel.class);
      } catch (ObjectNotFoundException e) {
        ui.handleException(e);
      }
    }
    model.initializeMarketplaceCategories(allMarketplaceCategories);
    model.setMarketplacePriceModel(
        marketplacePriceModel == null ? new POMarketplacePriceModel() : marketplacePriceModel);
    model.setMarketplacePartnerPriceModel(
        partnerPriceModel == null ? new POPartnerPriceModel() : partnerPriceModel);
    initializeLists(marketplaceId, partnerPriceModel);
  }

  private void initializeLists(String marketplaceId, POPartnerPriceModel partnerPriceModel) {
    ...
    if (!ADMStringUtils.isBlank(marketplaceId) && model.isSupplier()) {
      Response response =
          getPublishService().getBrokers(model.getServiceDetails().getService().getKey());
      model.setBrokers(response.getResultList(POPartner.class));
      response = getPublishService().getResellers(model.getServiceDetails().getService().getKey());
      model.setResellers(response.getResultList(POPartner.class));
    }
    setDefaultRevenueShares(
        model.getBrokers(),
        model.getServicePartnerPriceModel().getRevenueShareBrokerModel(),
        partnerPriceModel == null ? null : partnerPriceModel.getRevenueShareBrokerModel());

    setDefaultRevenueShares(
        model.getResellers(),
        model.getServicePartnerPriceModel().getRevenueShareResellerModel(),
        partnerPriceModel == null ? null : partnerPriceModel.getRevenueShareResellerModel());

    if (!model.isSupplier()) {
      if (model.getServicePartnerPriceModel().getRevenueShareBrokerModel() != null) {
        model
            .getMarketplacePartnerPriceModel()
            .setRevenueShareBrokerModel(
                model.getServicePartnerPriceModel().getRevenueShareBrokerModel());
      }
      if (model.getServicePartnerPriceModel().getRevenueShareResellerModel() != null) {
        model
            .getMarketplacePartnerPriceModel()
  ...
  private void setDefaultRevenueShares(
      List<POPartner> list, PORevenueShare rsPartner, PORevenueShare rsMarketplace) {
    if (list != null) {
      final PORevenueShare rsDefault =
          rsPartner == null || rsPartner.getKey() <= 0 ? rsMarketplace : rsPartner;
      for (POPartner partner : list) {
        if (partner.getRevenueShare() == null) {
          partner.setRevenueShare(rsDefault);
        }
        model.assignedPermissions.put(
            Long.valueOf(partner.getKey()), Boolean.valueOf(partner.isSelected()));
      }
    }
  }
  ...
  public void marketplaceChanged(AjaxBehaviorEvent event) {
    initializeModel(
        ...
        ((UIOutput) event.getSource()).getValue() == null
            ? ""
            : (String) ((UIOutput) event.getSource()).getValue(),
        true);
  }
  ...
  public void synchronizeUIWithObjects() {
    ...
    if (model.getServiceDetails() != null && model.getCategorySelection() != null) {
      ...
      List<VOCategory> changedCategories =
          getChangedAndSelectedCategories(
              model.getServiceDetails().getCatalogEntry().getCategories(), categories);
      model.setChangedCategories(changedCategories);
      model.getServiceDetails().getCatalogEntry().setCategories(categories);
    }
  }

  List<VOCategory> getChangedAndSelectedCategories(
      List<VOCategory> originalCategories, List<VOCategory> newCategories) {
  ...
  private boolean isCategoryExistInList(List<VOCategory> categories, VOCategory targetCategory) {
  ...
  public String save() {
    ...
    if (model.isSupplier()) {
      toGrant.addAll(getPermissionList(model.getBrokers(), OfferingType.BROKER, true));
      toGrant.addAll(getPermissionList(model.getResellers(), OfferingType.RESELLER, true));
      toRevoke.addAll(getPermissionList(model.getBrokers(), OfferingType.BROKER, false));
      toRevoke.addAll(getPermissionList(model.getResellers(), OfferingType.RESELLER, false));
    }
    try {
      ...
      if (marketplace.isRestricted()) {
        handleRestrictedMarketplaceAccess(marketplace, model.getServiceDetails().getService());
      }

      getCategorizationService().verifyCategoriesUpdated(model.getChangedCategoriess());
      Response response =
          getPublishService().updateAndPublishService(model.getServiceDetails(), toGrant, toRevoke);
      ui.handle(
          response, "info.service.saved", model.getServiceDetails().getService().getServiceId());

      updateAssignedPermissions(model.getBrokers(), model.getResellers());
      initRevenueShare(model.getSelectedServiceKey(), model.getServiceDetails().getMarketplaceId());
    } catch (SaaSApplicationException e) {
  ...
  private void initRevenueShare(long serviceKey, String marketPlaceId)
    ...
    response =
        getPricingService()
            .getPartnerRevenueShareForService(new POServiceForPricing(serviceKey, 0));
    servicePartnerPriceModel = response.getResult(POPartnerPriceModel.class);
    response = getPricingService().getOperatorRevenueShare(serviceKey);
    operatorPriceModel = response.getResult(POOperatorPriceModel.class);
    response = getPricingService().getMarketplaceRevenueShares(marketPlaceId);
    marketplacePriceModel = response.getResult(POMarketplacePriceModel.class);
    response = getPricingService().getPartnerRevenueSharesForMarketplace(marketPlaceId);
    marketplacePartnerPriceModel = response.getResult(POPartnerPriceModel.class);
    ...
    model.setMarketplacePriceModel(
        marketplacePriceModel == null ? new POMarketplacePriceModel() : marketplacePriceModel);
    model.setMarketplacePartnerPriceModel(
        marketplacePartnerPriceModel == null
            ? new POPartnerPriceModel()
            : marketplacePartnerPriceModel);
  }

  private void updateAssignedPermissions(List<POPartner> brokers, List<POPartner> resellers) {
    ...
    for (POPartner partner : allPartners) {
      if (partner.isSelected()
          != model.assignedPermissions.get(Long.valueOf(partner.getKey())).booleanValue()) {
        model.assignedPermissions.put(
            Long.valueOf(partner.getKey()), Boolean.valueOf(partner.isSelected()));
      }
    }
    ...
    if (partners != null) {
      for (POPartner partner : partners) {
        ...
        if (addToList) {
          ...
          permission.getGrantee().setOrganizationId(partner.getOrganizationId());
          permission
              .getGrantor()
              .setOrganizationId(model.getServiceDetails().getService().getSellerId());
          permission.getService().setKey(model.getSelectedServiceKey());
          permission
              .getService()
              .setServiceId(model.getServiceDetails().getService().getServiceId());
  ...
  private boolean isExternalPMUsed() {
    VOPriceModel priceModel = model.getServiceDetails().getService().getPriceModel();
  ...
  private void handleRestrictedMarketplaceAccess(VOMarketplace mpl, VOService service)
      throws ObjectNotFoundException, OperationNotPermittedException {

    boolean displayWarnMsg = false;
    String warnedOrgs = "";

    List<VOCustomerService> customerServices =
        serviceProvisioning.getServiceCustomerTemplates(model.getServiceDetails().getService());
    ...
    for (VOCustomerService customerService : customerServices) {
      ...
      try {
        isAccessible =
            mplService.doesOrganizationHaveAccessMarketplace(mpl.getMarketplaceId(), orgId);
