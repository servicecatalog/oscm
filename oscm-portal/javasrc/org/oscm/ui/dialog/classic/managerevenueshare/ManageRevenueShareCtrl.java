...
public class ManageRevenueShareCtrl implements Serializable {
  ...
  boolean initPricingOfMarketplaceForSelectedTemplate(long serviceKey)
    ...
    Response response = getPricingService().getMarketplacePricingForService(service);
    model.setPricingOfMarketplaceForSelectedTemplate(
        response.getResult(POMarketplacePricing.class));
    return model.getPricingOfMarketplaceForSelectedTemplate() != null;
  }

  void initPartnerPriceModelForSelectedTemplate(long serviceKey) throws SaaSApplicationException {
    POServiceForPricing service = new POServiceForPricing(serviceKey, 0);
    Response response = getPricingService().getPartnerRevenueShareForAllStatesService(service);
    model.setPartnerPriceModelForSelectedTemplate(response.getResult(POPartnerPriceModel.class));
  }

  void initPartnerServicePricings(long serviceKey)
      throws ObjectNotFoundException, ServiceOperationException {
    POServiceForPricing service = new POServiceForPricing(serviceKey, 0);
    Response response = pricingService.getPartnerServicesWithRevenueShareForTemplate(service);
    List<POServicePricing> servicePricings = response.getResultList(POServicePricing.class);
    model.setBrokerServicePricings(new ArrayList<POServicePricing>());
    model.setResellerServicePricings(new ArrayList<POServicePricing>());
    for (POServicePricing servicePricing : servicePricings) {
      if (servicePricing.getPartnerPriceModel().getRevenueShareBrokerModel() != null) {
        model.addBrokerServicePricing(servicePricing);
      }
      if (servicePricing.getPartnerPriceModel().getRevenueShareResellerModel() != null) {
  ...
  private void initTemplateServiceSelector() {
    ...
    List<POServiceForPricing> templates = response.getResultList(POServiceForPricing.class);
    Collections.sort(templates, new POServiceIdComparator());
    for (POServiceForPricing templateService : templates) {
      uiTemplateServices.add(
          new SelectItem(
              Long.valueOf(templateService.getKey()),
              templateService.getServiceId()
                  + "("
  ...
  String save(List<POServicePricing> partnerServicePricings) {
    ...
    if (model.isServicePublished()) {
      ...
      pricingForTemplate.setPartnerPriceModel(model.getPartnerPriceModelForSelectedTemplate());
    ...
    try {
      Response response = getPricingService().savePartnerRevenueSharesForServices(pricings);
  ...
  PricingService getPricingService() {
    if (pricingService == null) {
      pricingService = new ServiceLocator().findService(PricingService.class);
    }
