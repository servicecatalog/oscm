...
public class ManageAccessCtrl {
  ...
  public void initSelectableMarketplaces() {
    ...
    List<VOMarketplace> marketplaces = marketplaceService.getMarketplacesOwned();
    ...
    for (VOMarketplace mp : marketplaces) {
      selectableMarketplaces.add(
          new SelectItem(
              mp.getMarketplaceId(),
              String.format("%s (%s)", mp.getName(), mp.getMarketplaceId())));
    }
  ...
  public void marketplaceChanged() {
    ...
    try {
      VOMarketplace marketplace = marketplaceService.getMarketplaceById(marketplaceId);
  ...
  private void populateOrganizations(String marketplaceId) throws ObjectNotFoundException {
    ...
    if (restricted) {
      ...
      for (VOOrganization voOrganization : marketplaceService.getAllOrganizations(marketplaceId)) {
      ...
      Collections.sort(
          organizations,
          new Comparator<POOrganization>() {
            @Override
            public int compare(POOrganization org1, POOrganization org2) {
              return Boolean.valueOf(org2.isSelected())
                  .compareTo(Boolean.valueOf(org1.isSelected()));
            }
          });
  ...
  public String save() {
    try {
      if (model.isSelectedMarketplaceRestricted()) {
        ...
        getMarketplaceService().openMarketplace(model.getSelectedMarketplaceId());
      }
  ...
  private POOrganization toPOOrganization(VOOrganization voOrganization) {
    POOrganization poOrganization =
        new POOrganization(voOrganization.getName(), voOrganization.getOrganizationId());
    poOrganization.setKey(voOrganization.getKey());
    poOrganization.setSelected(
        voOrganization.isHasGrantedAccessToMarketplace()
            || voOrganization.isHasSubscriptions()
            || voOrganization.isHasPublishedServices());
    poOrganization.setDisabled(
        voOrganization.isHasSubscriptions() || voOrganization.isHasPublishedServices());
    poOrganization.setHasSubscriptions(voOrganization.isHasSubscriptions());
    poOrganization.setHasPublishedServices(voOrganization.isHasPublishedServices());
