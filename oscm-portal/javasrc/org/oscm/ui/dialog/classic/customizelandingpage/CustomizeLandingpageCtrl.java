...
public class CustomizeLandingpageCtrl implements Serializable {
  ...
  LandingpageConfigurationService getLandingpageService() {
    if (configureLandingpage == null) {
      configureLandingpage =
          new ServiceLocator().findService(LandingpageConfigurationService.class);
    }
  ...
  void initSelectableMarketplaces() {
    List<POMarketplace> marketplaces = getLandingpageService().getMarketplaceSelections();
  ...
  void initNumOfServicesRange() {
    ...
    List<SelectItem> numOfServicesRange = SelectItemBuilder.buildSelectItems(range);
  ...
  private List<SelectItem> initMarketplaceSelector(List<POMarketplace> mpls) {
    ...
    for (POMarketplace mp : mpls) {
      uiMarketplaces.add(new SelectItem(mp.getMarketplaceId(), mp.getDisplayName()));
    }
  ...
  void loadFillinOptions(String selectedMarketplaceId) {
    List<FillinCriterion> fillinOptions =
        getLandingpageService().getFillinOptions(selectedMarketplaceId);
    model.setFillinItems(buildFillinItems(fillinOptions));
  }

  List<SelectItem> buildFillinItems(List<FillinCriterion> fillinOptions) {
    return new SelectItemBuilder(ui).buildSelectItems(fillinOptions, "FillinCriterion");
  }

  boolean loadPublicLandingpage(String marketplaceId) {
    try {
      Response r = getLandingpageService().loadPublicLandingpageConfig(marketplaceId);
   ...
   */
  void setPublicLandingpageModel(String marketplaceId, Response r) throws ObjectNotFoundException {
    POPublicLandingpageConfig landingpageConfig = r.getResult(POPublicLandingpageConfig.class);
    landingpageConfig.setFeaturedServices(
        adaptServiceNames(landingpageConfig.getFeaturedServices()));
    initNumOfServicesRange();
    model.setLandingpageType(getPublicLandingpageType());
    model.setLandingpageConfig(landingpageConfig);

    List<POService> availableServices =
        new ArrayList<POService>(landingpageConfig.getFeaturedServices());
    availableServices.addAll(adaptServiceNames(r.getResultList(POService.class)));

    model.setAvailableServices(availableServices);
    model.setFillinItems(buildFillinItems(r.getResultList(FillinCriterion.class)));
  }
  ...
  List<POService> adaptServiceNames(List<POService> services) {
    ...
    for (POService s : services) {
      if (Strings.isEmpty(s.getServiceName()) || checkServiceName(s.getServiceName())) {
  ...
  /**
   * Check if the service name contains a control character.(except for blank and tab)
   *
   * @param serviceName
  ...
  public boolean isFieldsDisabled() {
    if (model == null
        || model.getSelectedMarketplace() == null
  ...
  private void saveEnterpriseLandingpage()
      throws ObjectNotFoundException, OperationNotPermittedException, NonUniqueBusinessKeyException,
          ConcurrentModificationException {
    getLandingpageService().saveEnterpriseLandingpageConfig(model.getSelectedMarketplace());
    ui.handle(INFO_SAVED);
  }

  private void saveDefaultLandingpage()
    ...
    Response response =
        getLandingpageService().savePublicLandingpageConfig(model.getLandingpageConfig());
    ui.handle(INFO_SAVED);
    updateModelLandingpageConfig(response.getResult(POPublicLandingpageConfig.class));
  }

  boolean isPublicLandingpageActivated(String marketplaceId) throws ObjectNotFoundException {
  ...
  void updateModelLandingpageConfig(POPublicLandingpageConfig result) {
    result.setFeaturedServices(adaptServiceNames(result.getFeaturedServices()));
  ...
  public String resetToDefault() {
    ...
    try {
      ...
      Response response = getLandingpageService().resetLandingPage(marketplaceId);
      ui.handle(INFO_SAVED);
      model.setLandingpageType(defaultLandingpageType().name());
      updateModelLandingpageConfig(response.getResult(POPublicLandingpageConfig.class));
      updateModelAvailableServices(response.getResultList(POService.class));
  ...
  public boolean isHideManualChoicePanels() {
    return landingpageTypeNotSelected() && !LandingpageType.isDefault(model.getLandingpageType());
  }
  ...
  void switchtToPublicLandingpage(Response r) {
    ...
    model.setLandingpageConfig(landingpageConfig);
    List<POService> availableServices = adaptServiceNames(r.getResultList(POService.class));
    model.setAvailableServices(availableServices);
    model.setFillinItems(buildFillinItems(r.getResultList(FillinCriterion.class)));
  }
}
