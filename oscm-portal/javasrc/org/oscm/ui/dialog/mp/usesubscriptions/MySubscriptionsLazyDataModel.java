 ...
 *
 * <p>Creation Date: 29.04.15 08:11
 *
 * <p>*****************************************************************************
 */
...
public class MySubscriptionsLazyDataModel extends RichLazyDataModel<POSubscription> {
  ...
  @EJB private SubscriptionsService subscriptionsService;
  private static final Log4jLogger logger =
      LoggerFactory.getLogger(MySubscriptionsLazyDataModel.class);
  ...
  @Override
  public List<POSubscription> getDataList(
      int firstRow,
      int numRows,
    ...
    fullTextSearchFilterValue =
        fullTextSearchFilterValue == null ? null : fullTextSearchFilterValue.trim();
    ...
    if (selectedSubscription != null) {
      if (resultList.contains(selectedSubscription)) {
        ...
        selectedSubscription =
            subscriptionsService.getMySubscriptionDetails(selectedSubscription.getKey());
        if (selectedSubscription == null) {
          ...
          if (selectedSubscription.isShowProvisioningWarning()) {
            String provisioningWarning =
                JSFUtils.getText(
                    SubscriptionDetailsCtrlConstants.ERROR_SUBSCRIPTION_ABORT_MODIFY,
                    new Object[] {selectedSubscription.getProvisioningError()});
  ...
  public int getTotalCount() {
    try {
      ...
      pagination.setFullTextFilterValue(fullTextSearchFilterValue);
      setTotalCount(
          subscriptionsService.getMySubscriptionsSizeWithFiltering(pagination).intValue());
    } catch (Exception e) {
      logger.logError(Log4jLogger.SYSTEM_LOG, e, LogMessageIdentifier.ERROR);
    }
  ...
  /**
   * Check if the given URL is external URL. External URL's are all URL which do not contain the
   * BES-Base-URL
   *
  ...
  boolean isInternalURL(String serviceAccessURL) {
    ...
    return isMatch(serviceAccessURL, applicationBean.getServerBaseUrl())
        || isMatch(serviceAccessURL, applicationBean.getServerBaseUrlHttps());
  }
