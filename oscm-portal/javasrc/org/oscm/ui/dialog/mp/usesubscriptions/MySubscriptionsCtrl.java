...
public class MySubscriptionsCtrl implements Serializable {
  ...
  public static final String ERROR_SUBSCRIPTION_CONCURRENTMODIFY =
      "error.subscription.concurrentModify";
  ...
  @EJB TriggerProcessesService triggerProcessService;

  @EJB ConfigurationService config;
  ...
  public void initializeTriggerSubscriptions() {
    myTriggerProcessesModel.setWaitingForApprovalSubs(
        triggerProcessService
            .getMyWaitingForApprovalSubscriptions()
  ...
  public void executeOperation() throws SaaSApplicationException {
    ...
    try {
      subscriptionService.executeServiceOperation(sub.getVOSubscription(), operation);
  ...
  VOTechnicalServiceOperation findSelectedOperation(VOSubscription sub, String operationId) {
    List<VOTechnicalServiceOperation> ops = sub.getTechnicalServiceOperations();
  ...
  public void operationChanged() {
    ...
    if (Strings.isEmpty(operationId)) {
      ...
      VOTechnicalServiceOperation op =
          findSelectedOperation(subscription.getVOSubscription(), operationId);
      OperationModel operationModel = new OperationModel();
      operationModel.setOperation(op);

      try {
        operationModel.setParameters(convert(op, subscription.getVOSubscription()));
  ...
  List<OperationParameterModel> convert(VOTechnicalServiceOperation op, VOSubscription sub)
    ...
    if (requestValuesNecessary(op)) {
      List<VOServiceOperationParameterValues> values =
          subscriptionService.getServiceOperationParameterValues(sub, op);
  ...
  public void validateSubscriptionStatus() {
    ...
    POSubscription mySubscriptionDetails =
        subscriptionsService.getMySubscriptionDetails(Long.parseLong(subKey));
  ...
  public String getCustomTabUrlWithParameters() {
    ...
    String orgId = encodeBase64(model.getSelectedSubscription().getOrganizationId());
    String subId = encodeBase64(model.getSelectedSubscription().getSubscriptionId());
    String instId = encodeBase64(model.getSelectedSubscription().getServiceInstanceId());
    ...
    try {
      ...
      // build URI
      UriBuilder builder = UriBuilder.fromPath(model.getSelectedSubscription().getCustomTabUrl());
