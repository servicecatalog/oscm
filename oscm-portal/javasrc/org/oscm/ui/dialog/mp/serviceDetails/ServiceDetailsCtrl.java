...
public class ServiceDetailsCtrl {
  ...
  private static final Log4jLogger logger = LoggerFactory.getLogger(ServiceDetailsCtrl.class);
  ...
  public String getInitialize()
    ...
    if (model.getSelectedService() != null) {
      ...
      model.setServiceEvents(
          SteppedPriceHandler.buildPricedEvents(
              model.getSelectedService().getPriceModel().getConsideredEvents()));

      model.setServiceParameters(
          PricedParameterRow.createPricedParameterRowListForService(model.getService().getVO()));

      PriceModelContent selectedPriceModelContent =
          new PriceModelContent(
  ...
  void markReviewOwnership() {
    ...
    if (model.getSelectedServiceFeedback() != null) {
      for (POServiceReview poReview : model.getSelectedServiceFeedback().getReviews()) {
        ServiceReview review = new ServiceReview(poReview);
        review.setBelongsToLoggedInUser(
            ui.getUserFromSessionWithoutException() != null
                && ui.getUserFromSessionWithoutException()
                    .getUserId()
  ...
  public boolean isHasReviewForLoggedInUser() {
    if (model.getSelectedServiceReviews() == null || model.getSelectedServiceReviews().isEmpty())
      return false;
  ...
  public static final Vo2ModelMapper<VOService, Service> DEFAULT_VOSERVICE_MAPPER =
    ...
    try {
      viewId =
          extContext.getRequestContextPath()
              + viewId
              + ui.getSelectedServiceKeyQueryPart(String.valueOf(model.getSelectedServiceKey()))
      ...
      extContext.log(getClass().getName() + ".redirectToServiceDetails()", e);
    } finally {
      // reset requested key;
  ...
  /**
   * Indirectly redirects to the service list in case the service details page was accessed with an
   * invalid service key. The actual redirection will be executed be a navigation rule.
   *
   * @return the corresponding outcome to so the navigation rule redirects to the service list page.
   */
  public String redirectToServiceList() {
    model.setSelectedServiceKey(null);
    String errorKey = (String) ui.getRequest().getAttribute(Constants.REQ_ATTR_ERROR_KEY);
    if (errorKey == null) {
      ...
      ui.getRequest().setAttribute(Constants.REQ_ATTR_ERROR_KEY, ERROR_SERVICE_INVALID_KEY);
    }
    return OUTCOME_SHOW_SERVICE_LIST;
  }

  /**
   * The selectedServiceForDetails is empty if the passed serviceKey is not set, invalid or the
   * service is not active see former <c:if test="#{serviceDetailsCtrl.serviceNotAvailable}">
   */
  public boolean getServiceNotAvailable() {
    logger.logDebug("ServiceNotAvailable");
    if (model.getSelectedService() == null) return true;
    else return false;
  }
  ...
  public void selectService()
    ...
    if (key != null && key.longValue() > 0) {
      ...
      } else storeServiceKeyInSessionBean(model.getSelectedService().getKey());
    }
  }
  ...
  /**
   * get the key for the service to load from the request parameter if it is invalid register an
   * error message and return null;
   */
  private Long determineServiceKeyFromRequestParam() {
    Map<String, String> params = ui.getExternalContext().getRequestParameterMap();
    String keyFromUrlParam = params.get(Constants.REQ_PARAM_SELECTED_SERVICE_KEY);
   ...
   *
   * @param errorMsgKey the error message key the be set.
   */
  ...
  void getServiceFromServer(long serviceKey) throws IllegalArgumentException, IOException {
    ...
    try {
      ...
      PartnerService partnerService = ui.findService(PartnerService.class);

      String mpl = ui.getMarketplaceId();

      Response response =
          partnerService.getAllServiceDetailsForMarketplace(
              serviceKey, ui.getViewLocale().getLanguage(), mpl);
      ...
      if (response.getReturnCodes().isEmpty()) {
        ...
        POServiceFeedback feedback = response.getResult(POServiceFeedback.class);
        ...
        List<VOService> relatedServicesForMarketplace = response.getResultList(VOService.class);
        List<Service> relatedServices = DEFAULT_VOSERVICE_MAPPER.map(relatedServicesForMarketplace);
        ...
        Map<String, POVendorAddress> addressMap = response.getResult(HashMap.class);
        if (addressMap != null) {

          if (addressMap.get(POVendorAddress.SERVICE_SELLER_SUPPLIER) != null) {
            model.setServiceSupplier(addressMap.get(POVendorAddress.SERVICE_SELLER_SUPPLIER));
          }
          if (addressMap.get(POVendorAddress.SERVICE_SERVICE_PARTNER) != null) {
            model.setServicePartner(addressMap.get(POVendorAddress.SERVICE_SERVICE_PARTNER));
          }
        }
        generateWarning();
      }
      ...
      ui.getRequest()
          .setAttribute(Constants.REQ_ATTR_ERROR_KEY, BaseBean.ERROR_SERVICE_NOT_AVAILABLE);
    } catch (OperationNotPermittedException e) {
      System.out.println("OperationNotPermittedException");
      if (ui.isLoggedIn()) {
        System.out.println("loggedIn" + BaseBean.ERROR_SERVICE_NOT_AVAILABLE_LOGGED_IN);
        ui.getRequest()
            .setAttribute(
                Constants.REQ_ATTR_ERROR_KEY, BaseBean.ERROR_SERVICE_NOT_AVAILABLE_LOGGED_IN);
      } else {
        System.out.println("NOT loggedIn" + BaseBean.ERROR_SERVICE_NOT_AVAILABLE);
        ui.getRequest()
            .setAttribute(Constants.REQ_ATTR_ERROR_KEY, BaseBean.ERROR_SERVICE_NOT_AVAILABLE);
      }
    }
  ...
  /**
   * Get the selected service object for showing the service details. When the service is not
   * subscribable, a warning is generated.
   *
  ...
  public void generateWarning() {
    ...
    if (!checkIfUserCanSubscribeToService()) {
      ui.getRequest()
          .setAttribute(Constants.REQ_ATTR_WARNING_KEY, BaseBean.WARNING_SUBSCRIBE_ONLY_BY_ADMIN);
    }

    if (model.getSelectedService() != null && !model.getSelectedService().isSubscribable()) {
      ui.getRequest()
          .setAttribute(Constants.REQ_ATTR_WARNING_KEY, BaseBean.WARNING_SUBSCRIBE_ONLY_ONCE);
    }
  }
  ...
  /**
   * Returns the base URL of a service with new parameter <code>serviceid</code>.
   *
  ...
  public String getServiceUrl() {
    ...
    if (selectedService == null
        || selectedService.getBaseURL() == null
        || !selectedService.isExternal()) {
      return "";
    }

    try {
      String serviceUrl = ADMStringUtils.removeEndingSlash(selectedService.getBaseURL().trim());
      ...
      result.append("serviceid=" + URLEncoder.encode(selectedService.getServiceId(), "UTF-8"));
      return result.toString();

    } catch (UnsupportedEncodingException e) {
      logger.logError(Log4jLogger.SYSTEM_LOG, e, LogMessageIdentifier.ERROR_UNSUPPORTED_ENCODING);
  ...
  boolean isServiceAccessible(long serviceKey) {
    ...
    if (null != model.getInvisibleProductKeys()
        && model.getInvisibleProductKeys().contains(Long.valueOf(serviceKey))) {
      return false;
    }
    return true;
  }
  ...
  void redirectToAccessDeniedPage() throws IllegalArgumentException, IOException {
  ...
  List<Service> removeInvisibleServices(List<Service> services) {
    ...
    for (int i = 0; i < services.size(); i++) {
      ...
      if (!invisibleProductKeySet.contains(Long.valueOf(service.getKey()))) {
  ...
  private void initInvisibleProductKeysForUser() throws ObjectNotFoundException {
    VOUserDetails voUserDetails = ui.getUserFromSessionWithoutException();
    if (null == voUserDetails || voUserDetails.hasAdminRole()) {
      model.setInvisibleProductKeys(null);
    } else {
      List<Long> invisibleProductKeys =
          getUserGroupService().getInvisibleProductKeysForUser(voUserDetails.getKey());
  ...
  public String getTargetUrl() {
    return Marketplace.MARKETPLACE_ROOT + "/serviceDetails.jsf";
  }
}
