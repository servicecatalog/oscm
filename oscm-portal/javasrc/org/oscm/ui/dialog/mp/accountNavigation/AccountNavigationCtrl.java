...
public class AccountNavigationCtrl extends BaseBean implements Serializable {
  ...
  /**
   * Check if reporting box should be displayed - reporting must be available and user must be
   * logged in and have the administrator role.
   *
   * @return <code>true</code> if report box is available, <code>false</code> otherwise.
   */
  ...
  /**
   * @param index the index of model.link or model.title
   * @return <code>true</code> if the index-1 of getHiddenElement() is available, <code>false</code>
   *     otherwise.
   */
  public boolean isLinkVisible(int index) {
    ...
    if (HiddenUIConstants.MARKETPLACE_MENU_ITEM_ACCOUNT_REPORTS.equals(hiddenElement)) {
      return isReportingAvailable();
    } else if (HiddenUIConstants.MARKETPLACE_MENU_ITEM_ACCOUNT_USERS.equals(hiddenElement)) {
      return isLoggedInAndAdmin() && !applicationBean.isUIElementHidden(hiddenElement);
    } else if (HiddenUIConstants.MARKETPLACE_MENU_ITEM_ACCOUNT_PAYMENT.equals(hiddenElement)) {
      return isPaymentAvailable();
    } else if (HiddenUIConstants.MARKETPLACE_MENU_ITEM_ACCOUNT_SUBSCRIPTIONS.equals(
        hiddenElement)) {
      return (isLoggedInAndAdmin()
              || isLoggedInAndSubscriptionManager()
              || isLoggedInAndUnitAdmin())
          && !applicationBean.isUIElementHidden(hiddenElement);
    } else if (HiddenUIConstants.MARKETPLACE_MENU_ITEM_ACCOUNT_UNITS.equals(hiddenElement)) {
      return isLoggedInAndUnitAdmin() && !applicationBean.isUIElementHidden(hiddenElement);
    } else if (HiddenUIConstants.MARKETPLACE_MENU_ITEM_ACCOUNT_ADMINISTRATION.equals(
        hiddenElement)) {
      return isAdministrationAccess() && !applicationBean.isUIElementHidden(hiddenElement);
    }
