...
public class OperationRecordCtrl {
  ...
  public void onFilterOperationsChange(final AjaxBehaviorEvent event) {
    ...
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug(
          String.format(
              "onFilterOperationsChange(event=%s) triggered, reloading records with myOperationsFlag=%s",
              event, myOpsOnlyNegated));
    }
  ...
  private void loadOperationRecords(final Boolean myOperationsOnlyFromSession) {
    ...
    final List<POOperationRecord> poOperationRecords =
        this.getOperationRecordService().getOperationRecords(myOperationsOnlyFromSession, language);
  ...
  protected List<OperationRecord> initOperationRecords(
      final List<POOperationRecord> poOperationRecords) {
    ...
    for (final POOperationRecord record : poOperationRecords) {
      ...
      if ((oldOpRecord = this.model.getOperationRecordByTransactionId(record.getTransactionId()))
          != null) {
