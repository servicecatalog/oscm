...
public class AppConfigurationCtrlTest {
        ...
        public void getInitialize_Success() throws Exception {
                // given
                doReturn(items).when(appConfigService)
        ...
        public void save_Add_Success() throws Exception {
                ...
                assertEquals(Boolean.FALSE,
                        Boolean.valueOf(model.isInitialized()));
        }
        ...
        public void restart_Failed() {
                ...
                doReturn(Boolean.FALSE).when(appTimerService)
                        .restart(anyBoolean());
                doReturn(session).when(request).getSession();
                ...
                assertEquals(Boolean.TRUE,
                        Boolean.valueOf(model.isRestartRequired()));
        }
        ...
        public void restart() {
                ...
                model.setToken(model.getToken());
                model.setRestartRequired(true);
                doReturn(Boolean.TRUE).when(appTimerService)
                        .restart(anyBoolean());
                doReturn(session).when(request).getSession();
                doReturn("uid").when(session).getAttribute(
                        SessionConstants.SESSION_USER_ID);
        ...
        public void shouldInvokeCanPingImplementation_whenCanPingIsRequested_givenWMWareControllerId() {
                ...
                try {
                        ...
                        fail("Should not thrown an exception: " + e
                                .getMessage());
                }

                verify(model, never()).addCanPingException(any());
        }

        @Test
        public void shouldUpdateVisibilityMap_whenCanPingIsRequested_givenUnsupportedControllerId() {
        ...
        @Test
        public void shouldUpdateVisibilityMapwhenCanPingIsRequested_givenExceptionThrown() {
                ...
                try {
                        doThrow(new ConfigurationException(
                                ...
                                .when(serviceController).canPing();
                } catch (ConfigurationException e) {
                }
        ...
        public void shouldPassPing_whenPingIsRequested_givenWMWareControllerId() {
                ...
                try {
                        when(serviceController.ping(anyString()))
                                .thenReturn(true);

                        ctrl.invokePing(Configuration.VMWARE_CONTROLLER_ID);

                        verify(serviceController, atLeastOnce())
                                .ping(Configuration.VMWARE_CONTROLLER_ID);
        ...
        public void shouldFailPing_whenPingIsRequested_givenWMWareControllerId() {
                ...
                verify(ctrl, atLeastOnce())
                        .addError("app.message.error.controller.unreachable");
        ...
        public void shouldShowErrorWithDetails_whenPingIsRequested_givenExceptionThrown() {
                ...
                try {
                        doThrow(new ServiceNotReachableException(
                                "Service Not Reachable Exception"))
                                .when(serviceController).ping(controllerId);
        ...
        private void setServiceControllerContext(String controllerId) {
                ...
                try {
                        ...
                        context.bind(
                                APPlatformController.JNDI_PREFIX + controllerId,
                                serviceController);
