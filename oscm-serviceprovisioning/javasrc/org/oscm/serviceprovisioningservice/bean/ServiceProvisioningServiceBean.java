...
public class ServiceProvisioningServiceBean
  ...
  private static final Log4jLogger logger =
      LoggerFactory.getLogger(ServiceProvisioningServiceBean.class);
  ...
  @EJB private TenantProvisioningServiceBean tenantProvisioning;
  ...
  @EJB CommunicationServiceLocal commService;

  @EJB LandingpageServiceLocal landingpageService;

  @EJB PriceModelAuditLogCollector priceModelAudit;

  @EJB ServiceAuditLogCollector serviceAudit;

  @EJB SubscriptionAuditLogCollector subscriptionAudit;

  @EJB ConfigurationServiceLocal configurationService;

  @EJB BillingAdapterLocalBean billingAdapterLocalBean;
  ...
  public List<VOService> getServicesForMarketplace(String marketplaceId) {
    return getServicesForMarketplace(marketplaceId, PerformanceHint.ALL_FIELDS);
  }

  public List<VOService> getServicesForMarketplace(
      String marketplaceId, PerformanceHint performanceHint) {

    ArgumentValidator.notEmptyString("marketplaceId", marketplaceId);

    Query query = dm.createNamedQuery("Product.getProductsForCustomerOnMarketplace");
    query.setParameter("customer", dm.getCurrentUser().getOrganization());
    query.setParameter("marketplaceId", marketplaceId);
    List<Product> productList =
        filterProducts(
            ParameterizedTypes.list(query.getResultList(), Product.class), marketplaceId);
    ...
    List<VOService> voServices = new ArrayList<>();
    for (Product product : productList) {
      voServices.add(ProductAssembler.toVOProduct(product, facade, performanceHint));
    }
  ...
  public VOServiceEntry getServiceForMarketplace(
    ...
    Product product = dm.getReference(Product.class, serviceKey.longValue());
    ...
    if (dm.getCurrentUserIfPresent() != null) {
      ...
      for (Marketplace marketplace : userMps) {
        if (marketplace.getMarketplaceId().equals(marketplaceId)) {
          returnSuspended = currentUser.hasRole(UserRoleType.MARKETPLACE_OWNER);
          break;
        }
      }
      ...
      if (product.getTargetCustomer() != null) {
        if (product.getTargetCustomer() == org) {
          ...
          if (product.getStatus() == ServiceStatus.ACTIVE
            ...
            if (!existsCatalogEntryForMarketplace(template, marketplaceId)) {
              return null;
            }
          } else {
          ...
          // allowed
          OperationNotPermittedException onp =
      ...
      CatalogEntry catalogEntry = getCatalogEntryForMarketplace(product, marketplaceId);
      ...
      if (catalogEntry.getMarketplace() == null
          || !marketplaceId.equals(catalogEntry.getMarketplace().getMarketplaceId())) {
    ...
    VOServiceEntry result =
        ProductAssembler.toVOServiceEntry(product, facade, subscriptionLimitReached);
  ...
  Product getCopyForCustomer(Product template, Organization customer) {
    ...
    List<Product> resultList = ParameterizedTypes.list(query.getResultList(), Product.class);
  ...
  void verifyNoSubscriptionCopy(Product product) throws OperationNotPermittedException {
    if (product.getTemplate() == null) {
      return;
    }
    if (ServiceType.isSubscription(product.getType())) {
      OperationNotPermittedException onp =
          new OperationNotPermittedException("Access to product copy not allowed.");
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG | Log4jLogger.AUDIT_LOG,
          onp,
  ...
  /**
   * Returns the catalog entry for the marketplace identified by the passed marketplace id. Returns
   * <code>null</code> if no entry exists.
   */
  private CatalogEntry getCatalogEntryForMarketplace(Product product, String marketplaceId) {
    ArgumentValidator.notEmptyString("marketplaceId", marketplaceId);
  ...
  /**
   * If a catalog entry for the passed marketplace exists the function returns <code>true</code>
   * otherwise <code>false</code>.
   */
  boolean existsCatalogEntryForMarketplace(Product product, String marketplaceId) {
    ArgumentValidator.notEmptyString("marketplaceId", marketplaceId);
    return getCatalogEntryForMarketplace(product, marketplaceId) != null ? true : false;
  }

  /**
   * The result list may contain marketing products that are available for all organizations, as
   * well as some that are particularly dedicated to the current customer and have been created as
   * copy of those original ones. Remove those from the list also remove inactive products
   *
   * @param products the list which should be filtered
   * @param marketplaceId the marketplace identifier
   */
  List<Product> filterProducts(List<Product> products, String marketplaceId) {
    Set<Long> prodKeysToBeRemoved = new HashSet<>();
    Map<Long, Product> prodKeysToBeReplaced = new HashMap<>();
    PlatformUser currentUser = dm.getCurrentUserIfPresent();

    boolean currentOrgIsMpOwner = isOrganizationMarketplaceOwner(currentUser, marketplaceId);
    for (Product product : products) {
      ...
      } else if (template != null && product.getType() == ServiceType.CUSTOMER_TEMPLATE) {
        // if the customer specific one wasn't removed but it has a
        // template, replace the template by the specific copy
        prodKeysToBeReplaced.put(Long.valueOf(template.getKey()), product);
  ...
  boolean isOrganizationMarketplaceOwner(PlatformUser currentUser, String marketplaceId) {
    if (currentUser != null && currentUser.isOrganizationAdmin()) {
      List<Marketplace> ownedMarketplaces = currentUser.getOrganization().getMarketplaces();
  ...
  public List<VOService> getRelatedServicesForMarketplace(
    ...
    if (dm.getCurrentUserIfPresent() != null) {
      ...
      query = dm.createNamedQuery("Product.getRelatedProductsForMarketplace");
      ...
      query = dm.createNamedQuery("Product.getRelatedPublicProductsForMarketplace");
      query.setParameter("marketplaceId", marketplaceId);
      query.setParameter("technicalProduct", prod.getTechnicalProduct());
      query.setParameter("vendor", prod.getVendor());
    }
  ...
  boolean hasOneSubscription(Product product) {
    Query query = dm.createNamedQuery("Subscription.numberOfVisibleSubscriptions");
    query.setParameter("productKey", Long.valueOf(product.getTechnicalProduct().getKey()));
    query.setParameter("orgKey", Long.valueOf(dm.getCurrentUser().getOrganization().getKey()));
  ...
  public List<VOService> getSuppliedServices(PerformanceHint performanceHint) {
    ...
    List<Product> productList = getProductsOfSupplier(currentUsersOrg, serviceTypes);
    LocalizerFacade facade = new LocalizerFacade(localizer, dm.getCurrentUser().getLocale());
    ProductAssembler.prefetchData(productList, facade, performanceHint);
    List<VOService> voList = new ArrayList<>();
    for (Product product : productList) {
      voList.add(ProductAssembler.toVOProduct(product, facade, performanceHint));
    }

    return voList;
  }

  private EnumSet<ServiceType> getServiceTypesForOrg(Organization currentUsersOrg) {
  ...
  /**
   * Retrieves the marketing products a supplier has defined so far. Obsolete and deleted products
   * are removed from the list.
   *
   * @param supplier The supplier organization for which the products have to be retrieved.
   * @return A list of subscribable products.
   */
  private List<Product> getProductsOfSupplier(
      Organization supplier, EnumSet<ServiceType> serviceTypes) {
    ...
    query.setParameter(
        "filterOutWithStatus", EnumSet.of(ServiceStatus.OBSOLETE, ServiceStatus.DELETED));
  ...
  public String importTechnicalServices(byte[] xml)
    ...
    TechnicalProductImportParser parser =
        new TechnicalProductImportParser(
            dm,
            localizer,
    ...
    try {
      parser.parse(xml);
    } catch (TechnicalServiceActiveException
  ...
  public VOService activateService(VOService service)
      throws ServiceStateException, ObjectNotFoundException, OrganizationAuthoritiesException,
    ...
    VOService voProduct = setActivationState(service, true, null, visChecker);
  ...
  public void activateServiceInt(TriggerProcess tp)
      throws ObjectNotFoundException, ServiceOperationException, TechnicalServiceNotAliveException,
    ...
    TriggerProcessParameter tpParam = tp.getParamValueForName(TriggerProcessParameterName.PRODUCT);
    ...
    setStatus(prod, product, ServiceStatus.ACTIVE, ServiceStatus.INACTIVE, user);

    // Update visibility (if catalog entries are given)
    TriggerProcessParameter tpCatEntries =
        tp.getParamValueForName(TriggerProcessParameterName.CATALOG_ENTRIES);
    if (tpCatEntries != null) {
      List<VOCatalogEntry> entries =
          ParameterizedTypes.list(tpCatEntries.getValue(List.class), VOCatalogEntry.class);
  ...
  /**
   * Validates the input parameters and availability of the technical product for an activation of a
   * product.
   *
   * @param product The product to be activated.
   * @throws ObjectNotFoundException Thrown in case the product does not exist.
   * @throws TechnicalServiceNotAliveException Thrown in case the technical product cannot be
   *     reached.
   * @throws ServiceNotPublishedException Thrown in case the service is currently not published on
   *     any marketplace.
   */
  private Product validateForProductActivation(VOService product)
    ...
    if (prod.getVendor().getGrantedRoleTypes().contains(OrganizationRoleType.SUPPLIER)) {
      priceModel = prod.getPriceModel();
    } else {
      priceModel = prod.getTemplate().getPriceModel();
    }
    if (priceModel == null) {
      ServiceOperationException sof =
          new ServiceOperationException(ServiceOperationException.Reason.MISSING_PRICE_MODEL);
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG | Log4jLogger.AUDIT_LOG,
          sof,
    ...
    if (prod.getCatalogEntries() != null) {
      for (CatalogEntry ce : prod.getCatalogEntries()) {
        if (ce.getMarketplace() == null) {
          ...
          ServiceNotPublishedException snp = new ServiceNotPublishedException(params);
          logger.logWarn(
              Log4jLogger.SYSTEM_LOG | Log4jLogger.AUDIT_LOG,
              snp,
    ...
    try {
      ...
      TechnicalServiceNotAliveException ex =
          new TechnicalServiceNotAliveException(
              ...
              new Object[] {prod.getTechnicalProduct().getTechnicalProductId()},
              e);
      logger.logWarn(
  ...
  public VOService deactivateService(VOService service)
    ...
    try {
      ...
      logger.logError(Log4jLogger.SYSTEM_LOG, sse, LogMessageIdentifier.ERROR_DEACTIVATE_SERVICE);
      throw sse;
  ...
  public void deactivateServiceInt(TriggerProcess tp)
    ...
    VOService product =
        tp.getParamValueForName(TriggerProcessParameterName.PRODUCT).getValue(VOService.class);
    Product prod = dm.getReference(Product.class, product.getKey());
    setStatus(prod, product, ServiceStatus.INACTIVE, ServiceStatus.ACTIVE, tp.getUser());
  ...
  /**
   * Sets the product state to the given product and all of its subscribable copies. Before that the
   * product is checked for a valid state and if it has not been changed in the meantime.
   *
   * @param prod the product to set the state for read from the database
   * @param product the voproduct passed in
   * @param newStatus the new state to set
   * @param requiredState the state in which the product has to be or <code>null</code>
   * @param user The user on behalf of which the current operation is executed.
   * @throws ServiceStateException in case the product isn't in the required state
   * @throws OperationNotPermittedException in case the current organization is not the owner of the
   *     product
   */
  private void setStatus(
      ...
      ServiceStatus requiredState,
      PlatformUser user)
      throws ServiceStateException, OperationNotPermittedException, ServiceOperationException {
    validateForProductStatusChange(prod, product, newStatus, requiredState, user);
   ...
   *
   * @param prod The product to change the status of.
   * @param product The value object for the product to be changed.
   * @param newStatus The status to be set.
   * @param requiredState The required status of the product.
   * @param user The user that performs the status change.
   * @throws OperationNotPermittedException Thrown in case the caller tries to modify the object of
   *     another organization.
   * @throws ServiceStateException Thrown in case the product state change fails as the current
   *     product state does not allow the intended modification.
   */
  private void validateForProductStatusChange(
      Product prod,
      VOService product,
      ServiceStatus newStatus,
      ServiceStatus requiredState,
      PlatformUser user)
      throws OperationNotPermittedException, ServiceOperationException, ServiceStateException {
    ...
    if (prod.getOwningSubscription() != null) {
      ServiceOperationException pof =
          new ServiceOperationException(Reason.STATE_CHANGE_FAILED_USED_BY_SUB);
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG | Log4jLogger.AUDIT_LOG,
          pof,
  ...
  public List<VOTechnicalService> getTechnicalServices(
    ...
    List<VOTechnicalService> result = new ArrayList<>();
    PlatformUser currentUser = dm.getCurrentUser();
    ...
    if (performanceHint == PerformanceHint.ONLY_IDENTIFYING_FIELDS) {
      for (TechnicalProduct tProd : tProds) {
        result.add(
            TechnicalProductAssembler.toVOTechnicalProduct(
                tProd, null, null, null, false, PerformanceHint.ONLY_IDENTIFYING_FIELDS));
      }
    } else {
      ...
      for (TechnicalProduct tProd : tProds) {
        ...
        result.add(
            TechnicalProductAssembler.toVOTechnicalProduct(
                tProd, paramDefs, platformEvents, facade, excludeNonConfigurableParamDefs));
      }
    }
  ...
  public void validateTechnicalServiceCommunication(VOTechnicalService technicalService)
    ...
    TechnicalProduct techProduct =
        dm.getReference(TechnicalProduct.class, technicalService.getKey());
    ...
    if (currentUserOrg.hasRole(OrganizationRoleType.TECHNOLOGY_PROVIDER)) {
      accessable =
          getTechnicalProductsInt(currentUserOrg, OrganizationRoleType.TECHNOLOGY_PROVIDER)
              .contains(techProduct);
    }
    if (!accessable && currentUserOrg.hasRole(OrganizationRoleType.SUPPLIER)) {
      accessable =
          getTechnicalProductsInt(currentUserOrg, OrganizationRoleType.SUPPLIER)
              .contains(techProduct);
    }
    if (!accessable) {
      OperationNotPermittedException onp =
  ...
  public void deleteTechnicalService(VOTechnicalService technicalService)
    ...
    Organization currentUserOrg = currentUser.getOrganization();
    if (!currentUserOrg.hasRole(OrganizationRoleType.TECHNOLOGY_PROVIDER)) {
      OrganizationAuthoritiesException ioa =
          new OrganizationAuthoritiesException(
              "Delete technical product +'" + technicalService.getKey() + "' failed.",
    ...
    verifyTechnicalServiceIsUpToDate(technicalService, technicalProduct, false);

    TechnicalProductCleaner cleaner = new TechnicalProductCleaner(dm, tenantProvisioning);
  ...
  /**
   * Verifies that the technical service is up to date - the versions of it and all depending
   * objects match the ones stored on server side.
   *
   * @param technicalService The value object.
   * @param technicalProduct The domain object.
   * @param ignoreNonConfigurableParameters Flag indicating whether to consider non-configurable
   *     parameters or not.
  ...
  void verifyTechnicalServiceIsUpToDate(
    ...
    List<VOEventDefinition> eventDefinitions = technicalService.getEventDefinitions();
    verifyListConsistency(eventDefinitions, events);

    List<ParameterDefinition> parameterDefinitions =
        new ArrayList<>(technicalProduct.getParameterDefinitions());
    // filter out the non-configurable parameter definitions, if required
    Iterator<ParameterDefinition> iterator = parameterDefinitions.iterator();
    while (iterator.hasNext()) {
      ParameterDefinition currentParamDef = iterator.next();
      if (!currentParamDef.isConfigurable() && ignoreNonConfigurableParameters) {
        iterator.remove();
      }
    }
    List<VOParameterDefinition> voParamDefs = technicalService.getParameterDefinitions();
    verifyListConsistency(voParamDefs, parameterDefinitions);

    List<RoleDefinition> roleDefinitions = technicalProduct.getRoleDefinitions();
    List<VORoleDefinition> voRoleDefinitions = technicalService.getRoleDefinitions();
    verifyListConsistency(voRoleDefinitions, roleDefinitions);

    List<TechnicalProductOperation> technicalProductOperations =
        technicalProduct.getTechnicalProductOperations();
    List<VOTechnicalServiceOperation> technicalServiceOperations =
        technicalService.getTechnicalServiceOperations();
    verifyListConsistency(technicalServiceOperations, technicalProductOperations);
  }

  /**
   * Verifies the list of value objects against the list of domain objects. If the value object
   * element list contains an entry that has a version clash with the corresponding domain object or
   * in case one of the domain objects is not considered as value object (so it might have been
   * added on server side later), a ConcurrentModificationException is thrown.
   *
   * @param voList The value object element list to check.
   * @param doList The domain object element list to compare against.
   * @throws ConcurrentModificationException
   */
  private void verifyListConsistency(
      List<? extends BaseVO> voList, List<? extends DomainObject<?>> doList)
      throws ConcurrentModificationException {
    Map<Long, DomainObject<?>> keyToDomainObject = new HashMap<>();
    for (DomainObject<?> domainObject : doList) {
      keyToDomainObject.put(Long.valueOf(domainObject.getKey()), domainObject);
    }
    for (BaseVO valueObject : voList) {
      DomainObject<?> correspondingDomainObject =
          keyToDomainObject.remove(Long.valueOf(valueObject.getKey()));
      if (correspondingDomainObject != null) {
        BaseAssembler.verifyVersionAndKey(correspondingDomainObject, valueObject);
      }
    }
    if (!keyToDomainObject.isEmpty()) {
      ...
      DomainObject<?> object = keyToDomainObject.values().iterator().next();
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG,
          cme,
  ...
  public VOServiceDetails createService(
      VOTechnicalService technicalService, VOService service, VOImageResource voImageResource)
    ...
    TechnicalProduct technicalProduct =
        dm.getReference(TechnicalProduct.class, technicalService.getKey());
    verifyTechnicalServiceIsUpToDate(technicalService, technicalProduct, true);
    try {
      product = prepareMarketingProduct(technicalService.getKey(), service, true);
    } catch (ServiceStateException | ConcurrentModificationException e) {
      // this must not happen
      SaaSSystemException sse = new SaaSSystemException(e);
      logger.logError(Log4jLogger.SYSTEM_LOG, sse, LogMessageIdentifier.ERROR_CREATE_SERVICE);
      throw sse;
    }
  ...
  public VOServiceDetails updateService(VOServiceDetails service, VOImageResource imageResource)
    ...
    Product storedService = dm.getReference(Product.class, service.getKey());
    List<Product> customerProducts = getCustomerSpecificCopyProducts(storedService);
    validateProductStatus(customerProducts);

    Product product =
        prepareMarketingProduct(storedService.getTechnicalProduct().getKey(), service, false);
    ...
    if (service.getVersion() < createdProduct.getVersion() || isLocalizedTextChanged) {
      serviceAudit.updateService(
          dm,
          storedService,
          isShortDescriptionChanged,
          isDescriptionChanged,
          isCustomTabNameChanged,
          dm.getCurrentUser().getLocale());
    }
  ...
  void validateProductStatus(List<Product> customerProducts) throws ServiceStateException {
  ...
  void updateCustomerSpecificService(
      Product templateProduct, List<Product> customerProducts, VOServiceDetails createdProduct)
      throws ValidationException {
    for (Product prod : customerProducts) {
      Product product = ProductAssembler.updateCustomerTemplateProduct(prod, createdProduct);
      updateParametersForCustomerTemplate(templateProduct, product);
      dm.flush();
    }
  }

  void updateParametersForCustomerTemplate(
      Product templateProduct, Product customerTemplateProduct) {
    ParameterSet customerTemplateParameterSet = customerTemplateProduct.getParameterSet();
    ParameterSet templateParameterSet = templateProduct.getParameterSet();
    Map<Long, Parameter> obsoleteParameters = new HashMap<>();
    if (customerTemplateParameterSet != null) {
      for (final Parameter p : customerTemplateParameterSet.getParameters()) {
        obsoleteParameters.put(Long.valueOf(p.getParameterDefinition().getKey()), p);
      }
    }
    if (customerTemplateParameterSet == null) {
      customerTemplateProduct.setParameterSet(new ParameterSet());
      customerTemplateParameterSet = customerTemplateProduct.getParameterSet();
    }
    if (templateParameterSet == null) {
      templateParameterSet = new ParameterSet();
    }

    List<Parameter> customerTemplateParameters = customerTemplateParameterSet.getParameters();
    for (Parameter param : templateParameterSet.getParameters()) {
      final Parameter oldParam =
          obsoleteParameters.remove(Long.valueOf(param.getParameterDefinition().getKey()));
      if (oldParam == null) {
        // add new param
        customerTemplateParameters.add(param.copy(customerTemplateParameterSet));
      } else {
        // update param
  ...
  public VOServiceDetails getServiceDetails(Product product, LocalizerFacade facade) {
    ...
    VOServiceDetails createdProduct =
        ProductAssembler.toVOProductDetails(
            product, platformParameters, platformEvents, isImageDefined(product), facade);

    return createdProduct;
  }

  /**
   * Modifies or creates a marketing product according to the given parameters. The modifications
   * will be stored directly.
   *
   * @param technicalProductKey The key of the technical product the marketing product is based on.
   * @param productToModify The value object representation of the object as is should finally look
   *     like
   * @param isCreation Indicates whether the product has to be created from scratch or if it should
   *     only be updated.
   * @return The modified product.
   * @throws ObjectNotFoundException Thrown in case the technical product or the marketing product
   *     (in case of an update) cannot be found.
   * @throws OperationNotPermittedException Thrown in case an attempt is made to use parameters
   *     defined for another technical product or in case the specified technical product is not
   *     related to the marketing product.
   * @throws ValidationException Thrown in case the product identifier is not valid.
   * @throws NonUniqueBusinessKeyException Thrown in case a product with the same identifier does
   *     already exist.
   * @throws ServiceStateException Thrown in case the product is not in the state {@link
   *     ServiceStatus#INACTIVE}
   * @throws ConcurrentModificationException
   * @throws DeletionConstraintException
   */
  private Product prepareMarketingProduct(
      long technicalProductKey, VOService productToModify, boolean isCreation)
      throws ObjectNotFoundException, OperationNotPermittedException, ValidationException,
          NonUniqueBusinessKeyException, ServiceStateException, ConcurrentModificationException {
    ...
    TechnicalProduct tProd = dm.getReference(TechnicalProduct.class, technicalProductKey);
    PermissionCheck.hasMarketingPermission(tProd, currentUserOrg, dm, logger);
    ...
    if (isCreation) {
      product = ProductAssembler.toNewTemplateProduct(productToModify, tProd, currentUserOrg);
    } else {
      product = dm.getReference(Product.class, productToModify.getKey());
      ProductValidator.validateInactiveOrSuspended(
          ProductAssembler.getProductId(product), product.getStatus());
      oldProductId = product.getProductId();
      if (!oldProductId.equals(productToModify.getServiceId())) {
        validateChangedId(productToModify.getServiceId(), currentUserOrg);
      }
      product = ProductAssembler.updateProduct(product, productToModify);
    }

    // handle the parameter settings
    List<Parameter> parametersToLog =
        modifyParameters(productToModify, currentUser, tProd, product, isCreation);

    // now store the product. due to the cascade definitions, the parameters
    // will be stored as well.
    if (isCreation) {
      dm.persist(product);
      CatalogEntry catalogEntry = QueryBasedObjectFactory.createCatalogEntry(product, null);
      copyOperatorPriceModel(catalogEntry, currentUserOrg.getOperatorPriceModel());
    ...
    String oldDescription =
        localizer.getLocalizedTextFromDatabase(
            userLocale, product.getKey(), LocalizedObjectTypes.PRODUCT_MARKETING_DESC);
    String oldShortDescription =
        localizer.getLocalizedTextFromDatabase(
            userLocale, product.getKey(), LocalizedObjectTypes.PRODUCT_SHORT_DESCRIPTION);
    String oldProductName =
        localizer.getLocalizedTextFromDatabase(
            userLocale, product.getKey(), LocalizedObjectTypes.PRODUCT_MARKETING_NAME);
    String oldCustomTabName =
        localizer.getLocalizedTextFromDatabase(
            userLocale, product.getKey(), LocalizedObjectTypes.PRODUCT_CUSTOM_TAB_NAME);

    isLocalizedTextChanged = false;
    if (productName != null && !productName.equals(oldProductName)) {
      isLocalizedTextChanged = true;
      BLValidator.isName(ProductAssembler.FIELD_NAME_NAME, productName, false);
      localizer.storeLocalizedResource(
          userLocale, product.getKey(), LocalizedObjectTypes.PRODUCT_MARKETING_NAME, productName);
    }

    isDescriptionChanged = false;
    if (productDescription != null && !productDescription.equals(oldDescription)) {
      isLocalizedTextChanged = true;
    ...
    if (productShortDescription != null && !productShortDescription.equals(oldShortDescription)) {
      isLocalizedTextChanged = true;
    ...
    if (productCustomTabName != null && !productCustomTabName.equals(oldCustomTabName)) {
      isLocalizedTextChanged = true;
    ...
    if (isCreation) {
      ...
      List<VOLocalizedText> techDescriptions =
          localizer.getLocalizedValues(
              tProd.getKey(), LocalizedObjectTypes.TEC_PRODUCT_TECHNICAL_DESC);
      ...
      if (techDescriptions != null) {
        ...
        for (VOLocalizedText desc : techDescriptions) {
          ...
          if (!userLocale.equals(locale)) {
            localizer.storeLocalizedResource(
                locale, key, LocalizedObjectTypes.PRODUCT_MARKETING_DESC, desc.getText());
          }
        }
      }
    }
  ...
  void copyOperatorPriceModel(CatalogEntry catalogEntry, RevenueShareModel operatorPriceModel) {
    ...
    try {
      ...
      SaaSSystemException sse =
          new SaaSSystemException("Caught unexpected NonUniqueBusinessKeyException", e);
      logger.logError(
          Log4jLogger.SYSTEM_LOG, sse, LogMessageIdentifier.ERROR_UNEXPECTED_BK_VIOLATION);
      throw sse;
    }
  }
  ...
  public void copyDefaultPaymentEnablement(Product product, Organization vendor) {
    List<OrganizationRefToPaymentType> types = vendor.getDefaultServicePaymentTypes();
    for (OrganizationRefToPaymentType ref : types) {
      ProductToPaymentType ptpt = new ProductToPaymentType(product, ref.getPaymentType());
  ...
  /**
   * Adapts the parameter settings for a given product according to the specified input parameters.
   * Matching parameters will be updated, new ones will be added. Those that do not exist in the
   * input will be removed.
   *
   * @param productToModify The value object representation of the product. It contains the values
   *     that have to be persisted.
   * @param currentUser The user that invoked the operation.
   * @param tProd The technical product the marketing product to be changed is based upon.
   * @param product The product domain object that corresponds to the product input parameter.
   * @param isCreation true if define service,false if update service
   * @throws ObjectNotFoundException Thrown in case the parameter definitions cannot be found.
   * @throws OperationNotPermittedException Thrown in case an attempt is made to store parameters
   *     that are based on another technical product's parameter definitions.
   * @throws ValidationException Thrown in case the parameter values could not match the specified
   *     datatype.
   * @throws ConcurrentModificationException
   * @throws GeneralSecurityException
   * @throws DeletionConstraintException
   */
  private List<Parameter> modifyParameters(
      VOService productToModify,
    ...
    // removed.
    Map<Long, Parameter> obsoleteParameters = new HashMap<>();
    List<Parameter> parametersToLog = new ArrayList<>();
    ...
    if (parameters != null && !parameters.isEmpty()) {
      ...
      for (VOParameter parameter : parameters) {
        ...
        try {
          paramDef =
              dm.getReference(
                  ParameterDefinition.class, parameter.getParameterDefinition().getKey());
          ...
          OperationNotPermittedException onp =
              new OperationNotPermittedException(
                  String.format(
                      ...
                      Long.valueOf(paramDef.getKey()), Long.valueOf(currentUser.getKey())));
          logger.logWarn(
              Log4jLogger.SYSTEM_LOG,
              onp,
        ...
        if (paramDef.getParameterType() == ParameterType.SERVICE_PARAMETER
            && paramDef.getTechnicalProduct().getKey() != tProd.getKey()) {
          sessionCtx.setRollbackOnly();
          OperationNotPermittedException onp =
              new OperationNotPermittedException("Creation of marketing product failed");
          logger.logWarn(
              Log4jLogger.SYSTEM_LOG,
              onp,
        ...
        List<Parameter> storedParameters = currentParameterSet.getParameters();
        if (isParameterToBeSaved(isDirectAccess, parameter, paramDef)) {
          final Parameter existingParameter =
              obsoleteParameters.remove(Long.valueOf(parameter.getKey()));
          if (existingParameter == null) {
            final Parameter param = ParameterAssembler.toParameter(parameter);
            ...
            boolean isChanged =
                isDifferentFromExistingValue(
                    existingParameter, ParameterAssembler.toParameter(parameter));
            ParameterAssembler.updateParameter(existingParameter, parameter);
    ...
    if (currentParameterSet != null) {
      for (Parameter obsoleteParameter : obsoleteParameters.values()) {
        ...
        Parameter parameterToLog = obsoleteParameter.copy(obsoleteParameter.getParameterSet());
  ...
  /**
   * When defining service, if the parameter's value and userOption are equals with the default, it
   * will not be logged, otherwise it will be logged.
   */
  boolean isDifferentFromDefaultValue(Parameter parameter) {
    String defaultValue =
        parameter.getParameterDefinition().getDefaultValue() == null
            ? ""
            : parameter.getParameterDefinition().getDefaultValue();
    boolean defaultUserOption = false;
    String inputValue = parameter.getValue() == null ? "" : parameter.getValue();
    boolean inputUserOption = parameter.isConfigurable();
    if (defaultValue.equals(inputValue) && defaultUserOption == inputUserOption) {
      return false;
    }
    return true;
  }
  ...
  /**
   * When updating service, if the parameter's value and userOption are equals with the existing, it
   * will not be logged. Otherwise it will be logged.
   */
  ...
  private void logUpdateServiceParameters(
      DataService dataService, Product product, Parameter parameter) {
    String parameterValue = "";
    if (parameter.getParameterDefinition().getValueType().equals(ParameterValueType.BOOLEAN)) {
    ...
    serviceAudit.updateServiceParameters(
        dataService,
        product,
  ...
  /**
   * Removes all existing {@link PricedParameter}s based on the provided {@link Parameter}. Only to
   * be used on Products that are not billing relevant - so templates that have no subscription
   * assigned
   *
   * @param parameter the {@link Parameter} to remove the {@link PricedParameter}s for
   */
  private void removePricedParameters(Parameter parameter) {
    final Query query = dm.createNamedQuery("PricedParameter.getForParameter");
    query.setParameter("parameter", parameter);

    List<PricedParameter> list =
        ParameterizedTypes.list(query.getResultList(), PricedParameter.class);
  ...
  /**
   * Checks if the parameter can be saved. This is the case for parameters that have a value set or
   * if they are configurable and they are no user related platform parameters for technical
   * products with access type DIRECT.
   *
   * @param isDirectAccess <code>true</code> if the technical product uses direct access
   * @param parameter the value object to check the parameter value and the configurable flag at
   * @param paramDef the domain object to get the parameter definition id from
   * @return
   */
  private boolean isParameterToBeSaved(
      boolean isDirectAccess, VOParameter parameter, ParameterDefinition paramDef) {
    return (!GenericValidator.isBlankOrNull(parameter.getValue()) || parameter.isConfigurable())
        && !(isDirectAccess
            && (PlatformParameterIdentifiers.CONCURRENT_USER.equals(paramDef.getParameterId())
                || PlatformParameterIdentifiers.NAMED_USER.equals(paramDef.getParameterId())));
  }
  ...
  public VOServiceDetails getServiceDetails(VOService service)
    ...
    Product storedProduct = dm.getReference(Product.class, service.getKey());

    if (storedProduct.getType() == ServiceType.PARTNER_SUBSCRIPTION
        && org.getGrantedRoleTypes().contains(OrganizationRoleType.SUPPLIER)) {
      PermissionCheck.owns(storedProduct.getTemplate().getTemplate(), org, logger, sessionCtx);
    } else {
  ...
  public VOServiceDetails savePriceModel(VOServiceDetails service, VOPriceModel priceModel)
    ...
    Product product =
        prepareProductWithPriceModel(service, priceModel, null, ServiceType.TEMPLATE, null);
  ...
  void validateCurrencyUniqunessOfMigrationPath(
      VOPriceModel priceModel, VOService referencedService) throws PriceModelException {

    Product referencedProduct = dm.find(Product.class, referencedService.getKey());
    if (referencedProduct != null) {
      // service is referenced service
      List<ProductReference> compatibleProductList =
          referencedProduct.getCompatibleProductsTarget();
      for (ProductReference prodRef : compatibleProductList) {
        PriceModel referenceModel = prodRef.getSourceProduct().getPriceModel();
        if (!isCompatibleCurrency(referenceModel, priceModel)) {
          PriceModelException pme =
              new PriceModelException(PriceModelException.Reason.UNMODIFIABLE_CURRENCY);
          logger.logWarn(
              Log4jLogger.SYSTEM_LOG,
              pme,
              LogMessageIdentifier
                  .WARN_SAVE_PRICE_MODEL_FAILED_NOT_SAME_CURRENCY_COMPATIBLE_PRODUCT,
      ...
      for (ProductReference prodRef : compatibleProductList) {
        PriceModel referenceModel = prodRef.getTargetProduct().getPriceModel();
        if (!isCompatibleCurrency(referenceModel, priceModel)) {
          PriceModelException pme =
              new PriceModelException(PriceModelException.Reason.UNMODIFIABLE_CURRENCY);
          logger.logWarn(
  ...
  /**
   * Loop through the list of stepped prices and update the values for free amount and additional
   * price (the limit of the predecessor defines the values).
   *
  ...
  void updateFreeAmountAndAdditionalPrice(List<SteppedPrice> list) {
    ...
    for (int i = 1; i < size; i++) {
      ...
      if (prevStep.getLimit() == null) {
        ...
        list.get(i)
            .setAdditionalPrice(BigDecimal.ZERO.setScale(PriceConverter.NORMALIZED_PRICE_SCALING));
      } else {
        list.get(i).setFreeEntityCount(prevStep.getLimit().longValue());
        list.get(i)
            .setAdditionalPrice(
                (BigDecimal.valueOf(prevStep.getLimit().longValue())
                        .subtract(BigDecimal.valueOf(prevStep.getFreeEntityCount())))
                    .multiply(prevStep.getPrice())
                    .add(prevStep.getAdditionalPrice())
                    .setScale(PriceConverter.NORMALIZED_PRICE_SCALING, RoundingMode.HALF_UP));
      }
    }
    if (size > 0) {
  ...
  /**
   * Prepares and stores a product by setting a price model on it, considering all the settings of
   * the provided parameters.
   *
   * @param voProductDetails The details on the product to be updated.
   * @param voPriceModel The price model information to be set.
   * @param targetCustomer The target customer to be set in the copy. Will be ignored if no copy is
   *     required.
   * @param subscription The subscription of the price model.
   * @return The updated product.
   * @throws ObjectNotFoundException Thrown in case the product cannot be found.
   * @throws OperationNotPermittedException Thrown in case the operation cannot be performed, as an
   *     attempt is made to illegally use data.
   * @throws CurrencyException Thrown in case the specified currency is not supported by the server.
   * @throws ValidationException Thrown in case a price for the price model or one of its events is
   *     negative.
   * @throws ServiceStateException Thrown in case we don't create a copy and the product's state
   *     isn't {@link ServiceStatus#INACTIVE}
   * @throws ConcurrentModificationException
   */
  Product prepareProductWithPriceModel(
      VOServiceDetails voProductDetails,
      VOPriceModel voPriceModel,
      Organization targetCustomer,
      ServiceType productType,
      Subscription subscription)
      throws ObjectNotFoundException, OperationNotPermittedException, CurrencyException,
          ValidationException, ServiceStateException, ConcurrentModificationException {
    ...
    if (productType == ServiceType.CUSTOMER_TEMPLATE) {
      if (product.getTemplate() == null) {
        ...
        product = copyCustomerProduct(targetCustomer, currentUser, product);
    ...
    dm.flush();
    boolean descriptionChanged = localizePriceModel(voPriceModel, currentUser, priceModel);
    ...
    if (!ServiceType.isSubscription(productType)) {
      licenseChanged =
          saveLicenseInformationForPriceModel(
    ...
    if (licenseChanged || descriptionChanged) {
      if (ServiceType.isTemplate(productType)) {
        priceModelAudit.addLogEntryForAuditLogData(
            dm,
            product,
        ...
        subscriptionAudit.addLogEntryForAuditLogData(
            dm,
  ...
  void setPriceModelToFree(VOPriceModel voPriceModel, PriceModel priceModel) {
    ...
    PriceModelHandler priceModelHandler =
        new PriceModelHandler(dm, priceModel, DateFactory.getInstance().getTransactionTime());
    priceModelHandler.resetToNonChargeable(PriceModelType.FREE_OF_CHARGE);
    priceModelAudit.editPriceModelTypeToFree(
        dm, priceModel, voPriceModel.getKey(), oldPriceModelType);
  }
  ...
  private Product copyCustomerProduct(
      Organization targetCustomer, PlatformUser currentUser, Product product) {
  ...
  void setPriceModelToChargeable(
      VOPriceModel voPriceModel,
      PriceModel priceModel,
      boolean priceModelCreatedInTransaction,
      boolean isCreatePriceModel,
      ServiceType productType,
      Organization targetCustomer,
      Subscription subscription,
      boolean isTemplateExistsForCustomer)
      throws ValidationException, OperationNotPermittedException, ConcurrentModificationException,
          CurrencyException {
    ...
    // business code
    SupportedCurrency currency = getSupportedCurrency(voPriceModel, priceModel.getProduct());
    ...
    priceModel.setPricePerUserAssignment(voPriceModel.getPricePerUserAssignment());
    ...
    setEvents(voPriceModel, priceModel, product, priceModelCreatedInTransaction);

    List<Parameter> parameters = new ArrayList<>();
    if (product.getParameterSet() != null) {
      parameters = product.getParameterSet().getParameters();
    }
    List<VOPricedParameter> selectedParameters = voPriceModel.getSelectedParameters();
    List<PricedParameter> pricedParameters =
        convertAndValidateParameters(
            voPriceModel.getKey(),
            selectedParameters,
            parameters,
            priceModel,
    ...
    setPricedParametersForPriceModel(
        voPriceModel.getKey(),
        priceModel,
        pricedParameters,
        selectedParameters,
    ...
    List<VOPricedRole> roleSpecificUserPrices = voPriceModel.getRoleSpecificUserPrices();
    validatePricedProductRoles(roleSpecificUserPrices, priceModel.getProduct());
    setRoleSpecificPrices(
        voPriceModel.getKey(),
        priceModel,
        null,
        null,
    ...
    List<SteppedPrice> steppedPrices =
        convertAndValidateSteppedPrices(
            voPriceModel.getKey(),
            voPriceModel.getSteppedPrices(),
            priceModel,
            null,
            null,
            priceModelCreatedInTransaction);
    if (!steppedPrices.isEmpty()
        && BigDecimal.ZERO.compareTo(priceModel.getPricePerUserAssignment()) != 0) {
      ValidationException ve =
          new ValidationException(
              ValidationException.ReasonEnum.STEPPED_USER_PRICING,
              "pricePerUserAssignment",
              new Object[] {});
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG,
          ve,
    ...
    priceModelAudit.editPriceModelTypeToChargeable(
        dm,
        priceModel,
        voPriceModel.getKey(),
        oldCurrency,
        ...
        oldFreePeriod,
        oldPricingPeriod);
    priceModelAudit.editSubscriptionPrice(dm, priceModel, oldSubscriptionPrice, isCreatePriceModel);
    if (productType.equals(ServiceType.TEMPLATE)
        || productType.equals(ServiceType.CUSTOMER_TEMPLATE)) {
      priceModelAudit.editOneTimeFee(dm, priceModel, oldOneTimeFee, isCreatePriceModel);
    }
    if (voPriceModel.getSteppedPrices() != null && voPriceModel.getSteppedPrices().isEmpty()) {
      priceModelAudit.editUserPrice(dm, priceModel, oldUserPrice, isCreatePriceModel);
    }
  }
  ...
  void setEvents(
      ...
      throws ValidationException, OperationNotPermittedException, ConcurrentModificationException {
    TechnicalProduct tp = product.getTechnicalProduct();
    List<Event> events = new ArrayList<>(tp.getEvents());
    events.addAll(getPlatformEvents(tp));
    convertAndValidateEvents(
        voPriceModel.getKey(),
        voPriceModel.getConsideredEvents(),
        events,
        priceModel,
        priceModelCreatedInTransaction);
  }

  SupportedCurrency getSupportedCurrency(VOPriceModel priceModel, Product product)
    ...
    if (priceModel.isChargeable()) {
      if (currency == null) {
        ...
        // exception
        CurrencyException uc =
            new CurrencyException(
                "Creation of price model failed.", new Object[] {priceModel.getCurrencyISOCode()});
        logger.logWarn(
            Log4jLogger.SYSTEM_LOG,
            uc,
  ...
  boolean localizePriceModel(
      VOPriceModel priceModel, PlatformUser currentUser, PriceModel priceModelToStore) {
    ...
    if (!currentDescription.equals(newDescription)) {
      if (priceModel.isChargeable()) {
        ...
        localizer.removeLocalizedValues(
            priceModel.getKey(), LocalizedObjectTypes.PRICEMODEL_DESCRIPTION);
      }
  ...
  boolean saveLicenseInformationForPriceModel(
      long productKey,
      long priceModelKey,
      VOPriceModel priceModel,
      PlatformUser currentUser,
      boolean isCreateNewPriceModel) {
    ...
    for (VOLocalizedText localizedText : oldLicenses) {
      ...
      localizer.storeLocalizedResource(
          locale, priceModelKey, LocalizedObjectTypes.PRICEMODEL_LICENSE, oldLicense);
    }
    localizer.storeLocalizedResource(
        userLocale, priceModelKey, LocalizedObjectTypes.PRICEMODEL_LICENSE, newLicense);
    return !newLicense.isEmpty();
  }

  private List<VOLocalizedText> getOldLicenses(
      long productKey, long priceModelKey, boolean isCreateNewPriceModel) {
    if (isCreateNewPriceModel) {
      return localizer.getLocalizedValues(productKey, LocalizedObjectTypes.PRODUCT_LICENSE_DESC);
    }
    return localizer.getLocalizedValues(priceModelKey, LocalizedObjectTypes.PRICEMODEL_LICENSE);
  }

  /**
   * Validates the value object input for the priced product roles.
   *
   * @param roleSpecificUserPrices The value objects.
   * @param product The related product.
   * @throws OperationNotPermittedException Thrown in case a defined role definition is not
   *     compatible to the product.
   */
  void validatePricedProductRoles(List<VOPricedRole> roleSpecificUserPrices, Product product)
      throws OperationNotPermittedException {
    Map<Long, RoleDefinition> keyRoleMap = new HashMap<>();
    for (RoleDefinition rd : product.getTechnicalProduct().getRoleDefinitions()) {
    ...
    for (VOPricedRole voPpr : roleSpecificUserPrices) {
      ...
      RoleDefinition rdToSet = keyRoleMap.get(Long.valueOf(roleDefinitionKey));
      if (rdToSet == null) {
        OperationNotPermittedException onp =
            new OperationNotPermittedException(
                String.format(
   ...
   *
   * @param priceModel The price model to be enhanced.
   * @param pricedParameter The priced parameter to be enhanced. Only used if no price model is set.
   * @param pricedOption The priced option to be enhanced. Only used if no price model and priced
   *     parameter is set.
   * @param roleSpecificUserPrices The user prices to be stored.
   * @param targetCustomer The targetCustomer of the price model.
   * @param subscription The subscription of the price model.
   * @param priceModelCreatedInTransaction Indicates whether the price model was created in this
   * @param oldPricedProductRoles The list of PricedProductRole of template price model. Only used
   *     if price model for service exists as a template for price model for customer
   * @throws ValidationException Thrown in case a negative price is specified for the price per user
   *     field.
   * @throws ConcurrentModificationException Thrown in case the value object's version does not
   *     match the domain object's.
   * @throws OperationNotPermittedException Thrown in case a passed priced role value object key
   *     does not belong to the domain object.
   */
  void setRoleSpecificPrices(
      ...
      throws ValidationException, ConcurrentModificationException, OperationNotPermittedException {

    List<RoleDefinition> roleDefinitions = new ArrayList<>();
    List<PricedProductRole> existingProductRolePrices = new ArrayList<>();
    if (priceModel != null) {
      roleDefinitions = priceModel.getProduct().getTechnicalProduct().getRoleDefinitions();
      existingProductRolePrices = priceModel.getRoleSpecificUserPrices();
    } else if (pricedParameter != null) {
      roleDefinitions =
          pricedParameter.getPriceModel().getProduct().getTechnicalProduct().getRoleDefinitions();
      existingProductRolePrices = pricedParameter.getRoleSpecificUserPrices();
      ...
      roleDefinitions =
          pricedOption
              ...
              .getTechnicalProduct()
              .getRoleDefinitions();
      existingProductRolePrices = pricedOption.getRoleSpecificUserPrices();
    }
    ...
    for (VOPricedRole voPricedProductRole : roleSpecificUserPrices) {
      ...
      if (pprMap.containsKey(roleKey) && roleKey.longValue() != 0) {
        ...
        validateDomainObjectKey(
            voPricedProductRole, pricedProductRole, priceModelCreatedInTransaction);
        voPricedProductRole.setKey(pricedProductRole.getKey());
        updatePricedProductRole(
            voPriceModelKey,
            voPricedProductRole,
            pricedProductRole,
            priceModel,
  ...
  void updatePricedProductRole(
      long voPriceModelKey,
      VOPricedRole pricedProductRole,
      PricedProductRole pprToUpdate,
      PriceModel priceModel,
    ...
    PricedProductRoleAssembler.updatePricedProductRole(pricedProductRole, pprToUpdate);
    pprToUpdate.setRoleDefinition(roleDefinition);

    if (priceModel != null) {
      priceModelAudit.editServiceRolePrice(
          dm,
          voPriceModelKey,
          priceModel,
          pprToUpdate,
      ...
      priceModelAudit.editParameterUserRolePrice(dm, voPriceModelKey, pprToUpdate, oldPricePerUser);
    } else {
      priceModelAudit.editParameterOptionUserRolePrice(dm, pprToUpdate, oldPricePerUser);
    }
  }

  PricedProductRole createPricedProductRole(
      long voPriceModelKey,
      VOPricedRole pricedProductRole,
      PriceModel priceModel,
      RoleDefinition roleDefinition,
    ...
    PricedProductRole pprToCreate =
        PricedProductRoleAssembler.toPricedProductRole(pricedProductRole);
    pprToCreate.setRoleDefinition(roleDefinition);
    BigDecimal oldPrice = findOldPrice(oldPricedProductRoles, roleDefinition);
    ...
    if (priceModel != null) {
      ...
      priceModelAudit.editParameterUserRolePrice(
          dm, voPriceModelKey, pprToCreate, DEFAULT_PRICE_VALUE);
    } else if (pricedOption != null) {
      priceModelAudit.editParameterOptionUserRolePrice(dm, pprToCreate, oldPrice);
    }
    return pprToCreate;
  }

  private BigDecimal findOldPrice(
      List<PricedProductRole> oldPricedProductRoles, RoleDefinition roleDefinition) {
  ...
  /**
   * Validates the {@link VOPricedEvent}s and checks if they are defined based on the event
   * definitions from the products technical products. Tries to match each {@link VOPricedEvent} to
   * one of the existing {@link PricedEvent}s (retrieved from the provided {@link PriceModel}) - if
   * a matching domain object was found, it will be updated and added to the result list; if not, a
   * new one will be created and added to the result list. All remaining domain objects, where no
   * matching value object was found for, will be removed with the data manager.
   *
   * @param voPricedEvents the {@link VOPricedEvent}s specifying the new values - can be a mixture
   *     of existing and new priced events
   * @param eventDefinitions a list of {@link Event} that belongs to the products technical product
   *     used for validating if the {@link PricedEvent}s to save belong to this set of {@link
   *     Event}s
   * @param priceModel the {@link PriceModel} to get the currently existing {@link PricedEvent}s
   *     from and the one to set to the created {@link PricedEvent}s
   * @param priceModelCreatedInTransaction Indicates whether the price model was created in this
   *     transaction or not.
   * @return the list of {@link PricedEvent} that has to be set to the {@link PriceModel}
   * @throws ValidationException in case of an validation error when converting the value object to
   *     a domain object
   * @throws OperationNotPermittedException in case the priced event does not belong to an event of
   *     the products technical product
   * @throws ConcurrentModificationException Thrown in case an event's value object's version does
   *     not match the current domain object.
   */
  List<PricedEvent> convertAndValidateEvents(
      long voPriceModelKey,
      List<VOPricedEvent> voPricedEvents,
      List<Event> eventDefinitions,
      PriceModel priceModel,
      boolean priceModelCreatedInTransaction)
      throws ValidationException, OperationNotPermittedException, ConcurrentModificationException {
    // helper map to get existing priced events
    Map<Long, PricedEvent> keyToPricedEvent = new HashMap<>();
    ...
    for (VOPricedEvent voPricedEvent : voPricedEvents) {
      ...
      if (event == null) {
        OperationNotPermittedException onp =
            new OperationNotPermittedException("Event conversion failed");
        logger.logWarn(
            Log4jLogger.SYSTEM_LOG | Log4jLogger.AUDIT_LOG,
            onp,
      ...
      if (keyToPricedEvent.containsKey(Long.valueOf(voPricedEvent.getKey()))) {
        pricedEvent = keyToPricedEvent.remove(Long.valueOf(voPricedEvent.getKey()));
        updatePricedEvent(voPricedEvent, pricedEvent, event, priceModel);
      } else {
        PricedEvent existingPricedEvent = eventToPricedEvent.remove(event);
        validateDomainObjectKey(voPricedEvent, existingPricedEvent, priceModelCreatedInTransaction);
        pricedEvent = createPricedEvent(voPricedEvent, event, priceModel);
      }

      List<SteppedPrice> steppedPrices =
          convertAndValidateSteppedPrices(
              voPriceModelKey,
              voPricedEvent.getSteppedPrices(),
              priceModel,
              pricedEvent,
              null,
              priceModelCreatedInTransaction);
  ...
  void updatePricedEvent(
      VOPricedEvent voPricedEvent, PricedEvent pricedEvent, Event event, PriceModel priceModel)
      throws ValidationException, ConcurrentModificationException {

    BigDecimal oldPrice = pricedEvent.getEventPrice();
    pricedEvent = EventAssembler.updatePricedEvent(voPricedEvent, pricedEvent);
    pricedEvent.setEvent(event);
    pricedEvent.setPriceModel(priceModel);
    priceModelAudit.editEventPrice(dm, pricedEvent, oldPrice);
  }

  PricedEvent createPricedEvent(VOPricedEvent voPricedEvent, Event event, PriceModel priceModel)
      throws ValidationException {
  ...
  void removePricedEvents(long voPriceModelKey, Collection<PricedEvent> pricedEvents) {
  ...
  private void validateSteppedPrices(Event event, PricedEvent pe, List<SteppedPrice> steppedPrices)
      throws ValidationException {
    if (!steppedPrices.isEmpty() && BigDecimal.ZERO.compareTo(pe.getEventPrice()) != 0) {
      ValidationException ve =
          new ValidationException(
              ValidationException.ReasonEnum.STEPPED_EVENT_PRICING,
              "eventPrice",
              new Object[] {event.getEventIdentifier()});
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG,
          ve,
          LogMessageIdentifier.WARN_STEPPED_PRICING_MIXED_WITH_BASEPRICE,
  ...
  /**
   * Throw an OperationNotPermittedException if the key of the value object is not equal to the key
   * of the domain object. The validation is only performed if the flag
   * priceModelCreatedInTransaction is <code>true</code> .
   *
   * @param vo The value object representation.
   * @param domainObject The domain object representation.
   * @param priceModelCreatedInTransaction Indicates whether a key-clash can be ignored or not.
   * @throws OperationNotPermittedException Thrown if the key values do not match.
   */
  private void validateDomainObjectKey(
      BaseVO vo, DomainObject<?> domainObject, boolean priceModelCreatedInTransaction)
      throws OperationNotPermittedException {
    if (domainObject != null
      ...
      OperationNotPermittedException onp =
          new OperationNotPermittedException("Saving the price model failed.");
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG,
          onp,
          LogMessageIdentifier.WARN_STORE_DOMAIN_OBJECT_FAILED_WRONG_TECHNICAL_KEY,
          user.getUserId(),
          String.valueOf(domainObject));
      throw onp;
    }
  }

  /**
   * Converts the list of {@link VOSteppedPrice} to be set to a list of {@link SteppedPrice} that
   * can be saved. One of the three parent entities must be passed. Stepped prices that shall be
   * attached on a priced parameter will be validated according to their parameter based constraints
   * (numeric parameters only; no limits out of the defined range).
   *
   * @param voSteppedPrices the list of stepped prices to be set
   * @param priceModel The parent price model if the stepped pricing is done on the price per user
   *     of the price model
   * @param pricedEvent The parent priced event if the stepped pricing is done based on the number
   *     of occurrence of an event
   * @param pricedParameter the parent priced parameter if the stepped pricing is done based on the
   *     value of a parameter
   * @param priceModelCreatedInTransaction Indicates whether the price model was created in this
   *     transaction or not.
   * @return the list of {@link SteppedPrice} to be set at the parent
   * @throws ValidationException in case of stepped pricing on a parameter of a non numeric type or
   *     violated range limits
   * @throws ConcurrentModificationException Thrown in case the passed value object's version does
   *     not match the domain object.
   * @throws OperationNotPermittedException Thrown in case the passed value object key does not
   *     belong to a domain object.
   */
  List<SteppedPrice> convertAndValidateSteppedPrices(
      long voPriceModelKey,
      List<VOSteppedPrice> voSteppedPrices,
      PriceModel priceModel,
    ...
    List<SteppedPrice> existing = new ArrayList<>();

    if (pricedEvent == null && pricedParameter == null && priceModel != null) {
    ...
    Map<Long, SteppedPrice> keyToSteppedPrice = new HashMap<>();
    ...
    if (voSteppedPrices != null) {
      for (VOSteppedPrice voSteppedPrice : voSteppedPrices) {
        if (pricedParameter != null) {
          validateSteppedPriceContraints(pricedParameter, voSteppedPrice);
        }

        SteppedPrice steppedPrice;
        if (keyToSteppedPrice.containsKey(Long.valueOf(voSteppedPrice.getKey()))) {

          steppedPrice = keyToSteppedPrice.remove(Long.valueOf(voSteppedPrice.getKey()));
          updateSteppedPrice(
              voSteppedPrice, steppedPrice, priceModel, pricedEvent, pricedParameter);

        } else {
          if (!priceModelCreatedInTransaction && voSteppedPrice.getKey() != 0) {
            OperationNotPermittedException onp =
                new OperationNotPermittedException("Priced parameter with invalid key.");
            logger.logWarn(
                Log4jLogger.SYSTEM_LOG,
                onp,
          ...
          steppedPrice =
              createSteppedPrice(voSteppedPrice, priceModel, pricedEvent, pricedParameter);
        }
    ...
    removeSteppedPrices(
        voPriceModelKey, keyToSteppedPrice.values(), pricedEvent, pricedParameter, priceModel);

    return result;
  }

  SteppedPrice createSteppedPrice(
      VOSteppedPrice voSteppedPrice,
      PriceModel priceModel,
      PricedEvent pricedEvent,
      PricedParameter pricedParameter)
      throws ValidationException {

    SteppedPrice steppedPrice = SteppedPriceAssembler.toSteppedPrice(voSteppedPrice);
  ...
  void updateSteppedPrice(
    ...
    if (pricedEvent == null && pricedParameter == null && priceModel != null) {
      priceModelAudit.editUserSteppedPrice(dm, steppedPrice, oldPrice, oldLimit);
    } else if (pricedEvent != null) {
      priceModelAudit.editEventSteppedPrice(dm, steppedPrice, oldPrice, oldLimit);
    } else if (pricedParameter != null) {
      priceModelAudit.editParameterSteppedPrice(dm, steppedPrice, oldPrice, oldLimit);
    }
  }

  void removeSteppedPrices(
      long voPriceModelKey,
      Collection<SteppedPrice> steppedPrices,
      PricedEvent pricedEvent,
      PricedParameter pricedParameter,
      PriceModel priceModel) {
    for (SteppedPrice steppedPrice : steppedPrices) {
      ...
      if (voPriceModelKey > 0) {
        ...
        if (pricedEvent == null && pricedParameter == null && priceModel != null) {
          ...
          priceModelAudit.removeParameterSteppedPrice(dm, steppedPrice);
        }
      }
   ...
   *
   * <ul>
   *   <li>stepped pricing can only be done on parameters of type {@link
   *       ParameterValueType#DURATION}, {@link ParameterValueType#INTEGER} and {@link
   *       ParameterValueType#LONG}
   *   <li>the step limits must be within the data type limits and the defined parameter minimum and
   *       maximum value
   * </ul>
   *
   * @param pricedParameter the priced parameter the stepped prices will be attached to
   * @param steppedPrice the stepped price
   * @throws ValidationException in case of an validation error
   */
  void validateSteppedPriceContraints(PricedParameter pricedParameter, VOSteppedPrice steppedPrice)
      throws ValidationException {

    ParameterDefinition paramDef = pricedParameter.getParameter().getParameterDefinition();
    ...
    if (type == ParameterValueType.INTEGER) {
      BLValidator.isInteger(SteppedPriceAssembler.FIELD_NAME_LIMIT, String.valueOf(limit));
      BLValidator.isInRange(SteppedPriceAssembler.FIELD_NAME_LIMIT, limit, minValue, maxValue);
  ...
  public VOServiceDetails savePriceModelForCustomer(
    ...
    Organization targetCustomer = dm.getReference(Organization.class, customer.getKey());

    PlatformUser currentUser = dm.getCurrentUser();
    validateCustomersRole(targetCustomer, currentUser);
    PermissionCheck.supplierOfCustomer(
        currentUser.getOrganization(), targetCustomer, logger, sessionCtx);

    validateIfCustomerServiceExists(service, targetCustomer);
    validateCurrencyUniqunessOfMigrationPath(priceModel, service);

    Product product =
        prepareProductWithPriceModel(
            service, priceModel, targetCustomer, ServiceType.CUSTOMER_TEMPLATE, null);
  ...
  void validateCustomersRole(Organization targetCustomer, PlatformUser currentUser)
      throws OperationNotPermittedException {
    ...
    if (!targetCustomer.hasRole(OrganizationRoleType.CUSTOMER)) {
      OperationNotPermittedException onp =
          new OperationNotPermittedException("Creation of customer specific price model failed");
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG | Log4jLogger.AUDIT_LOG,
          onp,
  ...
  private void validateIfCustomerServiceExists(
      VOServiceDetails service, Organization targetCustomer)
    ...
    Product referencedProduct = dm.getReference(Product.class, service.getKey());
    if (referencedProduct.getTemplate() == null) {
      ...
      if (resultList.size() > 0) {
        ServiceOperationException sof =
            new ServiceOperationException(Reason.CUSTOMER_COPY_ALREADY_EXISTS);
        logger.logWarn(
            Log4jLogger.SYSTEM_LOG,
            sof,
    ...
    ProductValidator.validateActiveOrInactiveOrSuspended(
        referencedProduct.getProductId(), referencedProduct.getStatus());
  }
  ...
  public VOServiceDetails savePriceModelForSubscription(
    ...
    try {
      product =
          prepareProductWithPriceModel(
              service, priceModel, sub.getOrganization(), ServiceType.SUBSCRIPTION, sub);
  ...
  /**
   * test for not rewriting one-time fee for subscription price model for Price Model for
   * subscription we can not change one-time fee, new value has to be always 0
   */
  ...
  private void validatePriceModel(VOPriceModel priceModel, Product product, Subscription sub)
    ...
    if (priceModel.isChargeable()) {
      ...
      if (sub.getPaymentInfo() == null || sub.getBillingContact() == null) {
        PaymentInformationException pie =
            new PaymentInformationException(
                String.format(
    ...
    if (chargeableValueNew != chargeableValueOld) {
      PriceModelException pme =
          new PriceModelException(PriceModelException.Reason.UNMODIFIABLE_CHARGEABLE);
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG,
          pme,
  ...
  void validatePriceModelType(PriceModel existingPriceModel, VOPriceModel newPriceModel)
      throws PriceModelException {
    if (!existingPriceModel.getType().equals(newPriceModel.getType())) {
      PriceModelException pme =
          new PriceModelException(PriceModelException.Reason.UNMODIFIABLE_TYPE);
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG,
          pme,
  ...
  void validateCurrency(PriceModel existingPriceModel, VOPriceModel newPriceModel)
      throws PriceModelException {
  ...
  void validateTimeUnit(PriceModel existingPriceModel, VOPriceModel newPriceModel)
      throws PriceModelException {
    if (existingPriceModel.isChargeable()) {
      boolean isTimeUnitChanged =
          !(existingPriceModel.getPeriod().equals(newPriceModel.getPeriod()));
      if (isTimeUnitChanged) {
        PriceModelException pme =
            new PriceModelException(PriceModelException.Reason.UNMODIFIABLE_TIMEUNIT);
        logger.logWarn(
            Log4jLogger.SYSTEM_LOG,
            pme,
  ...
  private void validateExternalServiceMustBeFree(VOPriceModel priceModel, ServiceAccessType sat)
      throws ValidationException {

    if (priceModel.isChargeable() && sat == ServiceAccessType.EXTERNAL) {
      throw new ValidationException(ReasonEnum.EXTERNAL_SERVICE_MUST_BE_FREE_OF_CHARGE, null, null);
    }
  }
  ...
  public VOSubscriptionDetails validateSubscription(VOService service)
    ...
    PlatformUser currentUser = dm.getCurrentUser();
    Product product = dm.getReference(Product.class, service.getKey());
    Subscription subscription = validateSubscription(service, currentUser, product);
    VOSubscriptionDetails voSubscriptionDetails =
        subscriptionService.getSubscriptionDetailsWithoutOwnerCheck(subscription.getKey());
    return voSubscriptionDetails;
  }

  private Subscription validateSubscription(
      VOService service, PlatformUser currentUser, Product product)
    ...
    if (sub.getStatus() == SubscriptionStatus.EXPIRED
      ...
      SubscriptionStateException sse =
          new SubscriptionStateException(
              SubscriptionStateException.Reason.SUBSCRIPTION_STATE_CHANGED, null, params);
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG,
          sse,
  ...
  public List<VOService> getCompatibleServices(VOService service)
    ...
    for (Product compatibleProduct : compatibleProducts) {
      ...
      if (compatibleProduct.getStatus() != ServiceStatus.DELETED
        ...
        if (isCompatibleCurrency(prod.getPriceModel(), compatibleProduct.getPriceModel())) {
          result.add(ProductAssembler.toVOProduct(compatibleProduct, facade));
        }
      }
    }
  ...
  public void setCompatibleServices(VOService service, List<VOService> compatibleServices)
    ...
    // as well
    Product referenceProduct = dm.getReference(Product.class, service.getKey());
    BaseAssembler.verifyVersionAndKey(referenceProduct, service);
    ProductValidator.validateInactiveOrSuspended(
        ProductAssembler.getProductId(referenceProduct), referenceProduct.getStatus());
    ...
    Set<Long> currentCompatibleProductKeys = new HashSet<>();
    Set<Long> obsoleteCompatibleProductKeys = new HashSet<>();
    for (Product currentlyCompatibleProduct : referenceProduct.getCompatibleProductsList()) {
      currentCompatibleProductKeys.add(Long.valueOf(currentlyCompatibleProduct.getKey()));
      obsoleteCompatibleProductKeys.add(Long.valueOf(currentlyCompatibleProduct.getKey()));
    }
    ...
    for (Product prod : compProducts) {
      if (!currentCompatibleProductKeys.contains(Long.valueOf(prod.getKey()))) {
        ProductReference ref = new ProductReference(referenceProduct, prod);
    ...
    List<ProductReference> objToBeRemoved = new ArrayList<>();
    for (ProductReference currentlyCompatibleProductRef :
        referenceProduct.getAllCompatibleProducts()) {
      if (obsoleteCompatibleProductKeys.contains(
          Long.valueOf(currentlyCompatibleProductRef.getTargetProduct().getKey()))) {
    ...
    for (ProductReference newProductRef : newReferences) {
      try {
        ...
        SaaSSystemException sse =
            new SaaSSystemException("Persisting the product reference failed", e);
        logger.logError(
            Log4jLogger.SYSTEM_LOG,
            sse,
    ...
    logServiceUpDownGradeOptions(referenceProduct, newReferences, DEFINEIPDOWNGRADE_ON);
    logServiceUpDownGradeOptions(referenceProduct, objToBeRemoved, DEFINEIPDOWNGRADE_OFF);
  }

  void logServiceUpDownGradeOptions(
      Product referencrProdut, List<ProductReference> targetProducts, String upDowngrade) {
    for (ProductReference productReference : targetProducts) {
      serviceAudit.defineUpDownGradeOptions(
          dm, referencrProdut, productReference.getTargetProduct(), upDowngrade);
    }
  }
   ...
   *
   * @param referenceProduct the source product
   * @param compatibleProduct the target product
   * @throws ServiceCompatibilityException
   */
  private void validateMarketplaceCompatibility(Product referenceProduct, Product compatibleProduct)
      throws ServiceCompatibilityException {
    Set<Marketplace> mps = new HashSet<>();
    ...
    for (CatalogEntry ce : t.getCatalogEntries()) {
      if (ce.getMarketplace() != null && mps.contains(ce.getMarketplace())) {
        return;
      }
    }
    ServiceCompatibilityException ipc =
        new ServiceCompatibilityException(
  ...
  public void deleteService(VOService service)
    ...
    Product productToDelete = dm.getReference(Product.class, service.getKey());
  ...
  public void deleteProduct(Organization supplier, Product product)
    ...
    if (product.getOwningSubscription() != null) {
      ServiceOperationException sof =
          new ServiceOperationException(Reason.DELETION_FAILED_USED_BY_SUB);
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG,
          sof,
    ...
    else if (product.getTemplate() != null && product.getTargetCustomer() != null) {
      ProductValidator.validateInactiveOrSuspended(
          ProductAssembler.getProductId(product), product.getStatus());
    ...
    // all customer specific copies that are not used by any subscription
      ...
      long count = countNonSubscriptionCopiesInState(product, EnumSet.of(ServiceStatus.ACTIVE));
      if (count > 0) {
        String expected = ServiceStatus.INACTIVE.name() + ", " + ServiceStatus.SUSPENDED.name();
        throw new ServiceStateException(
            ServiceStatus.ACTIVE, expected, ProductAssembler.getProductId(product));
      }
      ...
      product.setProductId(
          product.getProductId() + "#" + String.valueOf(System.currentTimeMillis()));
    ...
    if (marketplace != null) {
      landingpageService.removeProductFromLandingpage(marketplace, product);
    }
  }
  ...
  public boolean statusAllowsDeletion(VOService service)
    ...
    BaseAssembler.verifyVersionAndKey(productToDelete, service);
    Set<ServiceStatus> invalidSates =
        EnumSet.of(ServiceStatus.ACTIVE, ServiceStatus.DELETED, ServiceStatus.OBSOLETE);
    boolean rc = !invalidSates.contains(productToDelete.getStatus());
    if (rc) {
      long count = countNonSubscriptionCopiesInState(productToDelete, invalidSates);
      rc = count == 0;
    }
    return rc;
  }

  private long countNonSubscriptionCopiesInState(Product template, Set<ServiceStatus> states) {
    Query query = dm.createNamedQuery("Product.countCustomerCopiesForTemplateInState");
    query.setParameter("template", template);
   ...
   */
  void validateNotExistingResalePermissions(Product template) throws ServiceOperationException {

    Query query = dm.createNamedQuery("Product.getPartnerCopiesForTemplateNotInState");
    query.setParameter("template", template);
    ...
    if (result.size() > 0) {
      ServiceOperationException sof =
          new ServiceOperationException(Reason.DELETION_FAILED_EXISTING_RESALE_PERMISSION);
      logger.logError(
          Log4jLogger.SYSTEM_LOG,
          sof,
  ...
  /**
   * Returns all customer specific product copies, that are not used in any subscription.
   *
   * @param template The product to retrieve the copies for.
  ...
  private List<Product> getCustomerSpecificProductCopies(Product template) {
    ...
    query.setParameter("template", template);
    List<Product> result = ParameterizedTypes.list(query.getResultList(), Product.class);

    return result;
  }

  /**
   * Returns the platform events defined in the system. No platform events are available if the
   * specified technical service is defined for the <code>DIRECT</code> or <code>USER</code> access
   * type.
   *
   * @param tp The technical product to retrieve the events for.
   ...
   *
   * <ul>
   *   <li>the user related ones for technical products of type <code>DIRECT</code> access
   *   <li>the maximum number of concurrent user per subscription for technical products of type
   *       <code>USER</code> access
   * </ul>
   *
   * @param tp the technical product to get the platform parameters for
   * @return the list of platform parameters for the technical product
   */
  private List<ParameterDefinition> getPlatformParameterDefinitions(TechnicalProduct tp) {

    Query query = dm.createNamedQuery("ParameterDefinition.getAllPlatformParameterDefinitions");
    query.setParameter("parameterType", ParameterType.PLATFORM_PARAMETER);
    List<ParameterDefinition> result =
        ParameterizedTypes.list(query.getResultList(), ParameterDefinition.class);
    if (tp.getAccessType() == ServiceAccessType.DIRECT
        || tp.getAccessType() == ServiceAccessType.USER) {
      List<ParameterDefinition> copy = new ArrayList<>(result);
      for (ParameterDefinition pd : copy) {
        if (PlatformParameterIdentifiers.CONCURRENT_USER.equals(pd.getParameterId())
            || (tp.getAccessType() == ServiceAccessType.DIRECT
                && PlatformParameterIdentifiers.NAMED_USER.equals(pd.getParameterId()))) {
          result.remove(pd);
        }
      }
    }
  ...
  /**
   * Reads the product result list from the given query's single result. If no result is found,
   * <code>null</code> is returned. If the result is not unique, a system exception will be thrown.
   *
   * @param cust The customer the product is requested for.
  ...
  public VOServiceLocalization getServiceLocalization(VOService service)
    ...
    ArgumentValidator.notNull("service", service);
    return spsLocalizer.getServiceLocalization(dm.getReference(Product.class, service.getKey()));
  }
  ...
  public VOPriceModelLocalization getPriceModelLocalization(VOPriceModel priceModel)
    ...
    if (!dm.getCurrentUser()
        .getOrganization()
        .getGrantedRoleTypes()
        .contains(OrganizationRoleType.SUPPLIER)) {
      throw new OperationNotPermittedException("only suppliers are allowed to call this method.");
    }
    PriceModel pm = dm.getReference(PriceModel.class, priceModel.getKey());
    if (pm.getProduct() == null) {
      throw new OperationNotPermittedException("price model has no product assigned.");
    }
  ...
  public List<VOLocalizedText> getPriceModelLicenseTemplateLocalization(VOServiceDetails service)
    ...
    List<VOLocalizedText> templates =
        localizer.getLocalizedValues(
            service.getTechnicalService().getKey(), LocalizedObjectTypes.PRODUCT_LICENSE_DESC);
  ...
  public void savePriceModelLocalization(
    ...
    spsLocalizer.savePriceModelLocalizationForSupplier(
        priceModel.getKey(), priceModel.isChargeable(), localization);
  }
  ...
  public List<VOService> getServicesForCustomer(VOOrganization customer)
    ...
    ArgumentValidator.notNull("customer", customer);
    PlatformUser user = dm.getCurrentUser();
    Organization cust = dm.getReference(Organization.class, customer.getKey());

    List<Product> list = getCustomerSpecificProducts(cust, user.getOrganization());
  ...
  public List<Product> getCustomerSpecificProducts(Organization cust, Organization seller)
    ...
    Query query = dm.createNamedQuery("Product.getCustomerSpecificProducts");
    query.setParameter("vendorKey", Long.valueOf(seller.getKey()));
    query.setParameter("customer", cust);
    List<Product> list = ParameterizedTypes.list(query.getResultList(), Product.class);
  ...
  public List<String> getSupportedCurrencies() {
    ...
    for (SupportedCurrency curr :
        ParameterizedTypes.iterable(query.getResultList(), SupportedCurrency.class)) {
  ...
  public VOImageResource loadImage(Long serviceKey) {
    ...
    if (product != null) {
      ...
      while (imageResource == null && product.getTemplate() != null) {
        ...
        imageResource = irm.read(product.getKey(), ImageType.SERVICE_IMAGE);
      }
   ...
   * @param voImageResource the image resource to store/delete
   */
  private void processImage(long productKey, VOImageResource voImageResource)
    ...
    if (voImageResource.getImageType() == null
      ...
      SaaSSystemException se =
          new SaaSSystemException("Only images belonging to a product can be saved.");
      logger.logError(
          Log4jLogger.SYSTEM_LOG, se, LogMessageIdentifier.ERROR_IMAGES_NOT_BELONG_TO_PRODUCT);
      throw se;
    }
    ...
    if (isImageDeleted) {
      ...
      try {
        ImageValidator.validate(
            voImageResource.getBuffer(), voImageResource.getContentType(), 80, 80, 80, 80);
  ...
  public byte[] exportTechnicalServices(List<VOTechnicalService> technicalServices)
    ...
    List<TechnicalProduct> toExport = new ArrayList<>();
    for (VOTechnicalService product : technicalServices) {
      TechnicalProduct techProd = findTechnicalProductAndCheckOwner(provider, product);
  ...
  private void setPricedParametersForPriceModel(
      long voPriceModelKey,
      PriceModel priceModel,
      List<PricedParameter> parametersToSet,
    ...
    if (parametersToSet.isEmpty()) {
      ...
      for (PricedParameter pricedParameter : priceModel.getSelectedParameters()) {
        dm.remove(pricedParameter);
      }
      priceModel.setSelectedParameters(parametersToSet);
    } else {
      List<PricedParameter> parametersToBeStored = new ArrayList<>();
      Map<String, PricedParameter> curParameters = new HashMap<>();
      for (PricedParameter pp : priceModel.getSelectedParameters()) {
        if (pp.getParameter().getParameterDefinition().getValueType()
          ...
          // fill the map
          curParameters.put(pp.getParameter().getParameterDefinition().getParameterId(), pp);
        }
      }
      for (PricedParameter parameterToSet : parametersToSet) {

        String parameterId =
            parameterToSet.getParameter().getParameterDefinition().getParameterId();
        PricedParameter pricedParameter = curParameters.get(parameterId);
        if (pricedParameter != null) {
          pricedParameter.setPricePerSubscription(parameterToSet.getPricePerSubscription());
          pricedParameter.setPricePerUser(parameterToSet.getPricePerUser());

          ParameterOptionAssembler.updatePriceOptions(pricedParameter, parameterToSet);

          curParameters.remove(
              pricedParameter.getParameter().getParameterDefinition().getParameterId());
        ...
        VOPricedParameter voPP = getPricedParamForParamId(selectedParameters, parameterId);
        setRoleSpecificPrices(
            voPriceModelKey,
            null,
            pricedParameter,
      ...
      Collection<PricedParameter> obsoletePricedParameters = curParameters.values();
  ...
  /**
   * Iterates over the parameters and returns the one with the given identifier.
   *
   * @param selectedParameters The list of vo priced parameters to parse.
   * @param parameterId The parameter identifier to look for.
  ...
  private VOPricedParameter getPricedParamForParamId(
    ...
    for (VOPricedParameter voPricedParameter : selectedParameters) {
      if (voPricedParameter.getVoParameterDef().getParameterId().equals(parameterId)) {
  ...
  /**
   * Checks if the given parameters to be set for the price model are contained in the parameter
   * list for the product. If they are, a list of consolidated priced parameters is returned.
   * Otherwise an OperationNotPermitted exception will be thrown.
   *
   * @param voPricedParameters The parameters to be set for the price model.
   * @param productParams The current parameter list of the considered product.
   * @param priceModel The price model the priced parameters should be added to.
   * @param targetCustomer The targetCustomer of the price model.
   * @param subscription The subscription of the price model.
   * @param priceModelCreatedInTransaction Indicates whether the price model was created in this
   *     transaction or not.
   * @param isTemplateExistsForCustomer Indicates whether price model for service exists as a
   *     template for price model for customer
  ...
  private List<PricedParameter> convertAndValidateParameters(
      long voPriceModelKey,
      List<VOPricedParameter> voPricedParameters,
      List<Parameter> productParams,
      PriceModel priceModel,
      boolean priceModelCreatedInTransaction,
      Organization targetCustomer,
      Subscription subscription,
    ...
    Map<Long, PricedParameter> pricedParameterMap = new HashMap<>();
    Map<Parameter, PricedParameter> paramToPricedParam = new HashMap<>();
    for (PricedParameter pricedParameter : priceModel.getSelectedParameters()) {
      pricedParameterMap.put(Long.valueOf(pricedParameter.getKey()), pricedParameter);
      paramToPricedParam.put(pricedParameter.getParameter(), pricedParameter);
    }
    Map<Long, Parameter> parameters = new LinkedHashMap<>();
    for (Parameter parameter : productParams) {
      parameters.put(Long.valueOf(parameter.getParameterDefinition().getKey()), parameter);
    }
    ...
    for (VOPricedParameter voPricedParameter : voPricedParameters) {
      ...
      PricedParameterChecks.isValidBaseParam(parameter, voPricedParameter);

      PricedParameter pricedParameter = null;
      if (pricedParameterMap.containsKey(Long.valueOf(voPricedParameter.getKey()))) {
        pricedParameter = pricedParameterMap.remove(Long.valueOf(voPricedParameter.getKey()));
        pricedParameter =
            handleParameterUpdate(
                voPricedParameter, pricedParameter, priceModelCreatedInTransaction);
        pricedParameterMap.remove(Long.valueOf(voPricedParameter.getKey()));
      } else {
        PricedParameter existingPricedParam = paramToPricedParam.remove(parameter);
        validateDomainObjectKey(
            voPricedParameter, existingPricedParam, priceModelCreatedInTransaction);
        pricedParameter = createPricedParameter(voPricedParameter, parameter, priceModel);
      }

      validateAndSetRolePricesForParam(
          voPriceModelKey,
          priceModel,
          voPricedParameter,
          pricedParameter,
      ...
      List<SteppedPrice> steppedPrices =
          convertAndValidateSteppedPrices(
              voPriceModelKey,
              voPricedParameter.getSteppedPrices(),
              priceModel,
              null,
              pricedParameter,
              priceModelCreatedInTransaction);
      if (!steppedPrices.isEmpty()
          && BigDecimal.ZERO.compareTo(pricedParameter.getPricePerSubscription()) != 0) {
        ValidationException ve =
            new ValidationException(
    ...
    removePricedParameters(voPriceModelKey, pricedParameterMap.values(), priceModel);

    return result;
  }

  PricedParameter createPricedParameter(
      VOPricedParameter voPricedParameter, Parameter parameter, PriceModel priceModel)
      throws ValidationException {

    PricedParameter pricedParameter = ParameterAssembler.toPricedParameter(voPricedParameter);
    pricedParameter.setParameter(parameter);
    pricedParameter.setPriceModel(priceModel);

    priceModelAudit.editParameterSubscriptionPrice(dm, pricedParameter, DEFAULT_PRICE_VALUE);
    priceModelAudit.editParameterUserPrice(dm, pricedParameter, DEFAULT_PRICE_VALUE);

    for (PricedOption pricedOption : pricedParameter.getPricedOptionList()) {
      priceModelAudit.editParameterOptionSubscriptionPrice(dm, pricedOption, DEFAULT_PRICE_VALUE);
      priceModelAudit.editParameterOptionUserPrice(dm, pricedOption, DEFAULT_PRICE_VALUE);
    }

    return pricedParameter;
  }

  void removePricedParameters(
      long voPriceModelKey, Collection<PricedParameter> pricedParameters, PriceModel priceModel) {
  ...
  /**
   * Updates a domain object parameter and all depending priced options according to the passed
   * value object.
   *
   * @param voPricedParameter The value object representation of the priced parameter.
   * @param pricedParameter The priced parameter domain object.
   * @return The updated priced parameter.
   * @throws ValidationException
   * @throws ConcurrentModificationException
   * @throws OperationNotPermittedException When a key of an priced parameter is set but unknown and
   *     the priceModelCreatedInTransaction flag is not set.
   */
  PricedParameter handleParameterUpdate(
      VOPricedParameter voPricedParameter,
      PricedParameter pricedParameter,
      boolean priceModelCreatedInTransaction)
      throws ValidationException, ConcurrentModificationException, OperationNotPermittedException {
    ...
    List<VOPricedOption> voPricedOptions = voPricedParameter.getPricedOptions();
    ...
    for (PricedOption pricedOption : pricedOptions) {
      storedOptionsMap.put(Long.valueOf(pricedOption.getKey()), pricedOption);
    }

    for (VOPricedOption voPricedOption : voPricedOptions) {
      ParameterOption paramOption =
          keyToParamOption.remove(Long.valueOf(voPricedOption.getParameterOptionKey()));
      if (paramOption == null) {
        OperationNotPermittedException onp =
            new OperationNotPermittedException(
                "No ParameterOption found for PricedOption value object.");
        logger.logWarn(
            Log4jLogger.SYSTEM_LOG, onp, LogMessageIdentifier.WARN_PRICEDOPTION_NO_OPTION_DEFINED);
        throw onp;
      }
      // new option
      PricedOption pricedOption = null;
      if (!storedOptionsMap.containsKey(Long.valueOf(voPricedOption.getKey()))) {
        if (!priceModelCreatedInTransaction && voPricedOption.getKey() != 0) {
          OperationNotPermittedException onp =
              new OperationNotPermittedException("Priced option value object with invalid key.");
          logger.logWarn(
              Log4jLogger.SYSTEM_LOG,
              onp,
        ...
        pricedOption = createPricedOption(voPricedOption, pricedParameter);
      } else {
        pricedOption = storedOptionsMap.remove(Long.valueOf(voPricedOption.getKey()));
  ...
  void updatePricedParameter(VOPricedParameter voPricedParameter, PricedParameter pricedParameter)
      throws ValidationException, ConcurrentModificationException {
    ...
    priceModelAudit.editParameterSubscriptionPrice(dm, pricedParameter, oldSubPrice);
    priceModelAudit.editParameterUserPrice(dm, pricedParameter, oldUserPrice);
  }

  PricedOption createPricedOption(VOPricedOption voPricedOption, PricedParameter pricedParameter) {
    PricedOption pricedOption =
        PricedOptionAssembler.toPricedOption(voPricedOption, pricedParameter);
  ...
  void updatePricedOption(VOPricedOption voPricedOption, PricedOption pricedOption)
    ...
    priceModelAudit.editParameterOptionSubscriptionPrice(dm, pricedOption, oldPOSubPrice);
    priceModelAudit.editParameterOptionUserPrice(dm, pricedOption, oldPOUserPrice);
  }
  ...
  Map<Long, ParameterOption> createParameterOptionMap(PricedParameter pricedParameter) {
    Map<Long, ParameterOption> keyToParamOption = new HashMap<>();
    for (ParameterOption parameterOption :
        pricedParameter.getParameter().getParameterDefinition().getOptionList()) {
      keyToParamOption.put(Long.valueOf(parameterOption.getKey()), parameterOption);
    }
    return keyToParamOption;
  }

  /**
   * Validates and sets the priced product role settings for the parameter and parameter options.
   *
   * @param priceModel The price model that should be modified.
   * @param voPP The value object representation of the priced parameter.
   * @param pp The domain object representation of the priced parameter.
   * @param targetCustomer The targetCustomer of the price model.
   * @param subscription The subscription of the price model.
   * @param priceModelCreatedInTransaction Indicates whether the price model was created in this
   *     transaction or not.
   ...
   * @throws ValidationException
   * @throws OperationNotPermittedException
   * @throws ConcurrentModificationException Thrown in case the priced role value object version
   *     does not match the domain object's one.
   */
  void validateAndSetRolePricesForParam(
      long voPriceModelKey,
      PriceModel priceModel,
      VOPricedParameter voPP,
      PricedParameter pp,
      boolean priceModelCreatedInTransaction,
      Organization targetCustomer,
      Subscription subscription,
    ...
    Map<Long, List<PricedProductRole>> oldPricedOptionMap = new HashMap<>();
    ...
    for (VOPricedOption pricedOption : pricedOptions) {
      validatePricedProductRoles(pricedOption.getRoleSpecificUserPrices(), priceModel.getProduct());
      ...
      for (PricedOption po : pp.getPricedOptionList()) {
        if (po.getParameterOptionKey() == pricedOption.getParameterOptionKey()) {
          if (isTemplateExistsForCustomer) {
            setRoleSpecificPrices(
                voPriceModelKey,
                null,
                ...
                targetCustomer,
                subscription,
                oldPricedOptionMap.get(Long.valueOf(po.getParameterOptionKey())));
    ...
    validatePricedProductRoles(voPP.getRoleSpecificUserPrices(), priceModel.getProduct());
    setRoleSpecificPrices(
        voPriceModelKey,
        null,
        pp,
  ...
  private Map<Long, List<PricedProductRole>> prepareOldPricedOptionMap(PriceModel priceModel) {
    Map<Long, List<PricedProductRole>> oldPricedOptionMap = new HashMap<>();
    List<PricedParameter> pricedParameters = priceModel.getSelectedParameters();
        ...
        if (pricedOptions != null) {
          for (PricedOption pricedOption : pricedOptions) {
            oldPricedOptionMap.put(
                Long.valueOf(pricedOption.getParameterOptionKey()),
                pricedOption.getRoleSpecificUserPrices());
          }
        }
      }
  ...
  public VOTechnicalService createTechnicalService(VOTechnicalService technicalService)
    ...
    TechnicalProduct domObj = TechnicalProductAssembler.toTechnicalProduct(technicalService);
    ...
    localizer.storeLocalizedResource(
        locale, domObj.getKey(), LocalizedObjectTypes.PRODUCT_LICENSE_DESC, license);
    String description = technicalService.getTechnicalServiceDescription();
    if (description == null) {
      description = "";
    }
    localizer.storeLocalizedResource(
        locale, domObj.getKey(), LocalizedObjectTypes.TEC_PRODUCT_TECHNICAL_DESC, description);
    ...
    if (accessType == ServiceAccessType.DIRECT || accessType == ServiceAccessType.USER) {
      localizer.storeLocalizedResource(
          "en", domObj.getKey(), LocalizedObjectTypes.TEC_PRODUCT_LOGIN_ACCESS_DESC, accessInfo);
    } else {
      localizer.storeLocalizedResource(
          locale, domObj.getKey(), LocalizedObjectTypes.TEC_PRODUCT_LOGIN_ACCESS_DESC, accessInfo);
    }

    // Store defined localized tags of this service
    tagService.updateTags(domObj, locale, TagAssembler.toTags(technicalService.getTags(), locale));
    ...
    if (org.hasRole(OrganizationRoleType.SUPPLIER)) {
      try {
        ms.addMarketingPermission(
            org, domObj.getKey(), Collections.singletonList(org.getOrganizationId()));
      } catch (ObjectNotFoundException e) {
    ...
    VOTechnicalService result =
        TechnicalProductAssembler.toVOTechnicalProduct(
            domObj, paramDefs, platformEvents, facade, false);
  ...
  public void saveTechnicalServiceLocalization(VOTechnicalService technicalService)
      throws ObjectNotFoundException, OperationNotPermittedException, UpdateConstraintException,
          ValidationException {

    ArgumentValidator.notNull("technicalService", technicalService);

    Organization provider = dm.getCurrentUser().getOrganization();
    TechnicalProduct techProd = findTechnicalProductAndCheckOwner(provider, technicalService);
    String locale = dm.getCurrentUser().getLocale();
    checkLicenseConstrainsAndStore(techProd, locale, technicalService.getLicense());
    String description = technicalService.getTechnicalServiceDescription();
    if (description != null) {
      localizer.storeLocalizedResource(
          locale,
    ...
    String accessInfo = technicalService.getAccessInfo();
    if (accessInfo != null) {
      createAccessInfoForDefaultLocale(locale, "en", technicalService, accessInfo);
      localizer.storeLocalizedResource(
          locale,
          technicalService.getKey(),
    ...
    for (VORoleDefinition role : roles) {
      ...
      RoleDefinition r = (RoleDefinition) dm.getReferenceByBusinessKey(roleDef);
      String name = role.getName();
      if (name != null) {
        BLValidator.isName("role definition name", name, false);
        localizer.storeLocalizedResource(
            locale, r.getKey(), LocalizedObjectTypes.ROLE_DEF_NAME, name);
      }
      String desc = role.getDescription();
      if (desc != null) {
        localizer.storeLocalizedResource(
            locale, r.getKey(), LocalizedObjectTypes.ROLE_DEF_DESC, desc);
      }
    }
    List<VOTechnicalServiceOperation> ops = technicalService.getTechnicalServiceOperations();
    for (VOTechnicalServiceOperation op : ops) {
      ...
      TechnicalProductOperation tpo =
          (TechnicalProductOperation) dm.getReferenceByBusinessKey(operation);
      String name = op.getOperationName();
      if (name != null) {
        BLValidator.isName("technical service operation name", name, false);
        localizer.storeLocalizedResource(
            locale, tpo.getKey(), LocalizedObjectTypes.TECHNICAL_PRODUCT_OPERATION_NAME, name);
      }
      ...
      List<VOServiceOperationParameter> params = op.getOperationParameters();
      for (VOServiceOperationParameter p : params) {
        ...
        if (parameterName != null) {
          ...
          OperationParameter param =
              (OperationParameter) dm.getReferenceByBusinessKey(operationParameter);
          BLValidator.isName("technical service operation parameter name", parameterName, false);
          localizer.storeLocalizedResource(
              locale,
              param.getKey(),
    ...
    for (VOEventDefinition event : events) {
      ...
      if (eventDescription != null) {
        if (event.getEventType() != EventType.PLATFORM_EVENT) {
          ...
          localizer.storeLocalizedResource(
              locale, e.getKey(), LocalizedObjectTypes.EVENT_DESC, eventDescription);
        }
      }
    }
    List<VOParameterDefinition> parameters = technicalService.getParameterDefinitions();
    for (VOParameterDefinition parameter : parameters) {
      ...
      if (parameter.getParameterType() != ParameterType.PLATFORM_PARAMETER) {
        ...
        ParameterDefinition p = (ParameterDefinition) dm.getReferenceByBusinessKey(parameterDef);

        // only service parameters can be modified
        if (parameterDescription != null) {
          localizer.storeLocalizedResource(
              locale, p.getKey(), LocalizedObjectTypes.PARAMETER_DEF_DESC, parameterDescription);
        }
        List<VOParameterOption> options = parameter.getParameterOptions();
        for (VOParameterOption option : options) {
          ...
          if (optionDescription != null) {
            ...
            ParameterOption o = (ParameterOption) dm.getReferenceByBusinessKey(parameterOption);
            localizer.storeLocalizedResource(
                locale,
                o.getKey(),
                LocalizedObjectTypes.OPTION_PARAMETER_DEF_DESC,
    ...
    List<Tag> tags = TagAssembler.toTags(technicalService.getTags(), locale);
    tagService.updateTags(techProd, locale, tags);
  }

  private void createAccessInfoForDefaultLocale(
      String userLocale,
    ...
    if (!userLocale.equals(defaultLocale)
      ...
      String accessInfoDefaultLocale =
          localizer.getLocalizedTextFromDatabase(
              defaultLocale,
              technicalService.getKey(),
              LocalizedObjectTypes.TEC_PRODUCT_LOGIN_ACCESS_DESC);
      if (accessInfoDefaultLocale == null || accessInfoDefaultLocale.trim().length() <= 0) {
        localizer.storeLocalizedResource(
            defaultLocale,
            technicalService.getKey(),
            LocalizedObjectTypes.TEC_PRODUCT_LOGIN_ACCESS_DESC,
  ...
  /**
   * Checks if the new license of the technical product can be changed. Saving is done when the
   * technical product doesn't have a license in the provided locale or if no marketing products
   * exist based on the technical product. In case the new license isn't set or equals the existing
   * one, it isn't saved.
   *
   * @param tp the technical product value object to get the new license from
   * @param locale the locale that is used for saving the license
   * @param license The license for the technical product.
   */
  private void checkLicenseConstrainsAndStore(TechnicalProduct tp, String locale, String license) {
    if (license == null) {
      return;
    }
    List<VOLocalizedText> localizedValues =
        localizer.getLocalizedValues(tp.getKey(), LocalizedObjectTypes.PRODUCT_LICENSE_DESC);
    ...
    if (!Strings.areStringsEqual(storedLicense, license)) {
      localizer.storeLocalizedResource(
          locale, tp.getKey(), LocalizedObjectTypes.PRODUCT_LICENSE_DESC, license);
    }
  }

  /**
   * Tries to find the technical product represented by the given value object and check if the
   * provided organization is the owner of it.
   *
   * @param provider the technology provider
   * @param voTechnicalProduct the value object to find the domain object for
   * @return the {@link TechnicalProduct}
   * @throws ObjectNotFoundException in case the technical product wasn't found
   * @throws OperationNotPermittedException in case the provided organization is not owner of the
   *     technical product
   */
  private TechnicalProduct findTechnicalProductAndCheckOwner(
    ...
    TechnicalProduct techProd =
        dm.getReference(TechnicalProduct.class, voTechnicalProduct.getKey());
  ...
  public List<VOCustomerService> getAllCustomerSpecificServices()
    ...
    Query query = dm.createNamedQuery("Product.getCustomerProductsForVendor");
    query.setParameter("vendorKey", Long.valueOf(organization.getKey()));
    List<Product> products = ParameterizedTypes.list(query.getResultList(), Product.class);
    List<VOCustomerService> result = new ArrayList<>();
    LocalizerFacade facade = new LocalizerFacade(localizer, dm.getCurrentUser().getLocale());
    Set<ServiceStatus> states =
        EnumSet.of(ServiceStatus.ACTIVE, ServiceStatus.INACTIVE, ServiceStatus.SUSPENDED);
    for (Product product : products) {
      if (states.contains(product.getStatus())) {
        result.add(ProductAssembler.toVOCustomerProduct(product, facade));
      }
    }
    return result;
  }
  ...
  public VOServiceDetails copyService(VOService service, String serviceId)
    ...
    if (product.getOwningSubscription() != null || product.getTargetCustomer() != null) {
      OperationNotPermittedException onp =
          new OperationNotPermittedException("Copying the service failed");
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG | Log4jLogger.AUDIT_LOG,
          onp,
    ...
    if (status != ServiceStatus.ACTIVE && status != ServiceStatus.INACTIVE) {
      ServiceStateException sse =
          new ServiceStateException(
              status,
              ServiceStatus.ACTIVE.name() + ", " + ServiceStatus.INACTIVE.name(),
    ...
    // categories
    CatalogEntry catalogEntry = QueryBasedObjectFactory.createCatalogEntry(copy, mp);
    for (Category cat : cats) {
      catalogEntry.addCategory(cat);
    }
    if (product.getCatalogEntries().size() > 0) {
      catalogEntry.setVisibleInCatalog(product.getCatalogEntries().get(0).isVisibleInCatalog());
      catalogEntry.setAnonymousVisible(product.getCatalogEntries().get(0).isAnonymousVisible());
    }

    if (!product.getCatalogEntries().isEmpty()) {
      copyOperatorPriceModel(
          catalogEntry, product.getCatalogEntries().get(0).getOperatorPriceModel());
    }
    ...
    List<VOLocalizedText> locDescs =
        localizer.getLocalizedValues(product.getKey(), LocalizedObjectTypes.PRODUCT_MARKETING_DESC);
    localizer.setLocalizedValues(
        copy.getKey(), LocalizedObjectTypes.PRODUCT_MARKETING_DESC, locDescs);
    List<VOLocalizedText> locShortDescs =
        localizer.getLocalizedValues(
            product.getKey(), LocalizedObjectTypes.PRODUCT_SHORT_DESCRIPTION);
    localizer.setLocalizedValues(
        copy.getKey(), LocalizedObjectTypes.PRODUCT_SHORT_DESCRIPTION, locShortDescs);
    List<VOLocalizedText> locNames =
        localizer.getLocalizedValues(product.getKey(), LocalizedObjectTypes.PRODUCT_MARKETING_NAME);
    localizer.setLocalizedValues(
        copy.getKey(), LocalizedObjectTypes.PRODUCT_MARKETING_NAME, locNames);
    List<VOLocalizedText> locCustomTabNames =
        localizer.getLocalizedValues(
            product.getKey(), LocalizedObjectTypes.PRODUCT_CUSTOM_TAB_NAME);
    localizer.setLocalizedValues(
        copy.getKey(), LocalizedObjectTypes.PRODUCT_CUSTOM_TAB_NAME, locCustomTabNames);

    PriceModel pm = product.getPriceModel();
    if (pm != null) {
      PriceModel pmCopy = copy.getPriceModel();
      List<VOLocalizedText> locPmDescs =
          localizer.getLocalizedValues(pm.getKey(), LocalizedObjectTypes.PRICEMODEL_DESCRIPTION);
      localizer.setLocalizedValues(
          pmCopy.getKey(), LocalizedObjectTypes.PRICEMODEL_DESCRIPTION, locPmDescs);
      // copy licenses
    ...
    serviceAudit.copyService(dm, copy, product.getProductId(), service.getNameToDisplay());
    return createdProduct;
  }
   ...
   *
   * @param source Original price model.
   * @param destination Copy price model.
   */
  private void copyLicenseInformation(PriceModel source, PriceModel destination) {
    // get local descriptions for technical service and
    // save for marketable service for all languages
    List<VOLocalizedText> licenseDescriptions =
        localizer.getLocalizedValues(source.getKey(), LocalizedObjectTypes.PRICEMODEL_LICENSE);
    ...
    if (licenseDescriptions != null) {
      for (VOLocalizedText localizedText : licenseDescriptions) {
        ...
        String locale = localizedText.getLocale();
        localizer.storeLocalizedResource(
            locale, destination.getKey(), LocalizedObjectTypes.PRICEMODEL_LICENSE, license);
      }
    }
  }
  ...
  public List<VOService> setActivationStates(List<VOServiceActivation> activations)
    ...
    List<VOService> resultList = new ArrayList<>();
  ...
  private VOService setActivationState(
      VOServiceActivation serviceActivation, ServiceVisibilityCheck visChecker)
      throws ObjectNotFoundException, ServiceStateException, OrganizationAuthoritiesException,
          OperationNotPermittedException, ServiceOperationException,
          TechnicalServiceNotAliveException, ServiceNotPublishedException,
          OperationPendingException, ConcurrentModificationException {
    ...
    List<VOCatalogEntry> catalogEntries = serviceActivation.getCatalogEntries();
    return setActivationState(service, active, catalogEntries, visChecker);
  }

  /**
   * Activates or deactivates the given service and updates the visibility flag for all given
   * catalog entries. If a trigger function is defined, the request will be queued.
   */
  private VOService setActivationState(
      VOService service,
      boolean activate,
      List<VOCatalogEntry> entries,
      ServiceVisibilityCheck visChecker)
      throws ServiceStateException, ObjectNotFoundException, OrganizationAuthoritiesException,
          OperationNotPermittedException, ServiceOperationException,
          TechnicalServiceNotAliveException, ServiceNotPublishedException,
          OperationPendingException, ConcurrentModificationException {
    ...
    Product prod = dm.getReference(Product.class, service.getKey());
    if (activate) {
      validateForProductStatusChange(
          prod, service, ServiceStatus.ACTIVE, ServiceStatus.INACTIVE, currentUser);
      validateForProductActivation(service);
    } else {
      validateForProductStatusChange(
          prod, service, ServiceStatus.INACTIVE, ServiceStatus.ACTIVE, currentUser);
    }

    TriggerProcessValidator triggerProcessValidator = new TriggerProcessValidator(dm);
    // If there is an existing pending operation throw an exception.
    if (triggerProcessValidator.isActivateOrDeactivateServicePending(service)) {
      OperationPendingException ope =
          new OperationPendingException(
              String.format(
    ...
    TriggerMessage message =
        new TriggerMessage(
            activate ? TriggerType.ACTIVATE_SERVICE : TriggerType.DEACTIVATE_SERVICE);
    List<TriggerProcessMessageData> list =
        triggerQS.sendSuspendingMessages(Collections.singletonList(message));
    TriggerProcess tProc = list.get(0).getTrigger();
    tProc.addTriggerProcessParameter(TriggerProcessParameterName.OBJECT_ID, service.getServiceId());
    tProc.addTriggerProcessParameter(TriggerProcessParameterName.PRODUCT, service);
    if (entries != null) {
    ...
    if (triggerDefinition == null) {
      ...
      try {
        ...
        if (entries != null && !entries.isEmpty()) {
          ...
          if (catalogEntry.getMarketplace() != null) {
            serviceAudit.activeOrDeactiveService(
                dm,
                prod,
        ...
        voProduct =
            ProductAssembler.toVOProduct(
                prod, new LocalizerFacade(localizer, currentUser.getLocale()));
      ...
      if (entries == null) {
        List<CatalogEntry> entry =
            dm.getReference(Product.class, service.getKey()).getCatalogEntries();
        if (!entry.isEmpty()) {
      ...
      tProc.setTriggerProcessIdentifiers(
          TriggerProcessIdentifiers.createDeactivateService(
   ...
   *
   * @throws ConcurrentModificationException when the passed entry has a different marketplace set
   *     as the persisted one
   */
  private void updateCatalogEntryVisibility(Product product, List<VOCatalogEntry> entries)
      throws ConcurrentModificationException {
    ...
    List<CatalogEntry> tempList =
        ParameterizedTypes.list(query.getResultList(), CatalogEntry.class);
    ...
    for (VOCatalogEntry voEntry : entries) {
      ...
      if (mp != null) {
        ...
        if (domEntry == null) {
          ...
          ConcurrentModificationException cme = new ConcurrentModificationException(voEntry);
          logger.logError(
              Log4jLogger.SYSTEM_LOG,
              cme,
  ...
  public VOOrganization getServiceSeller(long serviceKey, String locale)
    ...
    return OrganizationAssembler.toVOOrganization(
        product.getVendor(), false, new LocalizerFacade(localizer, locale));
  }

  @Override
  public VOOrganization getServiceSellerFallback(long serviceKey, String locale)
      throws ObjectNotFoundException {
    VOOrganization org = getServiceSeller(serviceKey, locale);
    if ((org.getDescription() == null || org.getDescription().isEmpty()) && !locale.equals("en")) {
      Product product = dm.getReference(Product.class, serviceKey);
      LocalizerFacade localizerEn = new LocalizerFacade(localizer, "en");
      String description =
          localizerEn.getText(
              product.getVendor().getKey(), LocalizedObjectTypes.ORGANIZATION_DESCRIPTION);
  ...
  public List<String> getInstanceIdsForSellers(List<String> organizationIds) {
    ...
    if (organizationIds.size() > 0) {
      Query query = dm.createNamedQuery("Subscription.instanceIdsForSuppliers");
      query.setParameter("providerKey", Long.valueOf(providerOrg.getKey()));
      query.setParameter("supplierIds", organizationIds);
      query.setParameter(
          "status", EnumSet.of(SubscriptionStatus.ACTIVE, SubscriptionStatus.SUSPENDED));
      List<String> instanceIds = ParameterizedTypes.list(query.getResultList(), String.class);
      if (instanceIds != null) {
  ...
  public VOService suspendService(VOService service, String reason)
    ...
    PlatformUser currentUser = dm.getCurrentUser();
    Product prod = dm.getReference(Product.class, service.getKey());
    Marketplace mp = validatePermissionForSuspendAndResume(currentUser, prod);
    ...
    if (tempOrSelf.getStatus() != ServiceStatus.ACTIVE) {
      ServiceStateException e =
          new ServiceStateException(ServiceStatus.ACTIVE, tempOrSelf.getStatus());
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG,
          e,
    ...
    for (PlatformUser user : users) {
      if (user.isOrganizationAdmin() || user.hasRole(UserRoleType.SERVICE_MANAGER)) {
        try {
          commService.sendMail(
              user,
              EmailType.SERVICE_SUSPENDED,
              new Object[] {tempOrSelf.getProductId(), reason, currentUser.getEmail()},
              mp);
    ...
    VOService voProduct =
        ProductAssembler.toVOProduct(
            prod, new LocalizerFacade(localizer, dm.getCurrentUser().getLocale()));
  ...
  public VOService resumeService(VOService service)
    ...
    if (tempOrSelf.getStatus() != ServiceStatus.SUSPENDED) {
      ServiceStateException e =
          new ServiceStateException(ServiceStatus.SUSPENDED, tempOrSelf.getStatus());
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG,
          e,
  ...
  /**
   * Check that the provided service is published to a marketplace owned by the provided user and
   * that it does not belong to a subscription.
   *
   * @param currentUser the calling user
   * @param prod the service to validate
   * @return the marketplace the service is published to
   * @throws OperationNotPermittedException in case the service is not published to a marketplace
   *     owned by the caller or the service belongs to a subscription
   */
  private Marketplace validatePermissionForSuspendAndResume(PlatformUser currentUser, Product prod)
      throws OperationNotPermittedException {
    if (prod.getOwningSubscription() != null) {
      String message = "Service '%s' is related to a subscription.";
      OperationNotPermittedException e =
          new OperationNotPermittedException(String.format(message, Long.valueOf(prod.getKey())));
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG,
          e,
    ...
    PermissionCheck.owns(mp, currentUser.getOrganization(), logger, sessionCtx);
    return mp;
  }

  /** compares the technical product key */
  private void validateTechnicalProductCompatibility(
      Product referenceProduct, Product compatibleProd) throws ServiceCompatibilityException {
    if (compatibleProd.getTechnicalProduct().getKey()
        != referenceProduct.getTechnicalProduct().getKey()) {
      ServiceCompatibilityException ipc =
          new ServiceCompatibilityException(
  ...
  private void validateCurrencyCompatibility(Product referenceProduct, Product compatibleProduct)
      throws ServiceCompatibilityException {
  ...
  /**
   * Returns true if one or both of the price models are free of charge, or the currencies are of
   * the same type.
   */
  private boolean isCompatibleCurrency(
      PriceModel referencePriceModel, PriceModel compatiblePriceModel) {
    if (referencePriceModel != null
        && compatiblePriceModel != null
        && referencePriceModel.isChargeable()
        && compatiblePriceModel.isChargeable()) {
      if (!referencePriceModel.getCurrency().equals(compatiblePriceModel.getCurrency())) {
        return false;
      }
    }
    return true;
   ...
   */
  private boolean isCompatibleCurrency(
      PriceModel referencePriceModel, VOPriceModel compatiblePriceModel) {
    if (referencePriceModel != null
        && compatiblePriceModel != null
        && referencePriceModel.isChargeable()
        && compatiblePriceModel.isChargeable()) {
      if (!referencePriceModel
          .getCurrency()
          .getCurrencyISOCode()
  ...
  public List<VOCompatibleService> getPotentialCompatibleServices(VOService service)
    ...
    List<VOCompatibleService> result = new ArrayList<>();
    if (mps.isEmpty()) {
    ...
    Query q = dm.createNamedQuery("Product.getPotentialCompatibleForProduct");
    ...
    q.setParameter("status", EnumSet.of(ServiceStatus.DELETED, ServiceStatus.OBSOLETE));
    List<Product> products = ParameterizedTypes.list(q.getResultList(), Product.class);
    products.remove(p);

    // get the currently configured targets
    Set<Long> targetKeys = new HashSet<>();
    q = dm.createNamedQuery("ProductReference.getTargetKeysForProduct");
    q.setParameter("product", p);
    targetKeys.addAll(ParameterizedTypes.list(q.getResultList(), Long.class));

    LocalizerFacade facade = new LocalizerFacade(localizer, dm.getCurrentUser().getLocale());
    ProductAssembler.prefetchData(products, facade, PerformanceHint.ONLY_FIELDS_FOR_LISTINGS);

    for (Product prod : products) {
      if (isCompatibleCurrency(p.getPriceModel(), prod.getPriceModel())) {
        VOCompatibleService s = ProductAssembler.toVOCompatibleService(prod, targetKeys, facade);
        result.add(s);
      }
    }
  ...
  public boolean isPartOfUpgradePath(long serviceKey) {
    ...
    query.setParameter("status", EnumSet.of(ServiceStatus.DELETED, ServiceStatus.OBSOLETE));
    Long count = (Long) query.getSingleResult();
  ...
  public List<Product> getCustomerSpecificCopyProducts(Product template)
    ...
    Query query = dm.createNamedQuery("Product.getCustomerSpecificCopiesForTemplate");
    query.setParameter("template", template);
  ...
  public void deleteTechnicalService(Long key)
    ...
    vo =
        TechnicalProductAssembler.toVOTechnicalProduct(
            tProd, paramDefs, platformEvents, facade, false);
