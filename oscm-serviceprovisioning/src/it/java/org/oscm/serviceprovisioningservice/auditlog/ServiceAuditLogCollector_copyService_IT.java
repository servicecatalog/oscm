...
public class ServiceAuditLogCollector_copyService_IT extends EJBTestBase {
    ...
    private final static String COPYNAME = "copyname";
    ...
    private ServiceAuditLogCollector copyService(Product product) {
        AuditLogData.clear();
        logCollector.copyService(dsMock, product, COPYNAME, COPYNAME);
        return logCollector;
    }
    ...
    private void verifyLogEntries() {
        ...
        assertEquals(COPYNAME, logParams.get(AuditLogParameter.COPY_SERVICE_ID));
        assertEquals(COPYNAME,
                logParams.get(AuditLogParameter.COPY_SERVICE_NAME));
    }
