...
public class OperatorServiceBean implements OperatorService {
  ...
  private static Log4jLogger logger = LoggerFactory.getLogger(OperatorServiceBean.class);
  ...
  @EJB protected TimerServiceBean timerMgmt;
  ...
  public VOOrganization registerOrganization(
      VOOrganization organization,
      VOImageResource voImageResource,
      VOUserDetails orgInitialUser,
      LdapProperties organizationProperties,
      String marketplaceID,
      OrganizationRoleType... rolesToGrant)
      throws NonUniqueBusinessKeyException, ValidationException, OrganizationAuthorityException,
    ...
    try {
      ...
      if (rolesAreInvalid(rolesToGrant)) {
        OrganizationAuthorityException iao =
      ...
      if (operatorRevenueShareIsInvalid(
          organization.getOperatorRevenueShare(), Arrays.asList(rolesToGrant))) {
        ValidationException validationException =
            new ValidationException(
                ValidationException.ReasonEnum.INVALID_OPERATOR_REVENUE_SHARE, null, null);
        logger.logError(
            Log4jLogger.SYSTEM_LOG,
            validationException,
      ...
      if (rolesToGrant.length > 0) {
        organizationToCreate = OrganizationAssembler.toVendor(organization);
      } else {
        ...
        organizationToCreate = OrganizationAssembler.toCustomer(organization);
      }
      if (organization.getOperatorRevenueShare() != null) {
        createOperatorPriceModel(organizationToCreate, organization.getOperatorRevenueShare());
      }
      ...
      Organization createdOrganization =
          accMgmt.registerOrganization(
              organizationToCreate,
              imageResource,
              orgInitialUser,
              organizationProperties != null ? organizationProperties.asProperties() : null,
  ...
  /**
   * when creating a customer, the roles should be null or empty, then the marketplaceID is
   * mandatory.
   *
   * @param marketplaceID
   * @param roles
   ...
   */
  private Marketplace checkMarketplaceIDForCreateOrganization(
   ...
   * @throws ObjectNotFoundException
   */
  private Marketplace validateMarketplaceID(String marketplaceId) throws ObjectNotFoundException {
   ...
   *
   * @param operatorRevenueShare an operator revenue share
   * @param roles the organization roles
   * @return <code>true</code> if operatorRevenueShare is <code>NULL</code>, but the organization is
   *     a supplier or if operatorRevenueShare is not <code>NULL</code>, but the organization is no
   *     supplier otherwise <code>false</code>
   */
  private boolean operatorRevenueShareIsInvalid(
      BigDecimal operatorRevenueShare, Collection<OrganizationRoleType> roles) {
   ...
   *
   * @param org a supplier organization domain object
   * @param operatorRevenueShare the operator revenue share
   */
  private void createOperatorPriceModel(Organization org, BigDecimal operatorRevenueShare)
      throws NonUniqueBusinessKeyException {
    RevenueShareModel operatorPriceModel = new RevenueShareModel();
    operatorPriceModel.setRevenueShare(operatorRevenueShare);
    operatorPriceModel.setRevenueShareModelType(RevenueShareModelType.OPERATOR_REVENUE_SHARE);
   ...
   * @param org a supplier organization domain object
   * @param operatorRevenueShare the operator revenue share
   * @throws ValidationException if the operator revenue share doesn't correspond to the
   *     organization role(s)
   */
  private void updateOperatorRevenueShare(Organization org, BigDecimal operatorRevenueShare)
      throws NonUniqueBusinessKeyException, ValidationException {
    if (operatorRevenueShareIsInvalid(operatorRevenueShare, org.getGrantedRoleTypes())) {
      ValidationException validationException =
          new ValidationException(
              ValidationException.ReasonEnum.INVALID_OPERATOR_REVENUE_SHARE, null, null);
      logger.logError(
      ...
      if (org.getOperatorPriceModel() != null) {
        org.getOperatorPriceModel().setRevenueShare(operatorRevenueShare);
      } else {
  ...
  /**
   * Adds INVOICE as payment type for the organization if the roles contain the supplier role.
   *
   * @param org The organization to add the payment type to.
   * @throws ObjectNotFoundException
   * @throws NonUniqueBusinessKeyException
   */
  private void addInvoice(Organization org)
    ...
    if (!org.canPermitPaymentType(PaymentType.INVOICE)) {
      ...
      OrganizationReference orgRef = findOrCreateOrganizationReference(operator, org);
      ...
      org.getSourcesForType(orgRef.getReferenceType()).get(0).getPaymentTypes().add(apt);
      dm.persist(apt);
    }
  }
  ...
  private OrganizationReference findOrCreateOrganizationReference(
    ...
    return OperatorServiceDataHandler.findOrCreateOrganizationReference(operator, org, refType, dm);
  }

  /**
   * Return true if {@link ConfigurationKey#SUPPLIER_SETS_INVOICE_AS_DEFAULT} has the value <code>
   * true</code>.
   *
   * @return <code>true</code> if the configuration key has the value <code>true</code>.
   */
  private boolean isSupplierSetsInvoiceAsDefaultEnabled() {

    ConfigurationSetting cs =
        configService.getConfigurationSetting(
            ConfigurationKey.SUPPLIER_SETS_INVOICE_AS_DEFAULT, Configuration.GLOBAL_CONTEXT);
    String value;
    if (cs == null || cs.getValue() == null) {
      value = ConfigurationKey.SUPPLIER_SETS_INVOICE_AS_DEFAULT.getFallBackValue();
    } else {
      value = cs.getValue();
    }
  ...
  private void addOrganizationToRoleIntern(String organizationId, OrganizationRoleType role)
      throws OrganizationAuthorityException, IncompatibleRolesException, ObjectNotFoundException,
    ...
    try {
      ...
      logger.logError(
          Log4jLogger.SYSTEM_LOG, sse, LogMessageIdentifier.ERROR_ADD_INVOICE_PAYMENT_TYPE);
      throw sse;
    }
  }
  ...
  @Override
  @RolesAllowed("PLATFORM_OPERATOR")
  public void addOrganizationToRole(String organizationId, OrganizationRoleType role)
      throws OrganizationAuthorityException, IncompatibleRolesException, ObjectNotFoundException,
  ...
  private void addAvailablePaymentTypesIntern(VOOrganization supplier, Set<String> types)
      throws ObjectNotFoundException, OrganizationAuthorityException,
          PSPIdentifierForSellerException, PaymentDataException {

    Organization operator = dm.getCurrentUser().getOrganization();

    Organization organization = getOrganizationInt(supplier.getOrganizationId());
    ...
    for (OrganizationToRole organizationToRole : grantedRoles) {
      if (organizationToRole.getOrganizationRole().getRoleName() == OrganizationRoleType.SUPPLIER
          || organizationToRole.getOrganizationRole().getRoleName()
              == OrganizationRoleType.RESELLER) {
      ...
      OrganizationAuthorityException ioa =
          new OrganizationAuthorityException(
              "Insufficient authorization. Required role for organization "
                  ...
                  + ", "
                  + OrganizationRoleType.RESELLER,
              new Object[] {OrganizationRoleType.SUPPLIER + ", " + OrganizationRoleType.RESELLER});
      logger.logWarn(
          ...
          OrganizationRoleType.SUPPLIER.name() + ", " + OrganizationRoleType.RESELLER.name());
      throw ioa;
    }

    List<OrganizationRefToPaymentType> availablePaymentTypes =
        organization.getPaymentTypes(false, role.getRoleName(), operator.getOrganizationId());
    for (String paymentTypeId : types) {
      ...
      try {
        ...
        for (PaymentType pt :
            ParameterizedTypes.iterable(query.getResultList(), PaymentType.class)) {
          if (notFirst) {
            validTypes.append(", ");
          }
          notFirst = true;
        ...
        throw new PaymentDataException(
            String.format(message, paymentTypeId, validTypes.toString()),
            Reason.UNKNOWN_PAYMENT_TYPE);
      }
      if (!orgHasAvailablePaymentType(paymentType, availablePaymentTypes)) {
        if (paymentType.getCollectionType() == PaymentCollectionType.PAYMENT_SERVICE_PROVIDER
            && (organization.getPspAccountForPsp(paymentType.getPsp()) == null
                || organization.getPspAccountForPsp(paymentType.getPsp()).getPspIdentifier()
                    == null)) {
          // validate that the supplier's organization has the
          // psp-identifier set for itself. If not, throw an exception
          PSPIdentifierForSellerException mpi =
              new PSPIdentifierForSellerException(
                  "Supplier '"
                      + organization.getKey()
        ...
        OrganizationReference orgRef = findOrCreateOrganizationReference(operator, organization);
        OrganizationRefToPaymentType apt = new OrganizationRefToPaymentType();
        apt.setOrganizationReference(orgRef);
        apt.setPaymentType(paymentType);
        apt.setOrganizationRole(role);
        try {
          ...
          throw new SaaSSystemException("Persisting new available payment failed.", e);
        }
      }
    }
  }
  ...
  @Override
  @RolesAllowed("PLATFORM_OPERATOR")
  public void addAvailablePaymentTypes(VOOrganization supplier, Set<String> types)
      throws ObjectNotFoundException, OrganizationAuthorityException,
          PSPIdentifierForSellerException, PaymentDataException {
  ...
  private boolean orgHasAvailablePaymentType(
      PaymentType type, List<OrganizationRefToPaymentType> available) {
  ...
  @Override
  @RolesAllowed("PLATFORM_OPERATOR")
  public boolean retryFailedPaymentProcesses() throws OrganizationAuthoritiesException {
  ...
  @Override
  @RolesAllowed("PLATFORM_OPERATOR")
  public List<VOTimerInfo> getTimerExpirationInformation() throws OrganizationAuthoritiesException {

    List<VOTimerInfo> expirationDates = timerMgmt.getCurrentTimerExpirationDates();
  ...
  public void setUserAccountStatus(VOUser voUser, UserAccountStatus newStatus)
    ...
    BLValidator.isUserId(UserDataAssembler.FIELD_NAME_USER_ID, voUser.getUserId(), true);
  ...
  @Override
  @RolesAllowed("PLATFORM_OPERATOR")
  public void setDistinguishedName(String organizationId, String distinguishedName)
      throws ObjectNotFoundException, DistinguishedNameException, OrganizationAuthoritiesException,
          ValidationException {
  ...
  public byte[] getOrganizationBillingData(long from, long to, String organizationId)
    ...
    try {
      List<BillingResult> resultList =
          billing.generateBillingForAnyPeriod(from, to, organization.getKey());
      if (resultList != null) {
  ...
  private Organization getOrganizationInt(String organizationId) throws ObjectNotFoundException {
    Organization organization = new Organization();
    organization.setOrganizationId(organizationId);
    organization = (Organization) dm.getReferenceByBusinessKey(organization);
    return organization;
  }
  ...
  public void addCurrency(String currencyISOCode)
    ...
    try {
      ...
      ValidationException ve =
          new ValidationException(
              ReasonEnum.INVALID_CURRENCY, "currencyISOCode", new Object[] {currencyISOCode});
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG, ve, LogMessageIdentifier.WARN_INVALID_CURRENCY, currencyISOCode);
      throw ve;
    }
    ...
    try {
      dm.persist(sp);
      logger.logInfo(
          Log4jLogger.SYSTEM_LOG, LogMessageIdentifier.INFO_CURRENCY_ADDED, currencyISOCode);
    } catch (NonUniqueBusinessKeyException e) {
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG,
  ...
  public List<VOConfigurationSetting> getConfigurationSettings()
    ...
    List<ConfigurationSetting> settings = configService.getAllConfigurationSettings();

    List<VOConfigurationSetting> result =
        ConfigurationSettingAssembler.toVOConfigurationSettings(settings);
  ...
  public void saveConfigurationSetting(VOConfigurationSetting setting)
      throws OrganizationAuthoritiesException, ValidationException, ConcurrentModificationException,
          DuplicateTenantIdException {

    ConfigurationSetting dbSetting =
        configService.getConfigurationSetting(setting.getInformationId(), setting.getContextId());
    dbSetting = ConfigurationSettingAssembler.updateConfigurationSetting(setting, dbSetting);
  ...
  public VOOperatorOrganization updateOrganization(
      VOOperatorOrganization voOrganization, VOImageResource voImageResource)
      throws OrganizationAuthoritiesException, ObjectNotFoundException, ValidationException,
          ConcurrentModificationException, DistinguishedNameException,
          OrganizationAuthorityException, IncompatibleRolesException,
          PSPIdentifierForSellerException, PaymentDataException, ImageException,
  ...
  VOOperatorOrganization updateOrganizationIntern(
      VOOperatorOrganization voOrganization, VOImageResource voImageResource)
      throws ObjectNotFoundException, ValidationException, ConcurrentModificationException,
          DistinguishedNameException, OrganizationAuthorityException, IncompatibleRolesException,
          PSPIdentifierForSellerException, PaymentDataException, ImageException,
          AddMarketingPermissionException, NonUniqueBusinessKeyException {
    ...
    Organization organizationObj = (Organization) dm.getReferenceByBusinessKey(tmpOrganization);

    if (isVendor(voOrganization)) {
      organizationObj = OrganizationAssembler.updateVendor(organizationObj, voOrganization);
    } else {
      if (voImageResource != null) {
        // only suppliers and technology providers are allowed to
        // save images
      ...
      organizationObj = OrganizationAssembler.updateCustomer(organizationObj, voOrganization);
    }
    ...
    updateOperatorRevenueShare(organizationObj, voOrganization.getOperatorRevenueShare());

    updateOrganizationDescription(organizationObj.getKey(), voOrganization.getDescription());
  ...
  private void updatePaymentTypes(VOOperatorOrganization voOrganization)
      throws ObjectNotFoundException, OrganizationAuthorityException,
          PSPIdentifierForSellerException, PaymentDataException {
    List<VOPaymentType> desiredPaymentypes = voOrganization.getPaymentTypes();
  ...
  private void updateOrganizationRoles(
      VOOperatorOrganization voOrganization, Organization organizationObj)
      throws OrganizationAuthorityException, IncompatibleRolesException, ObjectNotFoundException,
          AddMarketingPermissionException {

    List<OrganizationRoleType> desiredRoles = voOrganization.getOrganizationRoles();
    OrganizationRoleValidator.containsMultipleSellerRoles(
        desiredRoles, LogMessageIdentifier.WARN_ADDING_INCOMPATIBLE_ROLE_TO_ORGANIZATION_FAILED);
    if (desiredRoles != null && desiredRoles.size() > 0) {
      for (OrganizationRoleType role : desiredRoles) {
        if (!organizationObj.hasRole(role)) {
          addOrganizationToRoleIntern(voOrganization.getOrganizationId(), role);
        }
      }
    }
  }

  private void updateDomicileCountry(
      VOOperatorOrganization voOrganization, Organization organizationObj)
      throws ObjectNotFoundException {
    SupportedCountry sc = new SupportedCountry(voOrganization.getDomicileCountry());
    sc = (SupportedCountry) dm.getReferenceByBusinessKey(sc);
    organizationObj.setDomicileCountry(sc);
  }

  private void updateTenant(Organization organization, VOOperatorOrganization voOrganization)
      throws ObjectNotFoundException {
  ...
  private boolean isVendor(VOOperatorOrganization voOrganization) {
    ...
    if (voOrganization.getOrganizationRoles() != null) {
      for (OrganizationRoleType role : voOrganization.getOrganizationRoles()) {
  ...
  public List<VOOrganization> getOrganizationsWithLimit(
      ...
      Integer queryLimit)
      throws OrganizationAuthoritiesException {
    Query query = dm.createNamedQuery("Organization.findOrganizationsByIdAndRole");
  ...
  @Override
  @RolesAllowed("PLATFORM_OPERATOR")
  public List<VOUserDetails> getUsers() throws OrganizationAuthoritiesException {
    Query query =
        dm.createQuery(
  ...
  public List<VOUserDetails> getUnassignedUsersByOrg(Long subscriptionKey, Long organizationKey) {
    Query query =
  ...
  @Override
  @RolesAllowed("PLATFORM_OPERATOR")
  public boolean startPaymentProcessing() throws OrganizationAuthoritiesException {
  ...
  @Override
  @RolesAllowed("PLATFORM_OPERATOR")
  public byte[] getUserOperationLog(List<String> operationIds, long fromDate, long toDate)
      throws ValidationException, AuditLogTooManyRowsException {

    BLValidator.isValidDateRange(new Date(fromDate), new Date(toDate));
    byte[] resultCsv = auditLogService.loadAuditLogs(operationIds, fromDate, toDate);
    return resultCsv;
  }
  ...
  public Map<String, String> getAvailableAuditLogOperationGroups() {
    return AuditLogOperationGroups.getInstance().getAvailableOperationGroups();
  }
  ...
  public List<VOPSP> getPSPs() {
    ...
    List<PSP> psps = ParameterizedTypes.list(query.getResultList(), PSP.class);
    result =
        PSPAssembler.toVos(psps, new LocalizerFacade(localizer, dm.getCurrentUser().getLocale()));
  ...
  public VOPSP savePSP(VOPSP psp)
    ...
    if (dm.contains(pspToStore) && !pspToStore.getIdentifier().equals(psp.getId())
    ...
    VOPSP result =
        PSPAssembler.toVo(
            pspToStore, new LocalizerFacade(localizer, dm.getCurrentUser().getLocale()));

    return result;
  }

  /**
   * Handles the settings of the PSP, creates new ones, updates existing ones and deletes missing
   * ones.
   *
   * @param pspToStore The domain object containing the already existing settings.
   * @param psp The value object containing the target set of settings.
   * @throws ValidationException If the validation of the specified settings fails.
   * @throws ConcurrentModificationException Thrown in case the psp setting has changed in the
   *     meantime.
   * @throws ObjectNotFoundException Thrown in case the psp setting cannot be found.
   */
  private void handleSettings(PSP pspToStore, VOPSP psp)
      throws ConcurrentModificationException, ValidationException, ObjectNotFoundException {
    ...
    for (VOPSPSetting setting : pspSettings) {
      PSPSetting updatedSetting = existingSettings.remove(Long.valueOf(setting.getKey()));
      if (updatedSetting == null) {
        if (setting.getKey() != 0) {
          updatedSetting = dm.getReference(PSPSetting.class, setting.getKey());
        } else {
          updatedSetting = new PSPSetting();
          setting.setVersion(0);
        }
      }
      updatedSetting = PSPSettingAssembler.updatePSPSetting(setting, updatedSetting);
  ...
  public List<VOPSPAccount> getPSPAccounts(VOOrganization organization)
    ...
    Organization org = dm.getReference(Organization.class, organization.getKey());
    List<PSPAccount> pspAccounts = org.getPspAccounts();
    accounts =
        PSPAccountAssembler.toVos(
            pspAccounts, new LocalizerFacade(localizer, dm.getCurrentUser().getLocale()));
  ...
  @Override
  @RolesAllowed("PLATFORM_OPERATOR")
  public VOPSPAccount savePSPAccount(VOOrganization organization, VOPSPAccount account)
      throws ObjectNotFoundException, OrganizationAuthorityException,
    ...
    if (!org.hasRole(OrganizationRoleType.SUPPLIER)
        && !org.hasRole(OrganizationRoleType.RESELLER)) {
      OrganizationAuthorityException oae =
          new OrganizationAuthorityException(
              String.format(
    ...
    if (org.getPspAccountForPsp(psp) != null && !dm.contains(pspAccount)) {
      String[] params = new String[] {org.getOrganizationId(), psp.getIdentifier()};
      ValidationException ve =
          new ValidationException(ReasonEnum.DUPLICATE_PSP_ACCOUNT, "account", params);
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG, ve, LogMessageIdentifier.WARN_DUPLICATE_PSP_ACCOUNT, params);
      throw ve;
    }
    ...
    if (!dm.contains(pspAccount)) {
      try {
        ...
        SaaSSystemException sse = new SaaSSystemException("Creating of PSPAccount failed", e);
        logger.logError(LogMessageIdentifier.ERROR_NONUNIQUEKEY_EXCEPTION_ALTHOUGH_NO_BUSINESS_KEY);
        throw sse;
      }
    }
    ...
    dm.flush();
    VOPSPAccount result =
        PSPAccountAssembler.toVo(
            pspAccount, new LocalizerFacade(localizer, dm.getCurrentUser().getLocale()));
  ...
  @Override
  @RolesAllowed("PLATFORM_OPERATOR")
  public List<VOPaymentType> getPaymentTypes(VOPSP psp) throws ObjectNotFoundException {
    ...
    result.addAll(
        PaymentTypeAssembler.toVOPaymentTypes(
            paymentTypes, new LocalizerFacade(localizer, dm.getCurrentUser().getLocale())));
  ...
  public VOPaymentType savePaymentType(VOPSP psp, VOPaymentType paymentType)
    ...
    // if the identifier was changed or the object is not managed by the em
    if (dm.contains(pt) && !pt.getPaymentTypeId().equals(paymentType.getPaymentTypeId())
        || !dm.contains(pt)) {
      // validate BK uniqueness
    ...
    VOPaymentType result =
        PaymentTypeAssembler.toVOPaymentType(
            pt, new LocalizerFacade(localizer, dm.getCurrentUser().getLocale()));
    localizer.storeLocalizedResource(
        dm.getCurrentUser().getLocale(),
        result.getKey(),
  ...
  public byte[] getSupplierRevenueList(long month) {
    ...
    PriceConverter priceConverter =
        new PriceConverter(LocaleHandler.getLocaleFromString(dm.getCurrentUser().getLocale()));
    SupplierRevenueSqlResult result = SupplierRevenueSqlResult.executeQuery(dm, month);
    String header = String.format("FROM,TO,ID,NAME,AMOUNT,CURRENCY,MARKETPLACE%n");
    StringBuffer csvResult = new StringBuffer(header);
    List<SupplierRevenueSqlResult.RowData> rowDataList = result.getRowData();
    for (SupplierRevenueSqlResult.RowData rowData : rowDataList) {
      ...
      row[4] = priceConverter.getValueToDisplay(new BigDecimal(rowData.amount), false);
      row[5] = rowData.currency;
      row[6] = rowData.marketplace;
      csvResult.append(String.format("%s%n", CsvCreator.createCsvLine(row)));
    }
  ...
  public Map<String, String> getOrganizationIdentifiers(List<OrganizationRoleType> r) {
    ...
    if (ort.isEmpty()) {
      ...
      ort.add(OrganizationRoleType.CUSTOMER);
      ort.add(OrganizationRoleType.TECHNOLOGY_PROVIDER);
    }
