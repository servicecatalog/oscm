...
public class Organization extends DomainObjectWithHistory<OrganizationData> {
  ...
  private static final List<LocalizedObjectTypes> LOCALIZATION_TYPES = Collections
      .unmodifiableList(Arrays.asList(LocalizedObjectTypes.ORGANIZATION_DESCRIPTION));
  ...
  /**
   * 1:n relation to PlatformUser: the users registered for the organization. No
   * cascading option.
   */
  ...
  /**
   * 1:n relation to Product: the organization is the supplier of the marketing
   ...
   * users that have been created on the behalf of other users also. These users
   * should be excluded in the UI.
   * 
   * @return List of platform users
   */
   ...
   * 
   * @return <code>true</code> in case a remote LDAP has been activated for the
  ...
  /**
   * Defines if the organization supports users managed in a remote LDAP system
   * or not.
   * 
   * @param remoteLdapActive
   *          <code>true</code> in case the organization users are managed in an
  ...
  /**
   * @return The list with all active setting types of the current Organization
   *         which define a mapping of an LDAP attribute to a user property.
   */
   ...
   * 
   * @param usedAsDefault
   *          <code>true</code> if the configured default payment types should
   *          be returned; <code>false</code> if the available payment types
   *          should be returned
   * @param role
   *          the context role - a customer can have available payment types as
   *          well as a supplier
   * @param definingOrgId
   *          The identifier of the defining organization.
   * @return the list of <code>OrganizationToPaymentType</code>
   */
  public List<OrganizationRefToPaymentType> getPaymentTypes(boolean usedAsDefault,
      OrganizationRoleType role, String definingOrgId) {
  ...
  /**
   * Reads the service default payment types added for this organization in the
   * context of a certain role.
   * 
  ...
  /**
   * Returns the trigger definition stored for the current organization that is
   * configured to suspend the processing of BES. If more than one entry should
   * be there, a system exception will be thrown.
   * 
   * @param type
  ...
  /**
   * Evaluates the organization's subscriptions and returns those, that have the
   * specified status and that use the specified payment type.
   * 
   * @param states
   ...
   */
  public List<Subscription> getSubscriptionsForStateAndPaymentType(Set<SubscriptionStatus> states,
      String paymentType) {
    List<Subscription> result = new ArrayList<>();
    List<Subscription> subs = getSubscriptions();
    for (Subscription subscription : subs) {
      if (states.contains(subscription.getStatus()) && subscription.getPaymentInfo() != null
          && subscription.getPaymentInfo().getPaymentType().getPaymentTypeId()
              .equals(paymentType)) {
   ...
   * subscription is considered usable if a user can eventually subscribe to it.
   */
   ...
   * @return <code>true</code> in case the type is supported, <code>false</code>
   *         otherwise.
   */
  public boolean canPermitPaymentType(String type) {
    for (OrganizationRefToPaymentType ref : getPaymentTypes(false, OrganizationRoleType.SUPPLIER,
        OrganizationRoleType.PLATFORM_OPERATOR.name())) {
  ...
  /**
   * Returns the PSP account registered for the organization which refers to the
   * specified PSP.
   * 
   * @param psp
   ...
   * {@link OrganizationReferenceType} or <code>null</code> if it doesn't exist.
   * 
   * @param customer
