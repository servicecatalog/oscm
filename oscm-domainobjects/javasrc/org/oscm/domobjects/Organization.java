...
/**
 * We call the one who registers at the SaaS portal a organization. We consider this one also as the
 * contracting party in the legal sense. Part of a organization#s data is the payment information to
 * be used for collecting the subscription fees. (Entering the payment information is only mandatory
 * if the organization subscribes a chargeable product.)
 *
 * @author schmid
 */
@Entity
...
@NamedQueries({
  @NamedQuery(
      name = "Organization.getAdministrators",
      query =
          "SELECT pu FROM PlatformUser pu, RoleAssignment ra WHERE pu.organization.key=:orgkey and ra.user.key=pu.key and ra.userRole.dataContainer.roleName=org.oscm.internal.types.enumtypes.UserRoleType.ORGANIZATION_ADMIN"),
  @NamedQuery(
      name = "Organization.findByBusinessKey",
      query = "SELECT c FROM Organization c WHERE c.dataContainer.organizationId=:organizationId"),
  @NamedQuery(name = "Organization.getAllOrganizations", query = "SELECT c FROM Organization c"),
  @NamedQuery(
      name = "Organization.getAllSuppliers",
      query =
          "SELECT c FROM Organization c INNER JOIN c.grantedRoles otr INNER JOIN otr.organizationRole r WHERE r.dataContainer.roleName = org.oscm.internal.types.enumtypes.OrganizationRoleType.SUPPLIER"),
  @NamedQuery(
      name = "Organization.getForSupplierKey",
      query =
          "SELECT c FROM Organization c, OrganizationReference r WHERE r.target=c AND r.sourceKey=:supplierKey AND r.dataContainer.referenceType = :referenceType ORDER BY c.key ASC"),
  @NamedQuery(
      name = "Organization.getForSupplierKeyAndProduct",
      query =
          "SELECT c FROM Organization c, OrganizationReference r, Product p WHERE r.target=c AND r.sourceKey=:supplierKey AND r.dataContainer.referenceType = org.oscm.domobjects.enums.OrganizationReferenceType.SUPPLIER_TO_CUSTOMER AND c = p.targetCustomer AND p.template = :product AND NOT EXISTS (SELECT s.key FROM Subscription s WHERE s.product = p)"),
  @NamedQuery(
      name = "Organization.getForOffererKeyAndSubscriptionId",
      query =
          "SELECT c FROM Organization c, OrganizationReference r, Subscription sub WHERE r.target=c AND r.sourceKey=:offererKey AND r.dataContainer.referenceType IN (org.oscm.domobjects.enums.OrganizationReferenceType.SUPPLIER_TO_CUSTOMER,org.oscm.domobjects.enums.OrganizationReferenceType.BROKER_TO_CUSTOMER,org.oscm.domobjects.enums.OrganizationReferenceType.RESELLER_TO_CUSTOMER) AND sub.organization = c AND sub.dataContainer.subscriptionId = :subscriptionId AND sub.dataContainer.status IN (:states)"),
  @NamedQuery(
      name = "Organization.getOrganizationIdentifiersByRole",
      query =
          "SELECT DISTINCT o.dataContainer.organizationId, o.dataContainer.name FROM Organization o, OrganizationToRole o2r, OrganizationRole r WHERE o = o2r.organization and o2r.organizationRole = r and r.dataContainer.roleName IN (:organizationRoleTypes)"),
  @NamedQuery(
      name = "Organization.findOrganizationsByIdAndRole",
      query =
          "SELECT DISTINCT o FROM Organization o, OrganizationToRole o2r, OrganizationRole r where o.dataContainer.organizationId like :organizationId and o = o2r.organization and o2r.organizationRole = r and r.dataContainer.roleName IN (:organizationRoleTypes)"),
  @NamedQuery(
      name = "Organization.findOrganizationsByGroupId",
      query = "SELECT DISTINCT o FROM Organization o where o.dataContainer.groupid like :groupId "),
  @NamedQuery(
      name = "Organization.findOrganizationsByName",
      query =
          "SELECT o FROM Organization o where o.dataContainer.name like :name AND ((o.tenant IS NULL AND :tenantId IS NULL) OR EXISTS (SELECT t FROM Tenant t WHERE t=o.tenant AND t.dataContainer.tenantId = :tenantId))"),
  @NamedQuery(
      name = "Organization.countOrgsWithSameDN",
      query =
          "SELECT COUNT (DISTINCT organization) FROM Organization organization WHERE organization.dataContainer.distinguishedName = :distinguishedName AND organization != :organization"),
  @NamedQuery(
      name = "Organization.getLdapManagedOrganizations",
      query =
          "SELECT organization FROM Organization organization WHERE organization.dataContainer.remoteLdapActive=true"),
  @NamedQuery(
      name = "Organization.getOrgsForDN",
      query =
          "SELECT DISTINCT organization FROM Organization organization WHERE organization.dataContainer.distinguishedName IN (:dn)")
})
...
public class Organization extends DomainObjectWithHistory<OrganizationData> {
  ...
  private static final List<LocalizedObjectTypes> LOCALIZATION_TYPES =
      Collections.unmodifiableList(Arrays.asList(LocalizedObjectTypes.ORGANIZATION_DESCRIPTION));
  ...
  /**
   * The list with all active setting types of the current organization which define a mapping of an
   * LDAP attribute to a user property.
   *
   * @see OrganizationRoleType#CUSTOMER
   */
  @Transient private List<SettingType> ldapUserAttributes;
  ...
  /**
   * 1:n relation to PlatformUser: the users registered for the organization. No cascading option.
   */
  ...
  /**
   * 1:n relation to Subscription: the subscriptions of the organization. CascadeType: REMOVE
   *
   * @see OrganizationRoleType#CUSTOMER
   */
  @OneToMany(mappedBy = "organization", cascade = CascadeType.REMOVE, fetch = FetchType.LAZY)
  ...
  /**
   * 1:n relation to OrganizationReference which references the sources (this organization is the
   * target).
   */
  ...
  /**
   * 1:n relation to OrganizationReference which references the targets (this organization is the
   * source).
   */
  ...
  /**
   * 1:n relation to Product: the organization is the supplier of the marketing product.
   * CascadeType: REMOVE
   *
   * @see OrganizationRoleType#SUPPLIER
   */
  ...
  /**
   * 1:n relation to TechnicalProduct: the organization is technology provider of the technical
   * product. CascadeType: REMOVE
   *
  ...
  /**
   * OrganizationID of the platform operator. The platform operator is created by the setup script.
   */
  ...
  /**
   * Returns a list of all platform users of this organization. The list does not include users that
   * have been created on the behalf of other users. These users should be excluded in the UI.
   *
  ...
  /**
   * Returns all platform users of this organization. The result list includes users that have been
   * created on the behalf of other users also. These users should be excluded in the UI.
   *
   * @return List of platform users
   */
  ...
  /**
   * Returns all administrators of this organization. The result list includes users that have been
   * created on the behalf of other users also. These users should be excluded in the UI.
   *
   * @return List of platform users
   */
  ...
  /**
   * Returns a list of all usable subscriptions. A subscription is considered usable if a user can
   * eventually subscribe to it.
   */
  public List<Subscription> getUsableSubscriptions() {
    List<Subscription> usableSubscriptions = new ArrayList<>(subscriptions);
    for (Iterator<Subscription> i = usableSubscriptions.iterator(); i.hasNext(); ) {
   ...
   *
   * <p>Any organization has a role CUSTOMER
   *
   * @param roleToCheckFor The role the organization is supposed to be granted.
   * @return <code>true</code> in case the organization has been granted the given role, <code>false
   *     </code> otherwise.
   */
  ...
  public void setOrganizationSettings(List<OrganizationSetting> organizationSettings) {
   ...
   *
   * @return <code>true</code> in case a remote LDAP has been activated for the organization, <code>
  ...
  /**
   * Defines if the organization supports users managed in a remote LDAP system or not.
   *
   * @param remoteLdapActive <code>true</code> in case the organization users are managed in an
   *     external LDAP system, <code>false</code> otherwise.
   */
  ...
  /**
   * @return The list with all active setting types of the current Organization which define a
   *     mapping of an LDAP attribute to a user property.
   */
    ...
    if (ldapUserAttributes == null) {
      if (getOrganizationSettings() != null) {
        ...
        for (OrganizationSetting setting : getOrganizationSettings()) {
          if (SettingType.LDAP_ATTRIBUTES.contains(setting.getSettingType())) {
  ...
  /**
   * Returns the organization reference to payment type objects valid in the context of the defining
   * organization with the given identifier.
   *
   * @param definingOrgId The identifier of the defining organization.
   * @return The payment references.
   */
  public List<OrganizationRefToPaymentType> getPaymentTypes(String definingOrgId) {
  ...
  /**
   * Reads the payment types added for this organization in the context of a certain role
   *
   * @param usedAsDefault <code>true</code> if the configured default payment types should be
   *     returned; <code>false</code> if the available payment types should be returned
   * @param role the context role - a customer can have available payment types as well as a
   *     supplier
   * @param definingOrgId The identifier of the defining organization.
   * @return the list of <code>OrganizationToPaymentType</code>
   */
  public List<OrganizationRefToPaymentType> getPaymentTypes(
      boolean usedAsDefault, OrganizationRoleType role, String definingOrgId) {
  ...
  /**
   * Reads the service default payment types added for this organization in the context of a certain
   * role.
   *
  ...
  public List<OrganizationRefToPaymentType> getDefaultServicePaymentTypes() {
    ...
    for (OrganizationRefToPaymentType ref : types) {
      if ((ref.isUsedAsServiceDefault() == true)
          && (OrganizationRoleType.SUPPLIER.equals(ref.getOrganizationRole().getRoleName())
              || OrganizationRoleType.RESELLER.equals(ref.getOrganizationRole().getRoleName()))) {
        result.add(ref);
      }
    }
    return result;
  ...
  /**
   * Returns the trigger definition stored for the current organization that is configured to
   * suspend the processing of BES. If more than one entry should be there, a system exception will
   * be thrown.
   *
   * @param type The type the trigger must have.
   * @return The suspending trigger definition, <code>null</code> if none exists.
   */
  ...
  /**
   * Sets the domicile country for this organization. A supplier may restrict the list of supported
   * countries this organization can choose from.
   *
   * @param country
   */
   ...
   *
   * @param country SupportedCountry to be added.
   */
   ...
   *
   * @param countryCode The country code in ISO 3166.
   * @return boolean
   */
   ...
   *
   * @param vatRates The VAT rates to set.
   */
   ...
   *
   * @param orgRefType The reference type requested.
   * @return The organization references.
   */
  public List<OrganizationReference> getSourcesForType(OrganizationReferenceType orgRefType) {
  ...
  /**
   * Evaluates the organization's subscriptions and returns those, that have the specified status
   * and that use the specified payment type.
   *
   * @param states The status set of the subscription.
   * @param paymentType The payment type that has to be checked for.
  ...
  /**
   * Returns a list of all usable subscriptions for the given product. A subscription is considered
  ...
  public boolean isActingOnBehalf(Organization customer) {
    for (OrganizationReference ref :
        customer.getSourcesForType(OrganizationReferenceType.ON_BEHALF_ACTING)) {
   ...
   *
   * @param userList the user list to filter, could contain user from other organizations
   */
  private void filterOnBehalfUserList(List<PlatformUser> userList) {
    ...
    while (iterator.hasNext()) {
      ...
      if (!getPlatformUsers().contains(onbehalfUser.getMaster().getMasterUser())) {
        iterator.remove();
      }
    }
  }
  ...
  /**
   * Determines if the current organization may allow its customers to use the specified payment
   * type.
   *
   * @param type The payment type to check for.
   * @return <code>true</code> in case the type is supported, <code>false</code> otherwise.
   */
  public boolean canPermitPaymentType(String type) {
    for (OrganizationRefToPaymentType ref :
        getPaymentTypes(
            false, OrganizationRoleType.SUPPLIER, OrganizationRoleType.PLATFORM_OPERATOR.name())) {
  ...
  /**
   * Returns the PSP account registered for the organization which refers to the specified PSP.
   *
   * @param psp The PSP to find the account for.
   ...
   */
  public List<UdaDefinition> getReadableUdaDefinitions(OrganizationRoleType role) {
  ...
  public OrganizationRoleType getVendorRoleForPaymentConfiguration() {
    ...
    } else if (getGrantedRoleTypes().contains(OrganizationRoleType.RESELLER)) {
  ...
  /**
   * Returns the reference to the passed {@link Organization} of the passed {@link
   * OrganizationReferenceType} or <code>null</code> if it doesn't exist.
   *
   * @param customer the customer {@link Organization}
   * @param type the required {@link OrganizationReferenceType}
   * @return the {@link OrganizationReference} ore <code>null</code>
   */
  public OrganizationReference getCustomerReference(
      Organization customer, OrganizationReferenceType type) {
