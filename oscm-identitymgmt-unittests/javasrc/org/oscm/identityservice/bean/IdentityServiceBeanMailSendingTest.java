...
/**
 * Tests the correct generation of the context URL of Emails sent by this service.
 *
 * @author barzu
 */
public class IdentityServiceBeanMailSendingTest {
  ...
  public void setup() throws Exception {
    ...
    user.setEmail("user@oscm.org");

    supplier = new PlatformUser();
    Organization org2 = new Organization();
    org.setOrganizationId("supplier");
    supplier.setOrganization(org2);
    supplier.setKey(2345);
    supplier.setUserId("supplier");
    supplier.setEmail("supper@oscm.org");

    doReturn(user).when(idSrv.dm).getCurrentUser();
    doReturn(user).when(dm).getReference(eq(PlatformUser.class), eq(user.getKey()));
    ...
    when(dm.createNamedQuery(contains("TriggerProcessIdentifier"))).thenReturn(triggerQuery);
    ...
    ConfigurationSetting setting =
        new ConfigurationSetting(ConfigurationKey.BASE_URL, Configuration.GLOBAL_CONTEXT, BASE_URL);

    doReturn(setting).when(cs).getConfigurationSetting(any(ConfigurationKey.class), anyString());
    ...
    doReturn(
            Collections.singletonList(
                new TriggerProcessMessageData(new TriggerProcess(), new TriggerMessage())))
        .when(triggerQS)
        .sendSuspendingMessages(ParameterizedTypes.list(anyList(), TriggerMessage.class));
  ...
  public void confirmAccount_PublicMP() throws Exception {
    ...
    idSrv.confirmAccount(givenVOUser(user), "1");
  ...
  public void confirmAccount_AdminPortal() throws Exception {
    user.setStatus(UserAccountStatus.LOCKED_NOT_CONFIRMED);
    user.setAssignedRoles(UserRoles.createRoleAssignments(user, UserRoleType.MARKETPLACE_OWNER));
    idSrv.confirmAccount(givenVOUser(user), "1");
  ...
  /**
   * Test that on account confirmation user id and user key are passed to be contained in the mail
   * (Bug 9168)
   */
  @Test
  ...
  public void createOrganizationAdmin_PublicMP() throws Exception {
    ...
    idSrv.createOrganizationAdmin(
        getVOUserDetails(org, "user1"), org, "abc", Long.valueOf(123L), new Marketplace());
  ...
  /** Bug9301: Confirmation URL have to be encoded by using URLEncoder (e.g. '+' => %2B) */
  @Test
  public void createOrganizationAdmin_PublicMPWithJapaneseID() throws Exception {
    setupDsForRefreshingUserRoles();
    Organization org = Organizations.createOrganization("fdaskj");

    // pwd not auto generated -> test pwd confirmation mail
    idSrv.createOrganizationAdmin(
        getVOUserDetails(org, "富士次郎"), org, "abc", Long.valueOf(123L), new Marketplace());
  ...
  public void createOrganizationAdmin_AdminPortal() throws Exception {
    ...
    // pwd not auto generated -> test pwd confirmation mail
    idSrv.createOrganizationAdmin(userDetails, org, "abc", Long.valueOf(123L), new Marketplace());
  ...
  @Test
  public void createOrganizationAdmin_BaseUrlWithTwoSlashes() throws Exception {
  ...
  public void createOrganizationAdmin_AutoGenPwd_PublicMP() throws Exception {
    ...
    idSrv.createOrganizationAdmin(
        getVOUserDetails(org, "user1"), org, null, Long.valueOf(123L), new Marketplace());
  ...
  @Test
  public void createOrganizationAdmin_AutoGenPwd_AdminPortal() throws Exception {
    setupDsForRefreshingUserRoles();
    Organization org = Organizations.createOrganization("fdaskj");
    // non-customer role
    VOUserDetails userDetails = getVOUserDetails(org, "user1");
    ...
    // null password -> auto-generated -> skips pwd confirmation mail
    idSrv.createOrganizationAdmin(userDetails, org, null, Long.valueOf(123L), new Marketplace());
  ...
  @Test
  public void createOrganizationAdmin_RemoteLdapActive_PublicMP() throws Exception {
    setupDsForRefreshingUserRoles();
    Organization org = Organizations.createOrganization("fdaskj");
    // LDAP active
  ...
  @Test
  public void createOrganizationAdmin_RemoteLdapActive_AdminPortal() throws Exception {
    setupDsForRefreshingUserRoles();
    Organization org = Organizations.createOrganization("fdaskj");
    // LDAP active
    OrganizationSetting orgSetting = new OrganizationSetting();
  ...
  public void createUser_AdminPortal_NoRole() throws Exception {
    ...
    idSrv.createUser(
        getVOUserDetails(Organizations.createOrganization("orgId"), "userId"),
  ...
  public void sendMailToCreatedUser_NoManagerRole() throws Exception {
    ...
    idSrv.sendMailToCreatedUser("secret", true, marketplace, user);
  ...
  @Test
  public void sendMailToCreatedUser_LDAP_ManagerRole_GivenMID_B9806() throws Exception {
    // Given a user with the service manager role
    addRole(user, UserRoleType.SERVICE_MANAGER);
    ...
    // when sending a mail to the created user
    idSrv.sendMailToCreatedUser("secret", false, marketplace, user);
  ...
  @Test
  public void sendMailToCreatedUser_LDAP_ManagerRole_NoGivenMID_B9806() throws Exception {
    // Given a user with the service manager role
    addRole(user, UserRoleType.SERVICE_MANAGER);
  ...
  @Test
  public void sendMailToCreatedUser_LDAP_NoManagerRole_GivenMID_B9806() throws Exception {
    // Given a user with the service manager role
    addRole(user, UserRoleType.ORGANIZATION_ADMIN);
  ...
  public void sendMailToCreatedUser_MarketplaceIdSet() throws Exception {
    ...
    assertEquals(String.valueOf(user.getKey()), value[4]);
  }
  ...
  public void sendMailToCreatedUser_SwitchedOff() throws Exception {
    ...
    idSrv.sendMailToCreatedUser("secret", true, mp, user);
  ...
  public void sendMailToCreatedUser_NoManagerRole_SAML_SP() throws Exception {
    ...
    // when sending a mail to the created user
    idSrv.sendMailToCreatedUser("secret", true, new Marketplace(), user);
    ...
    verify(cm, times(1))
        .sendMail(
            eq(user), eq(EmailType.USER_CREATED_SAML_SP), ac.capture(), any(Marketplace.class));
    Object[] value = ac.getValue();
    assertEquals(user.getUserId(), value[0]);
  ...
  public void sendMailToCreatedUser_ManagerRole_SAML_SP() throws Exception {
    ...
    verify(cm, times(1))
        .sendMail(
            any(),
            eq(user),
            eq(EmailType.USER_CREATED_SAML_SP),
            ac.capture(),
            any(Marketplace.class));
    Object[] value = ac.getValue();
    assertEquals(user.getUserId(), value[0]);
  ...
  @Test
  public void sendMailToCreatedUser_MarketplaceIdSet_SAML_SP() throws Exception {
  ...
  @Test
  public void sendMailToCreatedUser_supplier_redirect() throws Exception {

    doReturn(Boolean.TRUE).when(cm).isMailMock();

    // given redirect
    SendMailControl.setMailTo(user.getEmail());
    addRole(user, UserRoleType.PLATFORM_OPERATOR);
    addRole(supplier, UserRoleType.SERVICE_MANAGER);

    Marketplace marketplace = new Marketplace();
    marketplace.setMarketplaceId("myMarketplaceId");

    // when sending a mail to the created user
    idSrv.sendMailToCreatedUser("secret", false, marketplace, supplier);
    ...
    verify(cm, times(1))
        .sendMail(
            eq(user.getEmail()),
            eq(supplier),
            eq(EmailType.USER_IMPORTED_WITH_MARKETPLACE),
            any(),
            any(Marketplace.class));
  }

  @Test
  public void sendMailToCreatedUser_supplier_noRedirect() throws Exception {
  ...
  private void setupIgnoreRoleCheck() throws Exception {
    doNothing().when(idSrv).checkRoleConstrains(any(PlatformUser.class), any(UserRoleType.class));
    setupDsForRefreshingUserRoles();
  }

  private ArgumentMatcher<Object[]> getArrayContainsStringMatcher(final String s) {
  ...
  private ArgumentMatcher<Object[]> getArrayNotContainsStringMatcher(final String s) {
    return new ArgumentMatcher<Object[]>() {
  ...
  private void verifyMailParameters() throws Exception {
    ...
    verify(cm, times(1))
        .sendMail(
            eq(user), eq(EmailType.USER_CONFIRM_ACKNOWLEDGE), ac.capture(), any(Marketplace.class));
    Object[] value = ac.getValue();
    assertEquals(user.getUserId(), value[1]);
    assertEquals(String.valueOf(user.getKey()), value[2]);
  }

  VOUser givenVOUser(PlatformUser po) {
    VOUser u = new VOUser();
    u.setKey(po.getKey());
    return u;
  }
  ...
  private void setupDsForRefreshingUserRoles() throws Exception {
    doAnswer(
        ...
        .when(idSrv.dm)
        .persist(any(RoleAssignment.class));
            ...
            new Answer<Void>() {
              ...
              public Void answer(InvocationOnMock invocation) throws Throwable {
                ...
                if (object instanceof PlatformUser) {
                  ((PlatformUser) object).getAssignedRoles().addAll(persistedRoleAssignments);
                }
                return null;
              }
            })
        .when(idSrv.dm)
        .refresh(any(PlatformUser.class));
  }
}
