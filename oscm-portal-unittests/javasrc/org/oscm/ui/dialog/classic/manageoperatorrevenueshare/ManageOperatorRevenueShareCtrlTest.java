...
public class ManageOperatorRevenueShareCtrlTest {
  ...
  public void setup() {
    ...
    doReturn(serviceSelector).when(ctrl.sl).findService(eq(ServiceSelector.class));
    ...
    doReturn(pricingService).when(ctrl.sl).findService(PricingService.class);
  }
  ...
  public void initializeModel_serviceSelected() throws Exception {
    ...
    doReturn(new Response(pm)).when(pricingService).getOperatorRevenueShare(anyLong());
  ...
  public void intializeRevenueShares() throws Exception {
    ...
    assertNotNull(ctrl.getModel().getOperatorRevenueShare());
    assertEquals(
        BigDecimal.valueOf(1), ctrl.getModel().getOperatorRevenueShare().getRevenueShare());
    assertNotNull(ctrl.getModel().getDefaultOperatorRevenueShare());
    assertEquals(
        BigDecimal.valueOf(2), ctrl.getModel().getDefaultOperatorRevenueShare().getRevenueShare());
  }

  @Test
  public void intializeRevenueShares_ObjectNotFoundException() throws Exception {
    // given
    ctrl.getModel().setSelectedTemplateKey(101L);
    doThrow(new ObjectNotFoundException()).when(pricingService).getOperatorRevenueShare(anyLong());
  ...
  public void save() throws Exception {
    ...
    doReturn(new Response()).when(pricingService).saveOperatorRevenueShare(eq(101L), eq(po));
    ...
    verify(pricingService, times(1)).saveOperatorRevenueShare(eq(101L), eq(po));
  }
  ...
  public void save_ServiceOperationException() throws Exception {
    // given
    ctrl.getModel().setSelectedTemplateKey(101L);
    doThrow(new ServiceOperationException())
        .when(pricingService)
        .saveOperatorRevenueShare(eq(101L), any(PORevenueShare.class));
