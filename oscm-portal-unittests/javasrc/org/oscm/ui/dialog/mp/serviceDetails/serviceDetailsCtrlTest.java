...
public class serviceDetailsCtrlTest {
  ...
  public void setup() throws Exception {
    ...
    doReturn(partnerService).when(ctrl.ui).findService(PartnerService.class);
    initServiceInServer();
    initCurrentUser();
  }
  ...
  public void getInitialize_redirectToAccessDeniedPage() throws Exception {
    ...
    doReturn(Arrays.asList(Long.valueOf(10001)))
        .when(ctrl.userGroupService)
        .getInvisibleProductKeysForUser(1L);
    doReturn(Arrays.asList(Long.valueOf(10001))).when(model).getInvisibleProductKeys();
  ...
  private void initServiceInServer() throws Exception {
    ...
    doReturn(response)
        ...
        .getAllServiceDetailsForMarketplace(
            Long.valueOf(model.selectedServiceKey).longValue(), "en", mpl);
  ...
  private void initCurrentUser() {
    ...
    doReturn(voUserDetails).when(ctrl.ui).getUserFromSessionWithoutException();
  }
}
