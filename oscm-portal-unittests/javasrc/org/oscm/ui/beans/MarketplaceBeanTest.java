...
public class MarketplaceBeanTest {
  ...
  public void setup() throws Exception {
    final MarketplaceService marketplaceService =
        new MarketplaceServiceStub() {

          @Override
          public VOServiceDetails publishService(
              VOService service, List<VOCatalogEntry> marketplaceEntries) {
            serviceCalled = true;
    ...
    bean =
        new MarketplaceBean() {
          ...
          @Override
          protected void addMessage(String clientId, Severity severity, String key, String param) {}
  ...
  private void assertMarketplaces(List<VOMarketplace> expected, List<Marketplace> actual) {
    assertEquals(expected.size(), actual.size());
    for (int i = 0; i < expected.size(); i++) {
      assertEquals(expected.get(i).getMarketplaceId(), actual.get(i).getMarketplaceId());
    }
  }
  ...
  public void testGetMarketplacesForOperator() throws Exception {
    returnedMarketplaces =
        Arrays.asList(localMarketplace, localMarketplace2, globalMarketplace, globalMarketplace2);
  ...
  public void testPublishService() throws Exception {
    ...
    Assert.assertEquals(
        ...
        passedMarketplaceEntries.get(0).getMarketplace().getMarketplaceId());
    Assert.assertEquals(
        expected.get(0).isAnonymousVisible(), passedMarketplaceEntries.get(0).isAnonymousVisible());
  }

  @Test
  public void testPublishService_noMarketplace_publicService() throws Exception {
    VOServiceDetails toPublish = new VOServiceDetails();
    ...
    Assert.assertTrue(serviceCalled);
    Assert.assertEquals(toPublish, passedService);
    Assert.assertEquals(null, passedMarketplaceEntries.get(0).getMarketplace());
    Assert.assertEquals(false, passedMarketplaceEntries.get(0).isAnonymousVisible());
  }

  @Test
  public void testPublishService_noMarketplace_nonpublicService() throws Exception {
    VOServiceDetails toPublish = new VOServiceDetails();
    bean.setMarketplaceId(null);
    ServiceDetails sd = new ServiceDetails(toPublish);
    sd.setPublicService(true);
    ...
    Assert.assertEquals(toPublish, passedService);
    Assert.assertEquals(null, passedMarketplaceEntries.get(0).getMarketplace());
    Assert.assertEquals(true, passedMarketplaceEntries.get(0).isAnonymousVisible());
  }
  ...
  public void testGetMarketplacesForService() throws Exception {
    ...
    List<VOCatalogEntry> catalogEntries = bean.getMarketplacesForService(service);

    Assert.assertEquals(service, passedService);
    Assert.assertTrue(serviceCalled);
    Assert.assertEquals(2, catalogEntries.size());
    Assert.assertEquals(
        localMarketplaceCE.getMarketplace().getMarketplaceId(),
        catalogEntries.get(0).getMarketplace().getMarketplaceId());
    Assert.assertEquals(
        globalMarketplaceCE.getMarketplace().getMarketplaceId(),
        catalogEntries.get(1).getMarketplace().getMarketplaceId());
  ...
  public void testGetSuppliersForMarketplace() throws Exception {
    ...
    for (int i = 0; i < list.size(); i++) {
      Assert.assertEquals(
          returnedSuppliers.get(i).getOrganizationId(), list.get(i).getOrganizationId());
    }
  ...
  @Test
  public void testAddSuppliersToMarketplace_NoMarketplaceIdSet() throws Exception {
  ...
  @Test
  public void testAddSuppliersToMarketplace_NoSupplierIdSet() throws Exception {
    bean.setMarketplaceId(GLOBAL_MP);
    bean.marketplaceChangedForManageSeller();
    bean.addSupplierMarketplaceRelation();
    Assert.assertTrue(serviceCalled);
  ...
  public void testRemoveSuppliersFromMarketplace() throws Exception {
    ...
    Assert.assertTrue(serviceCalled);
    Assert.assertEquals(GLOBAL_MP, passedMarketplaceId);
    Assert.assertEquals(returnedSuppliers.size(), passedOrganizationIds.size());
    for (int i = 0; i < list.size(); i++) {
      Assert.assertEquals(
          returnedSuppliers.get(i).getOrganizationId(), list.get(i).getOrganizationId());
    }
  ...
  public void testLiftBanForSuppliersOnMarketplace() throws Exception {
    ...
    Assert.assertEquals(bannedSuppliers.size(), bannedOrganizationIds.size());
    for (int i = 0; i < list.size(); i++) {
      Assert.assertEquals(
          bannedSuppliers.get(i).getOrganizationId(), list.get(i).getOrganizationId());
    }
    serviceCalled = false;
    bean.setToken(token);
    String outcome = bean.removeSupplierMarketplaceRelation();
  ...
  @Test
  public void testRemoveSuppliersFromMarketplace_NoSuppliersSelected() throws Exception {
    bean.setMarketplaceId(GLOBAL_MP);
    returnedSuppliers = createOrgsToReturn();
    bean.getSupplierMarketplaceRelation();
  ...
  @Test
  public void testRemoveSuppliersFromMarketplace_NoMarketplace() throws Exception {
    bean.setMarketplaceId(null);
    bean.removeSupplierMarketplaceRelation();
  ...
  public void testFindMarketplace() {
    ...
    bean.marketplaceChangedForManageSeller();
    Assert.assertEquals(
        globalMarketplace.getMarketplaceId(), bean.getMarketplace().getMarketplaceId());
  }
