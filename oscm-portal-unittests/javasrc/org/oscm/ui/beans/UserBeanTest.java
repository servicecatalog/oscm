...
public class UserBeanTest {
  ...
  public void setup() throws Exception {
    ...
    doReturn(new StringBuffer("https://something.de")).when(requestMock).getRequestURL();
    ...
    when(userBean.ui.findBean(eq(UserBean.APPLICATION_BEAN))).thenReturn(appBean);
    ...
    doReturn("")
        .when(userBean)
        .getLoginRedirect(any(HttpServletRequest.class), any(HttpSession.class), anyBoolean());

    doReturn(sessionMock).when(requestMock).getSession();
    serviceAccessMock = mock(ServiceAccess.class);
    doReturn(serviceAccessMock)
        .when(sessionMock)
        .getAttribute(eq(ServiceAccess.SESS_ATTR_SERVICE_ACCESS));
    doReturn(idServiceMock).when(serviceAccessMock).getService(eq(IdentityService.class));
    ...
    doReturn(true)
        ...
        .doesOrganizationHaveAccessMarketplace(anyString(), anyString());
  ...
  void mockErrorAttribute(HttpServletRequest requestMock) {
    doAnswer(
        ...
        .when(requestMock)
        .setAttribute(eq(Constants.REQ_ATTR_ERROR_KEY), anyString());
    ...
    doAnswer(
        ...
        .when(requestMock)
        .getAttribute(eq(Constants.REQ_ATTR_ERROR_KEY));
  }
  ...
  public void getUserId() {
    ...
    doReturn(userIdGiven).when(requestMock).getParameter(Constants.REQ_PARAM_USER_ID);
  ...
  public void isSelfRegistrationAllowed() {
    ...
    assertEquals(Boolean.TRUE, userBean.getSessionBean().getSelfRegistrationEnabled());
  }
  ...
  public void createClassic_ResetuserRolesForNewUser() throws Exception {
    ...
    doReturn(new VOUserDetails())
        .when(idServiceMock)
        .createUser(any(VOUserDetails.class), anyListOf(UserRoleType.class), anyString());
    // when
    userBean.createClassic();
    // then
    assertEquals(1, userBean.getUserRolesForNewUser().size());
    for (UserRole userRole : roles) {
      if (userRole.getUserRoleType().equals(UserRoleType.MARKETPLACE_OWNER)) {
  ...
  public void isInternalAuthMode_notInternalAuthMode() {
    // given
    when(Boolean.valueOf(appBean.isInternalAuthMode())).thenReturn(Boolean.FALSE);
    // when
    boolean result = userBean.isInternalAuthMode();
    // then
  ...
  public void getAuthSettings() throws Exception {
    // given
    ConfigurationService csMock = mockConfigurationService(AuthenticationMode.OIDC.name());
    ...
    assertEquals(authSettings.getIdentityProviderURL(), result.getIdentityProviderURL());
    assertEquals(
  ...
  public void isLoggedInAndSubscriptionManager_NullUser() {
    ...
    // when
    boolean isNotSubscriptionManager = userBean.isLoggedInAndSubscriptionManager();
  ...
  public void isLoggedInAndSubscriptionManager_SubscriptionManager() {
    ...
    doReturn(subscriptionManager).when(userBean).getUserFromSessionWithoutException();

    // when
    boolean isSubscriptionManager = userBean.isLoggedInAndSubscriptionManager();
  ...
  public void login_UserLocked_Internal() throws Exception {
    ...
    doThrow(new LoginException())
        .when(serviceAccessMock)
        .login(
            eq(voUser), anyString(), any(HttpServletRequest.class), any(HttpServletResponse.class));
    doReturn(BaseBean.MARKETPLACE_START_SITE).when(requestMock).getServletPath();
    // when
    String result = userBean.login();
    // then
    verify(requestMock, times(1))
        .setAttribute(eq(Constants.REQ_ATTR_ERROR_KEY), eq(BaseBean.ERROR_USER_LOCKED));
  ...
  public void login_UserLocked_SAMLSP() throws Exception {
    ...
    doReturn("userId")
        .when(requestMock)
        .getParameter(eq(UserBean.SAMPSP_FORM + Constants.REQ_PARAM_USER_ID));
    ...
    doThrow(new LoginException())
        ...
        .login(
            eq(voUser), anyString(), any(HttpServletRequest.class), any(HttpServletResponse.class));
    doReturn(BaseBean.SAML_SP_LOGIN_AUTOSUBMIT_PAGE).when(requestMock).getServletPath();
    // when
    String result = userBean.login();
    // then
    verify(requestMock, times(1))
  ...
  public void login_NonExistingUser_Internal() throws Exception {
    ...
    doThrow(new ObjectNotFoundException()).when(idServiceMock).getUser(any(VOUser.class));
    // when
    String result = userBean.login();
    // then
    verify(requestMock, times(1))
        .setAttribute(eq(Constants.REQ_ATTR_ERROR_KEY), eq(BaseBean.ERROR_LOGIN));
    assertEquals(BaseBean.OUTCOME_STAY_ON_PAGE, result);
  }
  ...
  @Test
  public void login_SAMLSP()
      throws ObjectNotFoundException, OrganizationRemovedException, OperationNotPermittedException,
          ValidationException {
    ...
    userBean.login();
    // then
    verify(userBean, times(1))
        .getLoginRedirect(any(HttpServletRequest.class), eq(sessionMock), eq(false));
  }

  /** Try to trigger import user action without uploaded file (actually cannot happen) */
  @Test
  public void importUsers() {
    ...
    // then
    verify(userBean.ui, times(1)).handleException(any(SaaSApplicationException.class));
  }
  ...
  public void createInt_MailOperationException_Internal() throws Exception {
    ...
    doReturn(mockConfigurationService(AuthenticationMode.INTERNAL.name()))
        .when(userBean)
        .getConfigurationService();
    when(idServiceMock.createUser(
            any(VOUserDetails.class), anyListOf(UserRoleType.class), anyString()))
        .thenThrow(new MailOperationException());
    when(Boolean.valueOf(authSettingsMock.isInternal())).thenReturn(Boolean.TRUE);
  ...
  public void createInt_MailOperationException_NotInternal() throws Exception {
    ...
    when(idServiceMock.createUser(
            any(VOUserDetails.class), anyListOf(UserRoleType.class), anyString()))
        .thenThrow(new MailOperationException());
    when(Boolean.valueOf(authSettingsMock.isInternal())).thenReturn(Boolean.FALSE);
  ...
  @Test
  public void checkAddSubscription_addSubscription_NoAccess() throws ObjectNotFoundException {
    // given
    ...
    // then
    assertEquals(BaseBean.MARKETPLACE_ACCESS_DENY_PAGE, userBean.getRequestedRedirect());
  }

  @Test
  public void checkAddSubscription_addSubscription_Access() throws ObjectNotFoundException {
    // given
    final long SERVICE_KEY = 12000L;
    userBean.setRequestedRedirect(SUBSCRIPTION_ADD_PAGE);
    userBean.getSessionBean().setSelectedServiceKeyForCustomer(SERVICE_KEY);
  ...
  @Test
  public void testLoginForClosedMarketplace()
      throws LoginException, ValidationException, OperationNotPermittedException,
          ObjectNotFoundException, OrganizationRemovedException {
    // given
    ...
    // then
    verify(requestMock, times(1))
        .setAttribute(Constants.REQ_ATTR_ERROR_KEY, BaseBean.ERROR_LOGIN_TO_CLOSED_MARKETPLACE);
  }

  @Test
  public void testLoginForClosedMarketplace_SAML()
      throws LoginException, ValidationException, OperationNotPermittedException,
          ObjectNotFoundException, OrganizationRemovedException {
    // given
    ...
    doReturn(false)
        .when(marketplaceService)
        .doesOrganizationHaveAccessMarketplace(anyString(), anyString());
    when(requestMock.getParameter("samlSPForm:" + Constants.REQ_PARAM_USER_ID))
        .thenReturn("UserId");
    when(requestMock.getParameter("samlSPForm:" + Constants.REQ_ATTR_PASSWORD))
        .thenReturn("Password");
    VOUser mockUser = mock(VOUser.class);
    doReturn(mockUser).when(idServiceMock).getUser(any(VOUser.class));
    // when
    ...
    // then
    verify(requestMock, times(1))
        .setAttribute(Constants.REQ_ATTR_ERROR_KEY, BaseBean.ERROR_ACCESS_TO_CLOSED_MARKETPLACE);
  }

  private ConfigurationService setupConfigurationMockForRegistrationenablement(boolean enabled) {
    ConfigurationService csMock = mock(ConfigurationService.class);
    when(csMock.getVOConfigurationSetting(
            eq(ConfigurationKey.CUSTOMER_SELF_REGISTRATION_ENABLED), anyString()))
        .thenReturn(
            createSetting(
                ConfigurationKey.CUSTOMER_SELF_REGISTRATION_ENABLED, String.valueOf(enabled)));

    doReturn(csMock).when(userBean).getConfigurationService();
    return csMock;
  }

  private ConfigurationService setupConfigurationMockForAuthentication(AuthenticationMode mode) {
    ConfigurationService csMock = mock(ConfigurationService.class);
    doReturn(csMock).when(userBean).getConfigurationService();
  ...
  private ConfigurationService mockConfigurationService(String mode) {
    ...
    doReturn(
            new VOConfigurationSetting(
                ConfigurationKey.AUTH_MODE, Configuration.GLOBAL_CONTEXT, mode))
        .when(cfgMock)
        .getVOConfigurationSetting(ConfigurationKey.AUTH_MODE, Configuration.GLOBAL_CONTEXT);
    doReturn(
            new VOConfigurationSetting(
                ConfigurationKey.BASE_URL, Configuration.GLOBAL_CONTEXT, RECIPIENT))
        .when(cfgMock)
        .getVOConfigurationSetting(ConfigurationKey.BASE_URL, Configuration.GLOBAL_CONTEXT);
    doReturn(
            new VOConfigurationSetting(
                ConfigurationKey.SSO_IDP_TRUSTSTORE, Configuration.GLOBAL_CONTEXT, KEYSTORE_PATH))
        .when(cfgMock)
        .getVOConfigurationSetting(
            ConfigurationKey.SSO_IDP_TRUSTSTORE, Configuration.GLOBAL_CONTEXT);
    ...
    doReturn(
        ...
        .when(cfgMock)
        .getVOConfigurationSetting(
            ConfigurationKey.SSO_IDP_TRUSTSTORE_PASSWORD, Configuration.GLOBAL_CONTEXT);
