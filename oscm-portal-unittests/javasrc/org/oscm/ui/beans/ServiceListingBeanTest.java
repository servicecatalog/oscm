...
public class ServiceListingBeanTest {
  ...
  /**
   * Categories are cached. ResetCategoriesForMarketplace must be called, if the SearchService is
   * called with a concurrently deleted category.
   */
  @Test
  ...
  /**
   * Categories are cached. ResetCategoriesForMarketplace must NOT be called, if the SearchService
   * is called successfully without problem.
   */
  @Test
  ...
  public void searchWithPhrase_CategorySelectionRemoved() {
    ...
    givenCategorySelectionBean().setSelectedCategoryId("selectedCategoryId");
  ...
  /**
   * Tags are cached. ResetTagForMarketplace must be called, if the SearchService is called with a
   * concurrently removed tag.
   */
  @Test
  ...
  /**
   * Tags are cached. ResetTagForMarketplace must be NOT called, if the SearchService is called
   * successfully with an existing tag.
   */
  @Test
  ...
  private BDDMyOngoingStubbing<VOServiceListResult> givenSearchService()
    ...
    BDDMyOngoingStubbing<VOServiceListResult> mock =
        given(
            searchServiceInternal.getServicesByCriteria(
                anyString(), anyString(), any(ListCriteria.class), any(PerformanceHint.class)));
  ...
  private ObjectNotFoundException categoryNotFoundException() {
    return new ObjectNotFoundException(ClassEnum.CATEGORY, "unknownCategoryId");
  }
  ...
  public void updateServiceListContainsChargeableResellerService_noService() {
    serviceListingBean.updateServiceListContainsChargeableResellerService(
        new ArrayList<VOService>());

    assertFalse(serviceListingBean.isServiceListContainsChargeableResellerService());
  }
  ...
  public void updateServiceListContainsChargeableResellerService_onlyNonResellerServices() {
    ...
    serviceListingBean.updateServiceListContainsChargeableResellerService(svcList);
  ...
  public void updateServiceListContainsChargeableResellerService_chargeableResellerService() {
    ...
    assertTrue(serviceListingBean.isServiceListContainsChargeableResellerService());
  }
  ...
  public void isPublicLandingPage_PublicLandingpageType() throws Exception {
    // given
    given(landingpageService().loadLandingpageType(anyString())).willReturn(LandingpageType.PUBLIC);
  ...
  LandingpageConfigurationService landingpageService() {
    LandingpageConfigurationService landingpageService =
        mock(LandingpageConfigurationService.class);
    serviceListingBean.landingpageService = landingpageService;
    return landingpageService;
  }

  /** If the the marketplace is set to enterprise should recieve a negative result */
  @Test
  public void isPublicLandingPage_EnterpriseLandingpageType() throws Exception {
    // given
