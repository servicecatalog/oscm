...
public class BrandBeanTest {
  ...
  public void setUp() throws Exception {
    ...
    doReturn(marketplaceServiceMock).when(brandBean).getMarketplaceService();
    doReturn(marketplaceServiceMock).when(marketplaceBean).getMarketplaceService();
    brandBean.setMarketplaceBean(marketplaceBean);
  }
  ...
  public void testProcessValueChange() throws Exception {
    ...
    doReturn(VALID_BRANDING_URL).when(marketplaceServiceMock).getBrandingUrl("newMarketplaceId");
    VOMarketplace mp = new VOMarketplace();
    doReturn(mp).when(marketplaceServiceMock).getMarketplaceById(anyString());
  ...
  @Test
  public void testProcessValueChange_ObjectNotFoundException() throws Exception {
    ValueChangeEvent eventMock = mock(ValueChangeEvent.class);
    doReturn("newMarketplaceId").when(eventMock).getNewValue();
  ...
  @Test
  public void testDownloadBrandingPackage_NullBrandingPackageData() throws Exception {
  ...
  public void testDownloadBrandingPackage() throws Exception {
    ...
    doNothing()
        .when(brandBean)
        .writeContentToResponse(eq(brandingPackageData), anyString(), anyString());
    brandBean.downloadBrandingPackage();

    verify(brandBean, times(1))
        .writeContentToResponse(eq(brandingPackageData), anyString(), anyString());
