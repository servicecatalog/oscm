...
public class GotoMarketplaceBeanTest {
  ...
  public void setup() {
    ...
    doReturn(mpServiceMock).when(marketplaceGotoBean).getMarketplaceService();
    doReturn(servletRequestMock).when(marketplaceGotoBean).getRequest();

    httpSessionMock = mock(HttpSession.class);
    when(servletRequestMock.getSession(anyBoolean())).thenReturn(httpSessionMock);

    doReturn(idServiceMock).when(marketplaceGotoBean).getIdService();

    doReturn(Boolean.TRUE).when(marketplaceGotoBean).isLoggedInAndMarketplaceOwner();
    doReturn(Boolean.TRUE).when(marketplaceGotoBean).isLoggedInAndVendorManager();
  ...
  /** Test if the goto marketplace button is disabled when marketplace ID is not set. */
  @Test
  ...
  /**
   * The user has only the MARKETPLACE_OWNER role, therefore only owned marketplaces must be loaded.
   */
  @Test
  ...
  /**
   * The user has only the SERVICE_MANAGER role, therefore only marketplaces with publishing rights
   * must be loaded.
   */
  @Test
