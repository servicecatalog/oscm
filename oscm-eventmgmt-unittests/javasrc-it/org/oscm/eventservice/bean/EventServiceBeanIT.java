...
public class EventServiceBeanIT extends EJBTestBase {
  ...
  public void testRecordEventForSubscription() throws Exception {
    ...
    GatheredEvent gatheredEvent =
        readEvent(ACTOR, TIMESTAMP, SUBSCRIPTION_KEY, EventType.SERVICE_EVENT);
    testSavedEvent(TIMESTAMP, MULTIPLIER, UNIQUEID, gatheredEvent, EventType.SERVICE_EVENT);
  }
  ...
  @Test(expected = ValidationException.class)
  public void testRecordEventForSubscription_ToLongUniqueId() throws Exception {
    final VOGatheredEvent evt = new VOGatheredEvent();
    evt.setActor(ACTOR);
    evt.setOccurrenceTime(TIMESTAMP);
    evt.setEventId(PlatformEventIdentifier.USER_LOGIN_TO_SERVICE);
  ...
  public void testRecordEventForSubscriptionDuplicateUniqueId() throws Exception {
  ...
  public void testRecordEventForSubscriptionWrongOrganization() throws Exception {
    ...
    runTX(
        new Callable<Void>() {
          ...
          public Void call() throws Exception {
            ...
            PlatformUser user = Organizations.createUserForOrg(mgr, provider, true, "admin");
  ...
  public void testRecordEventForInstanceObjectNotFound() throws Exception {
    ...
    evMgmt.recordEventForInstance(technicalProductIdLocal, instanceIdLocal, eventLocal);
  }
  ...
  public void testRecordEventForInstanceNoResult() throws Exception {
    ...
    instanceId = "123";
    evMgmt.recordEventForInstance(technicalProductId, instanceId, new VOGatheredEvent());
  }
   ...
   *
   * @param actor User saving event.
   * @param occurrenceTime Occurance time.
   * @param subKey Key of subscription.
   * @param eventType The type of the event to read
   * @return Event.
   * @throws Exception
   */
  private GatheredEvent readEvent(
      final String actor, final long occurrenceTime, final long subKey, final EventType eventType)
      throws Exception {
    GatheredEvent eventLocal =
        runTX(
            new Callable<GatheredEvent>() {
              @Override
              public GatheredEvent call() throws Exception {
                ...
                Iterator<GatheredEvent> gatheredEventIterator =
                    ParameterizedTypes.iterator(query.getResultList(), GatheredEvent.class);
   ...
   *
   * @param occurrenceTime Occurance time.
   * @param multiplier Event multiplier.
   * @param pEvent Event for testing.
   * @param eventType expected event type
   */
  private void testSavedEvent(
      final long occurrenceTime,
      final long multiplier,
      final String uniqueId,
      GatheredEvent pEvent,
      EventType eventType) {
    Assert.assertNotNull("Object was stored, so its reference must be found", pEvent);
    Assert.assertEquals("Stored information is wrong!", "anyUser", pEvent.getActor());
    Assert.assertEquals(
        "Stored information is wrong!", "USER_LOGIN_TO_SERVICE", pEvent.getEventId());
    Assert.assertEquals("Stored information is wrong!", occurrenceTime, pEvent.getOccurrenceTime());
    Assert.assertEquals(
        "Stored information is wrong!", SUBSCRIPTION_KEY, pEvent.getSubscriptionTKey());
    Assert.assertEquals("Stored information is wrong!", eventType, pEvent.getType());
    Assert.assertEquals(
        "Stored information is wrong!",
        PlatformEventIdentifier.USER_LOGIN_TO_SERVICE,
        pEvent.getEventId());
    Assert.assertEquals("Stored information is wrong!", multiplier, pEvent.getMultiplier());
    Assert.assertEquals("UniqueId is wrong.", uniqueId, pEvent.getUniqueId());
  }
  ...
  private void createTestData() throws Exception {
    supplier =
        Organizations.createOrganization(
            mgr, OrganizationRoleType.TECHNOLOGY_PROVIDER, OrganizationRoleType.SUPPLIER);
    ...
    TechnicalProduct technicalProduct =
        TechnicalProducts.createTechnicalProduct(
            mgr, supplier, "testTechProd", false, ServiceAccessType.LOGIN);
    technicalProductId = technicalProduct.getTechnicalProductId();
    Product product =
        Products.createProduct(supplier, technicalProduct, true, "PRODUCT_1", null, mgr);
    productId = product.getProductId();

    Organization customer = Organizations.createCustomer(mgr, supplier);
    customerId = customer.getOrganizationId();
    Subscription subscription =
        Subscriptions.createSubscription(mgr, customerId, productId, "SUBSCRIPTION_1", supplier);
