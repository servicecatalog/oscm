...
public class VMPropertyHandler {
    ...
    public void configureByPortgroupIPPool(int i) throws APPlatformException {
        ...
        if (i == 1) {
            ...
            settings.getParameters().put(TS_NIC2_IP_ADDRESS,
                    new Setting(TS_NIC2_IP_ADDRESS, ipAddress));
        } else if (i == 3) {
            settings.getParameters().put(TS_NIC3_IP_ADDRESS,
                    new Setting(TS_NIC3_IP_ADDRESS, ipAddress));
        } else if (i == 4) {
            settings.getParameters().put(TS_NIC4_IP_ADDRESS,
                    new Setting(TS_NIC4_IP_ADDRESS, ipAddress));
        }
    }
    ...
    public List<DistributedVirtualSwitch> getDistributedVirtualSwitch(
        ...
        DataAccessService das = getDataAccessService();
        try {
            distribtuedvirtualswitches = das
                    .getDistributedVirtualSwitch(cluster);
        } catch (Exception e) {
            logger.error("Failed to retrieve DistributedVirtualSwitch list.",
                    e);
