...
public class Template {
  ...
  public TaskInfo cloneVM(VMPropertyHandler paramHandler) throws Exception {
    ...
    VirtualMachineRelocateSpec relocSpec =
        setHostAndStorage(vmw, paramHandler, vmDataCenter, datacenter, cluster);
    cloneSpec.setLocation(relocSpec);
    cloneSpec.setPowerOn(false);
    cloneSpec.setTemplate(false);

    CustomizationSpec custSpec = getCustomizationSpec(configSpec, paramHandler);
  ...
  private CustomizationSpec getCustomizationSpec(
    ...
    if (isWindows) {
      ...
      if (paramHandler.isServiceSettingTrue(VMPropertyHandler.TS_WINDOWS_DOMAIN_JOIN)) {
        ...
        String domainName =
            paramHandler.getServiceSettingValidated(VMPropertyHandler.TS_DOMAIN_NAME);
        String domainAdmin =
      ...
      String adminPwd =
          paramHandler.getServiceSetting(VMPropertyHandler.TS_WINDOWS_LOCAL_ADMIN_PWD);
  ...
  private void setFixIp(int i, CustomizationIPSettings ipSettings, VMPropertyHandler paramHandler) {
      ...
      for (ManagedObjectReference storageRef : storageRefs) {
        ...
        for (DatastoreHostMount hm : hostMounts) {
          ...
          if (host.equals(hostThatLinksToThisStoarge)
            ...
            logger.debug("addStorage host: " + host + " storage: " + storageName);
