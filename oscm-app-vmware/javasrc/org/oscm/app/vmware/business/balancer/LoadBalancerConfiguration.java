...
public class LoadBalancerConfiguration {
	...
	protected synchronized void initialize(String xmlData, VMwareDatacenterInventory inventory) throws Exception {
		...
		for (int i = 0; i < hosts.getLength(); i++) {
			String name = hosts.item(i).getAttributes().getNamedItem("name").getTextContent();
			VMwareHost vmHost = inventory.getHost(name);

			Node hostNode = hosts.item(i);
			Node memory_limit = hostNode.getAttributes().getNamedItem("memory_limit");
			Node cpu_limit = hostNode.getAttributes().getNamedItem("cpu_limit");
			Node vm_limit = hosts.item(i).getAttributes().getNamedItem("vm_limit");
			String enabled = hostNode.getAttributes().getNamedItem("enabled").getTextContent();
			if (vmHost == null) {
				logger.warn("The configured host " + name + " is not available in the inventory.");
			} else {
				vmHost.setEnabled(Boolean.getBoolean(enabled));
				...
				List<Node> balencers = XMLHelper.getChildrenByTag(hostNode, "balancer");

				if (balencers.isEmpty()) {
					logger.warn("The configured host " + name + " has a wrong balancer configuration.");
				} else {

					VMwareBalancer<VMwareStorage> stb = parseBalancer(balencers.get(0), StorageBalancer.class,
		...
		for (int i = 0; i < storages.getLength(); i++) {
			String name = storages.item(i).getAttributes().getNamedItem("name").getTextContent();
			String enabled = storages.item(i).getAttributes().getNamedItem("enabled").getTextContent();
			...
			if (vmStorage == null) {
				...
				vmStorage.setEnabled(Boolean.getBoolean(enabled));
		...
		balancer = parseBalancer(storage, HostBalancer.class, EquipartitionHostBalancer.class, inventory);
	}

	@SuppressWarnings("unchecked")
	private <E extends VMwareBalancer<?>> E parseBalancer(Node balNode, Class<E> class1, Class<? extends E> class2,
			VMwareDatacenterInventory inventory) throws Exception {

		String balancerClass = balNode.getAttributes().getNamedItem("class").getTextContent();
		...
		if (balancerClass != null) {
			...
			if (!class1.isAssignableFrom(loadedClass)) {
				loadedClass = null;
				logger.warn("The configured balancer '" + balancerClass + "' is not of type " + class1.getSimpleName());
			}
		}
		if (loadedClass == null) {
			balancerClass = class2.getName();
			balNode.getAttributes().getNamedItem("class").setTextContent(balancerClass);
			loadedClass = class2;
		}
