...
public class PortgroupIpSettings {
    ...
    private static final Logger logger = LoggerFactory
            .getLogger(PortgroupIpSettings.class);
    ...
    PortgroupIpSettings(VMPropertyHandler ph, int nic) {
        ...
        dvs = getDistributedVirtualSwitch(ph.getSwitchUUID(nic),
                ph.getDistributedVirtualSwitch(cluster));
    ...
    private DistributedVirtualSwitch getDistributedVirtualSwitch(String dvsName,
            List<DistributedVirtualSwitch> distributedVirtualSwitches) {
        DistributedVirtualSwitch DistributedVirtualSwitch = null;
        for (int i = 0; distributedVirtualSwitches.size() > i; i++) {
            logger.debug("Available DistributedVIrtualSwitch: "
                    + distributedVirtualSwitches.get(i).getName() + " UUID "
    ...
    private Portgroup getPortgroup(String portgroupName,
            List<Portgroup> portgroups) {
        Portgroup portgroup = null;
        for (int i = 0; portgroups.size() > i; i++) {
            logger.debug("Available portgroups: " + portgroups.get(i).getName()
                    + " UUID " + portgroups.get(i).getUuid());
    ...
    public String getIpAdressFromIpPool() throws APPlatformException {
        ...
        for (int i = 0; portgroupIPPool.size() > i; i++) {
            if (!portgroupIPPool.get(i).isIn_use()) {
                try {
                    das.reservePortgroupIPAddress(
                            (portgroupIPPool.get(i).getTkey()));
                    portgroupIPPool.get(i).setIn_use(true);
                    logger.debug("Reserve IP: "
                            + portgroupIPPool.get(i).getIp_adress());
        ...
        for (int i = 0; portgroupIPPool.size() > i; i++) {
            if (portgroupIPPool.get(i).isIn_use()) {
                if (ip.equals(portgroupIPPool.get(i).getIp_adress())) {
                    try {
                        das.unReservePortgroupIPAddress(
                                portgroupIPPool.get(i).getTkey());
                    } catch (Exception e) {
