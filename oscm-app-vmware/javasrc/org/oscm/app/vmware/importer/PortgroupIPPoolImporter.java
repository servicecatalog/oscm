...
public class PortgroupIPPoolImporter implements Importer {
    ...
    private void save(PortgroupIPPool ipPool) throws Exception {
        String query = "INSERT INTO portgroup_ippool (tkey, ip_address, in_use, portgroup_fk) VALUES (DEFAULT,?,?,?)";
        int portgroupKey = this.getPortgroupTKey(ipPool.vCenter,
                ipPool.datacenter, ipPool.cluster, ipPool.portgroup,
                ipPool.distributedVirtualSwitch);
        try (PreparedStatement stmt = this.das.getDatasource().getConnection()
    ...
    private int getPortgroupTKey(String vCenter, String datacenter,
            String cluster, String portgroup, String dvs) throws Exception {
        String query = ""
    ...
    public void load(InputStream csvFile) throws Exception {
        try (PortgroupIPPoolParser parser = new PortgroupIPPoolParser(
                csvFile)) {
