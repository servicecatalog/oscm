...
public class UserServiceBean implements UserService {
  ...
  private static final Log4jLogger logger = LoggerFactory.getLogger(UserServiceBean.class);
  ...
  @Override
  @RolesAllowed("ORGANIZATION_ADMIN")
  public Response saveUser(POUserDetails user) throws SaaSApplicationException {

    try {
      // read user by key as id may have been changed
      PlatformUser existing = ds.getReference(PlatformUser.class, user.getKey());

      // keep a reference (not managed) with the old email
      PlatformUser old =
          existing.getEmail() != null ? UserDataAssembler.copyPlatformUser(existing) : null;
      ...
      if (user.getAssignedRoles().isEmpty()) {
        PlatformUser administrator = ds.getReference(PlatformUser.class, 1000);
  ...
  @Override
  @RolesAllowed("ORGANIZATION_ADMIN")
  public Response importUsersInOwnOrganization(byte[] users, String marketplaceId)
      throws SaaSApplicationException {
  ...
  public Response deleteUser(POUser user, String marketplaceId, String tenantId)
    ...
    try {
      ...
      PlatformUser u = isl.getPlatformUser(user.getUserId(), tenantId, true);
      VersionAndKeyValidator.verify(u, user.getKey(), user.getVersion());
  ...
  public POUserAndSubscriptions getNewUserData() {
    ...
    List<SubscriptionWithRoles> list =
        slsl.getSubcsriptionsWithRoles(
            user.getOrganization(), Subscription.ASSIGNABLE_SUBSCRIPTION_STATUS);
    Set<UserRoleType> availableRoles = isl.getAvailableUserRolesForUser(user);
    List<Long> roleKeys = toRoleKeyList(list);
    LocalizerFacade lf = new LocalizerFacade(lsl, user.getLocale());
    lf.prefetch(roleKeys, Collections.singletonList(LocalizedObjectTypes.ROLE_DEF_NAME));
    POUserAndSubscriptions result = dc.toPOUserAndSubscriptionsNew(list, availableRoles, lf);
  ...
  @Override
  @RolesAllowed("ORGANIZATION_ADMIN")
  public Response createNewUser(POUserAndSubscriptions user, String marketplaceId)
      throws SaaSApplicationException {

    VOUserDetails ud = convertToUser(user);
    ud.setOrganizationId(ds.getCurrentUser().getOrganization().getOrganizationId());
    ...
    try {
      ...
      if (ud == null) {
        ...
        PlatformUser pu = ds.getReference(PlatformUser.class, ud.getKey());
        isl.sendMailToCreatedUser(
            SendMailControl.getPassword(), true, SendMailControl.getMarketplace(), pu);
      }
    } catch (SaaSApplicationException e) {
      sc.setRollbackOnly();
      throw e;
  ...
  public POUserAndSubscriptions getUserAndSubscriptionDetails(String userId, String tenantId)
    ...
    return dc.toPOUserAndSubscriptionsExisting(
  ...
  public Response saveUserAndSubscriptionAssignment(
      POUserAndSubscriptions user, List<POUserGroup> allGroups) throws SaaSApplicationException {

    Response response = new Response();
    try {
      Map<UserGroup, UnitUserRole> groupsToBeAssigned = new HashMap<>();
      if (allGroups != null && !allGroups.isEmpty()) {
        groupsToBeAssigned = validateGroupsExist(allGroups, user.getGroupsToBeAssigned());
      }
      // read user by key as id may have been changed
      PlatformUser existing = ds.getReference(PlatformUser.class, user.getKey());
      // keep a reference (not managed) with the old email
      ...
      List<UsageLicense> assignments =
          slsl.getSubscriptionAssignments(existing, Subscription.ASSIGNABLE_SUBSCRIPTION_STATUS);
      ...
      // notify subscriptions
      ds.flush();
      PlatformUser userInDB = ds.getReference(PlatformUser.class, user.getKey());
  ...
  Map<String, VOUsageLicense> getAssignments(
      List<UsageLicense> assignments, List<POSubscription> subscriptions, VOUser u) {
  ...
  boolean sameRole(UsageLicense lic, POSubscription sub) {
    return lic.getRoleDefinition() == null && sub.getUsageLicense().getPoServieRole() == null
        || lic.getRoleDefinition()
            .getRoleId()
            .equals(sub.getUsageLicense().getPoServieRole().getId());
  }

  Set<String> getUnassignments(List<UsageLicense> assignments, List<POSubscription> subscriptions) {
  ...
  void updateUserAndRoles(POUserDetails user, PlatformUser existing)
    ...
    VersionAndKeyValidator.verify(existing, user.getKey(), user.getVersion());

    // validation
    PlatformUser updated =
        dc.updatePlatformUser(user, UserDataAssembler.copyPlatformUser(existing));
  ...
  private void updateUserGroups(
      ...
      POUserAndSubscriptions user,
      PlatformUser existing)
      throws NonUniqueBusinessKeyException, OperationNotPermittedException, ObjectNotFoundException,
    ...
    for (Entry<UserGroup, UnitUserRole> groupsWithRoles : userGroupsToBeAssigned.entrySet()) {
      groupsToBeAssignedMap.put(Long.valueOf(groupsWithRoles.getKey().getKey()), groupsWithRoles);
    }
    List<UserGroup> groupsAssigned =
        userGroupService.getUserGroupsForUserWithoutDefault(existing.getKey());
    ...
    for (Entry<Long, Entry<UserGroup, UnitUserRole>> entry : groupsToBeAssignedMap.entrySet()) {
  ...
  private void handleUnitRoleAssignments(
      POUserAndSubscriptions user,
      PlatformUser existing,
      Map<UserGroup, UnitUserRole> userGroupsToBeAssigned)
    ...
    allAvailableUnitRoleTypes = new ArrayList<>(Arrays.asList(UnitRoleType.values()));
    for (Entry<UserGroup, UnitUserRole> groupWithRoles : userGroupsToBeAssigned.entrySet()) {
      userGroupService.revokeUserRoles(
          existing, allAvailableUnitRoleTypes, groupWithRoles.getKey());
      userGroupService.grantUserRoles(
          existing,
          Collections.singletonList(groupWithRoles.getValue().getRoleName()),
          groupWithRoles.getKey());
    }

    Map<UserGroup, UnitRoleType> allUserAssignments =
        userGroupService.getUserGroupsForUserWithRoles(user.getUserId());
  ...
  private Map<UserGroup, UnitUserRole> validateGroupsExist(
      List<POUserGroup> allGroups, List<POUserGroup> groups) throws ObjectNotFoundException {
    Map<UserGroup, UnitUserRole> groupsToBeAssigned = new HashMap<>();
    ...
    for (POUserGroup group : allGroups) {
      ...
      try {
        ...
        groupsToBeAssignedMap.put(Long.valueOf(group.getKey()), userGroup);
      } catch (ObjectNotFoundException ex) {
        String groupName = group.getGroupName();
        ex.setMessageParams(new String[] {groupName});
        logger.logWarn(
            Log4jLogger.SYSTEM_LOG, ex, LogMessageIdentifier.WARN_GROUP_NOT_EXIST, groupName);
        throw ex;
      }
    }
    if (groups != null && !groups.isEmpty()) {
      for (POUserGroup group : groups) {
        ...
        groupsToBeAssigned.put(
            groupsToBeAssignedMap.remove(Long.valueOf(group.getKey())), unitUserRole);
      }
    }
    return groupsToBeAssigned;
  }

  boolean updateSubscriptionAssignment(POUserAndSubscriptions user, List<UsageLicense> assignments)
      throws SaaSApplicationException {

    Set<String> subsToRemove = getUnassignments(assignments, user.getSubscriptions());
    VOUser u = convertToUser(user);
    Map<String, VOUsageLicense> subsToAdd = getAssignments(assignments, user.getSubscriptions(), u);

    boolean done = true;

    for (String subId : subsToRemove) {
      done = done && ssl.addRevokeUser(subId, null, Collections.singletonList(u));
    }
    for (String subId : subsToAdd.keySet()) {
      VOUsageLicense lic = subsToAdd.get(subId);
      done = done && ssl.addRevokeUser(subId, Collections.singletonList(lic), null);
    }
  ...
  @Override
  public List<POSubscription> getUserAssignableSubscriptions(Pagination pagination, String userId)
      throws SaaSApplicationException {

    // TODO MULTITENANT
    PlatformUser user = isl.getPlatformUser(userId, ds.getCurrentUser().getTenantId(), true);

    List<POSubscription> subscriptions =
        slsl.getUserAssignableSubscriptions(
            pagination, user, Subscription.ASSIGNABLE_SUBSCRIPTION_STATUS);
  ...
  public Long getUserAssignableSubscriptionsNumber(
      PaginationInt pagination, String userId, String tenantId) throws SaaSApplicationException {

    PlatformUser user;
    user = isl.getPlatformUser(userId, tenantId, true);

    Long number =
        slsl.getUserAssignableSubscriptionsNumber(
            pagination, user, Subscription.ASSIGNABLE_SUBSCRIPTION_STATUS);
  ...
  @Override
  public String getDisplaySettings(long userTKey) {
    DisplaySettings doDS = new DisplaySettings();
    doDS.setUserTKey(userTKey);
    doDS = (DisplaySettings) ds.find(doDS);
    if (doDS == null) {
      return "";
    }
    return doDS.getData();
  ...
  private static void validateLength(String data, int maxLen) throws ValidationException {
    ValidationException vf = null;
    if (data == null || data.trim().length() == 0) {

      vf = new ValidationException(ReasonEnum.EMPTY_VALUE, "settings", new Object[] {""});

    } else if (data.length() > maxLen) {
      vf =
          new ValidationException(
              ReasonEnum.LENGTH, "settings", new Object[] {data, Integer.valueOf(maxLen)});
    }
    if (vf != null) {
      logger.logWarn(Log4jLogger.SYSTEM_LOG, vf, LogMessageIdentifier.WARN_VALIDATION_FAILED);
      throw vf;
    }
  }
}
