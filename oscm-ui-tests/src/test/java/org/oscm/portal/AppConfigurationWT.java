...
public class AppConfigurationWT {
  ...
  public void test01setConfiguration() throws Exception {
    ...
    tester.setBssUserId(tester.getPropertie(AppConfigurationTester.APP_ADMIN_USER_ID));
    tester.setBssUserPwd(tester.getPropertie(AppConfigurationTester.APP_ADMIN_USER_PWD));
  ...
  public void test02createNewControllerId() throws Exception {
    ...
    try {
      ...
      if (e.getMessage() != null
          && e.getMessage().contentEquals(AppConfigurationTester.ERROR_MSG_CONTROLLER_EXISTS))
    ...
    dbTester.insertSetting(
        "BSS_USER_ID", PlaygroundSuiteTest.supplierOrgAdminId, PlaygroundSuiteTest.controllerId);
    dbTester.insertSetting(
        "BSS_USER_KEY",
        PlaygroundSuiteTest.supplierOrgAdminUserkey,
        PlaygroundSuiteTest.controllerId);
    dbTester.insertSetting(
        "BSS_USER_PWD", PlaygroundSuiteTest.supplierOrgAdminPwd, PlaygroundSuiteTest.controllerId);
    dbTester.insertSetting(
        "BSS_ORGANIZATION_ID", "PLATFORM_OPERATOR", PlaygroundSuiteTest.controllerId);
    dbTester.insertSetting("VERSION", "1.0", PlaygroundSuiteTest.controllerId);
    dbTester.insertSetting(
        "APP_PROVISIONING_ON_INSTANCE", "false", PlaygroundSuiteTest.controllerId);
