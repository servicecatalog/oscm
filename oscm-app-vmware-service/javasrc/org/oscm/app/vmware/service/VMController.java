...
public class VMController implements APPlatformController {
  ...
  public InstanceDescription createInstance(ProvisioningSettings settings)
    ...
    try {
      ...
      logger.info(
          "createInstance({})", LogAndExceptionConverter.getLogText(id.getInstanceId(), settings));
  ...
  public InstanceStatus modifyInstance(
    ...
    try {
      ...
      validateParameters(
          currentParameters, newParameters, currentSettings.getOrganizationId(), instanceId);
  ...
  public InstanceStatus getInstanceStatus(String instanceId, ProvisioningSettings settings)
    ...
    try {
      ...
      if (errorMessage.contains(
          Messages.get(ph.getLocale(), "error_finalize_provisioning", new Object[] {instanceId})
              .toString())) {
        throw new AbortException(
            Messages.getAll("error_finalize_provisioning", ph.getInstanceName()),
            Messages.getAll("error_finalize_provisioning", ph.getInstanceName()));
      } else {
  ...
  private void updateProvisioningSettings(
      VMPropertyHandler ph, StateMachine stateMachine, String instanceId) throws Exception {

    String nextState = stateMachine.getStateId();
    switch (nextState) {
      case "REPEAT_FAILED_STATE":
        String failedState =
            stateMachine.loadPreviousStateFromHistory(ph.getProvisioningSettings());
        ...
        platformService.storeServiceInstanceDetails(
            Controller.ID,
            instanceId,
      ...
      case "ERROR":
        ...
        if (errorMessage.contains(
            Messages.get(ph.getLocale(), "error_import_vm", new Object[] {instanceId})
                .toString())) {
          throw new AbortException(
              Messages.getAll("error_import_vm_not_found", ph.getInstanceName()),
              Messages.getAll("error_import_vm_not_found", ph.getInstanceName()));
        } else {
          throw new SuspendException(errorMessage);
        }
  ...
  /**
   * Validates the given parameters before contacting VMware API. When both oldParams and newParams
   * are set, also modification rules (e.g. no disk reduce) are checked.
   *
  ...
  private void validateParameters(
      ...
      VMPropertyHandler newParameters,
  ...
  private void validateMemory(VMPropertyHandler newParameters) throws APPlatformException {
    ...
    if (memory % 4 != 0) {
      logger.warn("Validation error on memory size [" + memory + "MB]");
  ...
  private void validateWindowsSettings(VMPropertyHandler params) throws APPlatformException {
    ...
    boolean adminPwd =
        params.getServiceSetting(VMPropertyHandler.TS_WINDOWS_DOMAIN_ADMIN_PWD) != null;
  ...
  public InstanceStatus deactivateInstance(String instanceId, ProvisioningSettings settings)
      throws APPlatformException {
    logger.info(
        "deactivateInstance({})", LogAndExceptionConverter.getLogText(instanceId, settings));
    try {
  ...
  public boolean canPing() throws ConfigurationException {
    ...
    if (vCenter != null) areParamsValid = validateControllerParams(vCenter);

    if (areParamsValid) cacheCredentials(vCenter);
    return areParamsValid;
  }

  /**
   * Fetches controller setting from platformService. Throws an error when config is unable to
   * obtain or is incomplete
   *
  ...
  private HashMap<String, Setting> getControllerSettings() throws ConfigurationException {
    ...
    try {
      ...
      controllerSettings = platformService.getControllerSettings(Controller.ID, tpUser);
    ...
    if (controllerSettings != null && !controllerSettings.isEmpty()) return controllerSettings;
    else {
      exception =
          new ConfigurationException(
              getLocalizedErrorMessage("ui.config.error.unable.to.get.settings"));
   ...
   * @throws ConfigurationException
   */
  private VCenter getVCenter(HashMap<String, Setting> controllerSettings)
      throws ConfigurationException {
    ...
    if (vCenters != null && !vCenters.isEmpty()) return vCenters.get(0);
    else {
      exception =
          new ConfigurationException(
  ...
  protected List<VCenter> getVCenterList(HashMap<String, Setting> controllerSettings) {
    ProvisioningSettings settings =
        new ProvisioningSettings(new HashMap<>(), controllerSettings, Messages.DEFAULT_LOCALE);
   ...
   * @throws ConfigurationException
   */
  private boolean checkIfParamsAreNull(VCenter vCenter) throws ConfigurationException {
    if (vCenter.getUrl() == null || vCenter.getUserid() == null || vCenter.getPassword() == null) {
  ...
  private void cacheCredentials(VCenter vCenter) {
    cachedCredentials =
        new VMwareCredentials(vCenter.getUrl(), vCenter.getUserid(), vCenter.getPassword());
  }
  ...
  public List<OperationParameter> getOperationParameters(
      String arg0, String arg1, String arg2, ProvisioningSettings arg3) throws APPlatformException {
    return null;
  }
