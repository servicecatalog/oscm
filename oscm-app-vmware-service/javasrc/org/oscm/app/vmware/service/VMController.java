...
public class VMController implements APPlatformController {
        ...
        try {
            ...
            if (platformService.exists(Controller.ID, id.getInstanceId())) {
                ...
                throw new APPlatformException(Messages
                        .getAll("error_instance_exists", id.getInstanceId()));
            }
    ...
    public InstanceStatus getInstanceStatus(String instanceId,
        ...
        try {
            ...
            if (errorMessage
                    .contains(Messages.get(ph.getLocale(), "error_finalize_provisioning",
                            new Object[] { instanceId }).toString())) {
                throw new AbortException(
                        Messages.getAll("error_finalize_provisioning",
                                ph.getInstanceName()),
                        Messages.getAll("error_finalize_provisioning",
                                ph.getInstanceName()));
            } else {
    ...
    private void validateMemory(VMPropertyHandler newParameters)
        ...
        if (memory % 4 != 0) {
            ...
            throw new APPlatformException(Messages
                    .getAll("error_invalid_memory", Long.valueOf(memory)));
        }
    }
    ...
    public InstanceStatus executeServiceOperation(String userId,
        ...
        try {
            ...
            logger.warn("Failed to execute service operation " + operationId
        ...
        try {
            ...
            exception = new ServiceNotReachableException(
                    getLocalizedErrorMessage(
                            "ui.config.error.unable.to.connect.to.vmware"));
    ...
    /**
     * Fetches controller setting from platformService. Throws an error when
     * config is unable to obtain or is incomplete
     *
    ...
    private HashMap<String, Setting> getControllerSettings()
        ...
        try {
            ...
            exception = new ConfigurationException(getLocalizedErrorMessage(
                    "ui.config.error.unable.to.get.settings"));
            exception.setStackTrace(e.getStackTrace());
            throw exception;
        }
    ...
    private PasswordAuthentication getPasswordAuthentication() {
        ...
        return new PasswordAuthentication(userId.toString(),
                password.toString());
    }

    protected HttpSession getSession(FacesContext facesContext) {
        return (HttpSession) facesContext.getExternalContext()
                .getSession(false);
    }
    ...
    private VCenter getVCenter(HashMap<String, Setting> controllerSettings)
        ...
        if (vCenters != null && !vCenters.isEmpty())
            ...
            exception = new ConfigurationException(getLocalizedErrorMessage(
                    "ui.config.error.unable.to.get.settings.no.vcenters"));
            throw exception;
        }
    }
    ...
    protected List<VCenter> getVCenterList(
        ...
        ProvisioningSettings settings = new ProvisioningSettings(
                new HashMap<>(), controllerSettings, Messages.DEFAULT_LOCALE);
    ...
    private boolean checkIfParamsAreNull(VCenter vCenter)
            throws ConfigurationException {
        if (vCenter.getUrl() == null || vCenter.getUserid() == null
                || vCenter.getPassword() == null) {
            throw new ConfigurationException(getLocalizedErrorMessage(
                    "ui.config.error.controller.param.empty"));
        } else
            return true;
    }
    ...
    private boolean checkIfParamsAreEmpty(VCenter vCenter)
            throws ConfigurationException {
        if (vCenter.getUrl().isEmpty() || vCenter.getUserid().isEmpty()
    ...
    private void cacheCredentials(VCenter vCenter) {
        cachedCredentials = new VMwareCredentials(vCenter.getUrl(),
                vCenter.getUserid(), vCenter.getPassword());
    }
