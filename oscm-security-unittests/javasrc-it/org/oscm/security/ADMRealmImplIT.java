 ...
 * <p>Author: jaeger
 *
 * <p>Creation Date: 08.11.2011
 *
 * <p>Completion Time: 08.11.2011
 *
 * <p>*****************************************************************************
 */
...
public class ADMRealmImplIT extends EJBTestBase {
        ...
        runTX(
            new Callable<PlatformUser>() {
              ...
              public PlatformUser call() throws Exception {
                ...
                dm.flush();
                PlatformUser user = Organizations.createUserForOrg(dm, org, true, "some_username");
                user.setPasswordHash(PasswordHash.calculateHash(user.getPasswordSalt(), PWD));
                return user;
              }
            });

    userQuery = spy(realm.getUserQuery(String.valueOf(user.getKey())));
    doReturn(userQuery).when(realm).getUserQuery(String.valueOf(user.getKey()));

    organizationSettingQuery = spy(realm.getOrganizationSettingQuery(userQuery));
    doReturn(organizationSettingQuery)
        .when(realm)
        .getOrganizationSettingQuery(any(UserQuery.class));
    doReturn(new Properties()).when(organizationSettingQuery).getProperties();
    doNothing().when(organizationSettingQuery).execute();
    doReturn("")
        .when(realm)
        .findAndBind(any(Properties.class), anyString(), anyString(), anyString());
  }
  ...
  public void authenticate_WrongUser() throws Exception {
    try {
      ...
      assertEquals("Login failed for invalid user key 'wrong'", e.getMessage());
      throw e;
    }
  ...
  public void authenticate_userLocked() throws Exception {
    ...
    try {
      ...
      assertEquals(
          "Login for user '" + user.getKey() + "' failed as the user account is locked.",
          e.getMessage());
      validateUserSettings(0, UserAccountStatus.ACTIVE, 1);
    }
  }
  ...
  public void authenticate_lockingScenario() throws Exception {
    ...
    validateUserSettings(3, UserAccountStatus.LOCKED_FAILED_LOGIN_ATTEMPTS, 1);
  }
  ...
  @Test
  public void authenticate_lockingScenarioNonDefaultMaxAttempts() throws Exception {
    ...
    validateUserSettings(10, UserAccountStatus.LOCKED_FAILED_LOGIN_ATTEMPTS, 1);
  }
  ...
  public void getGroupNames_GoodCase() throws Exception {
    List<String> list = Collections.list(realm.getGroupNames(String.valueOf(user.getKey())));
  ...
  public void getGroupNames_WrongUserKey() throws Exception {
    try {
      ...
      assertEquals("PlatformUser with user key 'wrong' not found.", e.getMessage());
      throw e;
    }
  }
  ...
  public void handleOperatorClientCaller_positive() throws Exception {
    ...
    doNothing()
        .when(realm)
        .handleLoginAttempt(matches("1000"), matches("admin123"), any(UserQuery.class));
  ...
  private void validateUserSettings(
      final int counter, final UserAccountStatus status, final int version) throws Exception {
    runTX(
        new Callable<Void>() {
          @Override
          public Void call() throws Exception {
            PlatformUser storedUser = dm.getReference(PlatformUser.class, user.getKey());
