...
/**
 * Session Bean implementation to manage the properties, translations and images of a supplier.
 *
 * @author pock
 */
@Stateless
...
public class BrandServiceBean implements BrandService {
  ...
  private static final Log4jLogger logger = LoggerFactory.getLogger(BrandServiceBean.class);
   ...
   *
   * @param marketplaceId the identifier of the organization for which the marketplace object is
   *     requested.
   ...
   *
   * @param props the properties to be written.
  ...
  private String writeProperties(Properties props) {
    ...
    try {
      ...
      SaaSSystemException se = new SaaSSystemException("Storing properties failed!", e);
      logger.logError(
          Log4jLogger.SYSTEM_LOG, se, LogMessageIdentifier.ERROR_STORE_PROPERTIES_FAILED);
      throw se;
    }
  }
  ...
  private Marketplace getMarketplaceToWorkOn(Organization organization, PlatformUser user) {
  ...
  public void deleteAllMessageProperties(String marketplaceId)
    ...
    localizer.removeLocalizedValues(
        marketplace.getKey(), LocalizedObjectTypes.SHOP_MESSAGE_PROPERTIES);
  }
  ...
  public Properties loadMessageProperties(String marketplaceId, String localeString) {
    ...
    Properties mailProps =
        localizer.loadLocalizedPropertiesFromFile(
            LocalizedObjectTypes.MAIL_CONTENT.getSourceLocation(), localeString);
    if (mailProps != null) {
  ...
  public Properties loadMessagePropertiesFromDB(String marketplaceId, String localeString) {
    ...
    if (shop != null) {
      props.putAll(
          localizer.loadLocalizedPropertiesFromDatabase(
              0L, LocalizedObjectTypes.MESSAGE_PROPERTIES, localeString));
      props.putAll(
          localizer.loadLocalizedPropertiesFromDatabase(
              0L, LocalizedObjectTypes.MAIL_PROPERTIES, localeString));
      props.putAll(
          localizer.loadLocalizedPropertiesFromDatabase(
              shop.getKey(), LocalizedObjectTypes.SHOP_MESSAGE_PROPERTIES, localeString));
    }
  ...
  public void saveImages(List<VOImageResource> voImageResources, String marketplaceId)
    ...
    for (VOImageResource vo : voImageResources) {
      if (vo.getImageType() != null && vo.getImageType().getOwnerType() == ImageOwnerType.SHOP) {
        ImageValidator.validateImageType(vo.getBuffer(), vo.getContentType());
  ...
  public String getMarketplaceStage(String marketplaceId, String localeString) {
    ...
    String result =
        localizer.getLocalizedTextFromDatabase(
            localeString, mp.getKey(), LocalizedObjectTypes.MARKETPLACE_STAGE);
  ...
  public void setMarketplaceStage(String stageContent, String marketplaceId, String localeString)
    ...
    if (stageContent.trim().length() == 0) {
      localizer.removeLocalizedValue(
          mp.getKey(), LocalizedObjectTypes.MARKETPLACE_STAGE, localeString);
    } else {
      localizer.storeLocalizedResource(
          localeString, mp.getKey(), LocalizedObjectTypes.MARKETPLACE_STAGE, stageContent);
    }
  }
