...
/**
 * The timer management service bean that initializes the timers and controls the invocation of the
 * business logic as soon as a timer is expired.
 *
...
public class TimerServiceBean {
  ...
  /**
   * The time that has to pass before another timer of an already handled type can be handled again.
   */
  private static final int TIMER_HANDLING_DISTANCE = 10000;

  /**
   * The factor that determines the time corridor to realize timer processing entries as related to
   * the current timer expiration.
   */
  private static final double DEVIATION_FACTOR = 0.95;

  private static Log4jLogger logger = LoggerFactory.getLogger(TimerServiceBean.class);
  ...
  @Resource protected SessionContext ctx;
  ...
  /**
   * Reads the configuration settings for the timer intervals to be used and creates the timers
   * accordingly.
   */
  ...
  private void initAllTimers() throws ValidationException {
    ...
    for (TimerType timerType : supportedTimers) {
      ...
      if (timerInterval > 0) {
        createTimerWithInterval(timerService, timerType, timerInterval, timerOffset);
      } else if (period != null) {
        // if there is only a period, start a calendar timer.
        createTimerWithPeriod(timerService, timerType, timerOffset, period);
      } else {
        logger.logInfo(
    ...
    logger.logInfo(Log4jLogger.SYSTEM_LOG, LogMessageIdentifier.INFO_TIMER_INITIALIZED);
  }

  private void createTimerWithInterval(
      TimerService timerService, TimerType timerType, long timerInterval, long timerOffset)
    ...
    Date nextStart = getDateForNextTimerExpiration(timerInterval, timerOffset);
    ValidationException validationException;
    String[] params;
    if (nextStart.getTime() < 0) {
      if (timerType.name().equals(TimerType.USER_NUM_CHECK.name())) {
        params = new String[] {timerType.name(), timerType.getKeyForIntervalTime().name()};
        validationException =
        ...
        validationException =
            new ValidationException(
                ValidationException.ReasonEnum.TIMER_EXPIRATIONDATE_INVALID, null, params);
        logger.logError(
            Log4jLogger.SYSTEM_LOG,
            validationException,
    ...
    logger.logInfo(
        ...
        timerType.toString(),
        sdf.format(nextStart),
        Long.toString(timerInterval));
  }

  void createTimerWithPeriod(
      TimerService timerService, TimerType timerType, long timerOffset, Period period) {
    ...
    ScheduleExpression schedleExpression = getExpressionForPeriod(period, startDate);
    Timer timer = timerService.createCalendarTimer(schedleExpression, config);
  ...
  boolean isTimerCreated(TimerType timerType, TimerService timerService) {
    for (Timer timer : ParameterizedTypes.iterable(timerService.getTimers(), Timer.class)) {
      TimerType tType = (TimerType) timer.getInfo();
      if ((TimerType.BILLING_INVOCATION.equals(tType)
              && TimerType.BILLING_INVOCATION.equals(timerType))
          || (TimerType.DISCOUNT_END_CHECK.equals(tType)
              && TimerType.DISCOUNT_END_CHECK.equals(timerType))) {
   ...
   */
  private void cancelObsoleteTimer(TimerService timerService, TimerType timerType) {
    ...
    for (Timer timer : ParameterizedTypes.iterable(timerService.getTimers(), Timer.class)) {
      ...
      if (info != null && info instanceof TimerType && timerType == info) {
        ...
        timer.cancel();
        logger.logInfo(
            Log4jLogger.SYSTEM_LOG, LogMessageIdentifier.INFO_TIMER_REMOVED, String.valueOf(type));
      }
    }
  }
  ...
  private void cancelAllObsoleteTimer() {
    for (Timer timer :
        ParameterizedTypes.iterable(ctx.getTimerService().getTimers(), Timer.class)) {
      Serializable info = timer.getInfo();
  ...
  /**
   * Returns the timer details for the given timer type as specified in the database.
   *
  ...
  TimerIntervalDetails getTimerDetailsForTimerType(final TimerType timerType) {
    final long timerInterval =
        cfgMgmt.getLongConfigurationSetting(
            timerType.getKeyForIntervalTime(), Configuration.GLOBAL_CONTEXT);
    ...
    if (TimerType.BILLING_INVOCATION.equals(timerType)) {
      ...
      timerOffset =
          cfgMgmt.getLongConfigurationSetting(
              timerType.getKeyForIntervalOffset(), Configuration.GLOBAL_CONTEXT);
    }
   ...
   * If the timer is not a periodic timer, it will also be re-initialized.
   *
   * @param timer The expired timer provided by the system.
   */
  @Timeout
  public void handleTimer(Timer timer) {
    ...
    TimerProcessing processingData = createTimerProcessing(timerType, currentTime);
    ...
    if (processingData != null) {
      try {
        switch (timerType) {
          ...
          case TENANT_PROVISIONING_TIMEOUT:
            outcome = subMgmt.notifyAboutTimedoutSubscriptions(currentTime);
            break;
          ...
          case DISCOUNT_END_CHECK:
            outcome = accMgmt.sendDiscountEndNotificationMail(currentTime);
            break;
          ...
          case SYNCHRONIZE_USERS_WITH_OIDC_PROVIDER:
            if (!AuthenticationMode.INTERNAL
                .name()
                .equals(
                    cfgMgmt
                        ...
                        .getValue())) {
              outcome = idServiceLocal.synchronizeUsersAndGroupsWithOIDCProviderInt();
            }
            break;
          default:
        ...
        logger.logError(Log4jLogger.SYSTEM_LOG, e, LogMessageIdentifier.ERROR_HANDLE_TIMER_FAILED);
      }
      ...
      logger.logInfo(Log4jLogger.SYSTEM_LOG, LogMessageIdentifier.INFO_TIMER_NO_HANDLING);
    }
  }

  /**
   * Tries to create a timer processing data entry. If there has already been a handling for the
   * same timer type within 95% of the timer interval time, or if the database entry cannot be
   * written (concurrent handling scenario), the method will return <code>null</code> to indicate
   * that the handling can be aborted.
   *
   * @param type The type of the timer.
   * @param startTime The start time for the timer related task handling.
   * @return <code>null</code> in case the handling should be aborted, the timer processing data
   *     otherwise.
   */
  TimerProcessing createTimerProcessing(TimerType type, long startTime) {
    ...
    if (timerInterval == 0 && timerIntervalDetails.getPeriod() != null) {
      timerInterval = timerIntervalDetails.getPeriod().getDuration(startTime);
    }
    long maxDeviation = (long) (DEVIATION_FACTOR * timerInterval);
    long lowerTimeBound = startTime - maxDeviation;
    Query query = dm.createNamedQuery("TimerProcessing.findDataForTimeAndTimeframe");
  ...
  /**
   * Determines the next time the timer will expire. To do so the first day of the current year, at
   * 0:00:00 time will be used. The intervalTime will be added until we have a date that lies ahead
   * from now. Then this date will be returned.
   *
   * @param timerInterval Interval for the timer in milliseconds, is expected to not be below 0.
   * @return The date representing the next timer expiration time.
   */
  private Date getDateForNextTimerExpiration(long timerInterval, long offsetTime) {
    ...
    long nextExecutionTimeWithinOffset =
        startPoint + (numberPossibleExecutions * timerInterval) + offsetTime;
    if (nextExecutionTimeWithinOffset - currentTime > 0) {
      ...
      long nextExecutionTime = startPoint + (numberPossibleExecutions * timerInterval) + offsetTime;
  ...
  /**
   * Determines the next time the timer will expire. To do so the first day of the current year, at
   * 0:00:00 time will be used. The period will be added until we have a date that lies ahead from
   * now. Then this date will be returned. The offset will be added to this date as well.
   *
   * @param period The period to be considered.
   * @param timerOffset The offset to be added to the starting time.
  ...
  /**
   * Determines the calendar field object that corresponds to the given period.
   *
   * @param period The specifie period.
  ...
  /**
   * Wraps the timer interval and the timer offset settings for a particular timer type.
   *
   * @author Mike J&auml;ger
   */
  private class TimerIntervalDetails {
    ...
    TimerIntervalDetails(long intervalTime, long intervalOffset, Period period) {
  ...
  public List<VOTimerInfo> getCurrentTimerExpirationDates() {
    ...
    for (Timer timer :
      ...
      if (info != null && info instanceof TimerType) {
        TimerType type = (TimerType) info;
        long expirationTime = timer.getTimeRemaining() + System.currentTimeMillis();
