...
public class PropertyImportTest {
  ...
  public void setup() throws Exception {
    ...
    Mockito.when(sqlConn.prepareStatement(Matchers.anyString())).thenReturn(statement);
    Mockito.when(statement.executeQuery()).thenReturn(resultSet);

    Mockito.when(sqlConn.createStatement()).thenReturn(countStatement);
    Mockito.when(countStatement.executeQuery(Matchers.anyString())).thenReturn(countResultSet);
    Mockito.when(new Boolean(countResultSet.next()))
        .thenReturn(Boolean.TRUE)
        .thenReturn(Boolean.FALSE);
    Mockito.when(new Integer(countResultSet.getInt(Matchers.anyInt()))).thenReturn(new Integer(1));
  ...
  public void execute_update() throws Exception {
    ...
    Mockito.doAnswer(answerStatement).when(sqlConn).prepareStatement(Matchers.anyString());
    Mockito.when(secondStatement.executeQuery()).thenReturn(secondResult);
    // finding one entry
    Mockito.when(new Boolean(resultSet.next())).thenReturn(Boolean.TRUE).thenReturn(Boolean.FALSE);
    Mockito.when(new Integer(resultSet.getInt(Matchers.anyInt()))).thenReturn(new Integer(0));
    Mockito.when(new Boolean(secondResult.next()))
        .thenReturn(Boolean.TRUE)
        .thenReturn(Boolean.FALSE);
    Mockito.when(new Integer(secondResult.getInt(Matchers.anyInt()))).thenReturn(new Integer(4));
    ...
    String[] expected = new String[] {"SELECT ", "INSERT ", "UPDATE ", "DELETE "};
    int cnt = 0;
    assertEquals(expected.length, sqlStatementes.size());
    for (String sql : sqlStatementes) {
      assertEquals(Boolean.TRUE, Boolean.valueOf(sql.startsWith(expected[cnt++])));
    }
  }
  ...
  public void execute_nullMandatoryValueInInternalMode() throws Exception {
    ...
    try {
      ...
      assertEquals(
          "Mandatory attribute "
              + ConfigurationKey.AUTH_MODE.name()
  ...
  public void execute_prepareException() throws Exception {
    Mockito.when(sqlConn.prepareStatement(Matchers.anyString())).thenThrow(new SQLException());
    Properties p = getProperties();
    FileOutputStream fos = null;
    try {
      fos = new FileOutputStream(tempFile);
  ...
  private PropertyImport createImport(
    ...
    return new PropertyImport(
        driverClass, driverURL, userName, userPwd, propertyFile, overwriteFlag, contextId) {
      @Override
  ...
  private Properties getProperties() {
    ...
    p.put(
        ConfigurationKey.HIDDEN_UI_ELEMENTS.name(),
