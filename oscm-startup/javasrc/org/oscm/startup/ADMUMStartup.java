...
/**
 * Checks for existence of the mandatory configuration settings of the ADM UM product at startup
 * time. The result will be logged.
 *
...
public class ADMUMStartup extends HttpServlet {
  ...
  private static Log4jLogger logger = LoggerFactory.getLogger(ADMUMStartup.class);
  ...
  /**
   * Delegates the call to the product session management component to clean up the corresponding
   * database table.
   */
  ...
  private void initializeLogger() {
    String filePath =
        cs.getConfigurationSetting(ConfigurationKey.LOG_FILE_PATH, Configuration.GLOBAL_CONTEXT)
            .getValue();
    String logLevel =
        cs.getConfigurationSetting(ConfigurationKey.LOG_LEVEL, Configuration.GLOBAL_CONTEXT)
            .getValue();
    String logConfigFile =
        cs.getConfigurationSetting(ConfigurationKey.LOG_CONFIG_FILE, cs.getNodeName()).getValue();
    LoggerFactory.activateRollingFileAppender(filePath, logConfigFile, logLevel);
  }

  /**
   * Checks the currently existing configuration settings. If a mandatory setting is not present, an
   * exception will be logged. It also checks for the setting of the node name.
   */
  private void checkSettings() {
    ...
    for (ConfigurationKey key : keys) {
      if (key.isMandatory()) {
        try {
          cs.getConfigurationSetting(key, Configuration.GLOBAL_CONTEXT);
