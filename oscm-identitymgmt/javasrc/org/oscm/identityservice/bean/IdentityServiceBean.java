...
public class IdentityServiceBean implements IdentityService, IdentityServiceLocal {
  ...
  @Inject public ReviewServiceLocalBean rvs;

  @Inject UserGroupServiceLocalBean userGroupService;
  ...
  @EJB OidcSynchronizationBean oidc;
  ...
  public VOUserDetails createUser(
      VOUserDetails user, List<UserRoleType> roles, String marketplaceId)
  ...
  protected void importUsers(byte[] csvData, Organization organization, String marketplaceId)
    ...
    ensureNoRemoteLdapUsed(
        organization, LogMessageIdentifier.ERROR_ADDING_USER_FORBIDDEN_REMOTE_LDAP);
    ensureEmailSet();
  ...
  /**
   * The importing user must have an email defined. Otherwise, the import report cannot be sent by
   * mail.
   */
   ...
   * @return the user, otherwise throw if the specified user does not exist.
   * @throws ObjectNotFoundException - if the user is not found
   */
  PlatformUser loadUser(String userId, Tenant tenant) throws ObjectNotFoundException {
  ...
  public VOUserDetails createUserInt(TriggerProcess tp)
    ...
    List<UserRoleType> roles =
        ParameterizedTypes.list(
            tp.getParamValueForName(TriggerProcessParameterName.USER_ROLE_TYPE)
                .getValue(List.class),
            UserRoleType.class);
    ...
    Map<Long, UnitUserRole> userGroupKeyToRole =
        ParameterizedTypes.hashmap(
            tp.getParamValueForName(TriggerProcessParameterName.USER_GROUPS_WITH_ROLES)
                .getValue(Map.class),
            Long.class,
            UnitUserRole.class);
    ...
    try {
      ...
      if (userGroupKeyToRole != null && !userGroupKeyToRole.isEmpty()) {
        ...
        for (Entry<UserGroup, UnitUserRole> groupWithRole : userToGroup.entrySet()) {
          userGroupService.grantUserRoles(
              addedUser,
              Arrays.asList(groupWithRole.getValue().getRoleName()),
  ...
  private Map<UserGroup, UnitUserRole> getUserGroupToRoleMap(
    ...
    if (userGroupKeyToRole != null) {
      for (Map.Entry<Long, UnitUserRole> e : userGroupKeyToRole.entrySet()) {
        try {
          userGroupToRole.put(
              dm.getReference(UserGroup.class, e.getKey().longValue()), e.getValue());
  ...
  public void changePassword(String oldPassword, String newPassword)
    ...
    if (!PasswordHash.verifyPassword(
        pUser.getPasswordSalt(), pUser.getPasswordHash(), oldPassword)) {
      ...
      // exception.
      SecurityCheckException scf =
          new SecurityCheckException("Authentication failed, a wrong password has been specified.");
  ...
  public void resetUserPassword(PlatformUser platformUser, String marketplaceId)
    ...
    if (sessionsForUserKey.size() > 0) {
      UserActiveException uae =
          new UserActiveException(
              "Reset of password for user '"
                  + platformUser.getKey()
                  + "' failed, as the user is still active",
              new Object[] {platformUser.getUserId()});
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG, uae, LogMessageIdentifier.WARN_OPERATOR_RESET_PASSWORD_FAILED);
      throw uae;
    }
  ...
  public void deleteUser(PlatformUser pUser, String marketplaceId)
      throws OperationNotPermittedException, UserDeletionConstraintException,
          TechnicalServiceNotAliveException, TechnicalServiceOperationException {
  ...
  public void lockUserAccount(VOUser user, UserAccountStatus newStatus, String marketplaceId)
    ...
    ensureNoRemoteLdapUsed(
        userObj.getOrganization(), LogMessageIdentifier.ERROR_LOCK_OPERATION_FORBIDDEN_REMOTE_LDAP);
    ...
    if (newStatus == UserAccountStatus.ACTIVE
      ...
      OperationNotPermittedException onp =
          new OperationNotPermittedException("To lock an account a locking state must be set!");
      logger.logWarn(
    ...
    if (newStatus == UserAccountStatus.LOCKED_FAILED_LOGIN_ATTEMPTS
      ...
      OperationNotPermittedException onp =
          new OperationNotPermittedException("A user must not set those locking states!");
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG | Log4jLogger.AUDIT_LOG,
          onp,
          LogMessageIdentifier.WARN_OPERATOR_LOCK_FAILED,
  ...
  @Override
  @TransactionAttribute(TransactionAttributeType.MANDATORY)
  public PlatformUser modifyUserData(
      PlatformUser existingUser, VOUserDetails updatedUser, boolean modifyOwnUser, boolean sendMail)
      throws OperationNotPermittedException, NonUniqueBusinessKeyException,
          TechnicalServiceNotAliveException, TechnicalServiceOperationException,
    ...
    PlatformUser oldUser =
        existingUser.getEmail() != null ? UserDataAssembler.copyPlatformUser(existingUser) : null;
      ...
      if (status != SubscriptionStatus.PENDING && status != SubscriptionStatus.INVALID) {
        ...
        if (license != null) {
          UpdateUserPayload payload =
              new UpdateUserPayload(subscription.getKey(), license.getKey());
  ...
  public void sendAdministratorNotifyMail(PlatformUser administrator, String userId) {
    ...
    SendMailStatus<PlatformUser> mailStatus =
        cm.sendMail(
            EmailType.USER_UPDATED_WITH_NOROLE,
            new Object[] {userId},
            null,
            platformUsers.toArray(new PlatformUser[platformUsers.size()]));
  ...
  public void notifyOnLoginAttempt(VOUser user, boolean attemptSuccessful)
    ...
    if (attemptSuccessful) {
      ...
      if (failedAttempts >= getMaxRetryAttempts()) {
        if (pUser.getStatus().getLockLevel()
            <= UserAccountStatus.LOCKED_FAILED_LOGIN_ATTEMPTS.getLockLevel()) {
          pUser.setStatus(UserAccountStatus.LOCKED_FAILED_LOGIN_ATTEMPTS);
        }
      }
    }
  ...
  public void unlockUserAccount(VOUser user, String marketplaceId)
    ...
    ensureNoRemoteLdapUsed(
        pUser.getOrganization(), LogMessageIdentifier.ERROR_LOCK_OPERATION_FORBIDDEN_REMOTE_LDAP);
  ...
  private void setUserAccountStatusInternal(
      PlatformUser user, UserAccountStatus status, Marketplace marketplace) {
  ...
  public VOUser getUser(VOUser user)
    ...
    try {
      ...
      if (pUser.getOrganization().getDeregistrationDate() != null) {
        OperationNotPermittedException onp =
            new OperationNotPermittedException("The user doesn't belong to a active organization.");
        logger.logWarn(
            Log4jLogger.SYSTEM_LOG | Log4jLogger.ACCESS_LOG,
            onp,
            LogMessageIdentifier.WARN_OPERATOR_LOGIN_FAILED,
            pUser.getUserId());
        throw onp;
      }
    } catch (ObjectNotFoundException e) {
      ...
      Query query =
          dm.createNamedQuery("PlatformUserHistory.findUnconfirmedUserForRemovedOrganization");
      ...
      if (singleResult != null) {
        ...
        OrganizationRemovedException cre =
            new OrganizationRemovedException(
  ...
  public List<VOUserDetails> searchLdapUsers(final String userIdPattern)
    ...
    try {
      // read user from LDAP
      List<VOUserDetails> voUserList =
          ldapAccess.search(
              dirProperties, baseDN, getLdapSearchFilter(attrMap, userIdPattern), mapper, false);

      int size = voUserList.size();
      for (int i = 0; i < size; i++) {
        VOUserDetails voUser = voUserList.get(i);
        PlatformUser user =
            getPlatformUserByOrgAndReamUserId(organization, voUser.getRealmUserId());
        if (null != user) {
      ...
      logger.logError(
          Log4jLogger.SYSTEM_LOG, vf, LogMessageIdentifier.ERROR_LDAP_SYSTEM_CONNECTION_REFUSED);
      throw vf;
    }
  }
  ...
  public boolean searchLdapUsersOverLimit(final String userIdPattern) throws ValidationException {
    ...
    ILdapResultMapper<VOUserDetails> mapper = new LdapVOUserDetailsMapper(null, attrMap);
    try {
      return ldapAccess.searchOverLimit(
          dirProperties, baseDN, getLdapSearchFilter(attrMap, userIdPattern), mapper, false);
    } catch (NamingException e) {
      Object[] params = new Object[] {dirProperties.get(Context.PROVIDER_URL), e.getMessage()};
      ValidationException vf =
        ...
        if (size == 1) {
          ...
          ValidationException vf =
              new ValidationException(
                  ReasonEnum.LDAP_USER_NOT_FOUND, null, new Object[] {user.getRealmUserId()});
          logger.logError(
              Log4jLogger.SYSTEM_LOG,
              vf,
              LogMessageIdentifier.ERROR_VALIDATION_PARAMETER_LDAP_FOUND_ERROR,
          ...
          ValidationException vf =
              new ValidationException(
                  ReasonEnum.LDAP_USER_NOT_UNIQUE, null, new Object[] {user.getRealmUserId()});
          logger.logError(
              Log4jLogger.SYSTEM_LOG,
              vf,
              LogMessageIdentifier.ERROR_VALIDATION_PARAMETER_LDAP_FOUND_ERROR,
  ...
  public VOUserDetails createOnBehalfUser(String organizationId, String password)
    ...
    ensureNoRemoteLdapUsed(
        customer, LogMessageIdentifier.ERROR_ON_BEHALF_USER_OPERATION_REMOTE_LDAP);
   ...
   *
   * @param customerUser The user to grant the roles to.
   */
  ...
  public void createOrganizationAdmin(
    ...
    if (!isAutoGeneratedPassword) {
      ...
      try {
        ...
        MailOperationException mof =
            new MailOperationException("Confirmation URL creation failed!", e);
        logger.logError(
            Log4jLogger.SYSTEM_LOG, mof, LogMessageIdentifier.ERROR_UNSUPPORTED_ENCODING);
        throw mof;
      }
      ...
      cm.sendMail(
          organizationAdmin, EmailType.USER_CONFIRM, new Object[] {confirmationURL}, marketplace);
    }
  }
   ...
   *
   * @param userId The user identifying attributes' representation.
   * @param validateOrganization <code>true</code> if the calling user must be part of the same
  ...
  public PlatformUser getPlatformUser(String userId, String tenantId, boolean validateOrganization)
    ...
    if (platformUser == null) {
      ObjectNotFoundException onf =
          new ObjectNotFoundException(ObjectNotFoundException.ClassEnum.USER, userId);
  ...
  void deletePlatformUser(
      PlatformUser userToBeDeleted,
      boolean deleteLastAdmin,
      boolean selfDeletionAllowed,
      Marketplace marketplace)
      throws UserDeletionConstraintException, TechnicalServiceNotAliveException,
    ...
    if (subscriptions.size() > 0 && !ifAllExpired) {
      UserDeletionConstraintException udcv =
          new UserDeletionConstraintException(
              "User '"
    ...
    if (!selfDeletionAllowed) {
      if (sessionCtx
        ...
        logger.logWarn(
            Log4jLogger.SYSTEM_LOG, udcv, LogMessageIdentifier.WARN_USER_DELETION_FAILED);
        throw udcv;
      }
    }
    if (!deleteLastAdmin) {
    ...
    try {
      if (subscriptions.size() > 0 && ifAllExpired) {
        for (Subscription subscription : subscriptions) {
          sm.revokeUserFromSubscriptionInt(
              subscription, Collections.singletonList(userToBeDeleted));
        }
      }
    ...
    try {
      ...
      cm.sendMail(
          ...
          new Object[] {organizationName == null ? "" : organizationName},
          marketplace);
    } catch (MailOperationException e) {
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG, LogMessageIdentifier.WARN_SEND_USER_DELETED_MAIL_FAILED);
    }
  }

  PlatformUser addPlatformUser(
      VOUserDetails userDetails,
      Organization organization,
      String password,
      UserAccountStatus lockLevel,
      boolean sendMail,
      boolean userLocalLdap,
      Marketplace marketplace,
      boolean performRoleCheck)
      throws NonUniqueBusinessKeyException, MailOperationException, ValidationException,
          UserRoleAssignmentException {
    return addPlatformUser(
        userDetails,
        organization,
        password,
        lockLevel,
  ...
  private PlatformUser addPlatformUser(
      VOUserDetails userDetails,
      Organization organization,
      String password,
      UserAccountStatus lockLevel,
    ...
    if (!userLocalLdap) {
      ...
      if (!isUserIdUnique(userDetails.getRealmUserId())) {
        ...
        try {
          ...
          ValidationException vf =
              new ValidationException(
                  ReasonEnum.LDAP_CREATED_ID_INVALID, null, new Object[] {uniqueId});
          logger.logError(
              Log4jLogger.SYSTEM_LOG,
              vf,
              LogMessageIdentifier.ERROR_VALIDATION_PARAMETER_LDAP_FOUND_ERROR,
  ...
  /**
   * Returns the configured number of retry attempts. If none is specified, the fallback value will
   * be returned.
   *
  ...
  private int getMaxRetryAttempts() {
    return Integer.parseInt(
        cs.getConfigurationSetting(
                ConfigurationKey.MAX_NUMBER_LOGIN_ATTEMPTS, Configuration.GLOBAL_CONTEXT)
            .getValue());
  }
  ...
  void checkMinimumUserRoleConstrains(PlatformUser user) throws ValidationException {
    ...
    if (orgRoles.size() == 1 && orgRoles.iterator().next() == OrganizationRoleType.CUSTOMER) {
      ValidationException ve =
          new ValidationException(ReasonEnum.CUSTOMER_CREATION_ONLY_ON_MARKETPLACE, null, null);
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG | Log4jLogger.AUDIT_LOG,
          ve,
          LogMessageIdentifier.ERROR_USER_CREATION_FAILED_WITH_VALIDATION_ERROR);
  ...
  void checkRoleConstrains(PlatformUser user, UserRoleType role)
    ...
    if (!isAllowedUserRole(user.getOrganization(), role) && !role.isUnitRole()) {
      ...
      UserRoleAssignmentException ure =
          new UserRoleAssignmentException(msg, new Object[] {orgRole.name(), role.name()});
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG | Log4jLogger.AUDIT_LOG,
          ure,
          LogMessageIdentifier.WARN_USER_ROLE_REQUIRED,
    ...
    if (userRole == UserRoleType.ORGANIZATION_ADMIN) {
      ...
      if (isUserLastAdminForOrganization(user)) {
        UserModificationConstraintException e =
            new UserModificationConstraintException(
   ...
   *
   * @param userToBeDeleted The user the check is performed for
   * @return <code>true</code>in case the user is the last administrative user for the organization,
  ...
  /**
   * Retrieves all users with the given email (the search is performed case insensitive).
   *
   * @param email the email for the search
  ...
  private LdapConnector getLdapConnectionForOrganization(Organization organization)
      throws ValidationException {
    Properties resolvedProps = null;
    try {
      resolvedProps =
          ldapSettingsMS.getOrganizationSettingsResolved(organization.getOrganizationId());
    } catch (ObjectNotFoundException e) {
   ...
   *
   * @param attrMap the map with all LDAP attribute setting types.
   * @param pattern the pattern for the search.
  ...
  /**
   * Creates and persists a new platform user using the settings of the current user, except the
   * reference to the organization and the user identifier. The user will be created as
   * administrative user.
   *
   * @param password The password to be assigned to the user. In case the authentication mode is
   *     SAML_SP, the password parameter is used as the userId to be set for the created user.
   * @param currentUser The master user for the user to be created.
   * @param customer The customer organization.
   * @return
   * @throws NonUniqueBusinessKeyException
   */
  private PlatformUser createOnBehalfUser(
      String password, PlatformUser currentUser, Organization customer)
      throws NonUniqueBusinessKeyException {
    ...
    while (customerUser == null) {
      try {
        ...
        if (retryCount > 10) {
          logger.logWarn(
              Log4jLogger.SYSTEM_LOG, e, LogMessageIdentifier.WARN_NON_UNIQUE_BUSINESS_KEY);
          throw e;
        }
      } catch (MailOperationException e) {
        SaaSSystemException se =
            new SaaSSystemException(
                ...
                e);
        logger.logError(
            Log4jLogger.SYSTEM_LOG, se, LogMessageIdentifier.ERROR_MAIL_OPERATION_FAILED);
        throw se;
      } catch (ValidationException e) {
        // this never happens
        SaaSSystemException se =
            new SaaSSystemException("The user creation failed with a validation exception.", e);
        logger.logError(
            Log4jLogger.SYSTEM_LOG,
            se,
            LogMessageIdentifier.ERROR_USER_CREATION_FAILED_WITH_VALIDATION_ERROR);
   ...
   *
   * @param organizationId The identifier of the customer organization the user should be created
   *     for.
   * @param password The password to be given to the u ser.
   * @param currentUser The caller and master user for the user to be created.
   * @return The customer organization.
   * @throws ObjectNotFoundException
   * @throws OperationNotPermittedException
   */
  private Organization validateForOnBehalfUserCreation(
      String organizationId, String password, PlatformUser currentUser)
      throws ObjectNotFoundException, OperationNotPermittedException {
    ArgumentValidator.notNull("organizationId", organizationId);
  ...
  /**
   * Read the values from the remote LDAP and update the corresponding platform user attributes.
   *
   * @param pUser The platform user to be processed.
   * @throws ValidationException if not all mandatory LDAP parameters can be resolved for the
    ...
    try {
      List<VOUserDetails> list =
          ldapAccess.search(
              dirProperties,
              baseDN,
              getLdapSearchFilter(attrMap, pUser.getUserId()),
              mapper,
              false);
   ...
   *
   * @param user the user to find the usage license for
   * @param subscription the subscription to get the usage licenses from
   * @return The UsageLicense or <code>null</code> in case no matching license was found
   */
  ...
  /**
   * Tries to find the marketplace with the provided id. Returns <code>null</code> if the id is
  ...
  /**
   * The small helper function to create a user id which is unique in the context of BES id the
   * regular user id is not unique. The function follows the scheme: 1. use email as user id 2.
   * create string userid@orgid and us it as user id.
   *
   * @param userDetails Information about the user which should be created.
  ...
  public void setUserRoles(VOUser user, List<UserRoleType> roles)
      throws ObjectNotFoundException, UserModificationConstraintException,
          OperationNotPermittedException, UserRoleAssignmentException, UserActiveException {
  ...
  boolean isAllowedUserRole(Organization org, UserRoleType userRole) {
    if (isOrganizationSpecificRole(userRole)) {
      OrganizationRoleType orgTypesForUserRole =
          OrganizationRoleType.correspondingOrgRoleForUserRole(userRole);
  ...
  /**
   * This method already exists in UserRoleType class, but there are problem on Windows. The method
  ...
  /**
   * Checks if the specified organization uses a remote LDAP system for user authentication. If so,
   * an UnsupportedOperationException will be thrown.
   *
   * @param organization The organization to check.
   * @param messageId The log message identifier to use in case an exception is thrown.
   */
  ...
  @Override
  @RolesAllowed("ORGANIZATION_ADMIN")
  public VOUserDetails createUserWithGroups(
      VOUserDetails user,
      List<UserRoleType> roles,
      String marketplaceId,
      Map<Long, UnitUserRole> userGroupKeyToRole)
  ...
  private VOUserDetails createUser(
    ...
    triggerProcess.addTriggerProcessParameter(
        TriggerProcessParameterName.USER_GROUPS_WITH_ROLES, userGroupKeyToRole);
  ...
  @Override
  public PlatformUser getPlatformUserByOrganization(String userId, String orgId)
      throws ObjectNotFoundException {
  ...
  @Override
  public void addMemberToAccessGroupInOIDCProvider(
      String groupId, String tenantId, VOUserDetails userInfo) throws RegistrationException {
    try {
      ApiIdentityClient client = RestUtils.createClient(tenantId);
  ...
  protected boolean synchronizeUsersAndGroupsWithOIDCProviderIntImpl() {
    ...
    for (String tenantId : tenantIds) {
      ...
      for (Organization org : orgs) {
        ...
        removeDeletedUsersFromPlatform(org, members);

        addNewUsersToPlatform(tenantId, org, members);
      }
    }
    return true;
  }

  protected void addNewUsersToPlatform(
      String tenantId, Organization org, List<VOUserDetails> members) {
    for (VOUserDetails user : members) {
      ...
      if (!existsInDB(user, org)) {
        ...
        addUserToPlatform(model.getOrganization(), model.getUser(), model.getMarketplace());
      }
    }
  }

  protected void removeDeletedUsersFromPlatform(Organization org, List<VOUserDetails> members) {
    try {
  ...
  private boolean existsInDB(VOUserDetails user, Organization organization) {
    try {
      loadUser(user.getUserId(), organization.getTenant());
    } catch (ObjectNotFoundException e) {
      return false;
    }
    return true;
  ...
  private void addUserToPlatform(
      Organization organization, VOUserDetails user, Marketplace marketplace) {
    try {
      addPlatformUser(
          user, organization, "", UserAccountStatus.ACTIVE, true, true, marketplace, false);
    } catch (Exception e) {
      logger.logWarn(
          Log4jLogger.SYSTEM_LOG,
          e,
          LogMessageIdentifier.ERROR_ADD_CUSTOMER,
          "An error occured while trying to import the User " + user.getOrganizationId());
    }
  }
}
