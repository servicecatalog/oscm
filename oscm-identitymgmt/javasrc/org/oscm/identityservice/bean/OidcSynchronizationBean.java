...
/**
 * This EJB implementation is responsible for synchronizing organizations with user groups that are
 * managed with an OIDC provider.
 */
@Stateless
public class OidcSynchronizationBean {
  ...
  private static final Log4jLogger logger = LoggerFactory.getLogger(OidcSynchronizationBean.class);
  ...
  public List<VOUserDetails> getAllUsersFromGroup(String groupId, String tenantId) {
    try {
      ...
      logger.logWarn(
          ...
          String.format(
              "An error occurred while getting the members of the OIDC access group %s for tenant %s",
              groupId, tenantId));
    }
    return emptyList();
  }
  ...
  protected List<GroupInfo> getAllGroups(String tenantId) {
    ...
    try {
      ...
      logger.logInfo(
          ...
          LogMessageIdentifier.ERROR_SYNCHRONIZATION,
          String.format(
              "Cannot get OIDC access groups for tenant %s. Check if the tenant is configured propperly. ",
              tenantId));
    }
    return emptyList();
  }
  ...
  protected List<Organization> syncOrganizations(GroupInfo group, String tenantId) {
    try {
      ...
      logger.logWarn(
          ...
          LogMessageIdentifier.ERROR_SYNCHRONIZATION,
          String.format(
              "Error synchronizing organization: %s (group id and organization name must be unique)",
              e.getMessage()));
    } catch (EntityNotFoundException e) {
      logger.logInfo(
          Log4jLogger.SYSTEM_LOG,
          LogMessageIdentifier.INFO_SYNCHRONIZATION,
  ...
  protected Organization syncOrganization(GroupInfo group, String tenantId) {
    try {
      ...
      final String orgName = extractFromGroupName(group.getName());
      Organization org = findOrganizationByName(orgName, tenantId);
      setGroupId(org, group.getId());
      return org;
    }
  }

  private String extractFromGroupName(String grp) {
    if (grp.toUpperCase().startsWith("OSCM_")) {
      return grp.substring(5);
    }
    return grp;
  }
  ...
  protected Organization findOrganizationByGroupId(String groupId) throws NonUniqueResultException {
  ...
  protected Organization resultFrom(Query query, String entity)
      throws NonUniqueResultException, EntityNotFoundException {
    List<Organization> orgs = ParameterizedTypes.list(query.getResultList(), Organization.class);
  ...
  protected void setUserWithRoles(
      UserImportModel um, Organization o, VOUserDetails u, String tenantId) {
