...
/**
 * Assembler to convert the discount value object to the according domain object and vice versa.
 *
 * @author Enes Sejfi
 */
public class DiscountAssembler extends BaseAssembler {
   ...
   *
   * @param domObj domain discount object
   ...
   *
   * @param voObj VO discount object
   * @return new discount domain object
   * @throws ValidationException the validation exception
   */
  public static Discount toDiscount(VODiscount voObj) throws ValidationException {
    if (voObj == null) {
      return null;
    }
   ...
   * @throws ValidationException Thrown in case the discount violates discount validation rules.
   * @throws ConcurrentModificationException Thrown in case the value object's version does not
   *     match the current domain object's.
   */
   ...
   *
   * @param voObj VO discount object
   * @throws ValidationException A ValidationException is thrown if discount.value is null or the
   *     scale is not right, discount.value is less than 0 and greater than 100 or
  ...
  static void validate(VODiscount voObj) throws ValidationException {
    ...
    BLValidator.isInRange(
        FIELD_NAME_VALUE, voObj.getValue(), MIN_DISCOUNT_VALUE, MAX_DISCOUNT_VALUE);
    BLValidator.isEqual(FIELD_NAME_VALUE, voObj.getValue(), BigDecimal.ZERO);
    BLValidator.isValidPriceScale(FIELD_NAME_VALUE, voObj.getValue());
    if (voObj.getStartTime() != null) {
      BLValidator.isInRange(
          FIELD_NAME_STARTDATE,
          voObj.getStartTime().longValue(),
          voObj.getStartTime(),
          voObj.getEndTime());
    }
  }
