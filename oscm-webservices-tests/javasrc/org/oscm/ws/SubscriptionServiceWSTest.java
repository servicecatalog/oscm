...
public class SubscriptionServiceWSTest {
  ...
  private static final String ERROR_ABORT_ASYNC_SUB_EN =
      "provisioning_error_message_during_subscription_abortion_in_english";
  private static final String ERROR_ABORT_ASYNC_SUB_DE =
      "provisioning_error_message_during_subscription_abortion_in_german";
  private static final String ERROR_ABORT_ASYNC_SUB_JA =
      "provisioning_error_message_during_subscription_abortion_in_japanese";
  ...
  public static void setUpOnce() throws Exception {
    ...
    mpLocal =
        mpSrvOperator.createMarketplace(
            factory.createMarketplaceVO(supplier.getOrganizationId(), false, "Local Marketplace"));
    ...
    freeService = setup.createAndActivateService("Service", mpLocal, priceModel);

    asynFreeService = setup.createAndActivateAsynService("Service", mpLocal, priceModel);
    asynFreeService2 = setup.createAndActivateAsynService("Service2", mpLocal, priceModel);

    svcWithOp =
        setup.createAndActivateService(
            "Service OP", tpWithOp, mpLocal, new LinkedList<VOParameter>());
    ...
    subscrServiceForCustomer =
        ServiceFactory.getDefault()
            .getSubscriptionService(
                String.valueOf(customerUser.getKey()), WebserviceTestBase.DEFAULT_PASSWORD);

    subscrServiceForSupplier =
        ServiceFactory.getDefault()
            .getSubscriptionService(
                setup.getSupplierUserKey(), WebserviceTestBase.DEFAULT_PASSWORD);

    // reseller is created AFTER the customer, so the customerUser is not
    // assigned to him
    reseller = setup.createReseller(RESELLER_ORG_NAME);
    subscrServiceForReseller =
        ServiceFactory.getDefault()
            .getSubscriptionService(
                setup.getResellerUserKey(), WebserviceTestBase.DEFAULT_PASSWORD);

    accountService =
        ServiceFactory.getDefault()
            .getAccountService(setup.getSupplierUserKey(), WebserviceTestBase.DEFAULT_PASSWORD);
    accountServiceForCustomer =
        ServiceFactory.getDefault()
            .getAccountService(
                String.valueOf(customerUser.getKey()), WebserviceTestBase.DEFAULT_PASSWORD);
    ...
    subscrServiceForSubManager =
        ServiceFactory.getDefault()
            .getSubscriptionService(
                String.valueOf(subscriptionManager.getKey()), WebserviceTestBase.DEFAULT_PASSWORD);

    user = createVOUser();
    user = identityService.createUser(user, null, mpLocal.getMarketplaceId());

    orgAdmin = createVOUser();
    orgAdmin =
        identityService.createUser(
            orgAdmin, Arrays.asList(UserRoleType.ORGANIZATION_ADMIN), mpLocal.getMarketplaceId());
    orgAdmin.setKey(
        Long.parseLong(WebserviceTestBase.readLastMailAndSetCommonPassword(orgAdmin.getUserId())));
    subscrServiceForOrgAdmin =
        ServiceFactory.getDefault()
            .getSubscriptionService(
                String.valueOf(orgAdmin.getKey()), WebserviceTestBase.DEFAULT_PASSWORD);
  }
  ...
  public void setup() throws Exception {
    ...
    unitAdminSubManager =
        identityService.createUser(
            ...
            Arrays.asList(UserRoleType.SUBSCRIPTION_MANAGER),
            mpLocal.getMarketplaceId());
    unitAdminSubManager.setKey(
        Long.parseLong(
            WebserviceTestBase.readLastMailAndSetCommonPassword(unitAdminSubManager.getUserId())));
    ...
    unitBug12379 =
        organizationalUnitService.createUnit(
            "testUnit" + new SimpleDateFormat("yyyy.MM.dd.HH.mm.ss").format(new Date()),
            "testDescription",
            new SimpleDateFormat("yyyy.MM.dd.HH.mm.ss").format(new Date()));
    identityService.addRevokeUserUnitAssignment(
        unitBug12379.getName(),
        Arrays.<VOUser>asList(unitAdminSubManager),
        Collections.<VOUser>emptyList());

    organizationalUnitService.revokeUserRoles(
        unitAdminSubManager, Arrays.asList(UnitRoleType.USER), unitBug12379);
    organizationalUnitService.grantUserRoles(
        unitAdminSubManager,
        Arrays.asList(org.oscm.types.enumtypes.UnitRoleType.ADMINISTRATOR),
        unitBug12379);
    service4UnitAdminSubManager =
        ServiceFactory.getDefault()
            .getSubscriptionService(
                String.valueOf(subscriptionManager.getKey()), WebserviceTestBase.DEFAULT_PASSWORD);
    ...
    unitAdminOnly =
        identityService.createUser(
            unitAdminOnly, Collections.<UserRoleType>emptyList(), mpLocal.getMarketplaceId());
    unitAdminOnly.setKey(
        Long.parseLong(
            WebserviceTestBase.readLastMailAndSetCommonPassword(unitAdminOnly.getUserId())));
    identityService.addRevokeUserUnitAssignment(
        unitBug12379.getName(),
        Arrays.<VOUser>asList(unitAdminOnly),
        Collections.<VOUser>emptyList());

    service4UnitAdminOnly =
        ServiceFactory.getDefault()
            .getSubscriptionService(
                String.valueOf(unitAdminOnly.getKey()), WebserviceTestBase.DEFAULT_PASSWORD);
    ...
    subManagerOnly =
        identityService.createUser(
            ...
            Arrays.asList(UserRoleType.SUBSCRIPTION_MANAGER),
            mpLocal.getMarketplaceId());
    subManagerOnly.setKey(
        Long.parseLong(
            WebserviceTestBase.readLastMailAndSetCommonPassword(subManagerOnly.getUserId())));

    service4SubManagerOnly =
        ServiceFactory.getDefault()
            .getSubscriptionService(
                String.valueOf(subManagerOnly.getKey()), WebserviceTestBase.DEFAULT_PASSWORD);

    notUnitAdminNotSubManager = createVOUser();
    notUnitAdminNotSubManager =
        identityService.createUser(
            notUnitAdminNotSubManager,
            Collections.<UserRoleType>emptyList(),
            mpLocal.getMarketplaceId());
    notUnitAdminNotSubManager.setKey(
        Long.parseLong(
            WebserviceTestBase.readLastMailAndSetCommonPassword(
                notUnitAdminNotSubManager.getUserId())));

    service4notUnitAdminNotSubManager =
        ServiceFactory.getDefault()
            .getSubscriptionService(
  ...
  public void cleanup() throws Exception {
    if (createdSubscription != null) {
      try {
        subscrServiceForCustomer.unsubscribeFromService(createdSubscription.getSubscriptionId());
        subscrServiceForSubManager.unsubscribeFromService(createdSubscription.getSubscriptionId());
        subscrServiceForOrgAdmin.unsubscribeFromService(createdSubscription.getSubscriptionId());
      } catch (ObjectNotFoundException e) {
        // for unsubscribe test, it is already done
      }
    }
    accountService.saveUdaDefinitions(
        new ArrayList<VOUdaDefinition>(), accountService.getUdaDefinitions());
    setup.deleteTriggersForUser();
  }

  private void terminateAsyncSubscription() throws Exception {
    if (createdSubscription != null) {
      subscrServiceForSupplier.terminateSubscription(createdSubscription, "reason");
    }
  }

  private VOSubscription subscribe()
      throws ObjectNotFoundException, NonUniqueBusinessKeyException, ValidationException,
          PaymentInformationException, ServiceParameterException, ServiceChangedException,
    ...
    createdSubscription =
        subscrServiceForCustomer.subscribeToService(
            createdSubscription, freeService, usageLicences, null, null, new ArrayList<VOUda>());
    subscriptionID = createdSubscription.getSubscriptionId();
    return createdSubscription;
  }

  @Test
  public void bug12379_subscriptionToServiceWithUnitAssignedByApi_unitAdminSubManager()
      throws Exception {
    // given
    ...
    service4UnitAdminSubManager =
        ServiceFactory.getDefault()
            .getSubscriptionService(
                String.valueOf(unitAdminSubManager.getKey()), WebserviceTestBase.DEFAULT_PASSWORD);
    VOSubscription voSubscription =
        service4UnitAdminSubManager.subscribeToService(
            subscription, freeService, usageLicences, null, null, udasToSave);
    // then
  ...
  @Test
  public void bug12379_subscriptionToServiceWithoutUnitAssignedByApi_unitAdminSubManager()
      throws Exception {
    // given
    List<VOUda> udasToSave = prepareUdasForSave();
    VOSubscription subscription = createSubscription();
  ...
  public void bug12379_subscriptionToServiceWithUnitAssignedByApi_unitAdminOnly() throws Exception {
    ...
    subscription.setUnitName(unitBug12379.getName());
    organizationalUnitService.revokeUserRoles(
        unitAdminOnly, Arrays.asList(UnitRoleType.USER), unitBug12379);
    organizationalUnitService.grantUserRoles(
        unitAdminOnly,
        Arrays.asList(org.oscm.types.enumtypes.UnitRoleType.ADMINISTRATOR),
        unitBug12379);
    ...
    // when
    VOSubscription voSubscription =
        service4UnitAdminOnly.subscribeToService(
            subscription, freeService, usageLicences, null, null, udasToSave);
    waitForLogStored();
    // then
    assertEquals(voSubscription.getUnitKey(), unitBug12379.getKey());
  ...
  public void bug12379_subscriptionToServiceWithoutUnitAssignedByApi_unitAdminOnly()
      throws Exception {
    // given
    List<VOUda> udasToSave = prepareUdasForSave();
    VOSubscription subscription = createSubscription();
  ...
  @Test(expected = OperationNotPermittedException.class)
  public void bug12379_subscriptionToServiceWithUnitAssignedByApi_subManagerOnly()
      throws Exception {
    // given
    List<VOUda> udasToSave = prepareUdasForSave();
    VOSubscription subscription = createSubscription();
    ...
    // when
    VOSubscription voSubscription =
        service4SubManagerOnly.subscribeToService(
            subscription, freeService, usageLicences, null, null, udasToSave);
    // then
  ...
  @Test
  public void bug12379_subscriptionToServiceWithoutUnitAssignedByApi_subManagerOnly()
      throws Exception {
    // given
    List<VOUda> udasToSave = prepareUdasForSave();
    VOSubscription subscription = createSubscription();
  ...
  @Test
  public void bug12379_subscriptionToServiceWithoutUnitAssignedByApi_notUnitAdminNotSubManager()
      throws Exception {
    //        //given
    //        List<VOUda> udasToSave = prepareUdasForSave();
    //        VOSubscription subscription = createSubscription();
    //        //when
    //        try {
    //            service4notUnitAdminNotSubManager.subscribeToService(subscription, freeService,
    // usageLicences, null, null, udasToSave);
    //            fail();
  ...
  @Test
  public void bug12379_subscriptionToServiceWithUnitAssignedByApi_notUnitAdminNotSubManager()
      throws Exception {
    //        //given
    //        List<VOUda> udasToSave = prepareUdasForSave();
    //        VOSubscription subscription = createSubscription();
  ...
  public void subscribeToFreeService() throws Exception {
    ...
    createdSubscription.setSubscriptionId(Long.toHexString(System.currentTimeMillis()));
    createdSubscription.setUnitName("default");

    WebserviceTestBase.storeGlobalConfigurationSetting(ConfigurationKey.AUDIT_LOG_ENABLED, true);
    try {
      // when
      createdSubscription =
          subscrServiceForCustomer.subscribeToService(
              createdSubscription, freeService, null, null, null, new ArrayList<VOUda>());
      waitForLogStored();
      // then
      byte[] auditLogs =
          operatorService.getUserOperationLog(operations, fromDate, fromDate + 86400000L);
      verifySubscriptionAndAuditLogs(createdSubscription, freeService, auditLogs);
      ...
      WebserviceTestBase.storeGlobalConfigurationSetting(ConfigurationKey.AUDIT_LOG_ENABLED, false);
    }
  }

  /**
   * The log method will be invoked asynchronously, so some time is needed before retrieving log
   * entries.
   *
  ...
  public void subscribeToService_WithUdas() throws Exception {
    ...
    createdSubscription =
        subscrServiceForCustomer.subscribeToService(
            createdSubscription, freeService, usageLicences, null, null, udasToSave);
    assertEquals(customerUser.getUserId(), createdSubscription.getOwnerId());
  ...
  public void modifySubscription_WithUdas() throws Exception {
    ...
    createdSubscription =
        subscrServiceForCustomer.subscribeToService(
            createdSubscription, freeService, usageLicences, null, null, new ArrayList<VOUda>());
    String newSubscriptionId = "NewSubscrId" + Long.toHexString(System.currentTimeMillis());
    ...
    VOSubscriptionDetails modifiedSubscription =
        subscrServiceForCustomer.modifySubscription(createdSubscription, null, udasToSave);

    // then
    assertEquals(subscriptionManager.getUserId(), modifiedSubscription.getOwnerId());
    verifyUdas(udasToSave);
  }
  ...
  public void modifySubscription_AddOwnerWithoutRole() throws Exception {
    ...
    // when
    try {
      subscrServiceForCustomer.modifySubscription(createdSubscription, null, null);
      // then
      fail();
    } catch (Exception e) {
      assertTrue(e.getMessage().contains("Add subscription owner failed."));
      throw e;
    }
  }
  ...
  public void modifySubscriptionBug11736() throws Exception {
    ...
    createdSubscription.setOwnerId(subscriptionManager.getUserId());
    createdSubscription =
        subscrServiceForSubManager.subscribeToService(
            createdSubscription, freeService, usageLicences, null, null, new ArrayList<VOUda>());
    ...
    VOSubscriptionDetails modified =
        subscrServiceForSubManager.modifySubscription(createdSubscription, null, null);
    // then
  ...
  public void modifySubscriptionBug11736Admin() throws Exception {
    ...
    createdSubscription.setOwnerId(customerUser.getUserId());
    VOSubscriptionDetails modified =
        subscrServiceForOrgAdmin.modifySubscription(createdSubscription, null, null);
    // then
    String ownerId = modified.getOwnerId();
  ...
  public void modifySubscription_AddNoneExsitUserAsOwner() throws Exception {
    ...
    try {
      ...
      assertTrue(
          e.getMessage()
              .contains("Could not find object of type 'USER' with business key 'TestId'"));
      throw e;
    }
  }
  ...
  public void modifySubscription_RemoveOwner() throws Exception {
    ...
    // when
    VOSubscription modifiedSubscription =
        subscrServiceForCustomer.modifySubscription(createdSubscription, null, null);
    // then
  ...
  @Test(expected = OperationNotPermittedException.class)
  public void modifySubscription_SubscriptionManagerNoPermissions() throws Exception {
    // given
    createdSubscription = createSubscription();
    createdSubscription =
        subscrServiceForCustomer.subscribeToService(
  ...
  @Test
  public void modifySubscription_SubscriptionManagerPermissions() throws Exception {
    // given
    createdSubscription = createSubscription();
    createdSubscription =
        subscrServiceForSubManager.subscribeToService(
    ...
    // then
    List<VOSubscription> subList = subscrServiceForSubManager.getSubscriptionsForOrganization();
    VOSubscription subscription = getSubscriptionInList(createdSubscription, subList);
    assertNotNull(subscription);
    assertEquals("newOrderNumber", subscription.getPurchaseOrderNumber());
  }

  private VOSubscription getSubscriptionInList(
      VOSubscription subToGet, List<VOSubscription> subList) {
  ...
  public void modifySubscription_UpdateParameterOK() throws Exception {
    ...
    VOSubscriptionDetails subDetails =
        subscrServiceForCustomer.modifySubscription(createdSubscription, paramsForUpdate, null);

    assertEquals(subscriptionManager.getUserId(), subDetails.getOwnerId());
    assertEquals(createdSubscription.getSubscriptionId(), subDetails.getSubscriptionId());
    assertEquals(newParaValue, subDetails.getSubscribedService().getParameters().get(0).getValue());
  }
  ...
  public void asynModifySubscription() throws Exception {
    ...
    subscrServiceForSupplier.completeAsyncSubscription(
        createdSubscription.getSubscriptionId(), customerOrg.getOrganizationId(), instance);
    refreshSubscriptionDetails();
    String newSubscriptionId = "NewSubscrId" + Long.toHexString(System.currentTimeMillis());
    createdSubscription.setSubscriptionId(newSubscriptionId);
    // when
    createdSubscription =
        subscrServiceForCustomer.modifySubscription(
            createdSubscription, null, new ArrayList<VOUda>());
    subscrServiceForSupplier.completeAsyncModifySubscription(
        createdSubscription.getSubscriptionId(), customerOrg.getOrganizationId(), instance);

    // then
    assertEquals(SubscriptionStatus.PENDING_UPD, createdSubscription.getStatus());
    createdSubscription = subscrServiceForCustomer.getSubscriptionDetails(newSubscriptionId);
    assertTrue(createdSubscription.getProvisioningError().isEmpty());
  ...
  public void asynModifySubscription_PendingUpdate() throws Exception {
    ...
    refreshSubscriptionDetails();
    newSubscriptionId = "NewSubscrId" + Long.toHexString(System.currentTimeMillis());
    createdSubscription.setSubscriptionId(newSubscriptionId);
    // when
    try {
      createdSubscription =
    ...
    // then
    assertEquals(SubscriptionStatus.PENDING_UPD, createdSubscription.getStatus());
    createdSubscription = subscrServiceForCustomer.getSubscriptionDetails(oldSubscriptionId);
  ...
  public void completeAsynUpgradeSubscription() throws Exception {
    ...
    ServiceProvisioningService spsSvc =
        ServiceFactory.getDefault()
            .getServiceProvisioningService(
                setup.getSupplierUserKey(), WebserviceTestBase.DEFAULT_PASSWORD);
    spsSvc.setCompatibleServices(asynFreeService, Collections.singletonList(asynFreeService2));
    asynFreeService = setup.activateService(asynFreeService);
    createdSubscription = createSubscription();
    createdSubscription =
        subscrServiceForCustomer.subscribeToService(
            createdSubscription, asynFreeService, null, null, null, new ArrayList<VOUda>());
    VOInstanceInfo instance = new VOInstanceInfo();
    instance.setAccessInfo("PLATFORM");
    instance.setBaseUrl(asynFreeService.getBaseURL());
    instance.setInstanceId(createdSubscription.getSubscriptionId());
    ...
    refreshSubscriptionDetails();
    // when
    subscrServiceForCustomer.upgradeSubscription(
        createdSubscription,
        asynFreeService2,
        customerPaymentInfo,
  ...
  public void abortAsynUpgradeSubscription() throws Exception {
    ...
    subscrServiceForSupplier.abortAsyncUpgradeSubscription(
        createdSubscription.getSubscriptionId(), customerOrg.getOrganizationId(), null);
    // then
    assertEquals(SubscriptionStatus.PENDING_UPD, createdSubscription.getStatus());
    refreshSubscriptionDetails();
    assertEquals(SubscriptionStatus.ACTIVE, createdSubscription.getStatus());
  ...
  public void upgradeSubscription_WithUdas() throws Exception {
    ...
    ServiceProvisioningService spsSvc =
        ServiceFactory.getDefault()
            .getServiceProvisioningService(
                setup.getSupplierUserKey(), WebserviceTestBase.DEFAULT_PASSWORD);
    spsSvc.setCompatibleServices(freeService, Collections.singletonList(chargeableService));
  ...
  @Test(expected = OperationNotPermittedException.class)
  public void upgradeSubscription_SubscriptionManagerNoPermissions() throws Exception {
    // given
    freeService = setup.deactivateService(freeService);
    ServiceProvisioningService spsSvc =
        ServiceFactory.getDefault()
    ...
    // when
    subscrServiceForSubManager.upgradeSubscription(
        createdSubscription,
        chargeableService,
        customerPaymentInfo,
        customerBillingContact,
  ...
  @Test
  public void upgradeSubscription_SubscriptionManagerPermissions() throws Exception {
    // given
    List<VOUda> udasToSave = prepareUdasForSave();
    freeService = setup.deactivateService(freeService);
    ServiceProvisioningService spsSvc =
  ...
  public void subscribeToChargeableService_NoPayment() throws Exception {
    SubscriptionService subSvc = ServiceFactory.getDefault().getSubscriptionService();
    VOSubscription subscription = new VOSubscription();
    subscription.setSubscriptionId(Long.toHexString(System.currentTimeMillis()));
    PaymentInformationException expected = null;

    try {
      subSvc.subscribeToService(
          subscription,
          chargeableService,
          null,
          null,
      ...
      assertEquals("ex.PaymentInformationException", faultInfo.getMessageKey());
    }
    assertNotNull("Exception expected", expected);
  }

  @Test
  public void subscribeToChargeableService_NoBillingContact() throws Exception {
    VOSubscription subscription = new VOSubscription();
    subscription.setSubscriptionId(Long.toHexString(System.currentTimeMillis()));
    PaymentInformationException expected = null;
    ...
    try {
      subscrServiceForCustomer.subscribeToService(
          subscription, chargeableService, null, customerPaymentInfo, null, new ArrayList<VOUda>());
    } catch (PaymentInformationException e) {
      expected = e;
      ApplicationExceptionBean faultInfo = expected.getFaultInfo();
      assertNotNull(faultInfo);
  ...
  public void subscribeToChargeableService_WrongUser() throws Exception {
    ...
    try {
      ...
      ApplicationExceptionBean faultInfo = expected.getFaultInfo();
      assertNotNull(faultInfo);
      assertEquals("ex.OperationNotPermittedException", faultInfo.getMessageKey());
    }
  ...
  public void subscribeToChargeableService_WrongService() throws Exception {
    ...
    try {
      ...
      ApplicationExceptionBean faultInfo = expected.getFaultInfo();
      assertNotNull(faultInfo);
      assertEquals("ex.ObjectNotFoundException.SERVICE", faultInfo.getMessageKey());
  ...
  public void subscribeToChargeableService_DuplicateID() throws Exception {
    ...
    try {
      ...
      ApplicationExceptionBean faultInfo = expected.getFaultInfo();
      assertNotNull(faultInfo);
      assertEquals("ex.NonUniqueBusinessKeyException.SUBSCRIPTION", faultInfo.getMessageKey());
      assertNotNull(faultInfo.getMessageParams());
      assertEquals(1, faultInfo.getMessageParams().length);
      assertEquals(createdSubscription.getSubscriptionId(), faultInfo.getMessageParams()[0]);
      assertEquals(ClassEnum.SUBSCRIPTION, expected.getDomainObjectClassEnum());
    }
  ...
  public void getCustomersForSubscriptionId() throws Exception {
    ...
    // Check the customers
    List<VOOrganization> customers =
        subscrServiceForSupplier.getCustomersForSubscriptionId(
            createdSubscription.getSubscriptionId());
    assertEquals("Wrong numer of customers", 1, customers.size());
    assertEquals(
        "Wrong customer organization ID",
        customerOrg.getOrganizationId(),
  ...
  public void getCustomersSubscriptionForSupplier() throws Exception {
    ...
    List<VOSubscriptionIdAndOrganizations> customerSubscriptions =
        subscrServiceForSupplier.getCustomerSubscriptions();

    assertEquals("Wrong numer of customer subscriptions", 1, customerSubscriptions.size());
    assertEquals(
        "Wrong subscriptionId",
        createdSubscription.getSubscriptionId(),
        customerSubscriptions.get(0).getSubscriptionId());
    assertEquals(
        "Wrong customer organization ID",
        customerOrg.getOrganizationId(),
        customerSubscriptions.get(0).getOrganizations().get(0).getOrganizationId());
  }
  ...
  public void getSubscriptionForCustomer() throws Exception {
    ...
    subscrServiceForCustomer.subscribeToService(
        createdSubscription, freeService, usageLicences, null, null, new ArrayList<VOUda>());
    VOSubscriptionDetails subscriptionDetails =
        subscrServiceForSupplier.getSubscriptionForCustomer(
            customerOrg.getOrganizationId(), createdSubscription.getSubscriptionId());
    assertNotNull(subscriptionDetails);
    List<VOUsageLicense> usageLicences = subscriptionDetails.getUsageLicenses();
    assertEquals("Wrong number of usage licenses", 1, usageLicences.size());
    assertEquals(
        "Wrong userId", customerUser.getUserId(), usageLicences.get(0).getUser().getUserId());
  }
  ...
  public void getSubscriptionForCustomerUnauthorizedOrg() throws Exception {
    ...
    try {
      ...
      subscrServiceForReseller.getSubscriptionForCustomer(
          customerOrgId, createdSubscription.getSubscriptionId());
  ...
  public void getSubscriptionForCustomerWrongOrgId() throws Exception {
    try {
      subscrServiceForSupplier.getSubscriptionForCustomer("WRONG_ORG_ID", "4711");
  ...
  @Test
  public void getSubscriptionForCustomerWrongSubscriptionId() throws Exception {
    try {
      subscrServiceForSupplier.getSubscriptionForCustomer(
  ...
  public void getSubscriptionIdentifiersForSupplier() throws Exception {
    ...
    List<String> subscriptions = subscrServiceForSupplier.getSubscriptionIdentifiers();
    assertEquals("Wrong numer of subscriptions", 1, subscriptions.size());
    assertEquals(
        "Wrong subscription ID", createdSubscription.getSubscriptionId(), subscriptions.get(0));
  }
  ...
  public void getSubscriptionsForCurrentUser() throws Exception {
    ...
    List<VOUserSubscription> subscriptions =
        subscrServiceForCustomer.getSubscriptionsForCurrentUser();
    assertEquals("Wrong numer of subscriptions", 1, subscriptions.size());
    assertEquals(
        "Wrong subscription id",
        createdSubscription.getSubscriptionId(),
        subscriptions.get(0).getSubscriptionId());
    assertEquals(
        "Wrong supplier org name", SUPPLIER_ORG_NAME, subscriptions.get(0).getSellerName());
    assertEquals(
        "Wrong user id",
        customerUser.getUserId(),
        subscriptions.get(0).getLicense().getUser().getUserId());
    assertEquals(
        "Wrong user org id",
        customerOrg.getOrganizationId(),
    ...
    subscriptions = subscrServiceForSupplier.getSubscriptionsForCurrentUser();
  ...
  public void getSubscriptionsForUser() throws Exception {
    ...
    List<VOUserSubscription> subscriptions =
        subscrServiceForCustomer.getSubscriptionsForUser(customerUser);
    assertEquals("Wrong numer of subscriptions", 1, subscriptions.size());
    assertEquals(
        "Wrong subscription id",
        createdSubscription.getSubscriptionId(),
  ...
  public void getSubscriptionsForUserDifferentOrganization() throws Exception {
    ...
    try {
      ...
      assertTrue("Wrong exception message", e.getMessage().contains(exMsg));
    }
  }
  ...
  public void getSubscriptionsForOrganization() throws Exception {
    ...
    List<VOSubscription> subscriptions = subscrServiceForCustomer.getSubscriptionsForOrganization();
    ...
    assertEquals("Wrong numer of active subscriptions", 1, activeSubscriptions);
    if (activeSubscription != null) {
      ...
      assertEquals(
          "Wrong supplier org name", SUPPLIER_ORG_NAME, activeSubscription.getSellerName());
      assertEquals("Wrong number of users", 1, activeSubscription.getNumberOfAssignedUsers());
    } else {
      fail("No active subscription");
    }

    // The supplier org itself has no active subscription
    subscriptions = subscrServiceForSupplier.getSubscriptionsForOrganization();
    ...
    assertEquals("Wrong numer of active subscriptions", 0, activeSubscriptions);
  }
  ...
  public void getSubscriptionsForOrganizationWithFilter() throws Exception {
    ...
    List<VOSubscription> subscriptions =
        subscrServiceForCustomer.getSubscriptionsForOrganizationWithFilter(requiredStatus);
    assertEquals("Wrong numer of subscriptions", 0, subscriptions.size());
    ...
    assertEquals(
        "Wrong supplier org name", SUPPLIER_ORG_NAME, subscriptions.get(0).getSellerName());
    assertEquals("Wrong number of users", 1, subscriptions.get(0).getNumberOfAssignedUsers());
  }

  @Test(expected = OperationNotPermittedException.class)
  public void getServiceRolesForSubscription_SubscriptionManagerNoPermissions() throws Exception {
    // given
    createdSubscription = createSubscription();
    createdSubscription =
        subscrServiceForCustomer.subscribeToService(
  ...
  @Test
  public void getServiceRolesForSubscription_SubscriptionManagerPermissions() throws Exception {
    // given
    createdSubscription = createSubscription();
    createdSubscription =
        subscrServiceForSubManager.subscribeToService(
    ...
    // when
    List<VORoleDefinition> roleDefList =
        subscrServiceForSubManager.getServiceRolesForSubscription(subId);
  ...
  @Test(expected = OperationNotPermittedException.class)
  public void getSubscriptionDetails_SubscriptionManagerNoPermissions() throws Exception {
    // given
    createdSubscription = createSubscription();
    createdSubscription =
        subscrServiceForCustomer.subscribeToService(
  ...
  @Test
  public void getSubscriptionDetails_SubscriptionManagerPermissions() throws Exception {
    // given
    createdSubscription = createSubscription();
    createdSubscription =
        subscrServiceForSubManager.subscribeToService(
    ...
    // when
    VOSubscription subDetails = subscrServiceForSubManager.getSubscriptionDetails(subId);
  ...
  @Test(expected = OperationNotPermittedException.class)
  public void getUpgradeOptions_SubscriptionManagerNoPermissions() throws Exception {
    // given
    createdSubscription = createSubscription();
    createdSubscription =
        subscrServiceForCustomer.subscribeToService(
  ...
  @Test
  public void getUpgradeOptions_SubscriptionManagerPermissions() throws Exception {
    // given
    createdSubscription = createSubscription();
    createdSubscription =
        subscrServiceForSubManager.subscribeToService(
    ...
    // when
    List<VOService> serviceListSubManager = subscrServiceForSubManager.getUpgradeOptions(subId);

    // then
    List<VOService> serviceListAdmin = subscrServiceForCustomer.getUpgradeOptions(subId);
    assertEquals(serviceListAdmin.size(), serviceListSubManager.size());
  }

  @Test(expected = OperationNotPermittedException.class)
  public void modifySubscriptionPaymentData_SubscriptionManagerNoPermissions() throws Exception {
    // given
    createdSubscription = createSubscription();
    createdSubscription =
        subscrServiceForCustomer.subscribeToService(
    ...
    subscrServiceForSubManager.modifySubscriptionPaymentData(
        createdSubscription, new VOBillingContact(), new VOPaymentInfo());
  }

  @Test
  public void modifySubscriptionPaymentData_SubscriptionManagerPermissions() throws Exception {
    // given
    createdSubscription = createSubscription();
    ...
    newBillingContact = accountServiceForCustomer.saveBillingContact(newBillingContact);

    // when
    VOSubscriptionDetails voSubDetails =
        subscrServiceForSubManager.modifySubscriptionPaymentData(
            createdSubscription, newBillingContact, customerPaymentInfo);

    // then
    assertEquals(newBillingContact.getId(), voSubDetails.getBillingContact().getId());
  }

  @Test(expected = OperationNotPermittedException.class)
  public void addRevokeUser_SubscriptionManagerNoPermissions() throws Exception {
    // given
    createdSubscription = createSubscription();
    createdSubscription =
        subscrServiceForCustomer.subscribeToService(
    ...
    // when
    subscrServiceForSubManager.addRevokeUser(
        subId, new ArrayList<VOUsageLicense>(), new ArrayList<VOUser>());
  }
  ...
  public void addRevokeUser_SubscriptionManagerPermissions() throws Exception {
    ...
    // when
    subscrServiceForSubManager.addRevokeUser(
        subId, new ArrayList<VOUsageLicense>(), Arrays.asList((VOUser) customerUser));

    // then
    List<VOUserSubscription> subList =
        subscrServiceForCustomer.getSubscriptionsForUser(customerUser);
  ...
  public void hasCurrentUserSubscriptions() throws Exception {
    ...
    assertTrue(
        "Current user has no subscriptions",
        subscrServiceForCustomer.hasCurrentUserSubscriptions());

    assertFalse(
        "Supplier has subscriptions", subscrServiceForSupplier.hasCurrentUserSubscriptions());
  }
  ...
  public void modifySubscription() throws Exception {
    ...
    if (activeSubscription != null) {
      assertEquals(
          "Wrong subscription id", newSubscriptionId, activeSubscription.getSubscriptionId());
    } else {
      fail("No active subscription");
    }
  }
  ...
  public void modifyUnknownSubscription() throws Exception {
    ...
    try {
      subscrServiceForCustomer.modifySubscription(subscription, null, new ArrayList<VOUda>());
      fail("ObjectNotFoundException expected");
    } catch (ObjectNotFoundException e) {
      assertTrue(
          "Wrong exception message",
  ...
  public void subscribeToService_TriggerProcessPending() throws Exception {
    ...
    subscription =
        subSvc.subscribeToService(
            subscription, freeService, null, null, null, new ArrayList<VOUda>());

    VOSubscription subscription2 = new VOSubscription();
    subscription2.setSubscriptionId(subscriptionId);

    subSvc.subscribeToService(subscription2, freeService, null, null, null, new ArrayList<VOUda>());
  }
  ...
  @Test(expected = OperationNotPermittedException.class)
  public void unsubscribeFromService_SubscriptionManagerNoPermissions() throws Exception {
    // given
    createdSubscription = createSubscription();
    createdSubscription =
        subscrServiceForCustomer.subscribeToService(
  ...
  @Test
  public void unsubscribeFromService_SubscriptionManagerPermissions() throws Exception {
    // given
    createdSubscription = createSubscription();
    createdSubscription =
        subscrServiceForSubManager.subscribeToService(
    ...
    List<VOSubscription> subList =
        subscrServiceForSubManager.getSubscriptionsForOrganizationWithFilter(requiredStatus);
  ...
  public void modifySubscription_TriggerProcessPending() throws Exception {
    ...
    subscription =
        subSvc.subscribeToService(
            subscription, freeService, null, null, null, new ArrayList<VOUda>());
    subSvc.modifySubscription(subscription, new ArrayList<VOParameter>(), new ArrayList<VOUda>());
  ...
  public void upgradeSubscription_TriggerProcessPending() throws Exception {
    ...
    setup.createTriggerDefinition(triggerDef, String.valueOf(customerUser.getKey()));
  ...
  public void checkReceivedMailContent_CustomerLine() throws Exception {
    ...
    checkTag("[Customer]", customerOrg.getName(), customerOrg.getOrganizationId());
  }
  ...
  public void checkReceivedMailContent_ServiceLine() throws Exception {
    ...
    mailContentReceived = readContentFromServiceTicketMail();
    assertNotNull(mailContentReceived);
    checkTag("[Marketable Service]", freeService.getName(), freeService.getServiceId());
  }
  ...
  public void checkReceivedMailContent_TechnicalServicLine() throws Exception {
    ...
    checkTag(
        "[Technical Service]",
        freeService.getTechnicalId(),
        setup.getVoTechnicalService().getTechnicalServiceId(),
        setup.getVoTechnicalService().getTechnicalServiceBuildId());
  }
  ...
  public void reportIssueNoEmail() throws Exception {
    ...
    VOOperatorOrganization operatorOrg =
        operatorService.getOrganization(supplier.getOrganizationId());
  ...
  public void reportIssueForUserNotCustomer() throws Exception {
    ...
    IdentityService identityService = ServiceFactory.getDefault().getIdentityService();
    VOUserDetails voUser = factory.createUserVO(WebserviceTestBase.createUniqueKey());
    ...
    SubscriptionService subscrService =
        ServiceFactory.getDefault()
            .getSubscriptionService(
                String.valueOf(
                    WebserviceTestBase.readLastMailAndSetCommonPassword(voUser.getUserId())),
                WebserviceTestBase.DEFAULT_PASSWORD);

    // User does not have a subscription with this given id
    // --> ObjectNotFoundException
    subscrService.reportIssue(subscriptionID, mailSubjectToSend, mailContentToSend);
  }
  ...
  public void reportIssueTooLongContent() throws Exception {
    ...
    // call the reportIssue method
    subscrServiceForCustomer.reportIssue(subscriptionID, mailSubjectToSend, longContent);
  }

  @Test(expected = ObjectNotFoundException.class)
  public void reportIssueSubscriptionNotFound() throws Exception {
    // call the reportIssue method with "AnyID" as subscriptionID
    subscrServiceForCustomer.reportIssue("AnyID", mailSubjectToSend, mailContentToSend);
  }
  ...
  public void reportIssue_SubsciptionManagerNoPermissions() throws Exception {
    ...
    // when
    subscrServiceForSubManager.reportIssue(subscriptionID, mailSubjectToSend, "someText");
  }
  ...
  public void getServiceOperationParameterValues() throws Exception {
    VOTechnicalServiceOperation operation = tpWithOp.getTechnicalServiceOperations().get(0);
    createdSubscription = createSubscription();
    createdSubscription =
        subscrServiceForCustomer.subscribeToService(
            createdSubscription, svcWithOp, usageLicences, null, null, new ArrayList<VOUda>());

    List<VOServiceOperationParameterValues> values =
        subscrServiceForCustomer.getServiceOperationParameterValues(createdSubscription, operation);
    ...
    assertEquals(Arrays.asList("Server 1", "Server 2", "Server 3"), op.getValues());
  }
  ...
  public void executeServiceSubscription() throws Exception {
    ...
    // when
    subscrServiceForCustomer.executeServiceOperation(createdSubscription, operation);
  }

  @Test(expected = ValidationException.class)
  public void executeServiceSubscription_MandatoryValueMissing() throws Exception {
    // given
    VOTechnicalServiceOperation operation = tpWithOp.getTechnicalServiceOperations().get(0);
    createdSubscription = createSubscription();
    createdSubscription =
  ...
  @Test(expected = ValidationException.class)
  public void executeServiceSubscription_MandatoryValueIsNull() throws Exception {
    // given
    VOTechnicalServiceOperation operation = tpWithOp.getTechnicalServiceOperations().get(0);
    createdSubscription = createSubscription();
    createdSubscription =
  ...
  public void updateAccessInfo() throws Exception {
    ...
    subscrServiceForSupplier.updateAccessInformation(
        createdSubscription.getSubscriptionId(), customerOrg.getOrganizationId(), instanceInfo);

    // then
    VOSubscription subDetails =
        subscrServiceForCustomer.getSubscriptionDetails(createdSubscription.getSubscriptionId());
  ...
  public void abortAsyncCreateSubscription() throws Exception {
    ...
    subscrServiceForSupplier.abortAsyncSubscription(
        createdSubscription.getSubscriptionId(), customerOrg.getOrganizationId(), reason);
  ...
  void checkTag(String tag, String... values) {
    ...
    if (idx < 0) fail(mailContentReceived + " " + " does not contain required tag" + tag + ".");
    ...
    for (String value : values) {
      assertTrue(
          line + " " + " does not contain required value " + value + ".", line.contains(value));
    }
  }

  private void verifyUdas(List<VOUda> udas) throws Exception {
    // get the udas has been saved
    List<VOUda> resultUdas =
        accountService.getUdas(UdaTargetType.CUSTOMER.name(), customerOrg.getKey());
    // compare the uda's value with the created value
    assertEquals(udas.get(0).getTargetObjectKey(), resultUdas.get(0).getTargetObjectKey());
  ...
  private void verifySubscriptionAndAuditLogs(
      VOSubscription subscription, VOService freeService, byte[] auditLog) throws Exception {
    String auditLogs = new String(auditLog, "UTF-8");
    assertNotNull(auditLogs);
    assertNotNull(subscription);
    assertEquals(Boolean.TRUE, new Boolean(auditLogs.contains(subscription.getSubscriptionId())));
    assertEquals(Boolean.TRUE, new Boolean(auditLogs.contains(freeService.getServiceId())));
    assertEquals(Boolean.TRUE, new Boolean(auditLogs.contains(freeService.getName())));
  }
  ...
  private List<VOUda> prepareUdasForSave() throws Exception {
    ...
    accountService.saveUdaDefinitions(udaDefinitions, new ArrayList<VOUdaDefinition>());
    // get the saved udaDefinition
    udaDefinitions = accountService.getUdaDefinitions();
    // create uda object
    VOUda uda = createVOUda(udaDefinitions.get(0), "UdaValue1", customerOrg.getKey());
    List<VOUda> udasToSave = new ArrayList<VOUda>();
    udasToSave.add(uda);
    return udasToSave;
  }

  private VOUdaDefinition createVOUdaDefinition(
      String targetType,
      String udaId,
      String defaultValue,
  ...
  private VOUda createVOUda(VOUdaDefinition def, String value, long targetObjectKey) {
    VOUda uda = new VOUda();
   ...
   * @return
   */
  private VOParameterDefinition getParameterDefinitionByName(VOTechnicalService ts, String name) {
    List<VOParameterDefinition> paramDefinitions = ts.getParameterDefinitions();
    VOParameterDefinition parameterDefinition = new VOParameterDefinition();
    for (VOParameterDefinition voParamDef : paramDefinitions) {
      if (voParamDef.getParameterId().equals(name)) parameterDefinition = voParamDef;
    }
   ...
   * @throws Exception
   */
  private VOSubscription createAndSubscribeToServiceWithParameter(String paraValue)
      throws Exception {
    technicalServiceWithParameter = createTechnicalService("tp1");
    List<VOParameter> params =
        createParametersForTechnicalService(technicalServiceWithParameter, paraValue);
   ...
   * @throws Exception
   */
  private VOService createServiceWithParameter(List<VOParameter> params) throws Exception {
    return setup.createAndActivateService(
        "marketPlace", technicalServiceWithParameter, mpLocal, params);
  }
  ...
  private List<VOParameter> createParametersForTechnicalService(
    ...
    VOParameter standardParam =
        createParameterForTechnicalService(voTechnicalService, value, standardDefinitionID);
    VOParameter onetimeParam =
        createParameterForTechnicalService(voTechnicalService, value, onetimeDefinitionID);
  ...
  private VOParameter createParameterForTechnicalService(
    ...
    VOParameterDefinition standardDef = getParameterDefinitionByName(voTechnicalService, defID);
   ...
   * @throws Exception
   */
  private VOTechnicalService createTechnicalService(String serviceID) throws Exception {
   ...
   * @throws Exception
   */
  private List<VOParameter> getParametersForModifySubscription(String value) throws Exception {
   ...
   * @throws Exception
   */
  private List<VOParameter> getParametersForModifySubscription(
      int index, String value, VOSubscription subscription) throws Exception {
   ...
   * @throws Exception
   */
  private VOParameter getParameterFromSubscription(int index, VOSubscription subscription)
      throws Exception {
    VOSubscriptionDetails voSubscriptionDetails =
        subscrServiceForCustomer.getSubscriptionDetails(subscription.getSubscriptionId());
    VOParameter param = voSubscriptionDetails.getSubscribedService().getParameters().get(index);
    return param;
  }

  /** create VOUserDetail object */
  private static VOUserDetails createVOUser() throws Exception {
    VOUserDetails voUser = factory.createUserVO(Long.toHexString(System.currentTimeMillis()));
    voUser.setOrganizationId(customerOrg.getOrganizationId());
  ...
  private List<VOLocalizedText> getProvisioningErrorMessages() {
    ...
    VOLocalizedText enErrorMsg = new VOLocalizedText("en", ERROR_ABORT_ASYNC_SUB_EN);
    VOLocalizedText deErrorMsg = new VOLocalizedText("de", ERROR_ABORT_ASYNC_SUB_DE);
    VOLocalizedText jaErrorMsg = new VOLocalizedText("ja", ERROR_ABORT_ASYNC_SUB_JA);
