...
public class OnBehalfActingServiceWSTest {
  ...
  /**
   * In order to make the condition for onBehalfActing, the subscribable service is created by
   * hostUser('PaaS'), and technology provider subscribe it. If 'allowingOnBehalfActing' flag is
   * true, host user can create OnBehalf user of the technology provider.
   *
   * @param allowOnBehalfActing the flag setting 'allowBehalfActing' of technical service created
   * @throws Exception
   */
  private void createMarketableServiceAndSubscribe(boolean allowOnBehalfActing) throws Exception {
    ...
    subscription = factory.createSubscriptionVO("sub_" + WebserviceTestBase.createUniqueKey());
    ...
    SubscriptionService subscriptionSV =
        ServiceFactory.getDefault().getSubscriptionService(techProviderUserKey, commonUserPWD);
    subscription =
        subscriptionSV.subscribeToService(
            subscription, activatedService, users, null, null, new ArrayList<VOUda>());
  }

  private VOService createMarketableService(boolean allowOnBehalfActing) throws Exception {
    ServiceProvisioningService serviceProvisioningSV =
        ServiceFactory.getDefault().getServiceProvisioningService(hostUserKey, commonUserPWD);

    String tsxml =
        TSXMLForWebService.createTSXMLWithAllowingOnBehalfActingConnectable(
            Boolean.toString(allowOnBehalfActing));
    VOTechnicalService voTechnicalService =
        WebserviceTestBase.createTechnicalService(tsxml, serviceProvisioningSV);

    VOServiceDetails marketableServiceDetail =
        serviceProvisioningSV.createService(
            voTechnicalService,
            factory.createMarketableServiceVO("ms_" + WebserviceTestBase.createUniqueKey()),
            null);
    WebserviceTestBase.publishToMarketplace(
        marketableServiceDetail, true, mpSrvAsSupplier, localMarketplace);
    VOPriceModel priceModel = factory.createPriceModelVO();
    marketableServiceDetail =
        serviceProvisioningSV.savePriceModel(marketableServiceDetail, priceModel);
    VOService service = serviceProvisioningSV.activateService(marketableServiceDetail);
    return service;
  }

  /**
   * Test for 'Blue Line'. When subscription of technical service having 'true'
   * allowingOnBehalfActing flag has existed, creating onBehalfUser must be successful. After
   * subscription is unsubscribed, logging in by onBehalfUser and creating onBehalfUser must be
   * failed.
   *
   * @throws Exception
   */
  ...
  public void testConditionOfCreateBehalfUser() throws Exception {
    ...
    IdentityService identifySV =
        ServiceFactory.getDefault().getIdentityService(hostUserKey, commonUserPWD);
    VOUserDetails onBehalfUser =
        identifySV.createOnBehalfUser(techProviderOrg.getOrganizationId(), commonUserPWD);
    ...
    SessionService sessionSV =
        ServiceFactory.getDefault().getSessionService(onBehalfUserKey, commonUserPWD);
    ...
    sessionSV = ServiceFactory.getDefault().getSessionService(onBehalfUserKey, commonUserPWD);
    subscriptionId = subscription.getSubscriptionId();
    try {
      sessionSV.createServiceSession(subscription.getKey(), sessionId, "token2");
      fail();
    } catch (Exception e) {
      // expected
      assertNotNull(e);
    ...
    identifySV = ServiceFactory.getDefault().getIdentityService(hostUserKey, commonUserPWD);
    try {
      identifySV.createOnBehalfUser(techProviderOrg.getOrganizationId(), commonUserPWD);
      fail();
  ...
  /**
   * Test for 'Blue Line', however in this test onBehalfUser create another subscription. At that
   * case, if first subscription is deleted, the onBehalfUser must not be deleted and another
   * onBehalfUser also can be made.
   *
   * @throws Exception
   */
  @Test
  public void testCreateTwoSubscription() throws Exception {
    ...
    VOSubscription subscription2 =
        factory.createSubscriptionVO("sub2_" + WebserviceTestBase.createUniqueKey());
    List<VOUsageLicense> users = new ArrayList<VOUsageLicense>();
    users.add(factory.createUsageLicenceVO(techProviderUserId));
    SubscriptionService subscriptionSV =
        ServiceFactory.getDefault().getSubscriptionService(onBehalfUserKey, commonUserPWD);
    subscription2 =
        subscriptionSV.subscribeToService(
            subscription2, activatedService, users, null, null, new ArrayList<VOUda>());

    // By OnBehalfUser: unsubscribe the first subscription
    String subscriptionId = subscription.getSubscriptionId();
    subscriptionSV =
        ServiceFactory.getDefault()
            .getSubscriptionService(Long.toString(onBehalfUser.getKey()), commonUserPWD);
    subscriptionSV.unsubscribeFromService(subscriptionId);
  ...
  public void testChangeOnBehalfFlag() throws Exception {
    ...
    String tsxml = TSXMLForWebService.createTSXMLWithAllowingOnBehalfActingConnectable("false");
    ServiceProvisioningService serviceProvisioningSV =
        ServiceFactory.getDefault().getServiceProvisioningService(hostUserKey, commonUserPWD);
  ...
  /**
   * Test for 'Blue Line' with 'false' flag. If subscription of technical service has existed but it
   * has 'false' allowingOnBehalfActing flag, creating onBehalfUser must be failed.
   *
   * @throws Exception
   */
  ...
  /**
   * Test for restriction. Without subscription, creating OnBehalfUser must be failed.
   *
   * @throws Exception
   */
  @Test(expected = OperationNotPermittedException.class)
  public void testFailToCreateOnBehalfUserWithoutTechnicalService() throws Exception {
  ...
  public void testFailToCreateOnBehalfUserByOperator() throws Exception {
    ...
    String techProviderOrgId = techProviderOrg.getOrganizationId();
    IdentityService identifySV = ServiceFactory.getDefault().getIdentityService();
    try {
      identifySV.createOnBehalfUser(techProviderOrgId, commonUserPWD);
      fail();
    } catch (Exception e) {
