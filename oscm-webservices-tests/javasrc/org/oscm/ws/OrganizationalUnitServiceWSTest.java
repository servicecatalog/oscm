...
public class OrganizationalUnitServiceWSTest {
  ...
  private static final List<UserRoleType> ROLE_TYPES =
  ...
  public static void setUp() throws Exception {
    ...
    VOOrganizationalUnit unit = unitService.createUnit(unitName, unitDesc, unitRefId);

    userGroup = Converter.convert(unit, VOOrganizationalUnit.class, UserGroup.class);

    USER = identityService.createUser(createUser(), ROLE_TYPES, mpLocal.getMarketplaceId());
  }

  @Test
  @Order(order = 1)
  public void createUnit() throws OperationNotPermittedException, NonUniqueBusinessKeyException {
    // given
    final String unitName = randomString("TestUnit");
    final String unitDesc = randomString("TestDesc");
    final String unitRefId = randomString("refId");
  ...
  public void getOrganizationalUnits()
    ...
    // when
    List<VOOrganizationalUnit> units = unitService.getOrganizationalUnits(null);
  ...
  @Test
  @Order(order = 4)
  public void getOrganizationalUnitsWithPaging() throws OperationNotPermittedException {
    // given
    Pagination pagination = new Pagination(0, 2);

    // when
    List<VOOrganizationalUnit> units = unitService.getOrganizationalUnits(pagination);
  ...
  public void grantUserRoles()
      throws ObjectNotFoundException, NonUniqueBusinessKeyException, MailOperationException,
    ...
    Assert.assertNotNull(unit);
    unitService.revokeUserRoles(USER, Collections.singletonList(UnitRoleType.USER), unit);
    unitService.grantUserRoles(USER, Collections.singletonList(UnitRoleType.ADMINISTRATOR), unit);
  }
  ...
  public void revokeUserRoles()
    ...
    // when and then
    Assert.assertNotNull(unit);
    unitService.revokeUserRoles(USER, Collections.singletonList(UnitRoleType.ADMINISTRATOR), unit);
  }

  @Test
  @Order(order = 7)
  public void deleteUnit()
      throws NonUniqueBusinessKeyException, MailOperationException, ObjectNotFoundException,
  ...
  public void deleteUnitNotExist()
      throws OperationNotPermittedException, DeletionConstraintException, MailOperationException,
  ...
  private static VOUserDetails createUser() throws Exception {
    VOUserDetails userDetails = factory.createUserVO(Long.toHexString(System.currentTimeMillis()));
  ...
  private VOOrganizationalUnit getUnitWithName(String unitName)
    ...
    for (VOOrganizationalUnit unit : unitService.getOrganizationalUnits(null)) {
