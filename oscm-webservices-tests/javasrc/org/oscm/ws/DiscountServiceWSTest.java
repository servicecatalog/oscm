...
public class DiscountServiceWSTest {
  ...
  public void setUp() throws Exception {
    ...
    freeService = (VOServiceDetails) setup.createFreeService("FreeService1");
  ...
  /**
   * Tests the fetching of discount for a free service. Please note that the discount is defined on
   * a customer level, and not a service level. So the returned discount for free of charge services
   * is the discount defined by the supplier (33.24%).
   *
   * @throws Exception
   */
  @Test
  ...
  public void testGetDiscountNotDefined() throws Exception {
    ...
    Assert.assertNull(discountService.getDiscountForService(chargeableService.getKey()));
  }
  ...
  public void testGetDiscountForCustomer() throws Exception {
    ...
    discountService =
        ServiceFactory.getDefault()
            .getDiscountService(setup.getSupplierUserKey(), WebserviceTestBase.DEFAULT_PASSWORD);

    VODiscount voDiscount = discountService.getDiscountForCustomer(customer.getOrganizationId());
    Assert.assertEquals(voDiscount.getValue(), new BigDecimal("33.24"));
    Assert.assertEquals(voDiscount.getStartTime(), Long.valueOf(1));
    Assert.assertEquals(voDiscount.getEndTime(), Long.valueOf(2));
  }
