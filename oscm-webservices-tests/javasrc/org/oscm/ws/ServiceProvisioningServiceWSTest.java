...
public class ServiceProvisioningServiceWSTest {
  ...
  public void savePriceModel_NoPeriod() throws Exception {
    ...
    setup.getServiceProvisioningSrvAsSupplier().savePriceModel(serviceDetails, priceModel);
  }
  ...
  public void savePriceModel_FreePeriod() throws Exception {
    ...
    serviceDetails =
        setup.getServiceProvisioningSrvAsSupplier().savePriceModel(serviceDetails, priceModel);
    assertEquals(
        "Wrong PriceModel.freePeriod found", 5, serviceDetails.getPriceModel().getFreePeriod());
    ...
    serviceDetails =
        setup.getServiceProvisioningSrvAsSupplier().savePriceModel(serviceDetails, priceModel);
    assertEquals(
        "Wrong PriceModel.freePeriod found", 0, serviceDetails.getPriceModel().getFreePeriod());
    priceModel = serviceDetails.getPriceModel();
  ...
  public void savePriceModelForCustomer_FreePeriod() throws Exception {
    ...
    assertEquals(
        "Wrong PriceModel.freePeriod found for customer " + supplier.getOrganizationId(),
        5,
        serviceDetails.getPriceModel().getFreePeriod());
    priceModel = serviceDetails.getPriceModel();
  ...
  public void savePriceModelForSubscription_FreePeriod() throws Exception {
    ...
    VOSubscriptionDetails subscription = setup.createSubscription("Subscr1", service);
    ...
    priceModel = serviceDetails.getPriceModel();
    assertEquals(
        "Wrong PriceModel.freePeriod found for subscription " + subscription.getSubscriptionId(),
        5,
        serviceDetails.getPriceModel().getFreePeriod());
  ...
  @Ignore
  @Test(expected = PriceModelException.class)
  public void savePriceModelForSubscription_ChangePriceModelType() throws Exception {
    VOPriceModel priceModel = createVOPriceModel();
  ...
  public void createInvalidService() throws Exception {
    ServiceProvisioningService provisioningService = setup.getServiceProvisioningSrvAsSupplier();

    try {
      provisioningService.createService(setup.getVoTechnicalService(), new VOService(), null);
      fail("Validation Exception expected because the marketable service has no ID");
    } catch (ValidationException e) {
      assertEquals("Wrong number of exception parameters", 1, e.getMessageParams().length);
      assertEquals("Wrong exception parameter", "serviceId", e.getMessageParams()[0]);
    }
  }
  ...
  public void updateService() throws Exception {
    ...
    VOServiceDetails updatedService = provisioningService.updateService(serviceDetails, null);

    assertEquals(serviceDetails.getServiceId(), updatedService.getServiceId());
    assertEquals(serviceDetails.getConfiguratorUrl(), updatedService.getConfiguratorUrl());
  }
  ...
  public void createTechnicalService_WithOneTimeParameter() throws Exception {
    ...
    for (VOParameterDefinition para : voTS.getParameterDefinitions()) {
      if (para.getParameterId().equals("TEST")) {
        assertEquals(ParameterModificationType.ONE_TIME, para.getModificationType());
      }
    }
  }

  @Test
  public void createTechnicalService_WithNullModificationType() throws Exception {
    voTS = setup.createTechnicalServiceWithParameterDefinition("WebServiceTestExample1.0", null);
    for (VOParameterDefinition para : voTS.getParameterDefinitions()) {
      if (para.getParameterId().equals("TEST")) {
        assertEquals(ParameterModificationType.STANDARD, para.getModificationType());
      }
    }
  }

  @Test
  public void createTechnicalService_WithSTANDARDModificationType() throws Exception {
    voTS =
        setup.createTechnicalServiceWithParameterDefinition(
  ...
  @Test
  public void updateTechnicalServiceParameterDefiniton_WithNoProductExisting() throws Exception {
    voTS =
        setup.createTechnicalServiceWithParameterDefinition(
  ...
  @Test
  public void updateTechnicalServiceParameterDefiniton_WithNoChanges() throws Exception {
    voTS =
        setup.createTechnicalServiceWithParameterDefinition(
            "tp1", ParameterModificationType.ONE_TIME);
    setup.createService("ExampleTrial", voTS, mpLocal);
    setup.createTechnicalServiceWithParameterDefinition("tp1", ParameterModificationType.ONE_TIME);
  }

  @Test(expected = UpdateConstraintException.class)
  public void updateTechnicalServiceParameterDefiniton_WithProductExisting() throws Exception {
    voTS =
        setup.createTechnicalServiceWithParameterDefinition(
            "tp1", ParameterModificationType.ONE_TIME);
    setup.createService("ExampleTrial", voTS, mpLocal);
    setup.createTechnicalServiceWithParameterDefinition("tp1", ParameterModificationType.STANDARD);
  }

  @Test
  public void exportTechnicalServiceParameterDefiniton_CheckModificationType() throws Exception {
    String srvId = "exportTechnicalServiceParameterDefiniton_CheckModificationType".hashCode() + "";
    voTS =
        setup.createTechnicalServiceWithParameterDefinition(
            srvId, ParameterModificationType.ONE_TIME);
    ...
    verifyXmlForModificationType(srvId, xmlString, ParameterModificationType.ONE_TIME);
  }
  ...
  public void createService() throws Exception {
    ...
    List<VOParameterDefinition> voParamDefs = voTechService.getParameterDefinitions();
    ...
    VOServiceDetails voServiceDetails =
        provisioningService.createService(setup.getVoTechnicalService(), voService, null);
  ...
  /**
   * parse the XML content and verify the ModificationType has been exported as expected
   *
   * @param xmlString
   * @throws Exception
   */
  private void verifyXmlForModificationType(
      String srvId, String xmlString, ParameterModificationType currentModificationType)
      throws Exception {
    ...
    Element root =
        (Element)
            XMLConverter.getNodeByXPath(
                document, "//*[local-name(.)=\'TechnicalService\'][@id=\'" + srvId + "\']");
    NodeList serviceNode = root.getChildNodes();
    for (int index = 0; index < serviceNode.getLength(); index++) {
      if (serviceNode.item(index).getNodeName().equals("ParameterDefinition")) {
        // verify parameter definition attribute
        NamedNodeMap parameterDefinitionAttrs = serviceNode.item(index).getAttributes();
        ...
        if (parameterDefinitionAttrs.getNamedItem("id").equals("TEST")) {
          if (currentModificationType == null
              || currentModificationType.equals(ParameterModificationType.STANDARD)) {
            assertNull(parameterDefinitionAttrs.getNamedItem("modificationType"));
          } else {
  ...
  private VOSubscriptionDetails createSubscriptionAndSavaPricemodel(VOPriceModel priceModel)
      throws Exception {
    // create service, price model, customer, subscription
    serviceDetails =
        setup.getServiceProvisioningSrvAsSupplier().savePriceModel(serviceDetails, priceModel);
