...
public class BillingSetupTest {
  ...
  private void createSupplierWith3MP1Currency() throws Exception {
    ...
    VOMarketplace mpLocal =
        mpSrvOperator.createMarketplace(
            factory.createMarketplaceVO(
                supplier.getOrganizationId(), false, "Local Marketplace of Sicher-und-Heil"));
    ...
    VOServiceDetails serviceDetails = setup.createService("ExampleTrial", mpLocal);
    ...
    VOService service = setup.savePriceModelAndActivateService(priceModel, serviceDetails);
    ...
    mpDE =
        mpSrvOperator.createMarketplace(
            factory.createMarketplaceVO(supplier.getOrganizationId(), false, "Marketplace DE"));
    ...
    service = setup.savePriceModelAndActivateService(priceModel, serviceDetails);
    ...
    VOMarketplace mpAT =
        mpSrvOperator.createMarketplace(
            factory.createMarketplaceVO(supplier.getOrganizationId(), false, "Marketplace AT"));
  ...
  private void createSupplierWith1MP2Currencies() throws Exception {
    ...
    mpJP =
        mpSrvOperator.createMarketplace(
            factory.createMarketplaceVO(supplier.getOrganizationId(), false, "Marketplace JP"));
    ...
    VOServiceDetails serviceDetails = setup.createService("Sushi_Service", mpJP);
  ...
  private void createSupplierWith3MP2Currencies() throws Exception {
    ...
    VOMarketplace mpLocal =
        mpSrvOperator.createMarketplace(
            factory.createMarketplaceVO(
                supplier.getOrganizationId(), false, "Local Marketplace of Yamamoto-Line-Ltd"));

    // create technical service
    setup.createTechnicalService();

    // add service and subscription in JPY on local marketplace
    VOServiceDetails serviceDetails = setup.createService("Sake_Service", mpLocal);
    VOPriceModel priceModel = factory.createPriceModelVO("JPY");
    ...
    mpSrvAsJPOwner.addOrganizationsToMarketplace(
        Collections.singletonList(supplier.getOrganizationId()), mpJP.getMarketplaceId());
    ...
    mpSrvAsDEOwner.addOrganizationsToMarketplace(
        Collections.singletonList(supplier.getOrganizationId()), mpDE.getMarketplaceId());
