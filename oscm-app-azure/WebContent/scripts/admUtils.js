...
AdmUtils.stopObserving = function(element, eventName, handler) {
	if (AdmUtils.isFirefoxOrChrome()) {
		element.removeEventListener(eventName, handler, false);
			...
			&& typeof (Event.stopObserving) == 'function') {
...
AdmUtils.pageTo = function(target, value) {
	...
	Event.fire(target, 'rich:datascroller:onscroll', {
...
AdmUtils.setCaretToEnd = function() {
	// find the input field with focus
	var $s = jQuery('input:focus');
	if ($s.length > 0) {
		// cut and paste text to set the caret to the end position
		var $v = $s.val();
		$s.val('').val($v);
	}
}
...
AdmUtils.setFirstStepHeader = function() {
	...
	jQuery('#body_box2 .step:first').css('margin-top', '0px');
}
...
AdmUtils.initUploadPreview = function() {
	...
	jQuery('.fileChooser')
			.bind(
					'change',
					function(w, h) {
						// set preview for modern browsers that support
						// FileReader
						if (typeof (FileReader) !== "undefined") {
							var reader = new FileReader();
							if (typeof (reader.readAsDataURL) !== "undefined") {
								// choose the preview image on the page with the
								// same index as the upload input
								// allows multiple image upload components on a
								// single page
								var ix = jQuery('.fileChooser').index(
										jQuery(this));
								var $preview = jQuery(".previewImage:eq(" + ix
										+ ")");

								reader.onload = function(evt) {
									var img = new Image();
									var previewTarget = evt.target.result;
									img.onload = function(evt) {
										var width = this.width;
										var height = this.height;
										// find the sizing function
										var $preContainer = jQuery(".imagePreview:eq("
												+ ix + ")");
										var preview = eval($preContainer
												.attr("data-size"));
										...
										if (preview.apply(this,
											...
											$preview.fadeIn(600);
											// remove the no image defined text,
											// if available
											var $noImage = jQuery(".noImage",
													$preContainer);
											if ($noImage)
												$noImage.css("display", "none");
											// set image position to absolute to
											// cover place holder text
											// in case it's shown
											$preview
													.css("position", "absolute");
											// expand the container to match the
											// image size
								...
								var readFile = jQuery(this).attr("files")[0];
								var file = reader.readAsDataURL(readFile);
							}
						}
					});
}
...
AdmUtils.initFocus = function(parentId) {
	...
	jQuery(parentSelector + "input").focus(function() {
		AdmUtils.setFocus(this, true);
	});
	jQuery(parentSelector + "input").blur(function() {
		AdmUtils.setFocus(this, false);
	});

	jQuery(parentSelector + "select").focus(function() {
		AdmUtils.setFocus(this, true);
	});
	jQuery(parentSelector + "select").blur(function() {
		AdmUtils.setFocus(this, false);
	});

	jQuery(parentSelector + "textarea").focus(function() {
		AdmUtils.setFocus(this, true);
	});
	jQuery(parentSelector + "textarea").blur(function() {
		AdmUtils.setFocus(this, false);
	});

	jQuery("div[id$='focusButtons'] > span > a").focus(function() {
		AdmUtils.setFocus(this, true);
	});
	jQuery("div[id$='focusButtons'] > span > a").blur(function() {
		AdmUtils.setFocus(this, false);
	});

	jQuery(".pager select").unbind('focus');
	jQuery(".pager input").unbind('focus');
}
...
AdmUtils.richEditorInitFocus = function(e) {
	ed = tinyMCE.get(e.id + 'TextArea');
	var t = document.getElementById(e.id);

	tinymce.dom.Event.add(ed.getWin(), 'focus', function() {
		// check if the rich editor has a class defined which means it is
		// disabled
		// see bug 6620
		if (tinyMceParams.body_class) {
			return;
		}
		if (t) {
			AdmUtils.setFocus(t, true);
		}
	});
	tinymce.dom.Event.add(ed.getWin(), 'blur', function() {
		...
		if (t) {
			AdmUtils.setFocus(t, false);
		}
	});

	// Workaround for a bug in FF11 (Bug #9139): repaint the iframe by resizing
	// it.
	// Set a timeout to wait for the iframe to be rendered before resizing it.
	var iframe = document.getElementById(e.id + 'TextArea_ifr');
	if (AdmUtils.isFirefox11()) {
		setTimeout(function() {
			AdmUtils.resizeIframe(iframe)
		}, 100);
	}
}
