...
public class Consumer implements Runnable {
  ...
  private static final String TOPIC = "provisioning-release";
  private static final String CONSUMER_GROUP = "oscm-group";
  private KafkaConsumer<UUID, Object> consumer;
  private SubscriptionService subscriptionService;

  public Consumer() {
    ConfigurationService configService = ServiceLocator.findService(ConfigurationService.class);
    Properties props = new Properties();
    props.put(
        ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG,
        configService
            .getVOConfigurationSetting(ConfigurationKey.KAFKA_BOOTSTRAP_SERVERS, "global")
            .getValue());
    props.put(ConsumerConfig.GROUP_ID_CONFIG, CONSUMER_GROUP);
    consumer =
        new KafkaConsumer<>(props, new UUIDSerializer(), new DataSerializer(ReleaseRecord.class));
  ...
  public void run() {
    ...
    try {
      subscriptionService = ServiceLocator.findService(SubscriptionService.class);
    } catch (Exception e) {
      e.printStackTrace();
      return;
    }
    while (true) {
      try {
        final ConsumerRecords<UUID, Object> consumerRecords = consumer.poll(1000);
  ...
  void completeSubscription(ReleaseRecord release) {
    ...
    VOSubscription subscription = subscriptionService.getSubscription(release.getId());
    try {
      login();
      if (SubscriptionStatus.PENDING.equals(subscription.getStatus()))
        subscriptionService.completeAsyncSubscription(release.getId(), instanceInfo);
      else if (SubscriptionStatus.PENDING_UPD.equals(subscription.getStatus())) {
        subscriptionService.completeAsyncModifySubscription(release.getId(), instanceInfo);
      } else {
        // TODO error
      }
    } catch (Exception e) {
  ...
  void progressSubscription(ReleaseRecord release) {
    try {
      ...
      subscriptionService.updateAsyncSubscriptionProgress(
          release.getId(), release.getStatus().name());
    } catch (Exception e) {
      // TODO Auto-generated catch block
      e.printStackTrace();
    }
